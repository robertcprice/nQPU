...
impl SparseMatrix {
    ...
    pub fn vcat(&self, other: &SparseMatrix) -> SparseMatrix {
        assert_eq!(self.cols, other.cols, "vcat requires same number of columns");
...
impl std::fmt::Display for SparseMatrix {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "SparseMatrix({}x{}, nnz={})", self.rows, self.cols, self.nnz())?;
...
impl TannerGraph {
    ...
    pub fn from_parity_check(h: &SparseMatrix) -> Self {
        ...
        let check_to_var: Vec<Vec<usize>> = (0..num_checks)
            .map(|r| h.row_indices_vec(r))
            .collect();
...
impl HypergraphProductCode {
    ...
    pub fn from_classical(h1: &SparseMatrix, h2: &SparseMatrix) -> Self {
        ...
        let d1_est = (0..r1).map(|r| h1.row_weight(r)).filter(|&w| w > 0).min().unwrap_or(1);
        let d2_est = (0..r2).map(|r| h2.row_weight(r)).filter(|&w| w > 0).min().unwrap_or(1);
    ...
    pub fn decode(&self, syndrome: &[bool], error_rate: f64) -> Vec<bool> {
        ...
        for _iter in 0..self.max_iters {
            ...
            for c in 0..m {
                ...
                for &v in neighbors {
                    ...
                    let new_msg = match self.mode {
                        BPMode::MinSum => {
                            self.min_sum_check_update(c, v, &msg_v2c, syndrome_sign)
                        }
                        BPMode::SumProduct => {
            ...
            for v in 0..n {
                ...
                let total_llr: f64 = channel_llr
                    + checks.iter().map(|&c| msg_c2v[c][v]).sum::<f64>();
...
mod tests {
    ...
    fn test_bp_decoder_no_error() {
        ...
        assert!(result.iter().all(|&b| !b), "expected no error for zero syndrome");
    }
    ...
    fn test_bp_decoder_single_error() {
        ...
        let residual: Vec<bool> = error.iter().zip(correction.iter()).map(|(&e, &c)| e ^ c).collect();
    ...
    fn test_bivariate_bicycle_larger() {
        ...
        assert!(k > 0, "expected positive k for l=6,m=6 bicycle code, got {}", k);
    }
