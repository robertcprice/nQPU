...
impl LindbladEvolution {
    ...
    pub fn lindblad_rhs(&self, rho: &Array2<C64>) -> Array2<C64> {
        ...
        for diss in &self.dissipators {
            ...
            // ½ {L†L, ρ} = ½ (L†L ρ + ρ L†L)
            let anticomm =
                c64_scale(c64_one(), 0.5) * &(l_dag_l.dot(rho) + rho.dot(&l_dag_l));
        ...
        for step in 0..n_steps {
            ...
            rho = match self.config.method {
                ...
                IntegrationMethod::RungeKutta4 => {
                    ...
                    let k2 =
                        self.lindblad_rhs(&(&rho + c64_scale(c64_one(), 0.5 * dt) * &k1));
                    let k3 =
                        self.lindblad_rhs(&(&rho + c64_scale(c64_one(), 0.5 * dt) * &k2));
...
impl AdjointGradient {
    ...
    fn adjoint_rhs(&self, lambda: &Array2<C64>, h: &Hamiltonian) -> Array2<C64> {
        ...
        for diss in &self.dissipators {
            ...
            // ½ {L_k†L_k, λ}
            let anticomm = c64_scale(c64_one(), 0.5)
                * &(l_dag_l.dot(lambda) + lambda.dot(&l_dag_l));
    ...
    pub fn compute_gradient(
        ...
        let evol = LindbladEvolution::new(
            h.clone(),
        ...
        for _step in 0..n_steps {
            rho = match self.config.method {
                ...
                IntegrationMethod::RungeKutta4 => {
                    ...
                    let k2 = evol
                        .lindblad_rhs(&(&rho + c64_scale(c64_one(), 0.5 * dt) * &k1));
                    let k3 = evol
                        .lindblad_rhs(&(&rho + c64_scale(c64_one(), 0.5 * dt) * &k2));
                    let k4 =
                        evol.lindblad_rhs(&(&rho + c64_scale(c64_one(), dt) * &k3));
        ...
        for step in (0..n_steps).rev() {
            ...
            lambda = match self.config.method {
                ...
                IntegrationMethod::RungeKutta4 => {
                    ...
                    let k4 = self.adjoint_rhs(
                        &(&lambda + c64_scale(c64_one(), dt) * &k3),
                        &h_rebuilt,
                    );
    ...
    pub fn finite_difference_gradient(
        ...
        for k in 0..n_params {
            ...
            params_plus[k] += eps;
            let h_plus = ParameterizedHamiltonian::new(
                self.parameterized_h.terms().to_vec(),
                params_plus,
            );
            ...
            params_minus[k] -= eps;
            let h_minus = ParameterizedHamiltonian::new(
                self.parameterized_h.terms().to_vec(),
                params_minus,
            );
...
mod tests {
    ...
    fn test_purity_pure_state() {
        ...
        let rho = DensityMatrix::from_pure_state(&state);
        assert!((rho.purity() - 1.0).abs() < TOL, "Pure state should have purity 1");
    }
    ...
    fn test_hamiltonian_from_pauli_ising_2qubit() {
        ...
        let h = Hamiltonian::from_pauli(
            &[(1.0, "ZZ"), (0.5, "XI"), (0.5, "IX")],
            2,
        );
        assert_eq!(h.dim(), 4);
    ...
    fn test_depolarizing_operators() {
        ...
        assert_eq!(ops.len(), 3, "Depolarizing should have 3 operators (X, Y, Z)");
        for op in &ops {
            assert!(
                (op.rate - 0.2).abs() < TOL,
