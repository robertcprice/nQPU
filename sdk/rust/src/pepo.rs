...
impl Pepo {
        ...
        for row in 0..ly {
            ...
            for up_cfg in 0..up_total {
                ...
                for down_cfg in 0..down_total {
                    ...
                    let val = self.contract_row_horizontal(
                        row, bra, ket, &up_indices, &down_indices,
                    );
            ...
            if row == 0 {
                ...
                for up_cfg in 0..old_total.min(up_total) {
                    for dc in 0..down_total {
                        new_transfer[dc] += transfer[up_cfg] * row_transfer[up_cfg * down_total + dc];
                    }
                }
    ...
    pub fn expectation_value_single_site(
        &self,
        row: usize,
        col: usize,
        op: &[c64],
                    ...
                    for r in 0..dr {
                        for u in 0..du {
                            for dn in 0..dd {
                                let mut val = ZERO;
                                for pi_old in 0..d {
                                    val += op[pi_new * d + pi_old]
                                        * t.get(pi_old, po, l, r, u, dn);
                                }
        ...
        let corners = (0..ly)
            .map(|_| {
                (0..lx)
                    .map(|_| {
                        [
                            (0..chi * chi).map(|_| c64::new(rng.gen::<f64>(), 0.0)).collect(),
                            (0..chi * chi).map(|_| c64::new(rng.gen::<f64>(), 0.0)).collect(),
                            (0..chi * chi).map(|_| c64::new(rng.gen::<f64>(), 0.0)).collect(),
                            (0..chi * chi).map(|_| c64::new(rng.gen::<f64>(), 0.0)).collect(),
                        ]
...
mod tests {
    ...
    fn test_pepo_single_site_operator() {
        ...
        assert!(approx_eq(t.get(0, 0, 0, 0, 0, 0), ONE, TEST_TOL));
        assert!(approx_eq(t.get(1, 1, 0, 0, 0, 0), c64::new(-1.0, 0.0), TEST_TOL));
