...
fn run_batched_shots(
    ...
    (0..bs)
        .map(|shot| {
            let bit = 1u64 << shot;
            let det_events: Vec<bool> =
                detector_values.iter().map(|&d| d & bit != 0).collect();
            let obs_flips: Vec<bool> =
                observable_values.iter().map(|&o| o & bit != 0).collect();
...
impl QecCircuitLibrary {
    ...
    fn surface_code_memory(distance: usize, rounds: usize, x_basis: bool) -> QecCircuit {
        ...
        let obs_indices: Vec<usize> = (0..d)
            .map(|c| final_meas_start + c)
            .collect();
...
mod tests {
    ...
    fn test_config_builder_defaults() {
        ...
        assert!(matches!(config.error_model, ErrorModel::Depolarizing { p } if (p - 0.01).abs() < 1e-10));
    }
    ...
    fn test_single_z_error_no_flip_on_z_meas() {
        ...
        frame.inject_z(0);
        assert!(!frame.flips_measure_z(0), "Z error should not flip Z measurement");
    }
    ...
    fn test_detector_errors_cause_detections() {
        ...
        assert!(any_detection, "High error rate should produce some detections");
    }
    ...
    fn test_statistical_consistency() {
        ...
        let stderr = (result.logical_error_rate * (1.0 - result.logical_error_rate) / n as f64).sqrt();
    ...
    fn test_observable_tracking() {
        ...
        let has_flip = result.observable_flips.iter().any(|obs| obs.iter().any(|&b| b));
    ...
    fn test_shots_per_second() {
        ...
        assert!(result.elapsed_secs >= 0.0, "Elapsed time should be non-negative");
    }
    ...
    fn test_batched_frames_cx() {
        ...
        assert_eq!(frames2.z_frames[0], even_mask, "Control gains Z from target");
    }
