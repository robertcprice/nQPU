    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CatQubitError::InvalidAlpha(a) => {
                write!(f, "invalid coherent state amplitude alpha={:.4}: must be positive and finite", a)
            }
...
impl CatQubitState {
    ...
    pub fn new(logical: u8, alpha: f64, fock_cutoff: usize) -> Result<Self, CatQubitError> {
        ...
        for n in 0..dim {
            fock_coefficients[n] = plus_alpha.coefficients[n]
                + C64::new(sign, 0.0) * minus_alpha.coefficients[n];
        }
    ...
    pub fn fidelity(&self, other: &CatQubitState) -> f64 {
        let len = self.fock_coefficients.len().min(other.fock_coefficients.len());
        let mut overlap = C64::new(0.0, 0.0);
        for i in 0..len {
...
impl CatQubitOperator {
    ...
    pub fn x_gate(alpha: f64, fock_cutoff: usize) -> Self {
        ...
        let cat0 = CatQubitState::new(0, alpha, fock_cutoff)
            .expect("valid alpha for X gate construction");
...
impl fmt::Display for CatQubitOperator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "CatQubitOp({})[{}x{}]", self.name, self.dim(), self.dim())
    }
}
...
impl CatQubitSimulator {
    ...
    pub fn new(config: CatQubitConfig) -> Result<Self, CatQubitError> {
        let noise = BiasedNoiseModel::from_params(
            config.alpha,
            config.kappa_1,
            config.kappa_2,
        );
    ...
    pub fn sweep_distance(&self, distances: &[usize], rounds_per_point: usize) -> Vec<(usize, f64)> {
...
pub fn required_distance(
    ...
    for d in (1..=101).step_by(2) {
        let t = (d + 1) / 2;
        let logical_rate =
            binomial_coefficient(d, t) * p_pf.powi(t as i32) + d as f64 * p_bf;
...
impl ConcatenatedCatCode {
    ...
    pub fn logical_error_rate(&self) -> f64 {
        let d = self.config.num_cats;
        let t = (d + 1) / 2;
        let outer = binomial_coefficient(d, t)
            * self.phase_flip_per_round.powi(t as i32);
...
mod tests {
    ...
    fn test_concatenation_result() {
        ...
        assert!(result.bit_flip_rate < result.phase_flip_rate, "bit flips should be rarer than phase flips");
    ...
    fn test_minimum_weight_decoder_satisfies_syndrome_constraints() {
        for d in [3usize, 5, 7] {
            ...
            for mask in 0..(1usize << syndrome_bits) {
                let syndrome: Vec<bool> = (0..syndrome_bits)
                    .map(|i| ((mask >> i) & 1) == 1)
                    .collect();
                let correction = minimum_weight_decoder(&syndrome, d);
        ...
        for d in [3usize, 5, 7] {
            ...
            for mask in 0..(1usize << syndrome_bits) {
                ...
                for corr_mask in 0..(1usize << d) {
                    ...
                    let cand_syndrome: Vec<bool> = (0..syndrome_bits)
                        .map(|i| cand[i] ^ cand[i + 1])
                        .collect();
    ...
    fn test_overhead_analysis() {
        ...
        assert_eq!(
            result.num_ancilla_qubits,
            result.required_distance - 1
        );
        assert_eq!(
