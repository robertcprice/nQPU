...
mod qasm_lsp_impl {
    ...
    impl QasmLanguageServer {
        ...
        pub fn complete(
            ...
            if !word.is_empty() {
                items.retain(|item| {
                    item.label.starts_with(word)
                        || item.insert_text.starts_with(word)
                });
            }
                ...
                if let QASM3Statement::GateCall { qubits, .. } = stmt {
                    for q in qubits {
                        if let Some(&size) = reg_sizes.get(&q.reg) {
                            if q.index >= size {
                                let line = Self::find_qubit_ref_line(
                                    source,
                                    &q.reg,
                                    q.index,
                                );
        ...
        fn check_qasm2_register_bounds(
            source: &str,
            diagnostics: &mut Vec<QasmDiagnostic>,
        ) {
            ...
            let single_qubit_gates = [
                "h", "x", "y", "z", "s", "t", "sx", "sdg", "tdg", "id",
                "reset",
            ];
            let two_qubit_gates = [
                "cx", "cz", "swap", "iswap", "cy", "ch",
            ];
            let three_qubit_gates = [
                "ccx", "ccz", "cswap",
            ];
            let param_gates = [
                "rx", "ry", "rz", "rxx", "ryy", "rzz",
                "crx", "cry", "crz",
                "u1", "u2", "u3", "u", "cu", "cu1", "cu3",
                "p", "cp",
            ];
        ...
        fn check_gate_fusion_opportunities(
            source: &str,
            diagnostics: &mut Vec<QasmDiagnostic>,
        ) {
            let single_qubit_gates = [
            ...
            for (line_num, line) in source.lines().enumerate() {
                ...
                if let Some((ref prev_name, ref prev_qubit, prev_line)) = prev_gate {
                    if &qubit_ref == prev_qubit
                        && fusion_reported_at != Some(prev_line)
                    {
                        diagnostics.push(QasmDiagnostic {
                            line: line_num,
                            col: 0,
        ...
        fn cursor_after_gate_name(prefix: &str) -> bool {
            ...
            let gates = [
                "h", "x", "y", "z", "s", "t", "sx", "cx", "cz", "swap", "iswap",
                "ccx", "ccz", "cswap", "measure", "reset", "barrier",
            ];
        ...
        fn word_at_cursor(prefix: &str) -> &str {
            ...
            while end > 0 && (bytes[end - 1] as char).is_alphanumeric() || (end > 0 && bytes[end - 1] == b'_') {
                end -= 1;
            }
            &prefix[end..]
        }
        ...
        fn extract_qasm3_decl(line: &str, keyword: &str) -> Option<(usize, String)> {
            ...
            let name = rest[close + 1..].trim().trim_end_matches(';').trim().to_string();
            if name.is_empty() {
                return None;
            }
            Some((size, name))
...
"#;
        ...
        let errors: Vec<_> = diags
            ...
            .filter(|d| d.severity == DiagnosticSeverity::Error)
            .collect();
        assert!(errors.is_empty(), "Expected no errors for valid QASM 2.0, got: {:?}", errors);
    }
...
"#;
    ...
    fn test_complete_keywords() {
        ...
        let labels: Vec<&str> = completions.iter().map(|c| c.label.as_str()).collect();
        assert!(labels.contains(&"qubit"), "Missing 'qubit' keyword in completions");
        assert!(labels.contains(&"gate"), "Missing 'gate' keyword in completions");
    }
    ...
    fn test_complete_after_gate() {
        ...
        assert!(labels.contains(&"q[1]"), "Missing 'q[1]' in post-gate completions");
    }
...
"#;
    ...
    fn test_complete_version_after_openqasm() {
        ...
        let labels: Vec<&str> = completions.iter().map(|c| c.label.as_str()).collect();
        assert!(labels.contains(&"3.0"), "Missing '3.0' in version completions");
        assert!(labels.contains(&"2.0"), "Missing '2.0' in version completions");
    }
    ...
    fn test_incremental_edit_delete() {
        ...
        let result = QasmLanguageServer::apply_incremental_edit(
            doc,
            3, 0,
            3, 6,  // "x q[1];" is 7 chars but we delete "x q[1];" part
            "",
        );
        assert!(
            !result.contains("x q[1]"),
...
"#;
    ...
    fn test_goto_definition_no_match() {
        ...
        assert!(def.is_none(), "Expected no definition for built-in gate 'h'");
    }
...
"#;
    ...
    fn test_sim_resource_distributed_50_qubits() {
        ...
        assert!(
            sim_diags[0].message.contains("distributed or tensor network"),
