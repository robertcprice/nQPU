...
impl QuantumOttoEngine {
    ...
    pub fn run_cycle(&mut self) -> OttoCycleResult {
        ...
        let efficiency = if q_h.abs() > 1e-15 {
            work / q_h
        } else {
            0.0
        };
...
impl Ergotropy {
    ...
    ///
    /// W_max = Tr(H * rho) - Tr(H * rho_passive)
    pub fn compute_from_density_matrix(
        rho: &[Vec<C64>],
        hamiltonian: &[Vec<C64>],
    ) -> f64 {
        // Current energy
...
impl QuantumRefrigerator {
    ...
    pub fn run_cycle(&mut self) -> RefrigeratorResult {
        ...
        // Update state
        self.rho = ThermalState::gibbs_state(
            &self.build_hamiltonian(omega_2),
            t_c,
        )
...
mod tests {
    ...
    fn test_carnot_cycle_reversible() {
        ...
        assert!(
            ...
            "Carnot cycle should be reversible"
        );
    }
