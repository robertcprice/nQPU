...
impl std::fmt::Display for QspError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            QspError::ConvergenceFailed { iterations, residual } => {
...
impl BlockEncoding {
    ...
    pub fn from_matrix(a_flat: &[C64], dim: usize, subnormalization: f64) -> Self {
        ...
        let n_sys = if dim <= 1 { 0 } else { (dim as f64).log2().round() as usize };
...
fn matrix_sqrt_complement(m: &[C64], dim: usize) -> Vec<C64> {
    ...
    for i in 0..dim {
        for j in 0..dim {
            y[i * dim + j] = if i == j { c64(1.0, 0.0) } else { c64(0.0, 0.0) }
                - mtm[i * dim + j];
        }
    }
...
fn signal_matrix(a: f64, convention: QspConvention) -> Mat2 {
    ...
    match convention {
        QspConvention::Wx => [
            [c64(a, 0.0), c64(0.0, sq)],
            [c64(0.0, sq), c64(a, 0.0)],
        ],
        QspConvention::Wz => [
            [c64(a, 0.0), c64(sq, 0.0)],
            [c64(-sq, 0.0), c64(a, 0.0)],
        ],
    }
}
...
pub fn qsvt_hamiltonian_simulation(
    ...
    let val_at_m1: f64 = mono.iter().enumerate().map(|(k, &c)| if k % 2 == 0 { c } else { -c }).sum();
    let max_boundary = val_at_1.abs().max(val_at_m1.abs());

    // Scale so max boundary value = 1
    let scale = if max_boundary > 1e-10 { 1.0 / max_boundary } else { 1.0 };
...
pub fn extract_qsvt_polynomial(circuit: &QsvtCircuit, n_points: usize) -> Vec<(f64, C64)> {
    ...
    for i in 0..n {
        ...
        let qsp_val = evaluate_qsp_sequence(
            &circuit.phases.angles,
            x,
            QspConvention::Wx,
        );
...
mod tests {
    ...
    fn test_qsp_config_defaults() {
        ...
        let cfg2 = QspConfig::new().degree(5).tolerance(1e-8).convention(QspConvention::Wz);
    ...
    fn test_phase_factors_degree3_chebyshev() {
        ...
        assert!(result.is_ok(), "Phase computation should succeed: {:?}", result.err());
    ...
    fn test_conventions_match() {
        ...
        for &x in &[-0.8, 0.0, 0.5, 0.9] {
            ...
            assert!(
                (val_wx.re - x).abs() < TOL,
                "Wx: P({}) = {:.6}, expected {:.6}", x, val_wx.re, x
            );
            assert!(
                (val_wz.re - x).abs() < TOL,
                "Wz: P({}) = {:.6}, expected {:.6}", x, val_wz.re, x
            );
        }
    }
    ...
    fn test_qsvt_hamiltonian_simulation_pauli_x() {
        ...
        assert!(result.is_ok(), "Hamiltonian simulation should succeed: {:?}", result.err());
    ...
    fn test_qsvt_matrix_inversion_diagonal() {
        let a = vec![
            c64(0.8, 0.0), c64(0.0, 0.0),
            c64(0.0, 0.0), c64(0.4, 0.0),
        ];
        let block = BlockEncoding::from_matrix(&a, 2, 1.0);

        let result = qsvt_matrix_inversion(&block, 2.0, 1e-2);
        assert!(result.is_ok(), "Matrix inversion should succeed: {:?}", result.err());
    ...
    fn test_qsvt_amplitude_amplification() {
        ...
        for (i, &phi) in circuit.phases.angles.iter().enumerate() {
            if i % 2 == 0 {
                assert!((phi).abs() < 1e-10, "Even phase {} should be 0, got {}", i, phi);
            } else {
                assert!((phi - PI).abs() < 1e-10, "Odd phase {} should be pi, got {}", i, phi);
            }
        }
    }
    ...
    fn test_block_encoding_subnormalization() {
        let alpha = 2.0;
        let a = vec![
            c64(1.5, 0.0), c64(0.0, 0.0),
            c64(0.0, 0.0), c64(0.8, 0.0),
        ];
        ...
        assert!(
            ...
            "recovered[0][0] = {:.6}, expected 1.5", recovered[0].re
        );
        assert!(
            (recovered[3].re - 0.8).abs() < 1e-10,
            "recovered[1][1] = {:.6}, expected 0.8", recovered[3].re
        );
        ...
        for &a in &[-0.9, -0.5, 0.0, 0.3, 0.7, 1.0] {
            for conv in &[QspConvention::Wx, QspConvention::Wz] {
                ...
                assert!(
                    ...
                    "W({})^dag W({}) [0][0] not 1: {:?}", a, a, uu[0][0]
                );
                assert!(
                    uu[0][1].norm() < 1e-10,
                    "W({})^dag W({}) [0][1] not 0: {:?}", a, a, uu[0][1]
                );
            }
        }
    }
    ...
    fn test_chebyshev_to_monomial() {
        ...
        assert!((mono[0] - 1.0).abs() < 1e-12, "a_0 = {}, expected 1", mono[0]);
        assert!((mono[1] - (-3.0)).abs() < 1e-12, "a_1 = {}, expected -3", mono[1]);
        assert!((mono[2]).abs() < 1e-12, "a_2 = {}, expected 0", mono[2]);
        assert!((mono[3] - 4.0).abs() < 1e-12, "a_3 = {}, expected 4", mono[3]);
    }
    ...
    fn test_constant_polynomial() {
        ...
        assert!(phases.angles[0].abs() < 1e-6, "Phase for P=1 should be ~0, got {}", phases.angles[0]);
    }
    ...
    fn test_block_encoding_extract() {
        let a = vec![
            c64(0.3, 0.0), c64(0.1, 0.0),
            c64(0.1, 0.0), c64(0.5, 0.0),
        ];
        let block = BlockEncoding::from_matrix(&a, 2, 1.0);
        let recovered = block.extract_block();
    ...
    fn test_zero_phases_give_chebyshev() {
        ...
        for &x in &[-0.9, 0.0, 0.5, 0.9] {
            ...
            assert!((val.re - expected).abs() < 1e-10, "T2({}) mismatch: {} vs {}", x, val.re, expected);
        }
        ...
        for &x in &[-0.9, 0.0, 0.5, 0.9] {
            ...
            assert!((val.re - expected).abs() < 1e-10, "T3({}) mismatch: {} vs {}", x, val.re, expected);
        }
    }
    ...
    fn test_phase_rotated_linear() {
        ...
        for &x in &[-0.9, 0.0, 0.5] {
            ...
            assert!(
                ...
                "e^{{i*pi/4}}*{}: got {:?}, expected {:?}", x, val, expected
            );
        }
    }
    ...
    fn test_mixed_chebyshev_polynomial() {
        ...
        let result = compute_phase_factors(&poly, QspConvention::Wx, 1e-3);
        assert!(result.is_ok(), "Mixed Chebyshev should converge: {:?}", result.err());
