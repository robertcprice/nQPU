...
impl DifferentiableMPS {
    ...
    pub fn apply_gate(&mut self, site: usize, gate: &[C64]) {
                    ...
                    for jp in 0..2 {
                        ...
                        for i in 0..2 {
                            for j in 0..2 {
                                ...
                                let t_idx =
                                    a * 2 * 2 * chi_r + i * 2 * chi_r + j * chi_r + b;
        ...
        let (u_data, s_data, vt_data, rank) =
            self.compute_truncated_svd(&mat, m_rows, n_cols);
...
impl MPSVariationalAnsatz {
    ...
    pub fn new(num_qubits: usize, max_bond_dim: usize, num_layers: usize) -> Self {
        assert!(num_qubits >= 2, "Need at least 2 qubits for variational ansatz");
        Self {
            num_qubits,
            max_bond_dim,
            num_layers,
...
mod tests {
    ...
    fn test_dmps_norm_initial() {
        let mps = DifferentiableMPS::new(4, 8);
        let n = mps.norm();
        assert!((n - 1.0).abs() < TOL, "Initial state norm should be 1, got {}", n);
    }
    ...
    fn test_dmps_apply_cnot_creates_entanglement() {
        ...
        assert!(mps.bond_dims[0] >= 2, "CNOT should create entanglement, bond_dim={}", mps.bond_dims[0]);

        // Norm should be approximately 1
        let n = mps.norm();
        assert!((n - 1.0).abs() < 0.1, "Norm after Bell state creation: {}", n);
    }
