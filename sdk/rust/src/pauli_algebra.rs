...
impl fmt::Display for WeightedPauliString {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "({:.4}+{:.4}i) {}", self.coeff.re, self.coeff.im, self.pauli)
    }
}
...
mod tests {
    ...
    fn test_weighted_multiplication() {
        let wp1 = WeightedPauliString::new(
            PauliString::single(1, 0, 'X'),
            C64::new(2.0, 0.0),
        );
        let wp2 = WeightedPauliString::new(
            PauliString::single(1, 0, 'Y'),
            C64::new(3.0, 0.0),
        );
    ...
    fn test_pauli_sum_simplify() {
        ...
        // Find the XZ term
        let xz_term = sum.terms.iter().find(|t| t.pauli.to_string() == "XZ").unwrap();
