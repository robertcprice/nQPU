...
pub fn heisenberg_hamiltonian(
    ...
    for &(i, j) in adjacency {
        ...
        let term = mat_add(&mat_add(&mat_scale(&xx, jx), &mat_scale(&yy, jy)), &mat_scale(&zz, jz));
...
pub fn compact_svd(
    ...
    let mut eigvals: Vec<(f64, usize)> = (0..n)
        .map(|i| (matrix[i * n + i].re.max(0.0), i))
        .collect();
...
pub fn truncated_svd(
    ...
    let n_cols_vt = if full_k > 0 { vt_full.len() / full_k } else { 0 };
    ...
    for r in 0..rows {
        for c in 0..cols {
            ...
            if c + 1 < cols {
                ...
                bonds.push((s, t, 1, 3));
            } else if matches!(bc, BoundaryConditions::Periodic | BoundaryConditions::Cylindrical)
            {
                let t = site_idx(r, 0);
                if t != s {
                    adjacency[s].push((t, 1, 3));
...
impl PepsSimulator {
    ...
    pub fn simple_update(
        ...
        let theta_gated = self.apply_gate_to_theta(&theta, gate, d, other_a, other_b, row_dim, col_dim);
        ...
        self.reconstruct_tensors(site_a, site_b, leg_a, leg_b, &u, &vt,
                                  new_bond_dim, d, other_a, other_b, row_dim, col_dim);
    ...
    pub fn full_update(
        ...
        let (u, sv, vt, trunc_err) = truncated_svd(&weighted_theta, row_dim, col_dim, max_d, cutoff);
        let new_bond_dim = sv.len().max(1);
    ...
    fn find_bond(&self, site_a: usize, site_b: usize) -> PepsResult<(usize, usize, usize)> {
        ...
        Err(PepsError::InvalidLattice(format!(
            "No bond between sites {} and {}", site_a, site_b
        )))
    }
    ...
    fn build_theta_tensor(
        ...
        let other_a: usize = tensor_a.virtual_dims.iter().enumerate()
            .filter(|&(i, _)| i != leg_a)
            .map(|(_, &dim)| dim)
            .product::<usize>()
            .max(1);
        let other_b: usize = tensor_b.virtual_dims.iter().enumerate()
            .filter(|&(i, _)| i != leg_b)
            .map(|(_, &dim)| dim)
            .product::<usize>()
            .max(1);
        ...
        for i in 0..tensor_a.num_virtual {
            if i != leg_a { row_legs_a.push(i + 1); }
        }
    ...
    fn compute_environment_weight(&self, site_a: usize, site_b: usize) -> f64 {
        ...
        if weight > EPSILON { 1.0 / weight } else { 1.0 }
    }
    ...
    fn norm_exact(&self) -> PepsResult<f64> {
        ...
        let all_trivial = self.state.tensors.iter().all(|t| t.virtual_dims.iter().all(|&d| d <= 1));
        if all_trivial {
    ...
    fn norm_ctm(&self, chi: usize) -> PepsResult<f64> {
        ...
        let mut corners: Vec<Vec<c64>> = (0..4).map(|_| {
            let mut m = vec![c64::new(0.0, 0.0); effective_chi * effective_chi];
            for i in 0..effective_chi { m[i * effective_chi + i] = c64::new(1.0, 0.0); }
            m
        }).collect();
        ...
        for iter in 0..max_iter {
            ...
            for c in &mut corners {
                ...
                if n > EPSILON { for x in c.iter_mut() { *x /= n; } }
            }
        }
    ...
    fn norm_bp(&self, damping: f64) -> PepsResult<f64> {
        ...
        for _iter in 0..max_iter {
            ...
            for bond_idx in 0..num_bonds {
                ...
                let local_a: f64 = self.state.tensors[a].data.iter().map(|x| x.norm_sqr()).sum();
                let local_b: f64 = self.state.tensors[b].data.iter().map(|x| x.norm_sqr()).sum();
                ...
                for dir in 0..2 {
                    ...
                    if sum > EPSILON { for v in &mut messages[idx] { *v /= sum; } }
                }
            }
    ...
    fn norm_from_tensors_and_weights(&self) -> PepsResult<f64> {
        let mut norm_sq = 1.0f64;
        for t in &self.state.tensors {
            norm_sq *= t.data.iter().map(|x| x.norm_sqr()).sum::<f64>().max(EPSILON);
        }
        for weights in &self.state.bond_weights {
            let w_sq: f64 = weights.iter().map(|w| w * w).sum();
            if w_sq > EPSILON { norm_sq *= w_sq; }
        }
    ...
    pub fn expectation_local(&self, site: usize, operator: &[c64]) -> PepsResult<c64> {
        let d = self.state.config.physical_dim;
        if operator.len() != d * d {
            return Err(PepsError::DimensionMismatch { expected: d * d, got: operator.len() });
        }
        ...
        if local_norm > EPSILON { exp_val /= local_norm; }
        Ok(exp_val)
    }

    /// Compute the expectation value of a two-site operator.
    pub fn expectation_two_site(
        &self, site1: usize, site2: usize, operator: &[c64],
    ) -> PepsResult<c64> {
        let d = self.state.config.physical_dim;
        let d2 = d * d;
        if operator.len() != d2 * d2 {
            return Err(PepsError::DimensionMismatch { expected: d2 * d2, got: operator.len() });
        }
        ...
        for s in 0..d {
            let mut idx = vec![0usize; 1 + t1.num_virtual]; idx[0] = s;
            v1[s] = t1.get(&idx);
        }
        for s in 0..d {
            let mut idx = vec![0usize; 1 + t2.num_virtual]; idx[0] = s;
            v2[s] = t2.get(&idx);
        }
    ...
    pub fn energy(&self, hamiltonian: &LocalHamiltonian) -> PepsResult<f64> {
        ...
        for term in &hamiltonian.terms {
            match term.sites.len() {
                1 => { total += self.expectation_local(term.sites[0], &term.operator)?; }
                2 => { total += self.expectation_two_site(term.sites[0], term.sites[1], &term.operator)?; }
                _ => return Err(PepsError::ContractionFailed("Only 1/2-site terms supported".into())),
            }
        }
        Ok(total.re)
    }
    ...
    pub fn imaginary_time_evolution(
        &mut self, hamiltonian: &LocalHamiltonian, dt: f64, steps: usize,
    ) -> PepsResult<Vec<f64>> {
        let d = self.state.config.physical_dim;
                ...
                if term.sites.len() == 2 {
                    ...
                    match &self.state.config.update_method {
                        ...
                        UpdateMethod::ClusterUpdate { cluster_size } => {
                            self.cluster_update(term.sites[0], term.sites[1], &gate, *cluster_size)?;
                        }
                    }
    ...
    pub fn variational_optimization(
        &mut self, hamiltonian: &LocalHamiltonian, learning_rate: f64, steps: usize,
    ) -> PepsResult<Vec<f64>> {
    ...
    pub fn string_order_parameter(
        &self, site_i: usize, site_j: usize, path: &[usize],
    ) -> PepsResult<f64> {
        let sz = pauli_z();
        let sx = pauli_x();
        let mut sim_copy = PepsSimulator { state: self.state.clone() };

        sim_copy.apply_single_site_gate(site_i, &sz)?;
        for &k in path { sim_copy.apply_single_site_gate(k, &sx)?; }
        sim_copy.apply_single_site_gate(site_j, &sz)?;

        let norm_sq = { let n = self.norm()?; n * n };
        let mut overlap = c64::new(1.0, 0.0);
        for s in 0..self.state.num_sites {
            let t1 = &self.state.tensors[s];
            let t2 = &sim_copy.state.tensors[s];
            let lo: c64 = t1.data.iter().zip(t2.data.iter())
                .map(|(&a, &b)| a.conj() * b).sum();
            overlap *= lo;
        }
        Ok(if norm_sq > EPSILON { (overlap / norm_sq).re } else { 0.0 })
    }
...
mod tests {
    ...
    fn test_config_builder() {
        let config = PepsConfig::new()
            .with_geometry(LatticeGeometry::Cubic { nx: 2, ny: 2, nz: 2 })
            .with_bond_dim(4)
    ...
    fn test_cubic_lattice_site_count() {
        let geom = LatticeGeometry::Cubic { nx: 2, ny: 3, nz: 4 };
    ...
    fn test_cubic_lattice_neighbors() {
        let config = PepsConfig::new()
            .with_geometry(LatticeGeometry::Cubic { nx: 3, ny: 3, nz: 3 });
    ...
    fn test_diamond_lattice() {
        let geom = LatticeGeometry::Diamond { nx: 2, ny: 2, nz: 2 };
        assert_eq!(geom.num_sites(), 16);
        assert_eq!(geom.num_virtual_legs(), 4);
        let config = PepsConfig::new().with_geometry(geom);
        let sim = PepsSimulator::new_product_state(config, &[0; 16]).unwrap();
    ...
    fn test_simple_update_identity_preserves_state() {
        ...
        assert!(approx_eq(norm_before, norm_after, 0.1),
    ...
    fn test_simple_update_single_qubit_gate() {
        ...
        assert!(approx_eq(exp_z.re, -1.0, 1e-10),
            "Expected <Z> = -1 for |1>, got {}", exp_z.re);
    }
    ...
    fn test_simple_update_bell_pair() {
        ...
        assert!(zz_corr > 0.0, "ZZ correlation should be positive for Bell state, got {}", zz_corr);
    }
    ...
    fn test_boundary_mps_norm_product_state() {
        ...
        assert!(approx_eq(norm, 1.0, 1e-10), "Product state norm should be 1, got {}", norm);
    }
    ...
    fn test_exact_contraction_small_system() {
        ...
        let sim = PepsSimulator::new_product_state(config, &[0, 0, 0, 0]).unwrap();
        let norm = sim.norm().unwrap();
        assert!(approx_eq(norm, 1.0, 1e-10), "Exact norm of |0000> should be 1, got {}", norm);
    }
    ...
    fn test_local_exp_z_on_zero() {
        ...
        assert!(approx_eq(exp.re, 1.0, 1e-10), "<0|Z|0> should be 1, got {}", exp.re);
    }
    ...
    fn test_local_exp_z_on_one() {
        ...
        let exp = sim.expectation_local(0, &pauli_z()).unwrap();
        assert!(approx_eq(exp.re, -1.0, 1e-10), "<1|Z|1> should be -1, got {}", exp.re);
    }
    ...
    fn test_local_exp_x_on_plus() {
        ...
        assert!(approx_eq(exp.re, 1.0, 1e-10), "<+|X|+> should be 1, got {}", exp.re);
    }
    ...
    fn test_two_site_product_state_factorized() {
        ...
        assert!(approx_eq(zz_corr, 1.0, 1e-10), "<ZZ> for |00> should be 1, got {}", zz_corr);
    }
    ...
    fn test_two_site_correlation_mixed() {
        ...
        assert!(approx_eq(zz_corr, -1.0, 1e-10), "<ZZ> for |01> should be -1, got {}", zz_corr);
    }
    ...
    fn test_energy_trivial_hamiltonian() {
        ...
        // <0|Z|0> + <0|Z|0> = 1 + 1 = 2
        assert!(approx_eq(energy, 2.0, 1e-10), "Energy should be 2, got {}", energy);
    }
    ...
    fn test_ite_energy_decreases() {
        ...
        assert!(final_energy <= initial_energy + 0.5,
            "Energy should not increase significantly: {} -> {}", initial_energy, final_energy);
    }
    ...
    fn test_magnetization_all_zero() {
        ...
        for m in &mags {
            assert!(approx_eq(*m, 1.0, 1e-10), "Magnetization of |0> should be 1, got {}", m);
        }
    }
    ...
    fn test_structure_factor_zero_momentum() {
        ...
        assert!(sf > 0.0, "Structure factor at q=0 should be positive, got {}", sf);
    }
    ...
    fn test_entanglement_entropy_product_state() {
        ...
        assert!(approx_eq(entropy, 0.0, 1e-10),
            "Product state entanglement should be 0, got {}", entropy);
    }
    ...
    fn test_bond_dimension_truncation() {
        ...
        assert!(sv.len() <= 2, "Should truncate to 2 singular values, got {}", sv.len());
        assert!(err >= 0.0);
    }
    ...
    fn test_svd_truncation_accuracy() {
        ...
        assert!(sv.len() >= 2, "Should have at least 2 significant singular values");
        assert!(sv[0] >= sv[1], "Singular values should be in decreasing order");
        ...
        let recon_err: f64 = data.iter().zip(reconstructed.iter())
            .map(|(&a, &b)| (a - b).norm_sqr()).sum::<f64>().sqrt();
        assert!(recon_err < 1e-6, "SVD reconstruction error too large: {}", recon_err);
    }
    ...
    fn test_3d_cubic_ground_state_search() {
        let config = PepsConfig::new()
            .with_geometry(LatticeGeometry::Cubic { nx: 2, ny: 2, nz: 1 })
            .with_max_bond_dim(2)
            .with_update_method(UpdateMethod::SimpleUpdate)
            .with_contraction_method(ContractionMethod::SimpleEnvironment);
    ...
    fn test_periodic_boundary_conditions() {
        ...
        for site in 0..9 {
            assert_eq!(sim.state.adjacency[site].len(), 4,
                "Site {} should have 4 neighbors with PBC, got {}",
                site, sim.state.adjacency[site].len());
        }
    }
