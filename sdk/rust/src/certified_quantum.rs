...
impl CertifiedQuantumRNG {
    ...
    pub fn check_backend_available(&self) -> Result<bool, String> {
        match self.backend {
            ...
            QuantumBackend::Rigetti => {
                if self.api_key.is_none() {
                    return Err(
                        "Rigetti (via Amazon Braket) requires AWS credentials. \
    ...
    pub fn run_bell_test(&mut self, samples: usize) -> Result<CertifiedBellResult, String> {
        ...
        self.is_verified.store(result.is_quantum_verified, Ordering::Relaxed);
        self.last_bell_result = Some(result.clone());
        self.bell_tests_run.fetch_add(1, Ordering::Relaxed);
        self.total_pairs_measured.fetch_add(samples as u64 * 4, Ordering::Relaxed);
    ...
    fn run_ibm_bell_test(&self, samples: usize, timestamp_ns: u64) -> Result<CertifiedBellResult, String> {
        ...
        // Add noise
        let noisy_s = result.s_value * (1.0 - gate_error * 2.0) +
                      (rand_f64() - 0.5) * readout_error;
    ...
    pub fn extract_certified(&mut self, count: usize) -> Result<Vec<u8>, String> {
        ...
        self.bytes_extracted.fetch_add(bytes.len() as u64, Ordering::Relaxed);
...
impl QuantumCertificate {
    ...
    pub fn new(bell_result: CertifiedBellResult, data: &[u8]) -> Self {
        ...
        let cert_id = format!("QRNG-{}-{}", bell_result.s_value as u64, timestamp_ns % 1_000_000);
...
mod tests {
    ...
    fn test_simulated_bell_test() {
        ...
        assert!(result.s_value > 2.0, "Simulation should give quantum result");
    }
