...
impl QuasiProbabilityDecomposition {
    ...
    pub fn decompose(&self) -> Result<QPDResult, PECError> {
        ...
        let ptm_inv = ptm
            .inverse()
            .ok_or_else(|| PECError::NonInvertibleChannel("PTM is singular".to_string()))?;
        ...
        if self.noise_channel.dim == 2 {
            ...
            Err(PECError::InvalidNoiseParameter(format!(
                "Unsupported channel dimension {}; only 1- and 2-qubit channels are supported",
                self.noise_channel.dim
            )))
        }
    }

    /// Decompose a single-qubit channel using PTM inverse.
    fn decompose_single_qubit(&self, ptm_inv: &PauliTransferMatrix) -> Result<QPDResult, PECError> {
        ...
        let sign_distribution: Vec<f64> = coefficients.iter().map(|c| c.abs() / overhead).collect();
    ...
    /// diagonal entries.
    fn decompose_two_qubit(&self, ptm_inv: &PauliTransferMatrix) -> Result<QPDResult, PECError> {
        let m = &ptm_inv.matrix;
        let n = m.dim().0;
        if n != 16 {
        ...
        let single_paulis: [Array2<Complex64>; 4] = [pauli_i(), pauli_x(), pauli_y(), pauli_z()];
...
impl NoiseTomography {
    ...
    pub fn characterize_single_qubit<F>(noisy_gate_fn: F, _num_shots: usize) -> NoiseChannel
    where
    ...
    pub fn characterize_two_qubit<F>(noisy_gate_fn: F, _num_shots: usize) -> NoiseChannel
    where
        F: Fn(&Array2<Complex64>) -> Array2<Complex64>,
    {
    ...
    pub fn process_fidelity(ideal: &Array2<Complex64>, noisy: &NoiseChannel) -> f64 {
        let dim = noisy.dim;
        let d = dim as f64;
...
impl PECSampler {
    ...
    pub fn sample_circuit(&self, rng_seed: u64) -> (Vec<ImplementableOperation>, f64) {
    ...
    pub fn estimate_expectation(
        ...
        let values: Vec<f64> = samples
            .iter()
            .map(|(outcomes, sign)| {
                let obs_val: f64 = outcomes.iter().sum::<f64>() / outcomes.len().max(1) as f64;
                sign * obs_val
            })
            .collect();
    ...
    pub fn total_overhead(&self) -> f64 {
        self.decompositions.iter().map(|qpd| qpd.overhead).product()
    }
...
impl PECMitigator {
    ...
    /// and noise strength.
    pub fn characterize_noise(&self, gates: &[Array2<Complex64>]) -> Vec<NoiseChannel> {
        gates
            .iter()
            .map(|_gate| match self.config.channel_model {
                ...
                ChannelModel::PauliChannel => {
                    ...
                    NoiseChannel::new_pauli(p, p, p).unwrap_or_else(|_| NoiseChannel::identity(2))
                }
    ...
    pub fn mitigate(
        ...
        let _obs_norm = if obs_dim > 0 {
            obs_trace / obs_dim as f64
        } else {
            0.0
        };
    ...
    pub fn estimate_cost(&self, circuit_depth: usize, avg_noise: f64) -> CostEstimate {
...
mod tests {
    ...
    fn test_qpd_depolarizing() {
        ...
        assert!(
            result.overhead > 1.0,
            "Overhead should be > 1 for noisy channel"
        );
    }
    ...
    fn test_process_fidelity_decreases_with_noise() {
        ...
        assert!(
            f_low > 0.9,
            "Low noise fidelity should be > 0.9, got {}",
            f_low
        );
    }
    ...
    fn test_pec_sampling_produces_valid_operations() {
        ...
        for seed in 0..100 {
            ...
            assert!(sign.is_finite(), "Sign should be finite, got {}", sign);
        }
    }
    ...
    fn test_prepare_decompositions() {
        let config = PECConfig::builder().noise_strength(0.05).build().unwrap();
    ...
    fn test_noise_tomography_single_qubit() {
        ...
        let noisy_fn = |rho: &Array2<Complex64>| -> Array2<Complex64> { original.apply(rho) };
