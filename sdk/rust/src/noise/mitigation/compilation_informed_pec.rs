...
impl PecConfig {
    ...
    pub fn validate(&self) -> Result<(), CiPecError> {
        ...
        if self.num_samples == 0 {
            return Err(CiPecError::InvalidConfig("num_samples must be > 0".into()));
        }
...
impl CompilationAwareNoise {
    ...
    pub fn for_strategy(
        ...
        match strategy {
            ...
            CompilationStrategy::Custom => Self::new(physical_error_rate, 0, 0, 0, 1.0, strategy),
        }
    }
}
...
impl ErrorBudgetOptimizer {
    ...
    pub fn optimize(&self) -> Result<BudgetAllocation, CiPecError> {
        ...
        let epsilon = 0.01;
        let pec_samples =
            (best.pec_overhead * best.pec_overhead * z * z / (epsilon * epsilon)).ceil() as usize;
    ...
    pub fn breakeven_depth(&self, high_distance: usize) -> Option<usize> {
        ...
        let qec_only_cost = high_ft_pec.qec_qubit_overhead(self.num_logical_qubits) as f64;
...
impl LayerMitigationPlan {
    ...
    pub fn top_k(layer_errors: Vec<f64>, k: usize) -> Self {
        let mut indexed: Vec<(usize, f64)> = layer_errors
            .iter()
            .enumerate()
            .map(|(i, &e)| (i, e))
            .collect();
        ...
        let threshold = if k < indexed.len() { indexed[k].1 } else { 0.0 };
...
impl PecAnalysis {
    ...
    pub fn new(physical_error_rate: f64, circuit_depth: usize, num_logical_qubits: usize) -> Self {
        Self {
            physical_error_rate,
            circuit_depth,
            num_logical_qubits,
    ...
    fn analyze_full_qec(&self, code_distance: usize) -> AnalysisResult {
        ...
        AnalysisResult {
            ...
            confidence_interval: 1.96 * (p_logical * self.circuit_depth as f64).sqrt(),
            samples_for_1pct: 1,
            physical_qubits: phys_qubits,
...
pub fn demo() {
    ...
    // 7. Full comparison
    println!(
        "7. Strategy Comparison (p={:.0e}, depth=50, 4 qubits):",
        physical_rate
    );
...
mod tests {
    ...
    fn test_compilation_fraction() {
        let noise =
            CompilationAwareNoise::new(1e-3, 10, 20, 5, 0.999, CompilationStrategy::CliffordT);
    ...
    fn test_compilation_noise_custom_zero() {
        ...
        let noise = CompilationAwareNoise::new(1e-3, 0, 0, 0, 1.0, CompilationStrategy::Custom);
    ...
    fn test_strategy_noise_profiles() {
        ...
        let ct =
            CompilationAwareNoise::for_strategy(CompilationStrategy::CliffordT, physical, depth);
        let pf =
            CompilationAwareNoise::for_strategy(CompilationStrategy::PauliFrame, physical, depth);
    ...
    fn test_pareto_frontier() {
        let optimizer = ErrorBudgetOptimizer::new(1e-3, CompilationStrategy::CliffordT, 50, 4);
        let frontier = optimizer.pareto_frontier(11);
    ...
    fn test_budget_optimization() {
        let optimizer = ErrorBudgetOptimizer::new(1e-3, CompilationStrategy::CliffordT, 20, 2);
    ...
    fn test_layer_plan_from_ft_compilation() {
        ...
        assert!(
            plan.strategies
                .iter()
                .any(|s| matches!(s, LayerStrategy::Pec)),
