    ...
    pub fn validate(&self) -> Result<(), PNAError> {
        ...
        match &self.noise_model {
            ...
            PNANoiseModel::Thermal(t1, t2) => {
                if *t1 <= 0.0 || *t2 <= 0.0 {
                    return Err(PNAError::InvalidConfig("T1 and T2 must be positive".into()));
                }
                if *t2 > 2.0 * t1 {
                    return Err(PNAError::InvalidConfig(
...
impl NoisePropagator {
    ...
    pub fn propagate_through_clifford(noise: &PauliChannel, gate: &CliffordGate) -> PauliChannel {
...
impl NoiseAbsorber {
    ...
    pub fn absorb(
        ...
        let channels_per_target = (propagated_noises.len() + n_targets - 1) / n_targets;
        ...
        for (t_idx, &location) in target_locations.iter().enumerate() {
            ...
            combined
                .probabilities
                .retain(|(ps, p)| ps.weight() == 0 || p.abs() >= self.threshold);
    ...
    pub fn find_optimal_absorption_points(
        ...
        for i in 1..circuit_layers.len() {
            ...
            let curr_has_two_qubit = circuit_layers[i].iter().any(|(g, _)| !g.is_single_qubit());
...
impl CliffordTwirler {
    ...
    pub fn sample_twirl(&mut self, channel: &PauliChannel, num_samples: usize) -> PauliChannel {
        let n = channel.num_qubits;
...
impl PNAMitigator {
    ...
    pub fn analyze_circuit(&self, layers: &[Vec<(CliffordGate, Vec<usize>)>]) -> CircuitAnalysis {
        ...
        let estimated_improvement = self.estimate_improvement(
            num_layers,
    ...
    pub fn propagate_and_absorb(
        ...
        for (layer_idx, layer) in layers.iter().enumerate() {
            for (_gate, _qubits) in layer {
                ...
                let max_depth = (layer_idx + self.config.max_propagation_depth).min(layers.len());
    ...
    pub fn mitigate(
        ...
        let raw_deviation: f64 =
            noisy_results.iter().map(|v| 1.0 - v.abs()).sum::<f64>() / noisy_results.len() as f64;

        let mitigated_deviation: f64 =
            mitigated.iter().map(|v| 1.0 - v.abs()).sum::<f64>() / mitigated.len() as f64;
    ...
    pub fn estimate_improvement(&self, circuit_depth: usize, noise_rate: f64) -> f64 {
        ...
        let propagation_fraction =
            self.config.max_propagation_depth.min(circuit_depth) as f64 / circuit_depth as f64;
        ...
        let reduced_points =
            (circuit_depth as f64 * (1.0 - propagation_fraction * 0.7)).max(1.0) as u32;
...
mod tests {
    ...
    fn test_channel_composition() {
        ...
        assert!(
            composed.total_error_rate() < ch1.total_error_rate() + ch2.total_error_rate() + 1e-10
        );
    }
    ...
    fn test_asymmetric_noise_propagation_through_h() {
        ...
        assert!((propagated.total_error_rate() - noise.total_error_rate()).abs() < 1e-10);
    }
    ...
    fn test_sample_twirl_converges() {
        ...
        assert!((twirled.total_error_rate() - ch.total_error_rate()).abs() < 0.02);
    }
    ...
    fn test_mitigation_improves_estimates() {
        let config = PNAConfig::new().noise_model(PNANoiseModel::Depolarizing(0.05));
