...
impl SymmetryVerifier {
    ...
    /// equal `expected`.
    pub fn add_particle_conservation(&mut self, qubits: Vec<usize>, expected: usize) -> &mut Self {
    ...
    /// symmetry verification.
    pub fn acceptance_ratio(&self, counts: &HashMap<usize, usize>, num_qubits: usize) -> f64 {
        let total: usize = counts.values().sum();
        if total == 0 {
            return 0.0;
        }
        let accepted: usize = self.filter_counts(counts, num_qubits).values().sum();
...
impl VirtualDistillation {
    ...
    /// * `observable_qubit` - Qubit on which to estimate <Z>.
    pub fn estimate_expectation(&self, states: &[&QuantumState], observable_qubit: usize) -> f64 {
        assert!(!states.is_empty(), "At least one state must be provided");
...
mod tests {
    ...
    fn test_virtual_distillation_higher_copies() {
        ...
        assert!(
            exp2.abs() < TOLERANCE,
            "M=2: <Z> should be ~0, got {}",
            exp2
        );
        assert!(
            exp3.abs() < TOLERANCE,
            "M=3: <Z> should be ~0, got {}",
            exp3
        );
        assert!(
            exp4.abs() < TOLERANCE,
            "M=4: <Z> should be ~0, got {}",
            exp4
        );
    }
