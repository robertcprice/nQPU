    ...
    pub fn insert_dd_with_stats(
        ...
        for qubit in 0..num_qubits {
            ...
            for &(start, length) in &idle_periods {
                ...
                let pulse_positions =
                    Self::compute_pulse_positions(&self.config.sequence, &dd_gates, start, length);
...
pub fn verify_dd_identity(seq: &DDSequence) -> bool {
    ...
    for gate_type in &gates {
        ...
        let g = [[mat[0][0], mat[0][1]], [mat[1][0], mat[1][1]]];
...
mod tests {
    ...
    fn test_schedule_parallel_gates() {
        ...
        let circuit = vec![Gate::single(GateType::H, 0), Gate::single(GateType::X, 1)];
    ...
    fn test_insert_dd_simple_two_qubit() {
        ...
        let config = DDConfig::new().sequence(DDSequence::XY4).min_idle_slots(2);
    ...
    fn test_no_insertion_below_threshold() {
        ...
        let circuit = vec![Gate::single(GateType::H, 0), Gate::single(GateType::X, 0)];

        let config = DDConfig::new().sequence(DDSequence::XY4).min_idle_slots(5);
    ...
    fn test_no_idle_periods_unchanged() {
        ...
        let config = DDConfig::new().sequence(DDSequence::XY4).min_idle_slots(1);
    ...
    fn test_stats_accuracy() {
        ...
        assert_eq!(augmented.len(), stats.original_gates + stats.inserted_gates);
    ...
    fn test_platonic_xy_insertion() {
        ...
        let circuit: Vec<Gate> = (0..8).map(|_| Gate::single(GateType::H, 0)).collect();
    ...
    fn test_udd_insertion() {
        let circuit: Vec<Gate> = (0..6).map(|_| Gate::single(GateType::H, 0)).collect();
