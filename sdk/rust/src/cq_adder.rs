...
pub trait QuantumAdder {
    ...
    fn add(&self, a_qubits: &[usize], b_qubits: &[usize]) -> Result<Vec<ArithmeticGate>, ArithmeticError>;

    /// Generate the inverse (subtraction) gate sequence.
    fn subtract(&self, a_qubits: &[usize], b_qubits: &[usize]) -> Result<Vec<ArithmeticGate>, ArithmeticError> {
...
mod tests {
    ...
    fn quantum_multiplier() {
        ...
        let gates = multiplier.multiply_by_classical(&a_qubits, &result_qubits, 4).unwrap();
        apply_gates(&mut state, &gates);
