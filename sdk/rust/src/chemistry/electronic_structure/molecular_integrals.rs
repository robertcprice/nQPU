...
impl QubitHamiltonian {
    ...
    pub fn add_term(&mut self, term: PauliTerm) {
        if term.operators.is_empty() || term.operators.iter().all(|(_, op)| *op == PauliOp::I) {
...
pub fn build_molecular_hamiltonian(fcidump: &FcidumpData) -> MolecularHamiltonian {
...
fn jw_one_body_pauli(p: usize, q: usize, _n: usize) -> Vec<(f64, Vec<(usize, PauliOp)>)> {
    if p == q {
        // (I - Z_p)/2
        vec![(0.5, vec![]), (-0.5, vec![(p, PauliOp::Z)])]
    } else {
        let (lo, hi) = if p < q { (p, q) } else { (q, p) };
        let mut z_chain: Vec<(usize, PauliOp)> = Vec::new();
        for k in (lo + 1)..hi {
...
fn multiply_pauli_strings(
    ...
    let mut combined: std::collections::BTreeMap<usize, PauliOp> =
...
fn bk_transform_term(term: &PauliTerm, _bk_matrix: &[Vec<bool>], n: usize) -> PauliTerm {
    ...
    let mut new_ops: std::collections::BTreeMap<usize, PauliOp> = std::collections::BTreeMap::new();
    let mut phase = 1.0;
...
pub fn parity_mapping(hamiltonian: &MolecularHamiltonian) -> QubitHamiltonian {
    ...
    for term in &jw.terms {
        ...
        let max_qubit = term.operators.iter().map(|&(q, _)| q).max().unwrap_or(0);
        ...
        if max_qubit < reduced_n {
            ...
            if extra_phase.abs() > 1e-15 {
                qh.add_term(PauliTerm::new(term.coefficient * extra_phase, new_ops));
            }
        }
    }
...
pub fn lithium_hydride() -> MolecularHamiltonian {
    ...
    let h_offdiag = [(0, 1, 0.1210), (0, 5, -0.0450), (1, 5, 0.1630)];
...
pub fn water_molecule() -> MolecularHamiltonian {
    ...
    let h_offdiag = [(0, 1, 0.2380), (1, 5, 0.1560), (2, 6, 0.1120)];
...
pub fn group_commuting_terms(h: &QubitHamiltonian) -> Vec<Vec<usize>> {
    ...
    for (i, term) in h.terms.iter().enumerate() {
        ...
        for group in &mut groups {
            let commutes_with_all = group.iter().all(|&j| term.commutes_with(&h.terms[j]));
...
fn tridiag_min_eigenvalue(alpha: &[f64], beta: &[f64]) -> f64 {
    ...
    for _iter in 0..1000 {
        ...
        let shift = diag[n - 1] - b2 / (d + d.signum() * (d * d + b2).sqrt().max(1e-15));
...
";
    ...
    fn test_parity_reduces_qubits() {
        ...
        assert_eq!(
            parity.num_qubits, 2,
            "Parity mapping should reduce by 2 qubits"
        );
    }
...
";
    ...
    fn test_max_pauli_weight() {
        ...
        assert!(
            w > 0 && w <= 4,
            "Max weight should be between 1 and 4, got {}",
            w
        );
    }
