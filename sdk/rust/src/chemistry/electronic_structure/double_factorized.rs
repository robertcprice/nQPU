...
impl fmt::Display for DoubleFactorizedError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DoubleFactorizedError::ConvergenceFailure {
                iterations,
                residual,
            } => {
                write!(
            ...
            for q in 0..n {
                for r in 0..n {
                    for s in 0..n {
                        ...
                        if (val - self.get(q, p, r, s)).abs() > tol {
                            return false;
                        }
                        if (val - self.get(p, q, s, r)).abs() > tol {
                            return false;
                        }
                        if (val - self.get(r, s, p, q)).abs() > tol {
                            return false;
                        }
                    }
                }
...
impl DoubleFactorizedHamiltonian {
    ...
    fn iterative_factorize(
        ...
        let mut ham =
            Self::cholesky_factorize(one_body, two_body, nuclear_repulsion, &chol_config)?;
                ...
                for p in 0..n {
                    for q in 0..n {
                        for r in 0..n {
                            for s in 0..n {
                                let reconstructed =
                                    self_reconstruct_element(&ham.leaf_tensors, p, q, r, s);
...
impl fmt::Display for ResourceEstimate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "Wall clock (1us/T): {:.3} s",
            self.wall_clock_estimate_secs
        )
    }
}
...
impl MolecularLibrary {
    ...
    pub fn h2(bond_length: f64) -> MolecularSystem {
        ...
        let one_body = OneBodyIntegrals {
            num_orbitals: n,
            data: vec![vec![h11, h12], vec![h12, h22]],
        };
    ...
    pub fn h2o() -> MolecularSystem {
        ...
        one_data[1][0] = one_data[0][1];
        one_data[1][2] = -0.432;
        one_data[2][1] = one_data[1][2];
        one_data[2][5] = -0.112;
        one_data[5][2] = one_data[2][5];
        one_data[3][6] = -0.098;
        one_data[6][3] = one_data[3][6];
    ...
    pub fn n2() -> MolecularSystem {
        ...
        let mut one_data = vec![vec![0.0; n]; n];
        let diag = [
            -15.636, -15.636, -1.472, -0.746, -0.632, -0.632, -0.632, -0.553, -0.553, -0.553,
        ];
        for i in 0..n {
            one_data[i][i] = diag[i];
        }
        one_data[0][2] = -0.315;
        one_data[2][0] = one_data[0][2];
        one_data[1][3] = -0.315;
        one_data[3][1] = one_data[1][3];
        one_data[2][3] = -0.178;
        one_data[3][2] = one_data[2][3];
...
mod tests {
    ...
    fn test_one_body_creation_and_symmetry() {
        let data = vec![vec![-1.25, -0.47], vec![-0.47, -0.48]];
    ...
    fn test_leaf_rotation_unitary() {
        ...
        for l in 0..df.rank {
            assert!(
                ...
                "Rotation matrix for leaf {} is not unitary",
                l
            );
        }
    }
