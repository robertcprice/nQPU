    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::BondDimOverflow { current, max } => {
                write!(f, "MPS bond dimension overflow: {} exceeds max {}", current, max)
            }
...
pub fn is_clifford(gate: &GateKind) -> bool {
    match gate {
        GateKind::H | GateKind::S | GateKind::Sdg | GateKind::X | GateKind::Y
        | GateKind::Z | GateKind::Cx | GateKind::Cz | GateKind::Swap => true,
...
impl CliffordTableau {
    ...
    pub fn new(num_qubits: usize) -> Self {
        ...
        CliffordTableau { num_qubits: n, x_table, z_table, phases }
    }
    ...
    pub fn apply_cz(&mut self, q1: usize, q2: usize) {
        assert!(q1 < self.num_qubits && q2 < self.num_qubits, "Qubit out of bounds");
    ...
    pub fn apply_x(&mut self, qubit: usize) {
        assert!(qubit < self.num_qubits, "Qubit index out of bounds");
        for i in 0..(2 * self.num_qubits) {
            if self.z_table[i][qubit] { self.phases[i] = (self.phases[i] + 2) % 4; }
        }
    }
    ...
    pub fn apply_z(&mut self, qubit: usize) {
        assert!(qubit < self.num_qubits, "Qubit index out of bounds");
        for i in 0..(2 * self.num_qubits) {
            if self.x_table[i][qubit] { self.phases[i] = (self.phases[i] + 2) % 4; }
        }
    }
    ...
    pub fn row_multiply(&mut self, target_row: usize, source_row: usize) {
        ...
        for j in 0..n {
            phase_contrib += pauli_product_phase(
                self.x_table[source_row][j], self.z_table[source_row][j],
                self.x_table[target_row][j], self.z_table[target_row][j],
            );
        }
        let total = (self.phases[source_row] as i64) + (self.phases[target_row] as i64) + phase_contrib;
    ...
    pub fn is_identity(&self) -> bool {
        let n = self.num_qubits;
        for i in 0..n {
            if self.phases[i] != 0 { return false; }
            for j in 0..n {
                if self.x_table[i][j] || self.z_table[i][j] != (i == j) { return false; }
            }
        }
        for i in 0..n {
            if self.phases[n + i] != 0 { return false; }
            for j in 0..n {
                if self.x_table[n + i][j] != (i == j) || self.z_table[n + i][j] { return false; }
            }
        }
        true
    }
...
impl fmt::Display for CliffordTableau {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for i in 0..n {
            let phase_str = match self.phases[i] { 0 => "+", 2 => "-", 1 => "+i", 3 => "-i", _ => "?" };
            write!(f, "  S{}: {} ", i, phase_str)?;
            for j in 0..n {
                let p = match (self.x_table[i][j], self.z_table[i][j]) {
                    (false, false) => "I", (true, false) => "X",
                    (true, true) => "Y", (false, true) => "Z",
                };
                write!(f, "{}", p)?;
            }
            writeln!(f)?;
...
impl MpsResidual {
    ...
    pub fn identity(num_qubits: usize, d_phys: usize) -> Self {
        ...
        MpsResidual {
            ...
            max_bond_dim: 64, svd_cutoff: 1e-10,
        }
    }
    ...
    pub fn apply_two_qubit(&mut self, q1: usize, q2: usize, gate: &[[C64; 4]; 4]) {
        assert!(q1 < self.num_qubits && q2 < self.num_qubits, "Qubit out of bounds");
        assert_eq!((q1 as i64 - q2 as i64).unsigned_abs(), 1, "Two-qubit gate requires adjacent qubits");
        ...
        for l in 0..bl { for r in 0..br { for k in 0..bond {
            for d1 in 0..2usize { for d2 in 0..2usize {
                theta[[l, d1, d2, r]] += tl[[l, d1, k]] * tr[[k, d2, r]];
            }}
        }}}

        let mut new_theta = ndarray::Array4::<C64>::zeros((bl, 2, 2, br));
        for l in 0..bl { for r in 0..br {
            for d1 in 0..2usize { for d2 in 0..2usize {
                let row = d1 * 2 + d2;
                for d1p in 0..2usize { for d2p in 0..2usize {
                    new_theta[[l, d1, d2, r]] += gate[row][d1p * 2 + d2p] * theta[[l, d1p, d2p, r]];
                }}
            }}
        }}
        ...
        for l in 0..bl { for d1 in 0..2usize { for d2 in 0..2usize { for r in 0..br {
    ...
    pub fn svd_truncate(&mut self, site: usize) {
        ...
        for l in 0..bl { for p in 0..d { for k in 0..keep {
        ...
        for k in 0..keep { for j in 0..br_old.min(vt.dim().1) {
            sv[[k, j]] = c64_scale(vt[[k, j]], s_vals[k]);
        }}
        let mut new_tr = Array3::zeros((keep, d2, br2));
        for k in 0..keep { for p in 0..d2 { for r in 0..br2 { for j in 0..br_old {
            new_tr[[k, p, r]] += sv[[k, j]] * tr[[j, p, r]];
        }}}}
        self.tensors[site + 1] = new_tr;
        if site < self.bond_dims.len() { self.bond_dims[site] = keep; }
    }

    pub fn total_bond_dim(&self) -> usize { self.bond_dims.iter().sum() }
    pub fn max_bond_dim_actual(&self) -> usize { self.bond_dims.iter().copied().max().unwrap_or(1) }
    ...
    pub fn norm(&self) -> f64 {
        ...
        for site in 0..self.num_qubits {
            ...
            for a in 0..bl_s { for ap in 0..bl_s {
                let e = env[[a, ap]];
                if e.norm() < 1e-15 { continue; }
                for p in 0..d_s { for b in 0..br_s { for bp in 0..br_s {
    ...
    pub fn expectation_z(&self, qubit: usize) -> f64 {
        ...
        for site in 0..self.num_qubits {
            ...
            for a in 0..bl_s { for ap in 0..bl_s {
                ...
                if e.norm() < 1e-15 { continue; }
                for p in 0..d_s {
                    let z_val: f64 = if site == qubit { if p == 0 { 1.0 } else { -1.0 } } else { 1.0 };
                    for b in 0..br_s { for bp in 0..br_s {
                        new_env[[b, bp]] += c64_scale(e * t[[a, p, b]] * t[[ap, p, bp]].conj(), z_val);
                    }}
                }
            }}
            env = new_env;
    ...
    pub fn is_identity(&self) -> bool {
        for t in &self.tensors {
            let (bl, d, br) = (t.dim().0, t.dim().1, t.dim().2);
            if bl != 1 || br != 1 { return false; }
            if (t[[0, 0, 0]] - c64_one()).norm() > 1e-10 { return false; }
            for p in 1..d { if t[[0, p, 0]].norm() > 1e-10 { return false; } }
        }
        true
    }
}
...
fn thin_svd_c64(mat: &ndarray::Array2<C64>) -> (ndarray::Array2<C64>, Vec<f64>, ndarray::Array2<C64>) {
    let (m, n) = (mat.dim().0, mat.dim().1);
    let k = m.min(n);
    if k == 0 {
        return (ndarray::Array2::zeros((m, 0)), vec![], ndarray::Array2::zeros((0, n)));
    }
    if m <= n {
        let mut a = ndarray::Array2::<C64>::zeros((m, m));
        for i in 0..m { for j in 0..m {
            let mut sum = c64_zero();
            for l in 0..n { sum += mat[[i, l]] * mat[[j, l]].conj(); }
            a[[i, j]] = sum;
        }}
        ...
        for (ki, &i) in idx.iter().take(k).enumerate() {
            ...
            sigma.push(sv);
            for r in 0..m { u[[r, ki]] = eigenvectors[[r, i]]; }
            if sv > 1e-15 {
                for c in 0..n {
                    let mut sum = c64_zero();
                    for r in 0..m { sum += eigenvectors[[r, i]].conj() * mat[[r, c]]; }
        ...
        for i in 0..n { for j in 0..n {
            let mut sum = c64_zero();
            for l in 0..m { sum += mat[[l, i]].conj() * mat[[l, j]]; }
            b[[i, j]] = sum;
        }}
        ...
        for (ki, &i) in idx.iter().take(k).enumerate() {
            let sv = eigenvalues[i].max(0.0).sqrt();
            sigma.push(sv);
            for c in 0..n { vt[[ki, c]] = eigenvectors[[c, i]].conj(); }
            if sv > 1e-15 {
                for r in 0..m {
                    let mut sum = c64_zero();
                    for c in 0..n { sum += mat[[r, c]] * eigenvectors[[c, i]]; }
...
fn hermitian_eigen_deflation(a: &ndarray::Array2<C64>) -> (Vec<f64>, ndarray::Array2<C64>) {
    let n = a.dim().0;
    if n == 1 {
        return (vec![a[[0, 0]].re], ndarray::Array2::from_elem((1, 1), c64_one()));
    }
    let mut mat = a.clone();
    ...
    for col in 0..n {
        ...
        for j in 0..n { v[j] = C64::new((j + 1) as f64, 0.3 * (j as f64)); }
        ...
        for prev in &prev_vecs {
            ...
            for i in 0..n { v[i] -= overlap * prev[i]; }
        }
        ...
        if init_norm < 1e-15 {
            ...
            if n2 < 1e-15 { eigenvalues.push(0.0); continue; }
            for j in 0..n { v[j] = c64_scale(v[j], 1.0 / n2); }
        } else {
            for j in 0..n { v[j] = c64_scale(v[j], 1.0 / init_norm); }
        }
        ...
        for _ in 0..200 {
            ...
            for prev in &prev_vecs {
                ...
                for i in 0..n { w[i] -= overlap * prev[i]; }
            }
            let norm_w: f64 = w.iter().map(|c| c.norm_sqr()).sum::<f64>().sqrt();
            if norm_w < 1e-15 { break; }
            for i in 0..n { v[i] = c64_scale(w[i], 1.0 / norm_w); }
            let mut lambda = c64_zero();
            for i in 0..n { for j in 0..n { lambda += v[i].conj() * mat[[i, j]] * v[j]; } }
            eigenvalue = lambda.re;
        }
        eigenvalues.push(eigenvalue);
        for i in 0..n { eigenvectors[[i, col]] = v[i]; }
...
impl Default for CampsConfig {
    fn default() -> Self {
        Self { max_bond_dim: 64, svd_cutoff: 1e-10, num_qubits: 1 }
    }
}

impl CampsConfig {
    pub fn with_max_bond_dim(mut self, d: usize) -> Self { self.max_bond_dim = d; self }
    pub fn with_svd_cutoff(mut self, c: f64) -> Self { self.svd_cutoff = c; self }
    pub fn with_num_qubits(mut self, n: usize) -> Self { self.num_qubits = n; self }
}
...
impl CampsState {
    pub fn new(config: CampsConfig) -> Self {
        ...
        CampsState { tableau, residual, config }
    }

    pub fn apply_gate(&mut self, gate_type: &GateKind, qubits: &[usize]) -> Result<(), CampsError> {
        let n = self.config.num_qubits;
        for &q in qubits {
            if q >= n { return Err(CampsError::QubitOutOfBounds { qubit: q, num_qubits: n }); }
        }
        if is_clifford(gate_type) { self.apply_clifford(gate_type, qubits) }
        else { self.apply_non_clifford(gate_type, qubits) }
    }

    pub fn apply_circuit(&mut self, gates: &[(GateKind, Vec<usize>)]) -> Result<(), CampsError> {
        for (gate, qubits) in gates { self.apply_gate(gate, qubits)?; }
        Ok(())
    }

    pub fn measure_z(&mut self, qubit: usize) -> Result<bool, CampsError> {
        if qubit >= self.config.num_qubits {
            return Err(CampsError::QubitOutOfBounds { qubit, num_qubits: self.config.num_qubits });
        }
        let (outcome, deterministic) = self.tableau.measure_z(qubit);
        if deterministic { Ok(outcome) }
        else {
    ...
    pub fn expectation_value_z(&self, qubit: usize) -> Result<f64, CampsError> {
        ...
        if self.is_clifford_only() {
            for i in 0..n {
                if !self.tableau.commutes_with_z(i, qubit) { return Ok(0.0); }
            }
            for i in 0..n {
                if self.tableau.z_table[i][qubit] && !self.tableau.x_table[i][qubit] {
                    ...
                    if only_z {
                        return Ok(if self.tableau.phases[i] == 0 { 1.0 } else { -1.0 });
                    }
                }
            }
    ...
    pub fn mps_bond_dim(&self) -> usize { self.residual.max_bond_dim_actual() }
    pub fn is_clifford_only(&self) -> bool { self.residual.is_identity() }
    pub fn tableau(&self) -> &CliffordTableau { &self.tableau }
    pub fn residual(&self) -> &MpsResidual { &self.residual }

    fn apply_clifford(&mut self, gate: &GateKind, qubits: &[usize]) -> Result<(), CampsError> {
        match gate {
            GateKind::H => { self.tableau.apply_h(qubits[0]); Ok(()) }
            GateKind::S => { self.tableau.apply_s(qubits[0]); Ok(()) }
            GateKind::Sdg => { self.tableau.apply_sdg(qubits[0]); Ok(()) }
            GateKind::X => { self.tableau.apply_x(qubits[0]); Ok(()) }
            GateKind::Y => { self.tableau.apply_y(qubits[0]); Ok(()) }
            GateKind::Z => { self.tableau.apply_z(qubits[0]); Ok(()) }
            GateKind::Cx => {
                if qubits.len() < 2 { return Err(CampsError::GateFailed("CX requires 2 qubits".into())); }
                self.tableau.apply_cx(qubits[0], qubits[1]); Ok(())
            }
            GateKind::Cz => {
                if qubits.len() < 2 { return Err(CampsError::GateFailed("CZ requires 2 qubits".into())); }
                self.tableau.apply_cz(qubits[0], qubits[1]); Ok(())
            }
            GateKind::Swap => {
                if qubits.len() < 2 { return Err(CampsError::GateFailed("SWAP requires 2 qubits".into())); }
                self.tableau.apply_swap(qubits[0], qubits[1]); Ok(())
            }
            GateKind::Rz(theta) => {
                for _ in 0..clifford_quarter_turns(*theta) { self.tableau.apply_s(qubits[0]); }
                Ok(())
            }
            GateKind::Rx(theta) => {
                let turns = clifford_quarter_turns(*theta);
                self.tableau.apply_h(qubits[0]);
                for _ in 0..turns { self.tableau.apply_s(qubits[0]); }
                self.tableau.apply_h(qubits[0]);
                Ok(())
            }
            GateKind::Ry(theta) => {
            ...
            _ => Err(CampsError::GateFailed(format!("Gate {} not handled as Clifford", gate))),
        }
    }
    ...
    fn apply_non_clifford(&mut self, gate: &GateKind, qubits: &[usize]) -> Result<(), CampsError> {
        match gate {
            GateKind::T => {
                ...
                self.residual.apply_single_qubit(qubits[0], &[[c64_one(), c64_zero()], [c64_zero(), phase]]);
                Ok(())
            }
            GateKind::Tdg => {
                let phase = C64::from_polar(1.0, -FRAC_PI_4);
            ...
            GateKind::Rz(theta) => {
                let half = theta / 2.0;
                self.residual.apply_single_qubit(qubits[0], &[
            ...
            GateKind::Rx(theta) => {
                ...
                self.residual.apply_single_qubit(qubits[0], &[[c, s], [s, c]]);
                Ok(())
            }
            GateKind::Ry(theta) => {
                let half = theta / 2.0;
            ...
            _ => Err(CampsError::GateFailed(format!("Unrecognized non-Clifford gate: {}", gate))),
        }
    }
}
...
impl fmt::Display for CampsState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "CampsState({} qubits, bond_dim={}, clifford_only={})",
            self.config.num_qubits, self.mps_bond_dim(), self.is_clifford_only())
    }
}
...
pub fn extract_clifford_prefix(
    ...
    for (i, (gate, _)) in circuit.iter().enumerate() {
        if is_clifford(gate) { split_idx = i + 1; } else { break; }
    }
...
mod tests {
    ...
    #[test] fn test_tableau_new() {
        ...
        for p in &tab.phases { assert_eq!(*p, 0); }
        assert!(tab.is_identity());
    }

    #[test] fn test_tableau_apply_h() {
    ...
    #[test] fn test_tableau_apply_s() {
        let mut tab = CliffordTableau::new(1);
        tab.apply_s(0);
        assert!(!tab.x_table[0][0] && tab.z_table[0][0]);
        tab.apply_s(0); tab.apply_s(0); tab.apply_s(0);
        assert!(tab.z_table[0][0] && !tab.x_table[0][0]);
    }

    #[test] fn test_tableau_apply_s_after_h() {
        let mut tab = CliffordTableau::new(1);
        tab.apply_h(0);
        tab.apply_s(0);
        assert!(tab.x_table[0][0] && tab.z_table[0][0]);
    ...
    #[test] fn test_tableau_apply_cx_bell() {
    ...
    #[test] fn test_tableau_measure_z_deterministic() {
        let mut tab = CliffordTableau::new(2);
        let (outcome, det) = tab.measure_z(0);
        assert!(det && !outcome);
    }

    #[test] fn test_tableau_measure_z_random_bell() {
        let mut tab = CliffordTableau::new(2);
        tab.apply_h(0); tab.apply_cx(0, 1);
        let (_, det) = tab.measure_z(0);
        assert!(!det);
    }

    #[test] fn test_tableau_commutes_with_z() {
        let tab = CliffordTableau::new(2);
        assert!(tab.commutes_with_z(0, 0));
        assert!(!tab.commutes_with_z(2, 0));
    }

    #[test] fn test_tableau_row_multiply() {
        let mut tab = CliffordTableau::new(2);
        tab.row_multiply(0, 1);
        assert!(tab.z_table[0][0] && tab.z_table[0][1] && tab.phases[0] == 0);
    }

    #[test] fn test_tableau_sdg_inverse() {
        let mut tab = CliffordTableau::new(1);
        tab.apply_s(0); tab.apply_sdg(0);
        assert!(tab.z_table[0][0] && !tab.x_table[0][0] && tab.phases[0] == 0);
    }

    #[test] fn test_mps_identity() {
    ...
    #[test] fn test_mps_apply_single_qubit_x() {
    ...
    #[test] fn test_mps_apply_two_qubit_cnot() {
        let mut mps = MpsResidual::identity(2, 2);
        let mut cnot = [[c64_zero(); 4]; 4];
        cnot[0][0] = c64_one(); cnot[1][1] = c64_one();
        cnot[2][3] = c64_one(); cnot[3][2] = c64_one();
        mps.apply_two_qubit(0, 1, &cnot);
        assert!(approx_eq(mps.norm(), 1.0));
    }

    #[test] fn test_mps_svd_truncate() {
        let mut mps = MpsResidual::identity(3, 2);
        mps.max_bond_dim = 4;
        let s = 1.0 / 2.0_f64.sqrt();
        mps.apply_single_qubit(0, &[[C64::new(s, 0.0), C64::new(s, 0.0)], [C64::new(s, 0.0), C64::new(-s, 0.0)]]);
        mps.svd_truncate(0);
        assert!(approx_eq(mps.norm(), 1.0));
    }

    #[test] fn test_mps_norm() { assert!(approx_eq(MpsResidual::identity(3, 2).norm(), 1.0)); }

    #[test] fn test_mps_expectation_z() {
    ...
    #[test] fn test_mps_bell_expectations() {
        let mut mps = MpsResidual::identity(2, 2);
        let s = 1.0 / 2.0_f64.sqrt();
        mps.apply_single_qubit(0, &[[C64::new(s, 0.0), C64::new(s, 0.0)], [C64::new(s, 0.0), C64::new(-s, 0.0)]]);
        let mut cnot = [[c64_zero(); 4]; 4];
    ...
    #[test] fn test_camps_clifford_only_stays_identity() {
    ...
    #[test] fn test_camps_t_on_zero() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(2));
        state.apply_gate(&GateKind::T, &[0]).unwrap();
        assert!(state.is_clifford_only());
    }

    #[test] fn test_camps_ry_non_clifford() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(2));
        state.apply_gate(&GateKind::Ry(PI / 3.0), &[0]).unwrap();
        assert!(!state.is_clifford_only());
    }

    #[test] fn test_camps_mixed_circuit() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(4).with_max_bond_dim(32));
        state.apply_gate(&GateKind::H, &[0]).unwrap();
        state.apply_gate(&GateKind::Cx, &[0, 1]).unwrap();
        assert!(state.is_clifford_only());
    ...
    #[test] fn test_camps_expectation_after_x() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(2));
        state.apply_gate(&GateKind::X, &[0]).unwrap();
        assert!(approx_eq(state.expectation_value_z(0).unwrap(), -1.0));
    }

    #[test] fn test_is_clifford_classification() {
    ...
    #[test] fn test_extract_clifford_prefix_mixed() {
        let circuit = vec![(GateKind::H, vec![0]), (GateKind::Cx, vec![0, 1]),
            (GateKind::S, vec![1]), (GateKind::T, vec![0]), (GateKind::H, vec![1])];
    ...
    #[test] fn test_extract_clifford_prefix_all() {
        let (p, r) = extract_clifford_prefix(&[(GateKind::H, vec![0]), (GateKind::Cx, vec![0, 1])]);
        assert_eq!(p.len(), 2); assert_eq!(r.len(), 0);
    }

    #[test] fn test_extract_clifford_prefix_none() {
        let (p, r) = extract_clifford_prefix(&[(GateKind::T, vec![0]), (GateKind::Rz(0.5), vec![1])]);
        assert_eq!(p.len(), 0); assert_eq!(r.len(), 2);
    }

    #[test] fn test_large_clifford_20q() {
        let n = 20;
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(n));
        for q in 0..n { state.apply_gate(&GateKind::H, &[q]).unwrap(); }
        for q in 0..(n-1) { state.apply_gate(&GateKind::Cx, &[q, q+1]).unwrap(); }
        for q in 0..n { state.apply_gate(&GateKind::S, &[q]).unwrap(); }
        assert!(state.is_clifford_only());
        assert_eq!(state.mps_bond_dim(), 1);
    }

    #[test] fn test_bond_dim_bounded() {
        let n = 6;
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(n).with_max_bond_dim(16));
        for q in 0..n { state.apply_gate(&GateKind::H, &[q]).unwrap(); }
        for q in 0..n { state.apply_gate(&GateKind::T, &[q]).unwrap(); }
        for q in 0..(n-1) { state.apply_gate(&GateKind::Cx, &[q, q+1]).unwrap(); }
        assert!(state.mps_bond_dim() <= 16);
    }

    #[test] fn test_apply_circuit() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(3));
        state.apply_circuit(&[(GateKind::H, vec![0]), (GateKind::Cx, vec![0, 1]),
            (GateKind::T, vec![2]), (GateKind::S, vec![1])]).unwrap();
    }

    #[test] fn test_qubit_out_of_bounds() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(2));
        match state.apply_gate(&GateKind::H, &[5]) {
            Err(CampsError::QubitOutOfBounds { qubit: 5, num_qubits: 2 }) => {}
            other => panic!("Expected QubitOutOfBounds, got {:?}", other),
        }
    }

    #[test] fn test_config_builder() {
        let c = CampsConfig::default().with_num_qubits(10).with_max_bond_dim(128).with_svd_cutoff(1e-12);
    ...
    #[test] fn test_gate_kind_display() {
    ...
    #[test] fn test_error_display() {
        let e = CampsError::QubitOutOfBounds { qubit: 5, num_qubits: 3 };
    ...
    #[test] fn test_clifford_rz() {
        let mut state = CampsState::new(CampsConfig::default().with_num_qubits(2));
        state.apply_gate(&GateKind::Rz(FRAC_PI_2), &[0]).unwrap();
        assert!(state.is_clifford_only());
    }

    #[test] fn test_rx_ry_non_clifford() {
