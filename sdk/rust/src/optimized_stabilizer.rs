...
impl TransposedTableau {
    ...
    pub fn h(&mut self, q: usize) -> Result<(), String> {
        ...
        for block_start in (0..num_chunks).step_by(self.cache_config.rows_per_block) {
            ...
            if block_end < num_chunks {
                ...
                let prefetch_end = (prefetch_start + self.cache_config.rows_per_block).min(num_chunks);
            ...
            for chunk_idx in block_start..block_end {
                ...
                // Swap X and Z bits
                self.xs[word_idx][chunk_idx] = (x_val & !bit_mask) | (if z_bit != 0 { bit_mask } else { 0 });
                self.zs[word_idx][chunk_idx] = (z_val & !bit_mask) | (if x_bit != 0 { bit_mask } else { 0 });
    ...
    pub fn cx(&mut self, c: usize, t: usize) -> Result<(), String> {
        if c >= self.num_qubits {
            return Err(format!("control {} out of range [0, {})", c, self.num_qubits));
        }
        if t >= self.num_qubits {
            return Err(format!("target {} out of range [0, {})", t, self.num_qubits));
        }
        if c == t {
...
pub fn benchmark_optimized(num_qubits: usize, num_gates: usize) -> f64 {
    ...
    for (q1, q2, gate_type) in gates.iter().take(100) {
        match gate_type {
            0 => { let _ = tab.h(*q1); }
            1 => { let _ = tab.s(*q1); }
            2 => { let _ = tab.cx(*q1, *q2); }
            _ => { let _ = tab.cz(*q1, *q2); }
        }
    }
