...
fn main() {
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("LSB-8 raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("LSB-8 + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("LSB-8 + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("LSB-8 delayed raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("LSB-8 delayed + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("LSB-8 delayed + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Middle bits raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("Middle bits + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Middle bits + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("High bits raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("High bits + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("High bits + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Diff raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("Diff + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Diff + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("SSD⊕CPU raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("SSD⊕CPU + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("SSD⊕CPU + VN", p, e));
    }
    ...
    let bits: Vec<u8> = (0..min_len)
        .map(|i| ssd_bits[i] ^ cpu_bits[i] ^ diff_bits[i])
        .collect();
    let (p, e, _) = test_bits("Triple XOR raw", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Triple XOR raw", p, e));
    }

    let bits = von_neumann(&bits);
    let (p, e, _) = test_bits("Triple XOR + VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Triple XOR + VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("LSB→SHA256", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Diff→SHA256", p, e));
    }

    println!("\n5c. (SSD⊕CPU) → SHA-256:");
    let bits = sha256_condition(&xor_combine(
        &extract_lsb(&ssd_raw, 4),
        &extract_lsb(&cpu, 4),
    ));
    let (p, e, _) = test_bits("(SSD⊕CPU)→SHA256", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("(SSD⊕CPU)→SHA256", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Diff→SHA→VN", p, e));
    }

    println!("\n6b. (SSD⊕CPU) → SHA256 → VN:");
    let bits = von_neumann(&sha256_condition(&xor_combine(
        &extract_lsb(&ssd_raw, 4),
        &extract_lsb(&cpu, 4),
    )));
    let (p, e, _) = test_bits("(SSD⊕CPU)→SHA→VN", &bits);
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("(SSD⊕CPU)→SHA→VN", p, e));
    }
    ...
    if p > best_result.map_or(0, |r| r.1) {
        best_result = Some(("Diff→VN→SHA", p, e));
    }
