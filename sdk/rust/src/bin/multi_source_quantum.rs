...
fn main() {
    ...
    println!(
        "  Collected {} bits, quantum estimate: ~74%",
        ssd_bits.len()
    );
    ...
    println!(
        "  Collected {} bits, quantum estimate: ~27%",
        cpu_bits.len()
    );
    ...
    println!(
        "  Collected {} bits, quantum estimate: ~40%",
        ram_bits.len()
    );
    ...
    println!(
        "  Collected {} bits, quantum estimate: ~50%",
        kernel_bits.len()
    );
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!(
        "║  SSD only                   │ {:>6}  │ ~74% (baseline)     ║",
        ssd_bits.len()
    );
    println!(
        "║  CPU jitter only            │ {:>6}  │ ~27%                ║",
        cpu_bits.len()
    );
    println!(
        "║  RAM timing only            │ {:>6}  │ ~40%                ║",
        ram_bits.len()
    );
    println!(
        "║  Kernel entropy             │ {:>6}  │ ~50%                ║",
    ...
    // Find best method
    let best_score = score_xor_all
        .max(score_ssd_ram)
        .max(score_cascade)
        .max(score_majority);
    let best_method = if best_score == score_xor_all {
        "XOR all sources"
    } else if best_score == score_ssd_ram {
        "SSD + RAM"
    } else if best_score == score_cascade {
        "Cascade XOR"
    } else {
        "Majority vote"
    };
    ...
    if best_score >= 80.0 {
        println!(
            "🎉 BEST RESULT: {} achieved {:.1}% - CERTIFIED QUANTUM!",
            best_method, best_score
        );
    } else if best_score >= 70.0 {
        println!(
            "✅ BEST RESULT: {} achieved {:.1}% - MIXED QUANTUM",
            best_method, best_score
        );
    } else {
        println!(
            "⚠️  BEST RESULT: {} achieved {:.1}%",
            best_method, best_score
        );
    }

    // Save best output
    let best_bits = if score_xor_all >= score_ssd_ram
        && score_xor_all >= score_cascade
        && score_xor_all >= score_majority
    {
        &xor_all
