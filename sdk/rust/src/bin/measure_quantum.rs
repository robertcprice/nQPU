...
fn measure_quantumness(timing_ns: &[u64], bits: &[u8]) -> QuantumMeasurement {
    ...
    QuantumMeasurement {
        quantum_score,
        confidence: if bits.len() > 10000 { 0.9 } else if bits.len() > 1000 { 0.7 } else { 0.5 },
...
fn measure_noise_spectrum(timing: &[u64]) -> (String, f64, String) {
    ...
    (format!("Noise Spectrum (Flatness = {:.3})", flatness), flatness,
...
fn measure_kurtosis(timing: &[u64]) -> (String, f64, String) {
    ...
    (format!("Kurtosis = {:.2}", kurtosis), score.max(0.0),
...
fn measure_entropy_rate(bits: &[u8]) -> (String, f64, String) {
    ...
    (format!("Entropy = {:.3} bits/bit", min_entropy), min_entropy,
...
fn measure_autocorrelation(timing: &[u64]) -> (String, f64, String) {
    ...
    (format!("Max Autocorr = {:.4}", max_corr), score,
...
fn measure_bit_independence(bits: &[u8]) -> (String, f64, String) {
    ...
    (format!("Independence = {:.3}", independence_score), independence_score.max(0.0),
...
fn collect_urandom(n_samples: usize) -> (Vec<u64>, Vec<u8>) {
    ...
    file.read_exact(&mut bytes).expect("Can't read from /dev/urandom");
...
fn print_report(source: &str, measurement: &QuantumMeasurement) {
    ...
    println!("║                                                                ║");
    println!("║   QUANTUM SCORE: {:>6.1}%                                    ║", measurement.quantum_score);
    println!("║   CONFIDENCE:    {:>6.1}%                                    ║", measurement.confidence * 100.0);
    println!("║   VERDICT:       {:<42}║",
        if measurement.is_quantum { "✅ QUANTUM CERTIFIED" } else { "⚠️ NOT FULLY QUANTUM" });
    println!("╠══════════════════════════════════════════════════════════════╣");
    ...
    println!("║   ----------------------------------------------------------- ║");
    println!("║   YOUR SOURCE ({}) {:>22.1}%     ║", source, measurement.quantum_score);
    println!("╚══════════════════════════════════════════════════════════════╝");
}
