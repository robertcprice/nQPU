...
#[allow(dead_code)]
fn matrix_exp_hermitian(
    eigenvalues: &[f64],
    eigenvectors: &[Vec<C64>],
    t: f64,
) -> Vec<Vec<C64>> {
...
impl QuantumChaosAnalyzer {
    ...
    pub fn new(config: ChaosConfig) -> Self {
        ...
        let hamiltonian = match &config.hamiltonian_type {
            ...
            ChaosHamiltonianType::XXZChain { delta } => {
                generate_xxz_chain(config.num_qubits, *delta, config.disorder_strength, &mut rng)
            }
    ...
    pub fn eth_analysis(
        &self,
        observable_matrix: &[Vec<C64>],
    ) -> Result<EthAnalysis, ChaosError> {
        ...
        let diagonal_variance = diag.iter().map(|&x| (x - diag_mean).powi(2)).sum::<f64>()
            / dim as f64;
        ...
        for step in 0..self.config.time_steps {
            ...
            for i in 0..dim {
                ...
                for k in 0..dim {
                    let phase = -eigenvalues[k] * t;
                    psi_i +=
                        eigenvectors[i][k] * c_orig[k] * C64::new(phase.cos(), phase.sin());
                }
...
impl fmt::Display for SpectralFormFactor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "  SFF(0):          {:.4}", self.sff_values.first().unwrap_or(&0.0))?;
        writeln!(
            f,
            "  SFF(T_H):        {:.4}",
...
mod tests {
    ...
    fn test_eth_random_matrix() {
        ...
        assert!(
            eth.offdiag_rms > 0.0,
            "Off-diagonal RMS should be positive"
        );
    }
