...
impl MetalStabilizerSimulator {
    ...
    pub fn measure(&mut self, qubit: u32) -> Result<bool, String> {
        ...
        let result = if let Some(p_row) = p {
            ...
            let row_multiply = |xs: &mut [u64], zs: &mut [u64], phases: &mut [u8],
                                dst: usize, src: usize, nw: usize| {
                let mut sign: u32 = 0;
                for w in 0..nw {
    ...
    pub fn run_circuit_batch_v2(&mut self, gates: &[StabilizerGate]) -> Result<Vec<bool>, String> {
        ...
        let batchable: Vec<u32> = gates.iter()
            .filter_map(|g| match g {
                StabilizerGate::H { .. } | StabilizerGate::S { .. } |
                StabilizerGate::CX { .. } | StabilizerGate::CZ { .. } => Some(Self::pack_gate(g)),
                _ => None,
            })
            .collect();
    ...
    pub fn run_circuit_batch_v1(&mut self, gates: &[StabilizerGate]) -> Result<Vec<bool>, String> {
        ...
        for (_i, gate) in gates.iter().enumerate() {
            match gate {
                StabilizerGate::H { .. } | StabilizerGate::S { .. } |
                StabilizerGate::CX { .. } | StabilizerGate::CZ { .. } => {
    ...
    pub fn benchmark_throughput_slow(&mut self, num_gates: usize) -> f64 {
        ...
        for i in 0..num_gates {
            ...
            match i % 5 {
                ...
                2 => gates.push(StabilizerGate::CX { control: q, target: q2 }),
    ...
    pub fn benchmark_throughput_batch(&mut self, num_gates: usize) -> f64 {
        ...
        for i in 0..num_gates {
            ...
            match i % 4 {
                ...
                1 => gates.push(StabilizerGate::S { qubit: q }),
                2 => gates.push(StabilizerGate::CX { control: q, target: q2 }),
                _ => gates.push(StabilizerGate::CZ { qubit1: q, qubit2: q2 }),
            }
        }
...
impl MetalStabilizerSimulator {
    ...
    pub fn bulk_sample(&mut self, _gates: &[StabilizerGate], _num_samples: usize) -> Result<Vec<u32>, String> {
        Err("Metal GPU stabilizer is only available on macOS".to_string())
    }
...
fn benchmark_cpu_stabilizer(num_qubits: usize, num_gates: usize) -> f64 {
    use crate::simd_stabilizer::{CircuitSimulator, StabilizerInstruction, SimdStabilizerConfig};

    let config = SimdStabilizerConfig { num_qubits, ..Default::default() };
...
mod tests {
    ...
    fn test_metal_stabilizer_creation() {
        ...
        assert!(sim.is_ok(), "Failed to create Metal stabilizer: {:?}", sim.err());
        let sim = sim.unwrap();
    ...
    fn test_metal_stabilizer_circuit() {
        ...
        let gates = vec![
            ...
            StabilizerGate::CX { control: 0, target: 1 },
    ...
    fn test_metal_stabilizer_batch() {
        ...
        let gates = vec![
            ...
            StabilizerGate::H { qubit: 2 },
            StabilizerGate::S { qubit: 3 },
            StabilizerGate::CZ { qubit1: 4, qubit2: 5 },
        ];
        let results = sim.run_circuit_batch(&gates);
        assert!(results.is_ok(), "Batch execution failed: {:?}", results.err());
    }
    ...
    fn test_metal_stabilizer_bell_state_correlation() {
        ...
        assert_eq!(r0, r1,
            "Bell state measurements must be correlated: q0={}, q1={}", r0, r1);
    }
    ...
    fn test_metal_stabilizer_ghz_correlation() {
        ...
        for q in 1..5 {
            ...
            assert_eq!(r0, rq,
                "GHZ state: qubit {} should agree with qubit 0 ({} vs {})", q, rq, r0);
        }
    }
