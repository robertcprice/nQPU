...
pub enum InfiniteMpsError {
    ...
    ConvergenceFailed { iterations: usize, residual: f64, tolerance: f64 },
...
impl fmt::Display for InfiniteMpsError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ConvergenceFailed { iterations, residual, tolerance } => {
                write!(f, "Not converged after {} iterations (residual={:.2e}, tol={:.2e})",
            ...
            Self::CtmrgFailed { iterations, residual } => {
                write!(f, "CTMRG not converged after {} iterations (residual={:.2e})", iterations, residual)
            }
...
impl VumpsConfig {
    ...
    pub fn with_bond_dim(mut self, d: usize) -> Self { self.bond_dim = d; self }
    pub fn with_phys_dim(mut self, d: usize) -> Self { self.phys_dim = d; self }
    ...
    pub fn with_lanczos_dim(mut self, d: usize) -> Self { self.lanczos_dim = d; self }
...
impl CtmrgConfig {
    pub fn new() -> Self { Self::default() }
    pub fn with_chi(mut self, chi: usize) -> Self { self.chi = chi; self }
    pub fn with_max_iterations(mut self, n: usize) -> Self { self.max_iterations = n; self }
    pub fn with_tolerance(mut self, t: f64) -> Self { self.tolerance = t; self }
    pub fn with_init_method(mut self, m: CtmrgInit) -> Self { self.init_method = m; self }
}
...
impl InfiniteTebdConfig {
    ...
    pub fn with_trotter_order(mut self, o: usize) -> Self { self.trotter_order = o; self }
    pub fn with_dt(mut self, dt: f64) -> Self { self.dt = dt; self }
    pub fn with_num_steps(mut self, n: usize) -> Self { self.num_steps = n; self }
}
...
impl UniformMps {
    ...
    pub fn random(phys_dim: usize, bond_dim: usize, rng: &mut impl Rng) -> Self {
        ...
        Self { tensors: vec![a], center: c, bond_dim, phys_dim }
    }
...
impl TransferMatrix {
    ...
    pub fn dominant_eigenvector(
        ...
        Err(InfiniteMpsError::ConvergenceFailed {
            iterations: max_iter, residual: 1.0, tolerance: tol
        })
    }
...
pub fn vumps(
    ...
    if config.bond_dim == 0 {
        return Err(InfiniteMpsError::InvalidBondDim("bond_dim must be > 0".into()));
    }
    ...
    for iteration in 0..config.max_iterations {
        ...
        let (_, l_fp) = tm.dominant_eigenvector(&mps.tensors[0], 100, 1e-12)
        ...
        let (_, r_fp) = tm_r.dominant_eigenvector(&a_dag, 100, 1e-12)
            .unwrap_or_else(|_| (c1(), Array2::eye(chi).mapv(|v| cr(v))));
        ...
        if gradient_norm < config.tolerance {
            return Ok(VumpsResult {
                mps, energy_per_site: energy, iterations: iteration + 1,
                gradient_norm, converged: true,
            });
        }
    }

    Ok(VumpsResult {
        mps, energy_per_site: energy, iterations: config.max_iterations,
        gradient_norm, converged: gradient_norm < config.tolerance,
    })
}
...
impl InfinitePeps {
    ...
    pub fn random(phys_dim: usize, bond_dim: usize, unit_cell: (usize, usize), rng: &mut impl Rng) -> Self {
        let dims = [bond_dim, bond_dim, bond_dim, bond_dim, phys_dim];
        let tensors = (0..unit_cell.0).map(|_| {
            (0..unit_cell.1).map(|_| PepsTensor::random(dims, rng)).collect()
        }).collect();
        Self { tensors, bond_dim, phys_dim, unit_cell }
    }
}
...
impl CtmEnvironment {
    ...
    pub fn new(chi: usize, bond_dim: usize, init: CtmrgInit, _rng: &mut impl Rng) -> Self {
        ...
        let (corners, edges) = match init {
            ...
            CtmrgInit::Random => {
                let make_c = || Array2::from_shape_fn((chi, chi), |_| {
                    C64::new(rand::random::<f64>() - 0.5, rand::random::<f64>() - 0.5)
                });
                let make_t = || Array3::from_shape_fn((chi, dd, chi), |_| {
                    C64::new(rand::random::<f64>() - 0.5, rand::random::<f64>() - 0.5)
                });
        ...
        Self { corners, edges, chi, converged: false, iterations: 0 }
    }
...
fn ctmrg_absorption_step(env: &mut CtmEnvironment, _peps: &InfinitePeps, chi: usize) {
    ...
    for dir in 0..4 {
        ...
        let trimmed = new_c.slice(ndarray::s![..chi.min(new_c.shape()[0]), ..new_chi]).to_owned();
...
pub fn infinite_tebd(
    ...
    for _step in 0..config.num_steps {
        ...
        let (new_mps, trunc_err) = apply_two_site_gate(&current, &gate, config.max_bond_dim, d, chi)?;
        current = new_mps;
...
fn apply_two_site_gate(
    ...
    let trunc_err = if total_weight > 0.0 { 1.0 - kept_weight / total_weight } else { 0.0 };
...
fn svd_truncate_mat(a: &Array2<C64>, max_dim: usize) -> (Array2<C64>, Vec<f64>, Array2<C64>) {
    ...
    for idx in 0..k {
        ...
        for i in 0..n {
            for j in 0..n {
                deflated_ata[[i, j]] = deflated_ata[[i, j]] - cr(sigma * sigma) * v[i] * v[j].conj();
            }
        }
    }
...
fn tensor_distance(a: &Array3<C64>, b: &Array3<C64>) -> f64 {
    a.iter().zip(b.iter()).map(|(x, y)| (x - y).norm_sqr()).sum::<f64>().sqrt()
}
...
fn unflatten_3d(v: &[C64], d0: usize, d1: usize, d2: usize) -> Array3<C64> {
    Array3::from_shape_vec((d0, d1, d2),
        v.iter().copied().chain(std::iter::repeat(c0())).take(d0 * d1 * d2).collect()
    ).unwrap()
}

fn unflatten_2d(v: &[C64], d0: usize, d1: usize) -> Array2<C64> {
    Array2::from_shape_vec((d0, d1),
        v.iter().copied().chain(std::iter::repeat(c0())).take(d0 * d1).collect()
    ).unwrap()
}

fn reshape_3d_to_mat(a: &Array3<C64>, rows: usize, cols: usize) -> Array2<C64> {
    let flat: Vec<C64> = a.iter().copied().collect();
    Array2::from_shape_vec((rows, cols),
        flat.into_iter().chain(std::iter::repeat(c0())).take(rows * cols).collect()
    ).unwrap()
}
...
fn reshape_mat_to_3d(a: &Array2<C64>, d0: usize, d1: usize, d2: usize) -> Array3<C64> {
    let flat: Vec<C64> = a.iter().copied().collect();
    Array3::from_shape_vec((d0, d1, d2),
        flat.into_iter().chain(std::iter::repeat(c0())).take(d0 * d1 * d2).collect()
    ).unwrap()
}
...
fn exponentiate_gate(h: &Array4<C64>, dt: f64) -> Array4<C64> {
    ...
    let result = &eye + &mat + &m2.mapv(|x| x / cr(2.0)) +
        &m3.mapv(|x| x / cr(6.0)) + &m4.mapv(|x| x / cr(24.0));
...
fn lanczos_ground(
    ...
    let mut v: Vec<C64> = initial.iter().copied().chain(std::iter::repeat(c0())).take(n).collect();
    let nrm = v.iter().map(|x| x.norm_sqr()).sum::<f64>().sqrt();
    if nrm < 1e-30 {
        for i in 0..n { v[i] = if i == 0 { c1() } else { c0() }; }
    } else {
    ...
    let kk = alpha.len();
    if kk == 0 {
        return Err(InfiniteMpsError::NumericalError("Lanczos produced no vectors".into()));
    }
...
mod tests {
    ...
    fn test_error_display_convergence() {
        let e = InfiniteMpsError::ConvergenceFailed { iterations: 100, residual: 1e-5, tolerance: 1e-10 };
        let s = format!("{}", e);
    ...
    fn test_error_display_all_variants() {
        let errors = vec![
            ...
            InfiniteMpsError::CtmrgFailed { iterations: 50, residual: 0.1 },
    ...
    fn test_error_is_std_error() {
        let e: Box<dyn std::error::Error> = Box::new(
            InfiniteMpsError::NumericalError("test".into())
        );
            ...
            for s2 in 0..2 {
                for s1p in 0..2 {
                    for s2p in 0..2 {
                        ...
                        assert!(diff < 1e-10, "Ising gate not Hermitian at [{},{},{},{}]", s1, s2, s1p, s2p);
                    }
                }
            }
        }
        ...
        for s1 in 0..2 {
            for s2 in 0..2 {
                for s1p in 0..2 {
                    for s2p in 0..2 {
                        let diff = (h[[s1, s2, s1p, s2p]] - h[[s1p, s2p, s1, s2]].conj()).norm();
                        assert!(diff < 1e-10, "Heisenberg gate not Hermitian at [{},{},{},{}]", s1, s2, s1p, s2p);
                    }
                }
            }
        }
    ...
    fn test_exponentiate_zero_gate() {
        ...
        for s1 in 0..2 {
            for s2 in 0..2 {
                ...
                assert!((u[[s1, s2, s1, s2]] - c1()).norm() < 1e-8,
                    "u[{},{},{},{}] = {:?}", s1, s2, s1, s2, u[[s1, s2, s1, s2]]);
            }
        }
    }
