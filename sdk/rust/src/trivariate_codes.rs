...
impl TrivariatePolynomial {
    ...
    pub fn from_monomials(l: usize, m: usize, p: usize, monomials: &[(usize, usize, usize)]) -> Self {
    ...
    pub fn add(&self, other: &TrivariatePolynomial) -> TrivariatePolynomial {
        assert_eq!(self.dims, other.dims, "dimension mismatch in polynomial add");
    ...
    pub fn multiply(&self, other: &TrivariatePolynomial) -> TrivariatePolynomial {
        assert_eq!(self.dims, other.dims, "dimension mismatch in polynomial multiply");
        let (l, m, p) = self.dims;
...
impl TrivariateCode {
    ...
    pub fn extract_syndrome_noisy(
        ...
        syndrome.meta_syndrome = self.meta_checks.as_ref().map(|mc| {
            mc.mul_vec(&syndrome.x_syndrome)
        });
...
impl TannerGraph {
    fn from_matrix(h: &SparseBinaryMatrix) -> Self {
        let check_to_var: Vec<Vec<usize>> = (0..h.rows)
            .map(|r| h.row_indices(r))
            .collect();
...
impl TrivariateCode {
    ...
    pub fn decode(&self, syndrome: &TTSyndrome) -> Result<DecodingResult, TrivariateError> {
        ...
        match &self.config.decoder {
            ...
            TTDecoder::BeliefPropagation { iterations, damping } => {
    ...
    fn decode_bp(
        ...
        let x_corr = self.bp_decode_single(
            &self.hz,
            &syndrome.x_syndrome,
            max_iterations,
            damping,
        ...
        let z_corr = self.bp_decode_single(
            &self.hx,
            &syndrome.z_syndrome,
            max_iterations,
            damping,
        ...
        for _iter in 0..max_iterations {
            ...
            for c in 0..num_checks {
                ...
                for &v in neighbors {
                    ...
                    check_to_var[c][v] =
                        damping * new_msg + (1.0 - damping) * check_to_var[c][v];
                }
            }
    ...
    fn uf_decode_single(
        ...
        for v in 0..num_vars {
            ...
            let active_in_var: Vec<usize> = checks
                .iter()
                .filter(|&&c| syndrome[c])
                .copied()
                .collect();
...
mod tests {
    ...
    fn test_polynomial_creation_and_display() {
        let poly = TrivariatePolynomial::from_monomials(3, 3, 2, &[(0, 0, 0), (1, 0, 0), (0, 1, 1)]);
    ...
    fn test_css_condition_holds() {
        ...
        assert!(product.is_zero(), "CSS condition must hold: H_X * H_Z^T = 0");
    }
    ...
    fn test_bp_decoder_convergence() {
        let config = {
            ...
            TTConfig::new(l, m, p, vec![a], vec![b])
                .with_decoder(TTDecoder::BeliefPropagation {
                    iterations: 100,
                    damping: 0.8,
                })
        };
        ...
        assert!(result.correction.len() <= n, "correction should not exceed n qubits");
    }
    ...
    fn test_bivariate_bicycle_backward_compatible() {
        ...
        assert_eq!(code.params.n, 18, "n = 2*3*3*1 = 18");
        assert!(code.meta_checks.is_none(), "meta-checks disabled for bivariate");
    }
