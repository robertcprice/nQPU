...
impl Molecule {
    ...
    pub fn estimated_log_p(&self) -> f64 {
        ...
        let aromatic_count = self.bonds.iter().filter(|b| b.bond_type == BondType::Aromatic).count();
...
impl QuantumMolecularGenerator {
    pub fn new(config: GeneratorConfig) -> Self {
        let num_params = config.num_qubits * config.num_layers * 3
            + (config.num_qubits - 1) * config.num_layers;
    ...
    pub fn sample(&self, num_samples: usize) -> Vec<GeneratedCandidate> {
        ...
        let mut candidates: Vec<(usize, f64)> = probs
            .iter()
            .enumerate()
            .map(|(i, &p)| (i, p))
            .collect();
...
pub fn lead_optimization(
    ...
    for iter in 0..num_iterations {
        ...
        let admet_penalty: f64 = admet_results
            .iter()
            .filter(|r| !r.passes)
            .count() as f64
            * 2.0;
...
mod tests {
    ...
    fn test_drug_likeness_aspirin() {
        ...
        assert_eq!(result.lipinski_violations, 0, "aspirin: {} violations", result.lipinski_violations);
        assert!(result.qed_score > 0.0 && result.qed_score <= 1.0, "QED = {}", result.qed_score);
        assert!(result.synthetic_accessibility > 0.0, "SA = {}", result.synthetic_accessibility);
    }
    ...
    fn test_drug_likeness_large_molecule() {
        ...
        assert!(result.lipinski_violations > 0, "large mol should have violations");
    }
    ...
    fn test_fingerprint_self_similarity() {
        ...
        assert!((tanimoto - 1.0).abs() < 1e-10, "self-similarity should be 1.0, got {}", tanimoto);
    }
    ...
    fn test_fingerprint_different_molecules() {
        ...
        assert!(tanimoto < 1.0, "different molecules should have Tanimoto < 1.0, got {}", tanimoto);
    ...
    fn test_quantum_kernel_self() {
        ...
        assert!((k - k2).abs() < 1e-12, "self-kernel should be deterministic");
    }
    ...
    fn test_quantum_kernel_matrix() {
        ...
        let fps: Vec<_> = mols.iter().map(|m| MolecularFingerprint::from_molecule(m, 64)).collect();
        let kernel = QuantumKernel::new(4);
        let matrix = kernel.kernel_matrix(&fps);
        assert_eq!(matrix.len(), 3);
        // Diagonal should be 1.0
        for i in 0..3 {
            assert!((matrix[i][i] - 1.0).abs() < 1e-6, "diagonal [{i}] = {}", matrix[i][i]);
        }
        // Symmetric
        for i in 0..3 {
            for j in 0..3 {
    ...
    fn test_docking_kernel_scoring() {
        ...
        assert!(result.score <= 0.0, "kernel score should be <= 0, got {}", result.score);
    }
    ...
    fn test_quantum_force_field() {
        ...
        assert!(energy.is_finite(), "interaction energy should be finite, got {}", energy);
    }
    ...
    fn test_binding_affinity_no_features() {
        ...
        assert!(estimator.estimate().is_err(), "should fail without features");
    }
    ...
    fn test_admet_prediction() {
        ...
        for r in &results {
            assert!(r.probability >= 0.0 && r.probability <= 1.0, "prob = {}", r.probability);
    ...
    fn test_lead_optimization() {
        ...
        assert!(!result.pareto_front.is_empty(), "Pareto front should be non-empty");
    ...
    fn test_pareto_dominance() {
        let a = ParetoPoint { objectives: vec![1.0, 2.0], candidate_idx: 0 };
        let b = ParetoPoint { objectives: vec![2.0, 3.0], candidate_idx: 1 };
    ...
    fn test_pareto_non_dominated() {
        let a = ParetoPoint { objectives: vec![1.0, 3.0], candidate_idx: 0 };
        let b = ParetoPoint { objectives: vec![2.0, 1.0], candidate_idx: 1 };
