...
impl PauliPropagationSimulator {
    ...
    pub fn from_sum(
        ...
        let frames: Vec<PauliFrame> = terms
            .into_iter()
            .map(PauliFrame::new)
            .collect();
    ...
    pub fn consolidate(&mut self) {
        ...
        for frame in self.frames.drain(..) {
            let entry = map
                .entry(frame.string.pauli)
                .or_insert(C64::new(0.0, 0.0));
...
impl DynamicTruncationPolicy {
    ...
    pub fn adaptive_threshold(&self, frames: &[PauliFrame], allowance: f64) -> f64 {
        ...
        let mut magnitudes: Vec<f64> = frames
            .iter()
...
impl DynamicPauliPropagator {
    ...
    fn apply_branch_and_bound(&mut self, gate: &Gate) {
        let max_coeff = self.inner.frames
            .iter()
            .map(|f| f.string.coeff.norm())
    ...
    fn dynamic_truncate(&mut self) {
        ...
        let threshold = self.dynamic_policy.adaptive_threshold(
            &self.inner.frames,
            allowance,
        );
    ...
    fn importance_sample(&mut self) {
        ...
        let total_weight: f64 = self.inner.frames.iter().map(|f| f.string.coeff.norm()).sum();
        if total_weight <= 0.0 {
            return;
        }
        ...
        let mut rng = StdRng::seed_from_u64(
            self.error_budget.layers_processed as u64 ^ 0xDEADBEEF,
        );
        ...
        for (i, &count) in counts.iter().enumerate() {
            if count > 0 {
                ...
                let _rescale = (count as f64) * (total_weight / n_target as f64) / (p_i * count as f64);
...
mod tests {
    ...
    fn test_weighted_coefficient_propagation() {
        let obs =
            WeightedPauliString::new(PauliString::single(1, 0, 'Z'), C64::new(0.0, 2.5));
        let mut sim = PauliPropagationSimulator::new(1, obs, default_policy());
    ...
    fn test_adaptive_threshold() {
        ...
        assert!(threshold >= 0.1 && threshold <= 0.2,
            "Threshold should discard small strings: got {}", threshold);
    ...
    fn test_branch_and_bound_freezing() {
        let obs = WeightedPauliString::unit(PauliString::single(1, 0, 'X'));
        let policy = DynamicTruncationPolicy::default()
            .with_branch_and_bound(0.5); // Freeze strings below 50% of max.
