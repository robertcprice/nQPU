...
pub use crate::decoding::{MWPMDecoder, BPDecoder};
...
impl DecoderMut for DecoderAdapter<crate::decoding::MWPMDecoder> {
    fn decode(&mut self, syndrome: &Syndrome) -> Vec<usize> {
        ...
        let production_syndrome = crate::decoding::mwpm::Syndrome::new(
            syndrome.stabilizer_measurements.clone()
        );
...
impl RepetitionCodeBitFlip {
    pub fn new(repetition: usize) -> Self {
        assert!(repetition >= 3, "Need at least 3 qubits for repetition code");
    ...
    fn correct_errors(&self, state: &mut QuantumState, syndrome: &Syndrome) -> QuantumState {
        ...
        for i in 0..self.num_physical_qubits {
            let error_count = if i > 0 && i < self.num_physical_qubits - 1 {
                ...
                (if syndrome.stabilizer_measurements[i - 1] { 1 } else { 0 }) +
                (if syndrome.stabilizer_measurements[i] { 1 } else { 0 })
            } else if i == 0 {
                if syndrome.stabilizer_measurements[0] { 1 } else { 0 }
            } else {
                if syndrome.stabilizer_measurements[i - 1] { 1 } else { 0 }
            };
...
impl RotatedSurfaceCode {
    ...
    pub fn measure_syndrome(&self, state: &QuantumState) -> SurfaceCodeSyndrome {
        let x_syndromes: Vec<bool> = self.layout.x_stabilizers.iter()
            .map(|qubits| measure_multi_x(state, qubits))
            .collect();
        let z_syndromes: Vec<bool> = self.layout.z_stabilizers.iter()
            .map(|qubits| measure_multi_z(state, qubits))
            .collect();

        SurfaceCodeSyndrome { x_syndromes, z_syndromes }
    }
    ...
    pub fn decode(&self, syndrome: &SurfaceCodeSyndrome) -> SurfaceCodeCorrection {
        ...
        SurfaceCodeCorrection { x_corrections, z_corrections }
    }
    ...
    fn decode_error_type(
        ...
        let defects: Vec<usize> = syndromes.iter()
            .enumerate()
            .filter(|(_, &s)| s)
            .map(|(i, _)| i)
            .collect();
        ...
        for &(pair_dist, i, j) in &pairs {
            if !matched[i] && !matched[j] {
                ...
                if bd_i + bd_j < pair_dist {
                    ...
                    corrections.extend(
                        self.correction_path(defects[i], defects[j], stabilizers),
                    );
                }
                matched[i] = true;
                matched[j] = true;
    ...
    fn boundary_distance(&self, pos: (f64, f64)) -> f64 {
        ...
        let (r, c) = pos;
        (r + 0.5).min(c + 0.5).min(d - 0.5 - r).min(d - 0.5 - c).max(0.0)
    }
    ...
    fn single_error_lookup(
        &self,
        syndromes: &[bool],
        stabilizers: &[Vec<usize>],
    ) -> Option<usize> {
        let syndrome_mask: u64 = syndromes.iter()
            .enumerate()
            .filter(|(_, &s)| s)
    ...
    fn correction_path(
        ...
        let shared: Vec<usize> = stabilizers[def_a].iter()
        ...
        for window in path.windows(2) {
            let s: Vec<usize> = stabilizers[window[0]].iter()
    ...
    fn stabilizer_bfs(stabilizers: &[Vec<usize>], start: usize, end: usize) -> Vec<usize> {
        ...
        while head < queue.len() {
            ...
            for next in 0..n {
                if !visited[next]
                    && stabilizers[current].iter().any(|q| stabilizers[next].contains(q))
                {
                    visited[next] = true;
                    parent[next] = current;
                    queue.push(next);
...
impl LogicalGates {
    ...
    pub fn distill_magic_state(
        noisy_states: Vec<QuantumState>,
        _rounds: usize,
    ) -> QuantumState {
...
mod tests {
    ...
    fn test_steane_encode_zero() {
        ...
        for &cw in &hamming {
            assert!((amps[cw].re - expected).abs() < 1e-10,
                "codeword {:#09b}: expected {}, got {}", cw, expected, amps[cw].re);
        }
    }
    ...
    fn test_steane_syndrome_lookup_all_qubits() {
        ...
        for q in 0..7 {
            ...
            assert!(seen.insert(syndrome_val), "qubit {} has duplicate syndrome {}", q, syndrome_val);
            let lookup_result = SteaneSevenQubitCode::syndrome_lookup(syndrome_val);
            assert_eq!(lookup_result, Some(q),
                "qubit {}: syndrome {} maps to {:?}", q, syndrome_val, lookup_result);
        }
    }
    ...
    fn test_steane_x_error_correction() {
        for error_qubit in 0..7 {
            ...
            let x_syn = (syndrome.stabilizer_measurements[0] as usize) << 2
                | (syndrome.stabilizer_measurements[1] as usize) << 1
                | syndrome.stabilizer_measurements[2] as usize;
            assert_ne!(x_syn, 0, "X error on qubit {} needs non-trivial Z syndrome", error_qubit);

            SteaneSevenQubitCode::correct_errors(&mut encoded, &syndrome);

            let expected = 1.0 / (8.0_f64).sqrt();
            let amp = encoded.amplitudes_ref()[0].re;
            assert!((amp - expected).abs() < 1e-10,
                "after X correction on qubit {}: expected {}, got {}", error_qubit, expected, amp);
        }
    }
    ...
    fn test_steane_z_error_correction() {
        for error_qubit in 0..7 {
            ...
            let z_syn = (syndrome.stabilizer_measurements[3] as usize) << 2
                | (syndrome.stabilizer_measurements[4] as usize) << 1
                | syndrome.stabilizer_measurements[5] as usize;
            assert_ne!(z_syn, 0, "Z error on qubit {} needs non-trivial X syndrome", error_qubit);
            ...
            let amp = encoded.amplitudes_ref()[0].re;
            assert!((amp - expected).abs() < 1e-10,
                "after Z correction on qubit {}: expected {}, got {}", error_qubit, expected, amp);
        }
    }
    ...
    fn test_steane_full_decode_with_error() {
        ...
        assert!((alpha_sq - 1.0).abs() < 1e-6,
            "decoded |0L⟩ probability: {}", alpha_sq);
    }
    ...
    fn test_shor_x_syndrome_identifies_block_pair() {
        ...
        for (error_qubit, (exp_s6, exp_s7)) in cases {
            ...
            assert_eq!((s6, s7), (exp_s6, exp_s7),
                "Z error on qubit {} (plus basis): expected X syndromes ({}, {}), got ({}, {})",
                error_qubit, exp_s6, exp_s7, s6, s7);
        }
    }
    ...
    fn test_surface_code_single_x_error_d3() {
        ...
        for error_qubit in 0..n {
            ...
            assert!(has_z_defect,
                "X error on qubit {} should trigger Z syndromes", error_qubit);
            ...
            assert!(post.z_syndromes.iter().all(|&s| !s),
                "d=3 X error on qubit {}: Z syndrome not trivial after correction: {:?}",
                error_qubit, post.z_syndromes);
        }
    }
    ...
    fn test_surface_code_single_z_error_d3() {
        ...
        for error_qubit in 0..n {
            ...
            assert!(has_x_defect,
                "Z error on qubit {} (Hadamard basis) should trigger X syndromes", error_qubit);

            let correction = code.decode(&syndrome);
            RotatedSurfaceCode::apply_correction(&mut state, &correction);

            let post = code.measure_syndrome_deterministic(&state);
            assert!(post.x_syndromes.iter().all(|&s| !s),
                "d=3 Z error on qubit {}: X syndrome not trivial after correction: {:?}",
                error_qubit, post.x_syndromes);
        }
    }
    ...
    fn test_surface_code_no_error() {
        ...
        let syndrome = code.measure_syndrome_deterministic(&state);
        assert!(syndrome.x_syndromes.iter().all(|&s| !s), "X syndromes should be trivial");
        assert!(syndrome.z_syndromes.iter().all(|&s| !s), "Z syndromes should be trivial");
    ...
    fn test_surface_code_syndrome_equivalence_d3() {
        ...
        for q in 0..n {
            ...
            let syndrome = code.measure_syndrome_deterministic(&state);
            syndrome_map.entry(syndrome.z_syndromes).or_default().push(q);
        }
            ...
            if qubits.len() > 1 {
                for i in 0..qubits.len() {
                    for j in (i + 1)..qubits.len() {
                        ...
                        assert!(is_x_stab,
                            "qubits {:?} have same Z syndrome but don't differ by X stabilizer", diff);
                    }
                }
            }
        }
    ...
    fn test_mwpm_decoder_adapter() {
        ...
        assert!(no_errors.is_empty(), "Trivial syndrome should produce no corrections");
        ...
        for &pos in &error_positions {
            assert!(pos < num_qubits, "Error position {} should be valid for {} qubits", pos, num_qubits);
        }
    }

    #[test]
    fn test_bp_decoder_adapter() {
        use crate::decoding::bp::{BPDecoder, BPConfig, TannerGraph};
        ...
        assert!(error_positions.len() <= 3, "Error positions should be within variable count");

        // Test with trivial syndrome
        let trivial = Syndrome::new(vec![false, false]);
        let no_errors = adapter.decode(&trivial);
        assert!(no_errors.is_empty() || no_errors.len() <= 3,
    ...
    fn test_decoder_adapter_conversion() {
        ...
        for &pos in &positions {
            assert!(pos < 9, "Error position {} should be valid for d=3 code (9 qubits)", pos);
        }
    }
}
