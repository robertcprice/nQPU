...
impl CircuitProfile {
    ...
    pub fn is_clifford_t_only(&self, circuit: &[Gate]) -> bool {
        circuit
            .iter()
            .all(|g| is_clifford(g) || matches!(g.gate_type, GateType::T))
    }
...
pub fn select_backend_profiled(gates: &[Gate], num_qubits: usize, noisy: bool) -> SimBackend {
    ...
    if noisy && num_qubits >= config.pauli_prop_min_qubits && analysis.clifford_fraction >= 0.5 {
    ...
    if num_qubits > config.mps_min_qubits && analysis.max_entanglement_width <= num_qubits / 2 {
        #[cfg(target_os = "macos")]
        if metal_gpu_available() {
            return SimBackend::MetalMPS;
        }
...
pub enum SimBackend {
    ...
    /// Physics-based trapped-ion simulator (IonQ, Quantinuum, Oxford style).
    TrappedIon,
    /// Physics-based superconducting transmon simulator (IBM, Google, Rigetti style).
    Superconducting,
}
...
impl std::fmt::Display for SimBackend {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SimBackend::TrappedIon => write!(f, "Trapped-Ion (Physics)"),
            SimBackend::Superconducting => write!(f, "Superconducting Transmon (Physics)"),
        }
    }
}
    ...
    pub fn record(&mut self, obs: RuntimeObservation) {
        ...
        if entry.len() >= self.min_observations {
            let factor: f64 = entry
                ...
                .map(|(est, act)| act / est)
                .sum::<f64>()
                / entry.iter().filter(|(est, _)| *est > 0.0).count().max(1) as f64;
...
impl AutoSimulator {
    ...
    /// Create with trapped-ion physics backend.
    pub fn with_trapped_ion(num_qubits: usize) -> Self {
        Self::with_backend(SimBackend::TrappedIon, num_qubits)
    }

    /// Create with superconducting transmon physics backend.
    pub fn with_superconducting(num_qubits: usize) -> Self {
        Self::with_backend(SimBackend::Superconducting, num_qubits)
    }
    ...
    fn execute_inner(&self, gates: &[Gate]) -> Result<Vec<f64>, String> {
        ...
        let result = match &self.backend {
            ...
            // Physics-based hardware backends: run through the full-physics simulators.
            SimBackend::TrappedIon => {
                use crate::trapped_ion::{IonSpecies, TrapConfig, TrappedIonSimulator};
                use crate::traits::QuantumBackend;
                let config = TrapConfig::builder()
                    .num_ions(self.num_qubits)
                    .species(IonSpecies::yb171())
                    .build()
                    .map_err(|e| format!("{}", e))?;
                let mut sim = TrappedIonSimulator::new(config)
                    .map_err(|e| format!("{}", e))?;
                for gate in gates {
                    QuantumBackend::apply_gate(&mut sim, gate)
                        .map_err(|e| format!("{}", e))?;
                }
                QuantumBackend::probabilities(&sim)
                    .map_err(|e| format!("{}", e))
            }
            SimBackend::Superconducting => {
                use crate::superconducting::{test_processor, TransmonSimulator};
                use crate::traits::QuantumBackend;
                let proc = test_processor(self.num_qubits);
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            for gate in gates {
                let applied_on_gpu = match &gate.gate_type {
                    GateType::H => executor
                        .apply_h(&mut state_vec, self.num_qubits, gate.targets[0])
                        .is_ok(),
                    GateType::X => executor
                        .apply_x(&mut state_vec, self.num_qubits, gate.targets[0])
                        .is_ok(),
                    GateType::CNOT => executor
                        .apply_cnot(
                            &mut state_vec,
                            self.num_qubits,
                            gate.controls[0],
...
pub fn estimate_runtime(backend: &SimBackend, profile: &CircuitProfile) -> f64 {
    ...
    match backend {
        ...
        SimBackend::TrappedIon => {
            // Physics-based: O(2^n * depth) with ~3x overhead for gate compilation
            // and noise channel application (MS gate decomposition, heating models).
            2.0_f64.powf(n) * d * 3.0
        }
        SimBackend::Superconducting => {
            // Physics-based: O(2^n * depth) with ~2.5x overhead for native gate
            // compilation (ECR/sqrt-iSWAP/CZ) and decoherence channels.
            2.0_f64.powf(n) * d * 2.5
        }
    }
}
...
fn estimate_bond_dim(profile: &CircuitProfile) -> f64 {
    ...
    let depth_factor = 1.0 + (profile.depth as f64 / profile.n_qubits.max(1) as f64).min(5.0);
...
mod tests {
    ...
    fn test_new_backend_variants_display() {
        assert_eq!(
            format!("{}", SimBackend::PauliPropagation),
            "Pauli Propagation"
        );
        assert_eq!(
            format!("{}", SimBackend::DecisionDiagram),
            "Decision Diagram"
        );
        assert_eq!(
            format!("{}", SimBackend::NearClifford),
            "Near-Clifford (CH-form)"
        );
        assert_eq!(
    ...
    fn test_optimizer_reduces_gate_count() {
        ...
        let sim = AutoSimulator::new(&gates, 2, false).with_optimization(OptimizationLevel::Basic);
    ...
    fn test_optimizer_none_level_is_noop() {
        ...
        let sim = AutoSimulator::new(&gates, 2, false).with_optimization(OptimizationLevel::None);
    ...
    fn test_with_strategy_builder() {
        let gates = vec![Gate::h(0)];
        let sim =
            AutoSimulator::new(&gates, 1, false).with_strategy(BackendSelectionStrategy::Estimated);
        assert_eq!(*sim.strategy(), BackendSelectionStrategy::Estimated);
    }
    ...
    fn test_runtime_tracker_with_auto_simulator() {
        ...
        let gates = vec![Gate::h(0), Gate::cnot(0, 1)];
        let sim = AutoSimulator::new(&gates, 2, false).with_runtime_tracker(tracker.clone());
    ...
    fn test_estimated_vs_heuristic_agreement() {
        ...
        for tc in &test_cases {
            let estimated_backend = select_backend_estimated(&tc.gates, tc.num_qubits, tc.noisy);
            assert!(
