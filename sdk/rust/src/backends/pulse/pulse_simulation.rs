...
impl TransmonSystem {
    ...
    pub fn two_qubit(freq0: f64, freq1: f64, anhar0: f64, anhar1: f64, coupling_g: f64) -> Self {
        Self {
            n_qubits: 2,
            n_levels: 2,
    ...
    fn embed_single_qubit_op(&self, op: &DenseMatrix, qubit: usize) -> DenseMatrix {
        ...
        let mut result = if qubit == 0 { op.clone() } else { id.clone() };
...
impl PulseSimulator {
    ...
    fn rk4_step(&self, psi: &[Complex64], t: f64, dt: f64) -> Vec<Complex64> {
        ...
        for i in 0..dim {
            result[i] =
                psi[i] + c_re(dt6) * (k1[i] + c_re(2.0) * k2[i] + c_re(2.0) * k3[i] + k4[i]);
        }
        result
    }
    ...
    fn drho_dt(&self, rho: &DenseMatrix, jump_ops: &[(DenseMatrix, f64)], t: f64) -> DenseMatrix {
        let h = self.hamiltonian_at(t);
...
pub fn simulate_ramsey(qubit_freq: f64, detuning: f64, delays: &[f64], dt: f64) -> Vec<f64> {
    let system = TransmonSystem::single_qubit(qubit_freq, -0.3);
    let sigma = 4.0;
...
mod tests {
    ...
    fn test_drag_pulse_shape() {
        ...
        assert!(
            (center - 1.0).abs() < TIGHT_TOL,
            "DRAG center should be ~1.0, got {}",
            center
        );
        ...
        assert!(
            q_center.abs() < TIGHT_TOL,
            "DRAG quadrature at center should be 0"
        );
    }
    ...
    fn test_gaussian_square_pulse() {
        ...
        assert!(
            (mid - 1.0).abs() < TIGHT_TOL,
            "Flat-top should be 1.0, got {}",
            mid
        );
        ...
        assert!(
            rise < 1.0 && rise > 0.0,
            "Rise should be between 0 and 1, got {}",
            rise
        );
        ...
        assert!(
            fall < 1.0 && fall > 0.0,
            "Fall should be between 0 and 1, got {}",
            fall
        );
    }
    ...
    fn test_transmon_hamiltonian_2q() {
        ...
        assert!(
            has_coupling,
            "2-qubit Hamiltonian should have off-diagonal coupling"
        );
    }
    ...
    fn test_sx_gate_pulse() {
        ...
        let p0 = result.final_state[0].norm_sqr();
        let p1 = result.final_state[1].norm_sqr();
        assert!((p0 - 0.5).abs() < 0.15, "SX: p0 should be ~0.5, got {}", p0);
        assert!((p1 - 0.5).abs() < 0.15, "SX: p1 should be ~0.5, got {}", p1);
    }
    ...
    fn test_ramsey_experiment() {
        ...
        assert!(
            pops[0] > 0.0,
            "Ramsey should show nonzero signal at zero delay"
        );
    ...
    fn test_fidelity_computation() {
        ...
        assert!(f_xi < 0.9, "F(X, I) should be low, got {}", f_xi);
    ...
    fn test_process_fidelity() {
        ...
        assert!(
            (pf - 1.0).abs() < TIGHT_TOL,
            "Process fidelity I,I = {}",
            pf
        );

        let pf_xx = process_fidelity(&x, &x);
        assert!(
            (pf_xx - 1.0).abs() < TIGHT_TOL,
            "Process fidelity X,X = {}",
            pf_xx
        );
