...
pub fn classify_gate_precision(
    ...
    match context {
        ...
        GateContext::ForwardPass => {
            ...
            let is_multi_qubit = gate.targets.len() + gate.controls.len() > 2;
            if is_multi_qubit {
                (
                    GatePrecision::F64,
...
impl MixedPrecisionSimulator {
    ...
    pub fn execute_circuit_strict(&mut self, gates: &[Gate]) -> Result<PrecisionStats, String> {
...
pub fn qaoa_mixed_precision(gates: &[Gate], num_qubits: usize) -> (Vec<f64>, PrecisionStats) {
    let mut contexts = vec![GateContext::ForwardPass; gates.len()];
...
mod tests {
    ...
    fn test_classify_pre_measurement_always_f64() {
        let gate = Gate::h(0);
        let (prec, _) = classify_gate_precision(&gate, GateContext::PreMeasurement, 0.0, 1e-6);
        assert_eq!(prec, GatePrecision::F64);
    }
    ...
    fn test_classify_large_angle_parametric_f32() {
        ...
        let (prec, _) = classify_gate_precision(&gate, GateContext::ParametricSensitive, 0.0, 1e-6);
        assert_eq!(prec, GatePrecision::F32);
    }
    ...
    fn test_classify_error_exceeds_threshold_forces_f64() {
        let gate = Gate::h(0);
        let (prec, reason) = classify_gate_precision(&gate, GateContext::ForwardPass, 1e-5, 1e-6);
        assert_eq!(prec, GatePrecision::F64);
    ...
    fn test_simulator_always_f32_bell_state() {
        ...
        let probs = sim.probabilities();
        assert!((probs[0] - 0.5).abs() < 1e-5, "|00> = {}", probs[0]);
        assert!((probs[3] - 0.5).abs() < 1e-5, "|11> = {}", probs[3]);
    }
    ...
    fn test_simulator_adaptive_uses_both_precisions() {
        ...
        let mut sim =
            MixedPrecisionSimulator::new(2, PrecisionPolicy::Adaptive).with_gate_contexts(contexts);
    ...
    fn test_simulator_custom_selector() {
        ...
        let mut sim = MixedPrecisionSimulator::new(2, PrecisionPolicy::Custom)
            .with_custom_selector(|_gate, idx| {
                if idx % 2 == 0 {
                    GatePrecision::F32
                } else {
                    GatePrecision::F64
    ...
    fn test_precision_error_computation() {
        ...
        assert!(error < 1e-6, "precision error should be small: {}", error);
    }
