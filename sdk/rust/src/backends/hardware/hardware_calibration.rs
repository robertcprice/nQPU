...
impl std::fmt::Display for LocalHardwareError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LocalHardwareError::TimingUnavailable => {
                write!(f, "High-resolution timing unavailable")
            }
            LocalHardwareError::CalibrationNotFound(name) => {
                write!(f, "Calibration not found: {}", name)
            }
        }
    }
}
