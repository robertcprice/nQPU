...
pub trait LiveCalibrationProvider {
    ...
    fn fetch_calibration(&self, device_name: &str)
    ...
    fn parse_calibration_json(&self, json: &str) -> Result<CalibrationData, LiveCalibrationError>;
...
impl LiveCalibrationProvider for IbmCalibrationProvider {
    fn fetch_calibration(
        ...
        #[cfg(feature = "qpu")]
        {
            let _url = format!("{}/api/backends/{}/properties", self.base_url, device_name);
    ...
    fn parse_calibration_json(&self, json: &str) -> Result<CalibrationData, LiveCalibrationError> {
        ...
        let qubits_arr = root
            ...
            .ok_or_else(|| {
                LiveCalibrationError::ParseError("IBM JSON missing 'qubits' array".to_string())
            })?;
        ...
        for (qi, q_entry) in qubits_arr.iter().enumerate() {
            ...
            let t2_us = Self::named_param(params, &["T2"]).unwrap_or((2.0 * t1_us).min(100.0));
            let frequency_ghz = Self::named_param(params, &["frequency"]).unwrap_or(5.0);
            let anharmonicity_ghz = Self::named_param(params, &["anharmonicity"]).unwrap_or(-0.34);
            let readout_error = Self::named_param(params, &["readout_error"]).unwrap_or(0.02);
        ...
        let gates_arr = root
            ...
            .and_then(|v| v.as_array())
            .ok_or_else(|| {
                LiveCalibrationError::ParseError("IBM JSON missing 'gates' array".to_string())
            })?;
        ...
        for g in gates_arr {
            ...
            let error_rate = params
                .and_then(|p| Self::named_param(p, &["gate_error", "error"]))
                .or_else(|| g.get("gate_error").and_then(|v| v.as_f64()))
                .unwrap_or(0.0);
            let gate_time_ns = params
                .and_then(|p| Self::named_param(p, &["gate_length", "gate_time"]))
        ...
        let timestamp_secs = root.get("timestamp").and_then(|v| v.as_u64()).unwrap_or(0);
...
impl LiveCalibrationProvider for IonqCalibrationProvider {
    ...
    fn parse_calibration_json(&self, json: &str) -> Result<CalibrationData, LiveCalibrationError> {
        ...
        .filter(|&x| x > 0)
        .ok_or_else(|| {
            LiveCalibrationError::ParseError("IonQ JSON missing qubit count".to_string())
        })?;
...
impl LiveCalibrationProvider for RigettiCalibrationProvider {
    ...
    fn parse_calibration_json(&self, json: &str) -> Result<CalibrationData, LiveCalibrationError> {
        ...
        let isa = root.get("isa").ok_or_else(|| {
            LiveCalibrationError::ParseError("Rigetti JSON missing 'isa' object".to_string())
        })?;
            ...
            for (gate_type, edges_obj) in gm {
                if let Some(edges) = edges_obj.as_object() {
                    for (edge_key, val) in edges {
                        ...
                        let error = val.get("error").and_then(|v| v.as_f64()).unwrap_or(0.006);
                        let duration_ns = val
                            .get("duration_ns")
                            .and_then(|v| v.as_f64())
                            .unwrap_or(12.0);
...
impl CalibrationCache {
    ...
    pub fn insert(&mut self, provider_name: &str, device_name: &str, data: CalibrationData) {
...
impl NoiseModelBuilder {
    ...
    pub fn build_realistic(
        ...
        for gc in &cal.gate_calibrations {
            model
                .gate_errors
                .insert((gc.gate_type.clone(), gc.qubits.clone()), gc.error_rate);
        }
...
pub fn mock_calibration(device_name: &str, num_qubits: usize) -> CalibrationData {
    let qubit_calibrations: Vec<QubitCalibration> = (0..num_qubits)
        .map(|qi| {
...
mod tests {
    ...
    fn test_realistic_from_ibm_json() {
        ...
        let cx_err = model.gate_errors.get(&("cx".to_string(), vec![0, 1]));
    ...
    fn test_provider_name() {
        assert_eq!(
            IbmCalibrationProvider::without_token().provider_name(),
            "ibm"
        );
        assert_eq!(
            IonqCalibrationProvider::without_key().provider_name(),
            "ionq"
        );
        assert_eq!(
