            ...
            CouplingTopology::Pinnacle {
                ...
                for i in 0..outer {
                    if i % boundary_stride.max(1) == 0 {
                        ...
                        let inner_boundary =
                            (i * inner / outer.max(1)).min(inner.saturating_sub(1));
        ...
        match gate {
            ...
            _ => {
                ...
                for &q in qubits {
                    ...
                    let p = self
                        .noise_model
                        .total_1q_error(self.chip.qubits[q].t1, self.chip.qubits[q].t2)
                        / boost;
                    errors.push((q, p));
                }
            }
...
mod tests {
    ...
    fn test_circuit_construction() {
        ...
        circuit.add_moment(vec![(PinnacleGate::H, vec![0]), (PinnacleGate::H, vec![1])]);
    ...
    fn test_quantum_volume_scales_with_fidelity() {
        ...
        let qv_low = PinnacleSimulator::new(cfg_low)
            .unwrap()
    ...
    fn test_total_2q_error_greater_than_1q() {
        ...
        assert!(p2 > p1, "2q error ({}) should exceed 1q error ({})", p2, p1);
    }
}
