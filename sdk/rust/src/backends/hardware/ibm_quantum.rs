...
impl std::fmt::Display for IBMError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            IBMError::TokenNotConfigured => write!(
                f,
                "IBM Quantum token not configured. Set IBM_QUANTUM_TOKEN env var"
            ),
            ...
            IBMError::RateLimited { retry_after } => {
    ...
    pub fn to_qasm(&self) -> String {
        ...
        for gate in &self.gates {
            match gate {
                ...
                IBMGate::Rx { qubit, theta } => {
                    qasm.push_str(&format!("rx({}) q[{}];\n", theta, qubit))
                }
                IBMGate::Ry { qubit, theta } => {
                    qasm.push_str(&format!("ry({}) q[{}];\n", theta, qubit))
                }
                IBMGate::Rz { qubit, theta } => {
                    qasm.push_str(&format!("rz({}) q[{}];\n", theta, qubit))
                }
                IBMGate::CX { control, target } => {
                    qasm.push_str(&format!("cx q[{}],q[{}];\n", control, target))
                }
                IBMGate::CZ { control, target } => {
                    qasm.push_str(&format!("cz q[{}],q[{}];\n", control, target))
                }
                IBMGate::Swap { qubit1, qubit2 } => {
                    qasm.push_str(&format!("swap q[{}],q[{}];\n", qubit1, qubit2))
                }
                IBMGate::P { qubit, theta } => {
                    qasm.push_str(&format!("p({}) q[{}];\n", theta, qubit))
                }
                IBMGate::U3 {
                    qubit,
                    theta,
...
impl IBMQuantumBackend {
    ...
    pub fn new(backend_name: &str) -> IBMResult<Self> {
        let token = std::env::var("IBM_QUANTUM_TOKEN").map_err(|_| IBMError::TokenNotConfigured)?;
...
impl IBMJob {
    ...
    pub fn wait_for_result(mut self) -> IBMResult<IBMJobResult> {
        ...
        self.result
            .ok_or(IBMError::JobFailed("No result".to_string()))
    }
...
mod tests {
    ...
    fn test_simulated_job() {
        ...
        let job = backend
            .run_circuit(&circuit, IBMJobConfig::default())
            .unwrap();
