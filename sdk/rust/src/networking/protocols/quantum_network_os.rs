    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            QNodeOSError::NoPairsAvailable { node_a, node_b } => {
                write!(
                    f,
                    "no entangled pairs between nodes {} and {}",
                    node_a, node_b
                )
            }
...
impl QuantumTeleportation {
    ...
    pub fn teleport(state: &[C64], link: &QuantumLink) -> Result<Vec<C64>, QNodeOSError> {
        ...
        let output: Vec<C64> = state
            .iter()
            .map(|&amp| C64::new(amp.re * scale + noise_weight, amp.im * scale))
            .collect();
        ...
        Ok(output
            .iter()
            .map(|c| C64::new(c.re * inv_norm, c.im * inv_norm))
            .collect())
    }
...
impl NetworkScheduler {
    ...
    pub fn execute_swap_chain(
        ...
        for hop in 1..(path.len() - 1) {
            let next_link = table.consume_pair(path[hop], path[hop + 1]).ok_or(
                QNodeOSError::SwapChainFailed {
                    hop,
                    reason: format!("no pair between {} and {}", path[hop], path[hop + 1]),
                },
            )?;
...
impl QNodeOS {
    ...
    pub fn request_link(&mut self, src: usize, dst: usize) -> Result<QuantumLink, QNodeOSError> {
        ...
        let path = NetworkScheduler::find_path(src, dst, self.config.num_nodes, &self.table)?;
...
mod tests {
    ...
    fn test_config_builder_defaults() {
        let config = QNodeOSConfig::builder().num_nodes(4).build().unwrap();
    ...
    fn test_dejmps_fidelity_identical_pairs() {
        ...
        assert!(
            f_out > 0.9,
            "distillation should improve fidelity: got {}",
            f_out
        );
    ...
    fn test_distill_pairs_struct_success() {
        ...
        assert!(
            link.fidelity > 0.85,
            "distilled fidelity should exceed input"
        );
