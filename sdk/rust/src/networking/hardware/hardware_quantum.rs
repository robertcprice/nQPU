...
impl HardwareQuantumExtractor {
    ...
    fn probe_hardware(&mut self) -> Result<(), String> {
        #[cfg(target_os = "macos")]
        {
            ...
            let camera_paths = ["/dev/video0", "/dev/video1"];
            ...
            let audio_paths = ["/dev/audio", "/dev/dsp"];
    ...
    fn extract_audio_noise(&mut self) -> Result<QuantumNoiseSample, String> {
        ...
        let raw_bytes = if let Some(ref path) = self.audio_path {
            ...
            let mut file =
                File::open(path).map_err(|e| format!("Failed to open audio device: {}", e))?;
    ...
    fn extract_radio_noise(&mut self) -> Result<QuantumNoiseSample, String> {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let output = std::process::Command::new("netstat").arg("-i").output();
...
impl QuantumTestResults {
    ...
    pub fn passes_tests(&self) -> bool {
        self.frequency_test > 0.9 && self.runs_test > 0.8 && self.entropy > 7.5
    }
}
...
mod tests {
    ...
    fn test_bytes_are_unique() {
        ...
        assert_ne!(
            bytes1, bytes2,
            "Extracted bytes should be different each time"
        );
    }
    ...
    fn test_quantum_noise_analyzer() {
        ...
        assert!(
            entropy > 3.0,
            "Shannon entropy should be > 3.0 for hardware noise, got {}",
            entropy
        );
    }
    ...
    fn test_quantum_test_results() {
        ...
        assert!(
            results.entropy > 3.0,
            "Entropy should be > 3.0, got {}",
            results.entropy
        );
        // Should see many unique bytes
        assert!(
            results.unique_bytes > 100,
            "Should see > 100 unique bytes, got {}",
            results.unique_bytes
        );
    }
    ...
    fn test_quantum_fraction_estimates() {
        ...
        assert!(
            sample.quantum_fraction > 0.1,
            "Camera should have higher quantum fraction"
        );
    }
}
