...
impl CameraQuantumRNG {
    ...
    pub fn self_test(&mut self, sample_size: usize) -> QuantumSelfTest {
        ...
        let runs_score = 1.0
            - ((runs as isize - expected_runs as isize).abs() as f64 / expected_runs as f64)
                .min(1.0);
...
"#,
            ...
            if self.autocorrelation.abs() < 0.1 {
                "✓"
            } else {
                "⚠"
            },
...
mod tests {
    ...
    fn test_shot_noise_physics() {
        ...
        assert!(
            mean > 30.0 && mean < 100.0,
            "Mean should be around expected count"
        );
    }
