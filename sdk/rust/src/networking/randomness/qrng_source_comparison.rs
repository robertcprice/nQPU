...
impl EntropyPoolSource {
    ...
    pub fn collect(&self) -> Result<Vec<u64>, String> {
        println!(
            "  Collecting {} bytes from /dev/urandom...",
            self.config.n_samples
        );
...
pub fn run_source_comparison(config: SourceConfig) -> Result<Vec<SourceResult>, String> {
    ...
    println!(
        "║  Debiasing: {:>46}║",
        if config.debias { "Von Neumann" } else { "None" }
    );
    println!("╚══════════════════════════════════════════════════════════════╝");
    println!();
    ...
    println!(
        "    Bits:   {} raw → {} final",
        entropy_raw_bits.len(),
        entropy_final_bits.len()
    );
...
fn print_comparison_summary(results: &[SourceResult]) {
    ...
    if cpu.stats.cv > ssd.stats.cv {
        ...
        println!(
            "ANSWER: YES - CPU jitter shows {:.1}x HIGHER variance than SSD timing.",
            ratio
        );
        println!();
        ...
        println!(
            "ANSWER: NO - SSD timing shows {:.1}x HIGHER variance than CPU jitter.",
            ratio
        );
        println!();
        println!("INTERPRETATION:");
    ...
    if let (Some(ssd_nist), Some(cpu_nist)) = (&ssd.nist_result, &cpu.nist_result) {
        ...
        println!(
            "  SSD:   {}/15 passed ({:.0}%)",
            ssd_nist.passed_count,
            ssd_nist.passed_count as f64 / 15.0 * 100.0
        );
        println!(
            "  CPU:   {}/15 passed ({:.0}%)",
            cpu_nist.passed_count,
            cpu_nist.passed_count as f64 / 15.0 * 100.0
        );
        println!();
...
fn save_comparison_results(results: &[SourceResult], config: &SourceConfig) -> Result<(), String> {
    ...
    let report_path = config
        .output_dir
        .join(format!("SOURCE_COMPARE_{}.md", timestamp));
    ...
    std::fs::write(&report_path, &report).map_err(|e| format!("Cannot write report: {}", e))?;
