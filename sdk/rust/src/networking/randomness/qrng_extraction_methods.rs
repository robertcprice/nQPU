...
pub fn run_extraction_experiments(
        ...
        let (pass_str, ent_str, rating) = if let Some(ref nist) = result.nist_result {
            ...
            (
                format!(
                    "{}/15 ({:.0}%)",
                    nist.passed_count,
                    nist.passed_count as f64 / 15.0 * 100.0
                ),
...
fn save_extraction_results(
    ...
    let report_path = config
        .output_dir
        .join(format!("EXTRACTION_METHODS_{}.md", timestamp));
