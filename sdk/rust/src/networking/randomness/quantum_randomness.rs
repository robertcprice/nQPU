...
impl RandomnessComparison {
    ...
    pub fn compare(count: usize) -> Self {
        ...
        let quantum_bytes: Vec<u8> = (0..count).map(|_| qrng.random_byte()).collect();
...
mod tests {
    ...
    fn test_entangled_pair_measurement() {
        ...
        assert!(
            count_00 + count_11 == 1000,
            "All measurements should give |00⟩ or |11⟩ for |Φ+⟩"
        );
    }
    ...
    fn test_bell_verification() {
        ...
        println!(
            "Quantum confidence = {:.1}%",
            verification.quantum_confidence() * 100.0
        );

        // Should exceed classical bound
        assert!(
            verification.s_value > 2.0,
