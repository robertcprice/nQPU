...
impl SsdQrngPhase2 {
    ...
    pub fn run(&mut self) -> Result<Phase2Result, String> {
        ...
        println!(
            "║  Methodology: {:>46}║",
            "SSD timing → LSB extraction → NIST"
        );
        println!("╚══════════════════════════════════════════════════════════════╝");
        println!();
        ...
        println!(
            "  Using {} least significant bits per timing",
    ...
    fn generate_methodology_notes(
        ...
        notes.push_str(&format!(
            "- Measured CV = {:.2}% (>1% = quantum-consistent)\n\n",
            stats.cv * 100.0
        ));
    ...
    fn save_results(&self, result: &Phase2Result) -> Result<(), String> {
        ...
        let method_path = self
            .config
            .output_dir
            .join(format!("{}_methodology.md", result.id));
        ...
        let data_path = self
            .config
            .output_dir
            .join(format!("{}_timings.csv", result.id));
        let mut file =
        ...
        let bits_path = self
            .config
            .output_dir
            .join(format!("{}_bits.txt", result.id));
        let bits_str: String = result
            .final_bits
            .iter()
            .map(|b| char::from(b + 48))
            .collect();
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            if result.timing_stats.cv > 0.01 {
                "✅ QUANTUM"
            } else {
                "❌ Classical"
            },
