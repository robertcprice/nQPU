...
impl CertifiedQrngSource {
    ...
    pub fn generate_certified_bits(&mut self) -> Result<BellTestCertificate, CertifiedQrngError> {
        ...
        for _ in 0..self.config.chsh_rounds {
            ...
            backend
                ...
                .map_err(|e| CertifiedQrngError::ExtractionFailed(format!("gate error: {}", e)))?;
            backend
            ...
            if a == 1 {
                backend
                    .apply_gate(&Gate::single(GateType::Ry(-std::f64::consts::FRAC_PI_2), 0))
                    .map_err(|e| {
    ...
    fn fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), QrngError> {
        ...
        while written < dest.len() {
            ...
            if available > 0 {
                ...
                self.generate_certified_bits()
                    .map_err(|e| QrngError::DeviceError(format!("certified QRNG error: {}", e)))?;
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_extraction_output_has_bytes() {
        let config = CertifiedQrngConfig::new().chsh_rounds(500).min_s_value(2.0);
        let mut source = CertifiedQrngSource::new(config);
