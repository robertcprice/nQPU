    ...
    fn open_device(&mut self) -> Result<&mut std::fs::File, QrngError> {
        if self.file.is_none() {
            let f = std::fs::File::open(&self.device_path).map_err(|e| {
                QrngError::DeviceError(format!("failed to open {}: {}", self.device_path, e))
            })?;
            self.file = Some(f);
        }
...
impl RngCore for QrngRng {
    ...
    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), rand::Error> {
        ...
        while written < dest.len() {
            ...
            if available == 0 {
                self.refill().map_err(|e| rand::Error::new(e))?;
                continue;
            }
...
impl HybridQrngSource {
    ...
    pub fn new(primary: Box<dyn QrngSource>, fallback: Box<dyn QrngSource>) -> Self {
        Self {
            primary,
            fallback,
            fallback_count: 0,
...
impl QrngSource for HybridQrngSource {
    ...
    fn source_info(&self) -> QrngSourceInfo {
        ...
        QrngSourceInfo {
            name: format!("Hybrid ({} -> {})", primary_info.name, fallback_info.name),
...
mod tests {
    ...
    fn hardware_source_nonexistent_device_returns_error() {
        let mut source = HardwareQrngSource::new("/dev/nonexistent_qrng_device_12345".into());
        let mut buf = [0u8; 8];
        let result = source.fill_bytes(&mut buf);
