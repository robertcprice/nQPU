...
impl QuantumLink {
    pub fn new(a: usize, b: usize, fidelity: f64, success_prob: f64, latency: f64, capacity: usize) -> Self {
        Self {
            node_a: a,
            node_b: b,
...
impl Route {
    ...
    pub fn num_hops(&self) -> usize {
        if self.path.len() < 2 { 0 } else { self.path.len() - 1 }
    }
...
pub fn shortest_path(network: &QuantumNetwork, src: usize, dst: usize) -> Result<Route, RoutingError> {
    dijkstra_route(network, src, dst, |_link| 1.0, &HashMap::new())
}

/// Find the highest-fidelity path using -log(fidelity) as edge weight.
pub fn highest_fidelity_path(network: &QuantumNetwork, src: usize, dst: usize) -> Result<Route, RoutingError> {
    dijkstra_route(
        network,
        src,
        dst,
...
pub fn max_throughput_path(network: &QuantumNetwork, src: usize, dst: usize) -> Result<Route, RoutingError> {
    dijkstra_route(
        network,
        src,
        dst,
...
fn dijkstra_route(
    ...
    dist[src] = 0.0;
    heap.push(DijkstraState { cost: 0.0, node: src });
    ...
    let fidelities: Vec<f64> = link_indices.iter().map(|&li| network.links[li].fidelity).collect();
    let probs: Vec<f64> = link_indices.iter().map(|&li| network.links[li].success_probability).collect();
    let latencies: Vec<f64> = link_indices.iter().map(|&li| network.links[li].latency_ms).collect();
    let capacities: Vec<usize> = link_indices.iter().map(|&li| network.links[li].capacity).collect();
...
fn evaluate_path(network: &QuantumNetwork, path: &[usize], link_loads: &HashMap<usize, usize>, alpha: f64) -> Route {
    if path.len() < 2 {
        return Route {
            path: path.to_vec(),
            end_to_end_fidelity: 1.0,
...
impl NetworkPerformance {
    ...
    pub fn from_routes(routes: &[Route], network: &QuantumNetwork) -> Self {
        ...
        let congestion = if network.num_links() > 0 {
            let total: f64 = loads.iter().map(|(&li, &load)| {
                let cap = network.links[li].capacity as f64;
                if cap > 0.0 { load as f64 / cap } else { 0.0 }
            }).sum();
    ...
    pub fn solve(
        ...
        for _iter in 0..self.max_iterations {
            ...
            for i in 0..n {
                ...
                let mut best_cost = evaluate_path(network, &current_paths[i], &loads, config.congestion_alpha)
                    .cost(0.0, 0.0);
        ...
        for i in 0..n {
            ...
            for path in &all_paths[i] {
                ...
                let cost: f64 = trial_paths
                    .iter()
                    .map(|p| evaluate_path(network, p, &trial_loads, config.congestion_alpha).cost(0.0, 0.0))
                    .sum();
...
impl RoutingGame {
    pub fn new(network: QuantumNetwork, requests: Vec<RoutingRequest>, strategy: RoutingStrategy) -> Self {
        Self {
            network,
            requests,
            strategy,
    ...
    pub fn solve(&self) -> Result<GameResult, RoutingError> {
        match &self.strategy {
            RoutingStrategy::ShortestPath => self.solve_independent(|net, src, dst, _loads| {
                shortest_path(net, src, dst)
            }),
            RoutingStrategy::HighestFidelity => self.solve_independent(|net, src, dst, _loads| {
                highest_fidelity_path(net, src, dst)
            }),
            RoutingStrategy::MaxThroughput => self.solve_independent(|net, src, dst, _loads| {
                max_throughput_path(net, src, dst)
            }),
    ...
    fn solve_independent(
        &self,
        route_fn: impl Fn(&QuantumNetwork, usize, usize, &HashMap<usize, usize>) -> Result<Route, RoutingError>,
        ...
        let poa = if opt_cost > 1e-12 { selfish_cost / opt_cost } else { 1.0 };
    ...
    fn solve_nash(&self) -> Result<GameResult, RoutingError> {
        ...
        let poa = if opt_cost > 1e-12 { nash_cost / opt_cost } else { 1.0 };
...
impl QuantumNetworkLibrary {
    ...
    pub fn line_network(n: usize) -> QuantumNetwork {
        ...
        for i in 0..n {
            net.add_node(
                QuantumNode::new(i, &format!("R{}", i), 5, 0.5)
    ...
    pub fn grid_network(rows: usize, cols: usize) -> QuantumNetwork {
        ...
        for r in 0..rows {
            for c in 0..cols {
                // Horizontal link
                if c + 1 < cols {
                    net.add_link(QuantumLink::new(idx(r, c), idx(r, c + 1), 0.92, 0.85, 3.0, 8));
                }
                // Vertical link
                if r + 1 < rows {
                    net.add_link(QuantumLink::new(idx(r, c), idx(r + 1, c), 0.92, 0.85, 3.0, 8));
                }
            }
        }
        net
    ...
    pub fn braess_scenario(num_agents: usize) -> (QuantumNetwork, Vec<RoutingRequest>, QuantumLink) {
...
mod tests {
    ...
    fn test_e2e_fidelity_two_hop() {
        ...
        assert!((f - expected).abs() < 1e-6, "got {}, expected {}", f, expected);
    }
    ...
    fn test_purification_requires_multiple_rounds() {
        ...
        assert!(rounds >= 2, "Low initial fidelity requires multiple rounds: got {}", rounds);
    }
    ...
    fn test_nash_equilibrium_converges() {
        let net = QuantumNetworkLibrary::diamond_network();
        let requests = vec![
            RoutingRequest { source: 0, destination: 3, fidelity_threshold: 0.5, num_pairs: 5 },
            RoutingRequest { source: 0, destination: 3, fidelity_threshold: 0.5, num_pairs: 5 },
        ];
    ...
    fn test_no_paradox_good_link() {
        ...
        let requests = vec![
            RoutingRequest { source: 0, destination: 2, fidelity_threshold: 0.5, num_pairs: 5 },
        ];
        let config = RoutingConfig::default();
    ...
    fn test_grid_network_multiple_paths() {
        ...
        assert!(paths.len() > 1, "Grid should have multiple paths: found {}", paths.len());
    }
    ...
    fn test_large_network_many_agents() {
        ...
        let game = RoutingGame::new(net, requests, RoutingStrategy::NashEquilibrium)
            .with_config(RoutingConfig::default().max_hops(8).nash_max_iterations(100));
