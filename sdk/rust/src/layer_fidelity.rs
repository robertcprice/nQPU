...
fn simulate_twirled_circuit(
    layer: &CircuitLayer,
    twirled: &TwirledCircuit,
) -> f64 {
    let dim = 1 << layer.num_qubits;
    let layer_u = build_layer_unitary(layer);
...
pub fn extract_noise_model(result: &LayerFidelityResult) -> LayerNoiseModel {
    ...
    let per_qubit_baseline = 1.0 - (1.0 - result.process_infidelity).powf(1.0 / result.num_qubits as f64);
...
mod tests {
    ...
    fn test_circuit_layer_creation() {
        ...
        let bad2 = CircuitLayer::new(2)
            .add_gate(LayerGateType::H, vec![5]);
        assert!(bad2.validate().is_err());

        // Invalid: wrong arity.
        let bad3 = CircuitLayer::new(3)
            .add_gate(LayerGateType::CNOT, vec![0]);
        assert!(bad3.validate().is_err());
    }
    ...
    fn test_single_qubit_layer_fidelity() {
        ...
        assert_eq!(result.sequence_lengths.len(), result.survival_probabilities.len());
    }
    ...
    fn test_decay_fitting() {
        ...
        let y: Vec<f64> = x.iter().map(|&xi| a_true * p_true.powf(xi) + b_true).collect();

        let (a_fit, p_fit, b_fit) = fit_exponential_decay(&x, &y).unwrap();

        assert!(
            approx_eq(a_fit, a_true, 0.05),
            "fitted A={} should be near {}", a_fit, a_true
        );
        assert!(
            approx_eq(p_fit, p_true, 0.02),
            "fitted p={} should be near {}", p_fit, p_true
        );
        assert!(
            approx_eq(b_fit, b_true, 0.05),
            "fitted B={} should be near {}", b_fit, b_true
        );
    ...
    fn test_scaling_fidelity_decreases_with_width() {
        ...
        let layer_1q = CircuitLayer::new(1)
            .add_gate(LayerGateType::H, vec![0]);
        ...
        assert!(
            ...
            "1q fidelity = {}", result_1q.layer_fidelity
        );
        assert!(
            result_2q.layer_fidelity > 0.75,
            "2q fidelity = {}", result_2q.layer_fidelity
        );
        assert!(
            result_3q.layer_fidelity > 0.60,
            "3q fidelity = {}", result_3q.layer_fidelity
        );
