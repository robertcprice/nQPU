...
impl BranchingUniverse {
    ...
    pub fn branch(&self, idx: usize) -> Result<&Branch, ManyWorldsError> {
        self.branches.get(idx).ok_or(ManyWorldsError::InvalidBranch {
            branch_id: idx,
    ...
    pub fn measure_all(&mut self, qubit: usize) -> Result<Vec<usize>, ManyWorldsError> {
        ...
        for (parent_idx, state, parent_weight, parent_history) in branch_data {
            let (branch_0, branch_1) =
                self.create_measurement_branches(&state, qubit, parent_weight, &parent_history, parent_idx, step);
    ...
    pub fn decoherence_functional(
        ...
        // Validate projectors
        for proj in history_alpha.projectors.iter().chain(history_beta.projectors.iter()) {
            if proj.qubit >= nq {
                return Err(ManyWorldsError::QubitOutOfRange {
                    qubit: proj.qubit,
                    num_qubits: nq,
    ...
    pub fn check_consistency(
        ...
        for alpha in 0..n {
            for beta in 0..n {
                let d = self.decoherence_functional(initial_state, &histories[alpha], &histories[beta])?;
...
impl UniverseSnapshot {
    ...
    pub fn children_of(&self, index: usize) -> &[usize] {
        self.children.get(&index).map(|v| v.as_slice()).unwrap_or(&[])
    }
...
mod tests {
    ...
    fn test_gate_applies_to_all_branches() {
        ...
        for b in universe.active_branches() {
            ...
            let p1_0: f64 = probs.iter().enumerate()
                .filter(|(i, _)| (i >> 1) & 1 == 0)
                .map(|(_, p)| p)
                .sum();
            let p1_1: f64 = probs.iter().enumerate()
                .filter(|(i, _)| (i >> 1) & 1 == 1)
                .map(|(_, p)| p)
                .sum();
            assert!(
    ...
    fn test_branch_interference_overlapping() {
        ...
        assert!(
            !universe3.can_interfere(active3[0], active3[1], 0.01).unwrap(),
    ...
    fn test_measurement_history_tracking() {
        ...
        for b in universe.active_branches() {
            ...
            assert_eq!(b.history[0].qubit, 0, "First measurement should be on qubit 0");
            assert_eq!(b.history[1].qubit, 1, "Second measurement should be on qubit 1");
    ...
    fn test_measure_specific_branch() {
        ...
        assert!(!universe.branches[0].active, "Original branch should be retired");
