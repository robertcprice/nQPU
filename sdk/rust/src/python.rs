...
impl PyDifferentiableCircuit {
    ...
    fn h(&mut self, qubit: usize) {
        self.inner.add_gate(pytorch_bridge::ParametricGate::H(qubit));
    }

    /// Add a Pauli-X gate.
    fn x(&mut self, qubit: usize) {
        self.inner.add_gate(pytorch_bridge::ParametricGate::X(qubit));
    }

    /// Add a Pauli-Z gate.
    fn z(&mut self, qubit: usize) {
        self.inner.add_gate(pytorch_bridge::ParametricGate::Z(qubit));
    }
    ...
    fn crx(&mut self, control: usize, target: usize, param_idx: usize) {
        self.inner
            .add_gate(pytorch_bridge::ParametricGate::CRx(control, target, param_idx));
    }

    /// Add a parametric controlled-Ry rotation.
    fn cry(&mut self, control: usize, target: usize, param_idx: usize) {
        self.inner
            .add_gate(pytorch_bridge::ParametricGate::CRy(control, target, param_idx));
    }

    /// Add a parametric controlled-Rz rotation.
    fn crz(&mut self, control: usize, target: usize, param_idx: usize) {
        self.inner
            .add_gate(pytorch_bridge::ParametricGate::CRz(control, target, param_idx));
    }
    ...
    fn u3(&mut self, qubit: usize, theta_idx: usize, phi_idx: usize, lambda_idx: usize) {
        self.inner
            .add_gate(pytorch_bridge::ParametricGate::U3(qubit, theta_idx, phi_idx, lambda_idx));
    }
...
impl PyJAXCircuit {
    ...
    fn __repr__(&self) -> String {
        format!("<JAXCircuit {} qubits, {} params>", self.n_qubits, self.inner.num_parameters())
    }
}
...
impl PyMockProvider {
    ...
    fn run(&self, circuit: &PyQPUCircuit, backend: &str, shots: usize) -> PyResult<std::collections::HashMap<String, usize>> {
        use crate::qpu::{QPUProvider, QPUJob, JobConfig};
        ...
        let result = self
            .runtime
            .block_on(job.wait_for_completion(
...
impl PySlidingWindowDecoder {
    ...
    fn new(window_size: usize, slide_step: usize, code_distance: usize, decoder: &str) -> PyResult<Self> {
        let inner_decoder = match decoder {
            ...
            other => return Err(PyValueError::new_err(
                format!("Unknown decoder '{}', expected 'greedy' or 'union_find'", other)
            )),
        };
        if window_size < 2 {
        ...
        if slide_step < 1 || slide_step > window_size {
            return Err(PyValueError::new_err("slide_step must be in [1, window_size]"));
        }
        Ok(Self {
            inner: crate::sliding_window_decoder::SlidingWindowDecoder::new(
                window_size, slide_step, code_distance, inner_decoder,
            ),
        })
    }
    ...
    fn push_round(&mut self, round_id: usize, syndrome: Vec<bool>, timestamp: f64) {
        self.inner.push_round(crate::sliding_window_decoder::SyndromeRound {
            round_id,
            syndrome,
            timestamp,
        });
    ...
    fn decode_window(&mut self, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        if !self.inner.ready() {
            return Err(PyValueError::new_err("Not enough rounds buffered; call push_round first"));
        }
...
impl PyBpOsdDecoder {
    ...
    fn new(
        ...
        let bp_method = match method {
            ...
            other => return Err(PyValueError::new_err(
                format!("Unknown BP method '{}', expected 'min_sum' or 'sum_product'", other)
            )),
        };
    ...
    fn decode(&self, py: Python<'_>, syndrome: Vec<bool>, p: f64) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        if p <= 0.0 || p >= 1.0 {
...
impl PyMagicStateFactory {
    ...
    #[new]
    fn new(protocol: &str, levels: usize, code_distance: usize, physical_error_rate: f64) -> PyResult<Self> {
        ...
        if physical_error_rate <= 0.0 || physical_error_rate >= 1.0 {
            return Err(PyValueError::new_err("physical_error_rate must be in (0, 1)"));
        }
        Ok(Self {
            inner: crate::magic_state_factory::MagicStateFactory::new(
                proto, levels, code_distance, physical_error_rate,
            ),
        })
    }
    ...
    fn distill(&self, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        ...
        dict.set_item("total_raw_states_consumed", result.total_raw_states_consumed)?;
    ...
    #[staticmethod]
    fn estimate_resources(py: Python<'_>, target_error: f64, physical_error: f64, code_distance: usize) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        if code_distance < 3 {
            return Err(PyValueError::new_err("code_distance must be >= 3"));
        }
        let est = crate::magic_state_factory::MagicStateFactory::estimate_resources(
            target_error, physical_error, code_distance,
        );
        let dict = pyo3::types::PyDict::new(py);
...
#[cfg(feature = "python")]
fn parse_distillation_protocol(s: &str) -> PyResult<crate::magic_state_factory::DistillationProtocol> {
    match s {
...
impl PyAdaptVqe {
    ...
    fn new(
        ...
        let pool = match pool_type {
            ...
            "qubit_adapt" | "qubit" => {
                crate::adapt_vqe::OperatorPool::qubit_adapt_pool(n_qubits)
            }
            other => return Err(PyValueError::new_err(
                format!("Unknown pool type '{}'. Use 'gsd' or 'qubit_adapt'", other)
            )),
        };
    ...
    fn run(&self, py: Python<'_>, hamiltonian: &PyMolecularHamiltonian) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
...
impl PyWalkGraph {
    ...
    fn line(n: usize) -> PyResult<Self> {
        if n < 2 {
            return Err(PyValueError::new_err("Line graph requires at least 2 vertices"));
        }
        Ok(Self { inner: crate::quantum_walk::Graph::line(n) })
    }
    ...
    fn cycle(n: usize) -> PyResult<Self> {
        if n < 3 {
            return Err(PyValueError::new_err("Cycle graph requires at least 3 vertices"));
        }
        Ok(Self { inner: crate::quantum_walk::Graph::cycle(n) })
    }
    ...
    fn complete(n: usize) -> PyResult<Self> {
        if n < 2 {
            return Err(PyValueError::new_err("Complete graph requires at least 2 vertices"));
        }
        Ok(Self { inner: crate::quantum_walk::Graph::complete(n) })
    }
    ...
    fn grid_2d(rows: usize, cols: usize) -> PyResult<Self> {
        ...
        Ok(Self { inner: crate::quantum_walk::Graph::grid_2d(rows, cols) })
    }
    ...
    fn hypercube(dim: usize) -> PyResult<Self> {
        ...
        Ok(Self { inner: crate::quantum_walk::Graph::hypercube(dim) })
    }
    ...
    fn star(n: usize) -> PyResult<Self> {
        if n < 2 {
            return Err(PyValueError::new_err("Star graph requires at least 2 vertices"));
        }
        Ok(Self { inner: crate::quantum_walk::Graph::star(n) })
    }
    ...
    fn __repr__(&self) -> String {
        format!("WalkGraph(vertices={}, max_degree={})", self.inner.n_vertices, self.inner.max_degree())
    }
}
...
impl PyContinuousWalk {
    ...
    fn new(graph: &PyWalkGraph, initial_vertex: usize, gamma: f64) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: crate::quantum_walk::ContinuousWalk::new(
                graph.inner.clone(), initial_vertex, gamma,
            ),
        })
    }
    ...
    #[staticmethod]
    fn search(py: Python<'_>, graph: &PyWalkGraph, marked: Vec<usize>, gamma: f64, t: f64) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        let result = crate::quantum_walk::ContinuousWalk::search(
            &graph.inner, &marked, gamma, t,
        );
        let dict = pyo3::types::PyDict::new(py);
...
impl PyDiscreteWalk {
    ...
    fn new(graph: &PyWalkGraph, initial_vertex: usize, coin: &str) -> PyResult<Self> {
        ...
        let coin_op = match coin {
            ...
            other => return Err(PyValueError::new_err(
                format!("Unknown coin '{}'. Use 'hadamard', 'grover', or 'dft'", other)
            )),
        };
        Ok(Self {
            inner: crate::quantum_walk::DiscreteWalk::new(
                graph.inner.clone(), initial_vertex, coin_op,
            ),
        })
    }
    ...
    fn search(py: Python<'_>, graph: &PyWalkGraph, marked: Vec<usize>, coin: &str, n_steps: usize) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        let coin_op = match coin {
            "hadamard" | "h" => crate::quantum_walk::CoinOperator::Hadamard,
            "grover" | "g" => crate::quantum_walk::CoinOperator::Grover,
            "dft" | "fourier" => crate::quantum_walk::CoinOperator::DFT,
        ...
        let result = crate::quantum_walk::DiscreteWalk::search(
            &graph.inner, &marked, coin_op, n_steps,
        );
        let dict = pyo3::types::PyDict::new(py);
        dict.set_item("success_probability", result.success_probability)?;
        dict.set_item("marked_probabilities", result.marked_probabilities)?;
...
impl PyIsingModel {
    ...
    fn from_qubo(n: usize, q: Vec<(usize, usize, f64)>) -> Self {
        Self { inner: crate::quantum_annealing::IsingModel::from_qubo(n, &q) }
    }

    /// Create a max-cut problem.
    #[staticmethod]
    fn max_cut(n_vertices: usize, edges: Vec<(usize, usize)>) -> Self {
        Self { inner: crate::quantum_annealing::problems::max_cut(n_vertices, &edges) }
    }

    /// Create a number partitioning problem.
    #[staticmethod]
    fn number_partitioning(numbers: Vec<f64>) -> Self {
        Self { inner: crate::quantum_annealing::problems::number_partitioning(&numbers) }
    }
    ...
    #[getter]
    fn n_spins(&self) -> usize { self.inner.n_spins }

    fn __repr__(&self) -> String {
        format!("IsingModel(n_spins={}, couplings={})", self.inner.n_spins, self.inner.j.len())
    }
}
...
impl PyQuantumAnnealer {
    ...
    fn new(
        ...
        let sched = match schedule {
            ...
            other => return Err(PyValueError::new_err(
                format!("Unknown schedule '{}'. Use 'linear' or 'quadratic'", other)
            )),
        };
        ...
        Ok(Self {
            inner: crate::quantum_annealing::QuantumAnnealer::new(
                model.inner.clone(), config,
            ),
        })
    }
    ...
    fn __repr__(&self) -> String {
        format!("QuantumAnnealer(n_spins={}, n_steps={})",
...
impl PyFermionicGaussianState {
    ...
    fn new(n_modes: usize) -> Self {
        Self { inner: crate::fermionic_gaussian::FermionicGaussianState::new(n_modes) }
    }
    ...
    fn occupation_numbers(&self) -> Vec<f64> {
        (0..self.inner.n_modes).map(|i| self.inner.occupation_number(i)).collect()
    }
    ...
    #[getter]
    fn n_modes(&self) -> usize { self.inner.n_modes }

    fn __repr__(&self) -> String {
        format!("FermionicGaussianState(n_modes={}, particles={:.1})",
            self.inner.n_modes, self.inner.total_particle_number())
    }
}
...
impl PyRBMState {
    ...
    fn new(n_visible: usize, n_hidden: usize, seed: u64) -> Self {
        Self { inner: crate::neural_quantum_states::RBMState::new_seeded(n_visible, n_hidden, seed) }
    }
    ...
    #[getter]
    fn n_visible(&self) -> usize { self.inner.n_visible }

    #[getter]
    fn n_hidden(&self) -> usize { self.inner.n_hidden }

    fn __repr__(&self) -> String {
        format!("RBMState(n_visible={}, n_hidden={}, params={})",
            self.inner.n_visible, self.inner.n_hidden, self.inner.num_params())
    }
}
...
impl PyVMCOptimizer {
    ...
    fn optimize_heisenberg(
        ...
        let ham = crate::neural_quantum_states::SpinHamiltonian::heisenberg_1d(n_sites, j_coupling, h_field);
        let optimizer = crate::neural_quantum_states::VMCOptimizer::new(self.config.clone());
        let result = optimizer.optimize(&mut state.inner, &ham)
            .map_err(|e| PyValueError::new_err(format!("{:?}", e)))?;
        let dict = pyo3::types::PyDict::new(py);
        dict.set_item("energy_history", result.energy_history)?;
    ...
    fn optimize_ising(
        ...
        let ham = crate::neural_quantum_states::SpinHamiltonian::ising_1d(n_sites, j_coupling, h_field);
        let optimizer = crate::neural_quantum_states::VMCOptimizer::new(self.config.clone());
        let result = optimizer.optimize(&mut state.inner, &ham)
            .map_err(|e| PyValueError::new_err(format!("{:?}", e)))?;
        let dict = pyo3::types::PyDict::new(py);
    ...
    fn __repr__(&self) -> String {
        format!("VMCOptimizer(iters={}, samples={}, lr={})",
...
use crate::quantum_drug_design::{
    self as drug_design, AdmetPredictor as RustAdmetPredictor, AdmetProperty, BondType as DrugBondType,
    DockingConfig, DockingResult, DrugLikenessResult as RustDrugLikenessResult,
    DrugProperties, Element as DrugElement, Molecule as RustMolecule,
    QuantumDockingScorer as RustQuantumDockingScorer, ScoringFunction,
    DrugDiscoveryPipeline as RustDrugDiscoveryPipeline, PipelineStage,
    lead_optimization, MolecularFingerprint,
};
...
impl PyMolecule {
    ...
    fn add_atom(&mut self, element: String, position: Vec<f64>, charge: Option<f64>) -> PyResult<usize> {
        if position.len() != 3 {
            return Err(PyValueError::new_err("Position must have 3 elements [x, y, z]"));
        }
        let elem = match element.as_str() {
            ...
            "Fe" => DrugElement::Fe,
            "Zn" => DrugElement::Zn,
            _ => return Err(PyValueError::new_err(format!("Unknown element: {}", element))),
        };
    ...
    fn add_bond(&mut self, atom1: usize, atom2: usize, bond_type: Option<String>) -> PyResult<()> {
        let bt = match bond_type.as_deref().unwrap_or("single") {
            ...
            _ => return Err(PyValueError::new_err(format!("Unknown bond type: {:?}", bond_type))),
        };
    ...
    fn __repr__(&self) -> String {
        format!("<Molecule '{}' atoms={} bonds={}>",
            self.inner.name, self.inner.atoms.len(), self.inner.bonds.len())
    }
}
...
impl PyDrugLikenessResult {
    ...
    fn properties(&self, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        ...
        dict.set_item("polar_surface_area", self.inner.properties.polar_surface_area)?;
        Ok(dict.into())
    }

    fn __repr__(&self) -> String {
        format!("<DrugLikenessResult qed={:.3} violations={}>",
...
impl PyAdmetPredictor {
    ...
    fn new(num_qubits: usize, properties: Vec<String>) -> Self {
        let props: Vec<AdmetProperty> = properties.iter().filter_map(|p| match p.as_str() {
    ...
    fn predict(&self, mol: &PyMolecule, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        let results = self.inner.predict(&mol.inner)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
    ...
    fn __repr__(&self) -> String {
        format!("<AdmetPredictor qubits={} properties={}>",
            self.inner.num_qubits, self.inner.properties.len())
    }
}
...
fn py_predict_admet(mol: &PyMolecule, properties: Option<Vec<String>>, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
    let props = properties.unwrap_or_else(|| vec![
...
impl PyQuantumDockingScorer {
    ...
    fn score(&self, protein: &PyMolecule, ligand: &PyMolecule, py: Python<'_>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        let result = self.inner.score(&protein.inner, &ligand.inner)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
    ...
    fn __repr__(&self) -> String {
        format!("<QuantumDockingScorer qubits={} conformations={}>",
...
impl PyDrugDiscoveryPipeline {
    ...
    fn run(&mut self, candidates: Vec<PyMolecule>, protein: &PyMolecule, py: Python<'_>) -> PyResult<Vec<pyo3::Py<pyo3::types::PyDict>>> {
        let rust_candidates: Vec<RustMolecule> = candidates.iter().map(|c| c.inner.clone()).collect();

        let stage_results = self.inner.run(&rust_candidates, &protein.inner)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
    ...
    fn __repr__(&self) -> String {
        format!("<DrugDiscoveryPipeline stages={} qubits={}>",
            self.inner.stages.len(), self.inner.num_qubits)
    }
}
...
mod tests {
    ...
    fn test_quantum_simulator_initial_state() {
        ...
        for i in 1..8 {
            assert!(amps[i].re.abs() < 1e-12, "amplitude[{}] re was {}", i, amps[i].re);
            assert!(amps[i].im.abs() < 1e-12, "amplitude[{}] im was {}", i, amps[i].im);
        }
    }
    ...
    fn test_hadamard_creates_superposition() {
        ...
        let probs: Vec<f64> = sim.state.amplitudes_ref()
            .iter()
            .map(|c| c.re * c.re + c.im * c.im)
            .collect();
    ...
    fn test_ghz_state_circuit() {
        ...
        assert!((probs[dim - 1] - 0.5).abs() < 1e-10, "P(1111) = {}", probs[dim - 1]);
        // All others are 0
        let sum_other: f64 = probs[1..dim - 1].iter().sum();
        assert!(sum_other.abs() < 1e-10, "sum of non-GHZ amplitudes = {}", sum_other);
    }
    ...
    fn test_grover_optimal_iterations() {
        for num_qubits in 2..=8 {
            ...
            let optimal = ((std::f64::consts::PI / 4.0) * (max_states as f64).sqrt()).ceil() as usize;
            // Grover optimal iterations grows as sqrt(N)
            assert!(optimal >= 1, "num_qubits={}, optimal={}", num_qubits, optimal);
    ...
    fn test_simulator_reset() {
        ...
        assert!(prob_000_before < 0.9, "State should not be |000> after gates");
        ...
        assert!((prob_000_after - 1.0).abs() < 1e-12, "After reset: P(000) = {}", prob_000_after);
    }
    ...
    fn test_qubit_bounds_validation_pattern() {
        ...
        assert!(3 >= num_qubits, "Qubit 3 should be invalid for 3-qubit circuit");
    ...
    fn test_unitarity_through_gate_sequence() {
        ...
        let norm_sq: f64 = sim.state.amplitudes_ref()
            .iter()
            .map(|c| c.re * c.re + c.im * c.im)
            .sum();
