...
#[cfg(feature = "cuda")]
use cudarc::driver::{CudaDevice, CudaFunction, DevicePtr, LaunchConfig, LaunchAsync};
#[cfg(feature = "cuda")]
...
impl CudaParallelQuantumExecutor {
    ...
    pub fn new() -> Result<Self, CudaError> {
        #[cfg(feature = "cuda")]
        {
            let device = CudaDevice::new(0).map_err(|e| CudaError::ContextError(format!("{:?}", e)))?;
            Ok(Self {
                device,
    ...
    pub fn load_kernels(&mut self) -> Result<(), CudaError> {
        #[cfg(feature = "cuda")]
        {
            ...
            self.device.load_ptx(ptx, "quantum_kernels", &[
    ...
    pub fn apply_h(&self, state: &mut [Complex64], num_qubits: usize, target: usize) -> Result<(), CudaError> {
        self.launch_gate_kernel("hadamard", state, num_qubits, target, &[])
    }

    /// Apply a Pauli-X gate on the GPU
    pub fn apply_x(&self, state: &mut [Complex64], num_qubits: usize, target: usize) -> Result<(), CudaError> {
        self.launch_gate_kernel("x_gate", state, num_qubits, target, &[])
    }

    /// Apply a CNOT gate on the GPU
    pub fn apply_cnot(&self, state: &mut [Complex64], num_qubits: usize, control: usize, target: usize) -> Result<(), CudaError> {
    ...
    fn launch_gate_kernel(
        ...
        {
            // 1. Copy Host -> Device
            let mut device_state = self.device.htod_copy(state.to_vec())
            ...
            let func = self.device.get_func("quantum_kernels", kernel_name)
                .ok_or_else(|| CudaError::LaunchError(format!("Kernel {} not found", kernel_name)))?;
            ...
            unsafe {
                if kernel_name == "cnot" {
                    ...
                    func.launch(cfg, (&mut device_state, num_qubits as i32, arg1 as i32, target))
                } else {
            ...
            // 4. Copy Device -> Host
            let result_vec = self.device.dtoh_sync_copy(&device_state)
    ...
    pub fn execute_batch(
        ...
        match gate_type {
            ...
            _ => return Err(CudaError::LaunchError(format!("Unsupported batch gate: {}", gate_type))),
        }
        Ok(())
    }
