...
impl PauliGroup {
    ...
    pub fn multiply_paulis(a: &GateType, b: &GateType) -> (C64, GateType) {
        let pa = PauliOp::from_gate_type(a)
            .expect("multiply_paulis: first argument is not a Pauli");
        let pb = PauliOp::from_gate_type(b)
            .expect("multiply_paulis: second argument is not a Pauli");
    ...
    pub fn compile(
        ...
        for i in 0..config.num_compilations {
            ...
            if config.twirl_single_qubit {
                compilations.push(self.compile_with_frame_tracking(
                    circuit, num_qubits, &mut rng,
                ));
            } else {
                compilations.push(self.compile_two_qubit_only(
                    circuit, num_qubits, &mut rng,
                ));
            }
        }
    ...
    for gate in gates {
        ...
        if let Some(curr_op) = current_pauli {
            ...
            let merge = if let Some(last) = result.last() {
                if last.is_single_qubit()
                    && last.targets[0] == gate.targets[0]
                {
...
mod tests {
    ...
    fn test_pauli_anticyclic_multiplication() {
        ...
        assert_eq!(result, PauliOp::Z);
        assert!(c64_approx_eq(phase, C64::new(0.0, -1.0), 1e-14), "Y*X phase = -i");

        // Z * Y = -iX
        let (phase, result) = PauliGroup::multiply_pauli_ops(PauliOp::Z, PauliOp::Y);
        assert_eq!(result, PauliOp::X);
        assert!(c64_approx_eq(phase, C64::new(0.0, -1.0), 1e-14), "Z*Y phase = -i");

        // X * Z = -iY
        let (phase, result) = PauliGroup::multiply_pauli_ops(PauliOp::X, PauliOp::Z);
        assert_eq!(result, PauliOp::Y);
        assert!(c64_approx_eq(phase, C64::new(0.0, -1.0), 1e-14), "X*Z phase = -i");
    }
    ...
    fn test_twirl_empty_circuit() {
        ...
        let mut twirler = PauliTwirler::with_seed(42);
        let twirled = twirler.twirl_circuit(&circuit, 2);
        assert!(twirled.is_empty(), "Twirling empty circuit should give empty");
    }
    ...
    fn test_randomized_compiler_with_frame_tracking() {
        ...
        let compiler = RandomizedCompiler::new();
        let config = RCConfig::new(4)
            .with_seed(42)
            .with_single_qubit_twirl(true);
        let compilations = compiler.compile(&circuit, 2, &config);
    ...
    fn test_is_pauli_identifies_paulis() {
        ...
        assert!(is_pauli(&GateType::Phase(0.0)), "Phase(0) is Pauli (Identity)");
    ...
    fn test_gate_commutation() {
        ...
        assert!(gate_commutes_with_pauli(&GateType::Phase(0.0), &GateType::X));
        assert!(gate_commutes_with_pauli(&GateType::X, &GateType::Phase(0.0)));
    ...
    fn test_simplify_paulis_merge() {
        ...
        assert_eq!(simplified[0].gate_type, GateType::Z, "X*Y = Z (up to phase)");
    }
