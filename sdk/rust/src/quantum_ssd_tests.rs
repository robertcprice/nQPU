...
impl SsdAntibunchingTest {
    ...
    pub fn run_test(&mut self, n_writes: usize) -> Result<SsdAntibunchingResult, String> {
        ...
        let time_span = (self.write_times.last().unwrap_or(&0) -
                        self.write_times.first().unwrap_or(&0)) as f64 / 1e9; // in seconds
        let _mean_rate = if time_span > 0.0 { total_writes / time_span } else { 1.0 };

        // Calculate g²(0) = variance / mean
        let mean_per_bin = total_writes / histogram.len() as f64;
        let variance: f64 = histogram.values()
        ...
        // Timing variance
        let timing_mean: f64 = self.write_times.iter().map(|&t| t as f64).sum::<f64>() / total_writes;
        let timing_variance: f64 = self.write_times.iter()
            .map(|&t| (t as f64 - timing_mean).powi(2))
            .sum::<f64>() / total_writes;
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            result.g2_zero,
            if result.is_quantum { "✅ QUANTUM" } else { "❌ Classical" },
...
impl MultiCellCorrelationTest {
    ...
    pub fn run_test(&mut self, n_writes: usize) -> Result<MultiCellCorrelationResult, String> {
        ...
        let is_quantum_like = adjacent_correlation.abs() > 0.3 &&
                              distant_correlation.abs() < 0.1;

        let interpretation = if is_quantum_like {
            "Adjacent correlation without distant correlation - possible quantum effect!".to_string()
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            if result.is_quantum_like { "✅ QUANTUM-LIKE" } else { "❌ Classical" },
            result.interpretation
        )
    }
}
...
impl CosmicRayLGTest {
    ...
    pub fn run_multi_trial(&mut self, n_trials: usize, duration_ms: u64) -> Result<CosmicRayLGResult, String> {
        let mut c12_sum = 0.0;
        let mut c23_sum = 0.0;
        let mut c13_sum = 0.0;
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            result.c13,
            result.k_value,
            if result.is_quantum { "✅ QUANTUM COHERENCE" } else { "❌ No quantum coherence" },
            result.confidence * 100.0
        )
    }
}
...
impl SimultaneousWriteTest {
    ...
    fn attempt_simultaneous_write(&mut self) -> (u64, u64, u64) {
        ...
        let diff = (write2_start.elapsed().as_nanos() as i64 -
                   write1_start.elapsed().as_nanos() as i64).abs() as u64;
    ...
    pub fn run_test(&mut self, n_attempts: usize) -> Result<SimultaneousWriteResult, String> {
        ...
        let avg_time_diff: f64 = self.time_diffs.iter().map(|&d| d as f64).sum::<f64>() /
                                  self.time_diffs.len() as f64;

        let variance: f64 = self.time_diffs.iter()
            .map(|&d| (d as f64 - avg_time_diff).powi(2))
            .sum::<f64>() / self.time_diffs.len() as f64;
        ...
        let interpretation = if min_time_diff > 100 {
            format!("Consistent minimum timing difference of {} ns detected. \
                    Could indicate quantum uncertainty limit!", min_time_diff)
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            result.std_diff_ns,
            if result.is_quantum { "✅ QUANTUM LIMIT" } else { "❌ Classical timing" },
            result.interpretation
        )
    }
}
...
impl HardwareQuantumSuite {
    ...
    pub fn run_all(&mut self) -> Result<HardwareQuantumReport, String> {
        ...
        if let Some(ref r) = ssd {
            if r.is_quantum { score += 1.0; }
            count += 1;
        }
        if let Some(ref r) = cells {
            if r.is_quantum_like { score += 1.0; }
            count += 1;
        }
