...
fn rz_matrix(theta: f64) -> [[Complex64; 2]; 2] {
    [
        [Complex64::new(0.0, -theta / 2.0).exp(), Complex64::new(0.0, 0.0)],
        [Complex64::new(0.0, 0.0), Complex64::new(0.0, theta / 2.0).exp()],
    ]
}
...
pub fn fubini_study_metric(
    ...
    for i in 0..n {
        for j in i..n {
            ...
            let di_dot_dj = inner_product_diff(&states_plus[i], &states_minus[i],
                                                &states_plus[j], &states_minus[j]);
        ...
        let nat_grad = match config.metric_method {
            MetricMethod::FullFubiniStudy => {
                ...
                let rhs = Array1::from_vec(
                    grad.iter().map(|g| -config.learning_rate * g).collect(),
                );
...
pub fn compare_optimizers(
    ...
    let vanilla_result = match optimize(circuit, cost_fn, initial_params, &vanilla_config) {
        Ok(r) => r,
        Err(QngError::ConvergenceFailed { iterations, final_cost }) => QngResult {
...
mod tests {
    ...
    fn test_qng_faster_than_vanilla() {
        ...
        let qng_better = qng.num_iterations <= vanilla.num_iterations
            || qng.final_cost < vanilla.final_cost;
        assert!(
            qng_better,
            "QNG should be better: QNG iters={} cost={:.6}, Vanilla iters={} cost={:.6}",
            qng.num_iterations, qng.final_cost,
            vanilla.num_iterations, vanilla.final_cost
        );
    }
