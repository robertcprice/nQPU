...
impl RealPhotonProbe {
    ...
    fn capture_from_system_entropy(&mut self, n_samples: usize) -> Result<Vec<u16>, String> {
        ...
        #[cfg(unix)]
        {
            ...
            self.samples = buffer.chunks_exact(2)
...
impl RealSsdProbe {
    ...
    pub fn run_test(&mut self, n_writes: usize) -> Result<RealSsdResult, String> {
        ...
        let interpretation = if !has_jitter {
            ...
            format!("High timing variance (CV={:.2}) - consistent with quantum tunneling uncertainty", cv)
        } else if cv > 0.01 {
            format!("Moderate timing variance (CV={:.2}) - possibly quantum-influenced", cv)
        } else {
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            result.cv,
            result.cv * 100.0,
            if result.is_quantum_consistent { "✅ QUANTUM-CONSISTENT" } else { "❌ Inconclusive" },
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            result.max_ns,
            result.entropy_bits,
            if result.is_quantum_consistent { "✅ QUANTUM-CONSISTENT jitter" } else { "❌ No detectable jitter" }
        )
    }
}
...
impl RealQuantumSuite {
    ...
    pub fn run_all(&mut self, samples_each: usize) -> RealQuantumReport {
        ...
        if let Some(ref r) = photon {
            if r.is_quantum_consistent { score += 1.0; }
            count += 1;
        }
