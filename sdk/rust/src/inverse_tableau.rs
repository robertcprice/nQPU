                            ...
                            if bit != q && self.get_z(base, bit) {
                                ...
                                for j in 0..n {
                                    ...
                                    if self.get_z(z_row, bit) && !self.get_x(z_row, bit) {
                                        ...
                                        for b in 0..n {
                                            if b != bit && (self.get_z(z_row, b) || self.get_x(z_row, b)) {
                                                is_pure = false;
                                                break;
                                            }
                                        }
                                        if is_pure {
                                            ...
                                            for w in 0..self.num_words {
                                                phase_adj += (self.xs[z_row][w] & temp_z[w]).count_ones();
                                                phase_adj += (self.zs[z_row][w] & temp_x[w]).count_ones();
                                            }
                                            if phase_adj % 2 == 1 {
                                                temp_sign = !temp_sign;
                                            }
...
mod tests {
    ...
    fn test_init_inverse_is_symplectic_dual() {
        ...
        for row in 0..n {
            ...
            assert!(!tab.xs[row].iter().all(|&w| w == 0) || row < n,
                "Forward row {} should have X bits", row);
            ...
            for bit in 0..n {
                ...
                if bit == row {
                    ...
                    assert!(!has_inv_x && has_inv_z,
                        "Inverse row {} should have Z_{}", row, row);
                } else {
                    assert!(!has_inv_x && !has_inv_z,
                        "Inverse row {} should only have one Z bit", row);
                }
            }
        }
        for i in 0..n {
            ...
            for bit in 0..n {
                ...
                if bit == i {
                    ...
                    assert!(has_inv_x && !has_inv_z,
                        "Inverse row {} should have X_{}", row, i);
                } else {
                    assert!(!has_inv_x && !has_inv_z,
                        "Inverse row {} should only have one X bit", row);
                }
            }
        }
    }
    ...
    fn test_cx_forward_update() {
        ...
        let has_xx = gens.iter().any(|g| g.get_x(0) && g.get_x(1) && !g.get_z(0) && !g.get_z(1));
        let has_zz = gens.iter().any(|g| !g.get_x(0) && !g.get_x(1) && g.get_z(0) && g.get_z(1));
    ...
    fn test_cz_both_tableaux() {
        ...
        let gens = tab.to_stabilizer_generators();
        let has_xz = gens.iter().any(|g| {
            g.get_x(0) && !g.get_x(1) && !g.get_z(0) && g.get_z(1)
        });
    ...
    fn test_swap_both_tableaux() {
        ...
        let gens = tab.to_stabilizer_generators();
        let has_x_on_2 = gens.iter().any(|g| !g.get_x(0) && !g.get_x(1) && g.get_x(2));
    ...
    fn test_measure_zero_state() {
        ...
        assert!(m.was_deterministic, "|0> measurement should be deterministic");
    }
    ...
    fn test_measure_one_state() {
        ...
        assert!(m.was_deterministic, "|1> measurement should be deterministic");
    }
    ...
    fn test_measurement_produces_valid_state() {
        ...
        for _ in 0..20 {
            ...
            assert!(m2.was_deterministic, "Post-measurement state should be deterministic");
    ...
    fn test_bell_state_correlations() {
        ...
        for _ in 0..50 {
            ...
            assert!(m1.was_deterministic, "Second Bell qubit should be deterministic after first");
        }
    }
    ...
    fn test_ghz_state_all_same() {
        ...
        for run in 0..30 {
            ...
            for i in 1..n {
                ...
                assert_eq!(m0.outcome, mi.outcome, "GHZ qubits must all agree (run {}, qubits 0 vs {})", run, i);
            }
        }
    }
    ...
    fn test_teleportation() {
        ...
        assert!(m2.was_deterministic, "Teleported state should be deterministic");
    }
    ...
    fn test_post_select_forces_outcome() {
        ...
        for desired in [false, true] {
            ...
            assert_eq!(m.outcome, desired, "PostSelect should force the desired outcome");
        }
    }
    ...
    fn test_circuit_execution() {
        ...
        assert_eq!(results[0].outcome, results[1].outcome, "Bell pair must correlate");
    }
    ...
    fn test_sample_circuit_statistics() {
        ...
        let num_shots = 1000;
        let results = InverseStabilizerTableau::sample_circuit(&circuit, num_shots, &mut rng).unwrap();
    ...
    fn test_entanglement_entropy_product_state() {
        ...
        assert!((entropy - 0.0).abs() < 1e-10, "Product state should have 0 entanglement");
    }
    ...
    fn test_pauli_multiply() {
        ...
        assert!(xz.sign, "X*Z should have negative sign (representing -iY -> -Y in Pauli group)");
    }
    ...
    fn test_pauli_commutation() {
        ...
        assert!(x0.commutes_with(&x1), "X on different qubits should commute");
    ...
    fn test_measure_x_basis() {
        ...
        assert!(m2.was_deterministic, "|+> in X basis should be deterministic");
    ...
    fn test_multiple_gates_inverse_stays_valid() {
        ...
        tab.y(2).unwrap();
        tab.z(3).unwrap();
        assert!(tab.validate_inverse().is_ok(), "Inverse should remain valid after many gates");
    }
    ...
    fn test_ghz_entanglement_entropy() {
        ...
        assert!((e1 - 1.0).abs() < 1e-10, "GHZ single-qubit entropy should be 1, got {}", e1);
        // Entanglement of first two qubits = 1 (not 2)
        let e2 = tab.entanglement_entropy(&[0, 1]);
        assert!((e2 - 1.0).abs() < 1e-10, "GHZ two-qubit entropy should be 1, got {}", e2);
    }
