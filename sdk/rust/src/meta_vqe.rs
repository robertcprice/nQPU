...
impl MetaVQE {
    ...
    pub fn train(&mut self) -> Result<(), String> {
        ...
        for ex in &self.training_data {
            let key = format!("e{}_o{}",
                ex.molecule.n_electrons,
                ex.molecule.n_orbitals
            );
    ...
    pub fn predict(&self, molecule: &MolecularDescriptor) -> Result<Vec<f64>, String> {
        ...
        // Predict parameters
        let params: Vec<f64> = self.param_weights
            .iter()
            .map(|weights| {
                weights.iter().map(|w| w * h).sum::<f64>().tanh() * PI
            })
            .collect();
    ...
    fn find_similar(&self, query: &MolecularDescriptor, k: usize) -> Vec<&TrainingExample> {
        let mut scored: Vec<_> = self.training_data
            .iter()
            .map(|ex| {
