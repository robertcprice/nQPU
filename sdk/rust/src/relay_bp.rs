    ...
    pub fn decode(
        ...
        for relay_round in 0..=self.config.max_relays {
            ...
            if relay_round < self.config.max_relays {
                if let Some(relay) =
                    self.detect_trapped_edge(&msg_v2c, &msg_c2v, total_iters)
                {
        ...
        for iter in 0..self.config.max_iterations {
            ...
            for c in 0..self.num_checks {
                ...
                for &v in neighbors {
                    ...
                    let new_llr = self.sum_product_check_update(
                        c,
                        v,
                        msg_v2c,
                        syndrome_sign,
                    ...
                    let damped =
                        self.config.damping_factor * new_llr
        ...
        for iter in 0..self.config.max_iterations {
            ...
            for c in 0..self.num_checks {
                let neighbors: Vec<usize> =
                    self.tanner.neighbors_of_check(c).to_vec();
                let syndrome_sign: f64 = if syndrome[c] { -1.0 } else { 1.0 };
                ...
                for &v in &neighbors {
                    ...
                    let total_llr: f64 =
                        channel_llr
                        + checks.iter().map(|&cc| msg_c2v[cc][v].llr).sum::<f64>();
                    hard_decision[v] = total_llr < 0.0;
    ...
    fn detect_trapped_edge(
        ...
        for c in 0..self.num_checks {
            for &v in self.tanner.neighbors_of_check(c) {
                ...
                if combined_var < self.config.relay_threshold
                    && msg_v2c[c][v].history.len() >= 3
                {
                    match &worst_edge {
                        None => {
...
mod tests {
    ...
    fn repetition_code_decode_single_error() {
        ...
        let result = decoder.decode(&syndrome, 0.1).unwrap();
        assert!(result.syndrome_satisfied, "Decoded error must satisfy syndrome");
