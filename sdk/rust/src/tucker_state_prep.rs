...
impl TuckerPrep {
    ...
    pub fn prepare(&mut self, target: &[C64]) -> Result<Vec<CircuitOp>, TuckerError> {
        ...
        let normalized: Vec<C64> = if norm > 1e-15 {
            target.iter().map(|c| C64::new(c.re / norm, c.im / norm)).collect()
        } else {
    ...
    fn compute_tucker(&self, state: &[C64]) -> Result<TuckerDecomposition, TuckerError> {
        ...
        if state.len() != total {
            return Err(TuckerError::InvalidDimensions {
                message: format!("State length {} does not match tensor size {}", state.len(), total),
            });
        }
        ...
        if self.config.iterative_refinement && n > 1 {
            ...
            for _iter in 0..self.config.max_iterations {
                ...
                for mode in 0..n {
                    ...
                    let updated_factor = self.svd_of_mode_unfolding(&y_k, &y_dims, mode, ranks[mode])?;
    ...
    fn svd_of_mode_unfolding(
        ...
        let n = dims.len();
        let rows = dims[mode];
        let cols: usize = dims.iter().enumerate()
            .filter(|&(k, _)| k != mode)
            .map(|(_, &d)| d)
            .product::<usize>()
            .max(1);
    ...
    fn decomposition_to_circuit(&self, decomp: &TuckerDecomposition) -> Result<Vec<CircuitOp>, TuckerError> {
        let mut ops = Vec::new();
        ...
        for (mode, factor) in decomp.factors.iter().enumerate() {
            ...
            if rank >= 1 && factor.len() >= 2 {
                ...
                let u10 = if factor.len() > rank { factor[rank] } else { C64::new(0.0, 0.0) }; // factor[1 * rank + 0]
    ...
    pub fn last_error(&self) -> f64 {
        self.last_decomp.as_ref().map(|d| d.error).unwrap_or(f64::MAX)
    }
...
fn compute_error_and_fidelity(original: &[C64], reconstructed: &[C64]) -> (f64, f64) {
    ...
    let fidelity = if denom > 1e-30 { inner_mag_sq / denom } else { 0.0 };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidStateLength { expected, got } => {
                write!(f, "Invalid state length: expected {}, got {}", expected, got)
            }
...
mod tests {
    ...
    fn test_prepare_bell_state() {
        ...
        let mut prep = TuckerPrep::new(2, TuckerConfig {
            max_rank: 2,
            iterative_refinement: true,
    ...
    fn test_prepare_ghz_3qubit() {
        ...
        let inv_sqrt2 = 1.0 / 2.0_f64.sqrt();
        let mut prep = TuckerPrep::new(3, TuckerConfig {
            max_rank: 2,
            iterative_refinement: true,
            ..TuckerConfig::default()
        });
    ...
    fn test_fidelity_tracking() {
        ...
        let fidelity = prep.last_fidelity();
        assert!(fidelity >= 0.0 && fidelity <= 1.01, "Fidelity should be in [0, 1], got {}", fidelity);
    }
    ...
    fn test_reconstruction_roundtrip() {
        ...
        let (error, fidelity) = compute_error_and_fidelity(
            ...
            &state.iter().map(|c| {
                let norm: f64 = state.iter().map(|v| v.re * v.re + v.im * v.im).sum::<f64>().sqrt();
