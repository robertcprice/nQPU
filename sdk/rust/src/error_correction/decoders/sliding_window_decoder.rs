    ...
    fn from_rounds(rounds: &[SyndromeRound], code_distance: usize) -> Self {
        ...
        for round in rounds {
            ...
            for (det_idx, &val) in syndrome.iter().enumerate() {
                let prev_val = prev.map_or(false, |p| p.get(det_idx).copied().unwrap_or(false));
        ...
        for i in 0..defects.len() {
            for j in (i + 1)..defects.len() {
                ...
                edges.push(DefectEdge { a: i, b: j, weight });
            }
        }
...
impl SlidingWindowDecoder {
    ...
    pub fn decode_window(&mut self) -> WindowResult {
        ...
        let num_detectors = window_rounds.first().map(|r| r.syndrome.len()).unwrap_or(0);
        ...
        let all_corrections = generate_corrections(&graph, &matches, &window_rounds, num_detectors);
        ...
        let committed_corr: Vec<Vec<bool>> =
            all_corrections.into_iter().take(self.slide_step).collect();
    ...
    pub fn flush(&mut self) -> Vec<WindowResult> {
        ...
        if !self.buffer.is_empty() {
            ...
            let num_detectors = remaining.first().map(|r| r.syndrome.len()).unwrap_or(0);
            ...
            let all_corrections = generate_corrections(&graph, &matches, &remaining, num_detectors);
...
mod tests {
    ...
    fn test_single_defect_boundary_match() {
        ...
        let mut dec_greedy = SlidingWindowDecoder::new(3, 1, 5, WindowInnerDecoder::Greedy);
        let mut dec_uf = SlidingWindowDecoder::new(3, 1, 5, WindowInnerDecoder::UnionFind);
        ...
        dec_greedy.push_round(make_round(
            1,
