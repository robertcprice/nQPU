...
impl TransformerLayer {
    ...
    fn layer_norm(&self, x: &[f64], weight: &[f64]) -> Vec<f64> {
        ...
        for i in 0..seq_len {
            ...
            let mean: f64 =
                x[start..end.min(x.len())].iter().sum::<f64>() / self.attention.embed_dim as f64;
...
impl TransformerDecoder {
    ...
    pub fn decode(&self, syndrome: &[bool]) -> CorrectionResult {
        ...
        let confidence =
            x_conf.iter().chain(z_conf.iter()).sum::<f64>() / (x_conf.len() + z_conf.len()) as f64;
        ...
        for pos in 0..max_len {
            for i in 0..embed_dim {
                ...
                if idx < encoding.len() {
                    if i % 2 == 0 {
                        encoding[idx] =
                            ((pos as f64) / 10000.0_f64.powf(i as f64 / embed_dim as f64)).sin();
                    } else {
                        encoding[idx] = ((pos as f64)
                            / 10000.0_f64.powf((i - 1) as f64 / embed_dim as f64))
                        .cos();
                    }
                }
            }
...
impl TransformerDecoder {
    ...
    pub fn pack_weights_for_gpu(&self) -> Vec<f32> {
        ...
        debug_assert_eq!(
            buf.len(),
            total,
            "Weight packing size mismatch: got {} expected {}",
            buf.len(),
            total
        );
        buf
    ...
    pub fn packed_weight_size(&self) -> usize {
        ...
        s * e + e * 3 * e + 3 * e + e * e + e + e * f + f + f * e + e + e * 2 * q + 2 * q
    }
}
...
mod tests {
    ...
    fn test_metal_pipeline_different_inputs() {
        ...
        let total_weights = (syndrome_len * embed_dim
            ...
            + ffn_dim * embed_dim
            + embed_dim
            + embed_dim * 2 * num_qubits
            + 2 * num_qubits) as usize;
