...
impl DenseLayer {
    ...
    pub fn new(in_dim: usize, out_dim: usize) -> Self {
        ...
        let weights = Array2::from_shape_fn((out_dim, in_dim), |_| rng.gen_range(-limit..limit));
...
impl AttentionLayer {
    ...
    pub fn new(feature_dim: usize, num_heads: usize) -> Self {
        ...
        let make_proj = |rng: &mut rand::rngs::ThreadRng| {
            Array2::from_shape_fn((feature_dim, feature_dim), |_| rng.gen_range(-limit..limit))
        };
        AttentionLayer {
...
impl CodeFamilyEmbedding {
    ...
    pub fn new(embedding_dim: usize) -> Self {
        ...
        let embeddings = Array2::from_shape_fn((CodeType::NUM_FAMILIES, embedding_dim), |_| {
    ...
    pub fn encode(&self, code_type: &CodeType) -> Vec<f64> {
        ...
        self.embeddings.row(idx).iter().cloned().collect()
    }
...
impl UnifiedNeuralDecoder {
    ...
    pub fn train_step(&mut self, syndromes: &[SyndromeData], labels: &[CorrectionLabel]) -> f64 {
        ...
        for idx in 0..n {
            ...
            let probs: Vec<f64> = logits
                .iter()
                .map(|&x| Activation::Sigmoid.apply(x))
                .collect();
    ...
    pub fn train(
        ...
        for epoch in 0..epochs {
            ...
            for chunk_start in (0..dataset.len()).step_by(bs) {
                ...
                let batch_s: Vec<SyndromeData> = syndromes[chunk_start..chunk_end]
                    .iter()
                    .map(|s| (*s).clone())
                    .collect();
                let batch_l: Vec<CorrectionLabel> = labels[chunk_start..chunk_end]
                    .iter()
                    .map(|l| (*l).clone())
                    .collect();
    ...
    pub fn evaluate(&self, test_data: &[(SyndromeData, CorrectionLabel)]) -> EvaluationResult {
...
mod tests {
    ...
    fn test_dense_forward_known_weights() {
        ...
        assert!(
            (output[0] - 3.5).abs() < 1e-9,
            "expected 3.5, got {}",
            output[0]
        );
        assert!(
            (output[1] - 6.5).abs() < 1e-9,
            "expected 6.5, got {}",
            output[1]
        );
    }
    ...
    fn test_code_type_embedding() {
        ...
        assert!(
            diff > 0.0,
            "Distinct code types should have distinct embeddings"
        );
    }
    ...
    fn test_transfer_learning_pretrain_finetune() {
        ...
        let finetune_history = TransferLearningManager::finetune(&mut decoder, &target, 1);
