...
impl MetalNeuralDecoder {
    ...
    pub fn decode(&self, syndrome: &[bool]) -> Vec<bool> {
        ...
        let soft_syndrome: Vec<f32> = syndrome
            .iter()
        ...
        for layer_idx in 0..self.weights.weights.len() {
            ...
            if is_last {
                ...
                encoder.set_bytes(
                    4,
                    4,
                    &layer_input_dim as *const u32 as *const std::ffi::c_void,
                );
                ...
                for b in 0..batch_size {
                    let in_offset =
                        (b * layer_input_dim as usize * std::mem::size_of::<f32>()) as u64;
            ...
            activations = unsafe { std::slice::from_raw_parts(ptr, out_count).to_vec() };
        }
    ...
    fn forward_gpu(&self, input: &[f32]) -> Option<Vec<f32>> {
        ...
        for layer_idx in 0..self.weights.weights.len() {
            ...
            let threadgroup_width = pipeline.thread_execution_width().min(if is_last {
                output_dim as u64
            } else {
                output_dim as u64
            });
            ...
            // Read back output
            let ptr = output_buf.contents() as *const f32;
            activations = unsafe { std::slice::from_raw_parts(ptr, output_dim as usize).to_vec() };
        }
...
mod tests {
    ...
    fn test_forward_gpu_batch_matches_individual_cpu() {
        ...
        let cpu_results: Vec<Vec<f32>> =
            inputs.iter().map(|inp| decoder.forward_cpu(inp)).collect();
