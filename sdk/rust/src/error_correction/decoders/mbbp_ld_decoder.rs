...
impl LocalDecoder {
    ...
    pub fn new(
        ...
        lookup_table.insert(vec![false; syndrome_bits.len()], vec![false; n]);
...
impl MbbpLdDecoder {
        ...
        for i in 0..num_syndromes {
            for j in (i + 1)..num_syndromes {
                ...
                if !shared.is_empty() {
                    ...
                    let noise_w =
                        noise.edge_weight(shared[0], shared.get(1).copied().unwrap_or(shared[0]));
    ...
    pub fn decode_multi_round(&mut self, syndrome_rounds: &[Vec<bool>]) -> MbbpLdResult {
        ...
        for w in syndrome_rounds.windows(2) {
            let diff_syndrome: Vec<bool> =
                w[0].iter().zip(w[1].iter()).map(|(&a, &b)| a ^ b).collect();
    ...
    fn apply_boundary_correction(&self, region1: usize, region2: usize, correction: &mut [bool]) {
        let data1 = self
            .stabilizer_map
            .get(&region1)
            .cloned()
    ...
    /// identically zero).
    fn syndromes_match(&self, original: &[bool], corrected: &[bool], _correction: &[bool]) -> bool {
        ...
        // contribution).
        original.iter().zip(corrected.iter()).all(|(&a, &b)| a == b)
    }
        ...
        for &d in &defects {
            if !matched.contains(&d) {
                ...
                if let Some(&q) = data.first() {
                    if let Some(pos) = self.topology.data_qubits.iter().position(|&dd| dd == q) {
                        correction[pos] ^= true;
                    }
                }
            }
...
mod tests {
    ...
    fn test_weight_computation() {
        ...
        assert!(
            mw > 0.0,
            "Measurement weight should be positive, got {}",
            mw
        );
    }
    ...
    fn test_heavy_hex_d7() {
        ...
        // least one edge
        let all_edge_qubits: HashSet<usize> =
            topo.edges.iter().flat_map(|&(u, v)| [u, v]).collect();
