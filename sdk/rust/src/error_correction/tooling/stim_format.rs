...
impl StimGate {
    ...
    pub fn is_measurement(&self) -> bool {
        matches!(
            self,
            StimGate::M { .. } | StimGate::MX { .. } | StimGate::MY { .. }
        )
    }
...
fn parse_line(line: &str) -> Result<Vec<StimGate>, String> {
    ...
    match gate_type.as_str() {
        ...
        "CX" | "CNOT" => {
            ...
            let pairs: Vec<(usize, usize)> = targets.chunks(2).map(|p| (p[0], p[1])).collect();
...
pub fn to_reference_frame_gates(circuit: &StimCircuit) -> Vec<crate::reference_frame::FrameGate> {
    ...
    for gate in &circuit.gates {
        match gate {
            ...
            StimGate::M { targets } => {
                for &q in targets {
                    gates.push(FrameGate::M {
                        qubit: q,
                        result_idx: measurement_idx,
                    });
                    measurement_idx += 1;
                }
            }
...
pub fn bell_state_circuit() -> StimCircuit {
    ...
    circuit.add_gate(StimGate::CX {
        pairs: vec![(0, 1)],
    });
    circuit.add_gate(StimGate::M {
        targets: vec![0, 1],
    });
    circuit
}
...
pub fn surface_code_cycle(distance: usize, p: f64) -> StimCircuit {
    ...
    // Prepare ancillas in |+⟩
    circuit.add_gate(StimGate::H {
        targets: (n_data..n_total).collect(),
    });
    ...
    for i in 0..n_ancilla {
        ...
        circuit.add_gate(StimGate::CX {
            pairs: vec![(anc, data1), (anc, data2), (anc, data3), (anc, data4)],
        });
    }
