...
impl DecoderAwareTranspiler {
    ...
    pub fn optimize(&mut self, circuit: &Circuit) -> OptimizationResult {
        let original_stats = CircuitStats::from_circuit(circuit, &self.decoder_sim);
        ...
        for pass in 0..self.config.max_passes {
            ...
            if self.config.enable_layout_opt {
                let layout_result = self
                    .layout_optimizer
                    .optimize(&current_circuit, &self.decoder_sim);
            ...
            if self.config.enable_rescheduling {
                let schedule_result = self
                    .gate_rescheduler
                    .reschedule(&current_circuit, &self.decoder_sim);
        ...
        let optimized_stats = CircuitStats::from_circuit(&best_circuit, &self.decoder_sim);
        ...
        OptimizationResult {
            ...
            overall_improvement: original_stats.decoder_difficulty
                - optimized_stats.decoder_difficulty,
            passes,
        }
    }
