    ...
    pub fn get_syndrome(&self) -> Vec<bool> {
        ...
        for y in 0..self.params.l - 1 {
            for x in 0..self.params.l - 1 {
                // Check plaquette
                let x1 = if x < self.params.l && y < x_stabs.len() {
                    x_stabs[y][x]
                } else {
                    false
                };
                let x2 = if x < self.params.l - 1 && y < x_stabs.len() {
                    x_stabs[y + 1][x]
                } else {
                    false
                };

                let y1 = if y < self.params.l - 1 && x < z_stabs.len() {
                    z_stabs[y][x]
                } else {
                    false
                };
                let y2 = if y < self.params.l - 1 && x < z_stabs.len() {
                    z_stabs[y + 1][x]
                } else {
                    false
                };
