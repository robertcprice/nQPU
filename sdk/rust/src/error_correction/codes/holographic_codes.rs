...
impl PerfectTensor {
    ...
    fn compute_code_space(generators: &[(u32, u32)], n: usize, dim: usize) -> Vec<Vec<C64>> {
    ...
    fn apply_stabilizer(state: &[C64], x_mask: u32, z_mask: u32, _n: usize) -> Vec<C64> {
        let dim = state.len();
    ...
    fn extract_independent_basis(vectors: &[Vec<C64>], dim: usize) -> Vec<Vec<C64>> {
        let mut basis: Vec<Vec<C64>> = Vec::new();
...
impl HolographicCode {
    ...
    pub fn encode_single_qubit(&self, logical_state: &[C64]) -> Result<Vec<C64>, HolographicError> {
    ...
    pub fn encode_bulk_state(&self, bulk_state: &[C64]) -> Result<Vec<C64>, HolographicError> {
    ...
    fn eigenvalues_hermitian(matrix: &[Vec<C64>], n: usize) -> Vec<f64> {
        ...
        for _ in 0..max_iter {
            ...
            new_a[p][p] =
                cos_t * cos_t * a[p][p] + 2.0 * cos_t * sin_t * a[p][q] + sin_t * sin_t * a[q][q];
            new_a[q][q] =
                sin_t * sin_t * a[p][p] - 2.0 * cos_t * sin_t * a[p][q] + cos_t * cos_t * a[q][q];
            new_a[p][q] = 0.0;
            new_a[q][p] = 0.0;
...
impl RyuTakayanagi {
    ...
    pub fn entropy(&self, boundary_region: &[usize]) -> Result<f64, HolographicError> {
        ...
        let all_boundary: HashSet<usize> =
            self.tiling.boundary_qubit_indices.iter().cloned().collect();
    ...
    fn compute_min_cut(&self, region: &HashSet<usize>) -> Result<usize, HolographicError> {
...
impl SubregionDuality {
    ...
    pub fn check(
        ...
        let all_boundary: HashSet<usize> = tiling.boundary_qubit_indices.iter().cloned().collect();
        let region_set: HashSet<usize> = boundary_region.iter().cloned().collect();
    ...
    pub fn find_all_reconstructing_regions(
        ...
        for start in 0..n {
            for len in 1..=n {
                let region: Vec<usize> = (0..len).map(|i| boundary[(start + i) % n]).collect();
...
mod tests {
    ...
    fn test_tiling_shortest_path() {
        ...
        for i in 1..6_usize {
            ...
            assert!(
                path.is_some(),
                "Path from center to tile {} should exist",
                i
            );
            assert_eq!(
                path.unwrap().len(),
    ...
    fn test_encoding_preserves_inner_product() {
        ...
        let plus = vec![C64::new(inv_sqrt2, 0.0), C64::new(inv_sqrt2, 0.0)];
    ...
    fn test_rt_entropy_single_qubit() {
        ...
        assert!(s >= 0.0, "RT entropy should be non-negative, got {}", s);
    }
    ...
    fn test_rt_surface_minimal() {
        ...
        if !tiling.boundary_qubit_indices.is_empty() {
            ...
            assert!(surface.is_ok(), "RT surface computation should succeed");
        }
    }
    ...
    fn test_rt_matches_exact_entropy() {
        ...
        let inv_sqrt2 = 1.0 / 2.0_f64.sqrt();
        let logical_plus = vec![C64::new(inv_sqrt2, 0.0), C64::new(inv_sqrt2, 0.0)];
        let boundary = code
        ...
        assert!(exact_s.is_ok(), "Exact entropy computation should succeed");
