...
impl YokedCodeConfig {
    ...
    fn num_yoke_checks(&self) -> usize {
        ...
        let vertical = match self.yoke_direction {
            ...
            YokeDirection::RowAndColumn => self.grid_rows.saturating_sub(1) * self.grid_cols,
        };
        horizontal + vertical
    }
...
impl YokedDecoder {
    ...
    pub fn decode(&self, syndrome: &YokedSyndrome) -> Result<Vec<PatchCorrection>, YokedCodeError> {
        ...
        for r in 0..self.config.grid_rows {
            for c in 0..(self.config.grid_cols.saturating_sub(1)) {
                if yoke_idx < syndrome.yoke_bits.len() && syndrome.yoke_bits[yoke_idx] {
                    ...
                    self.augment_correction(idx_a, idx_b, &patch_defect_counts, &mut corrections);
                }
                yoke_idx += 1;
            }
        }
...
impl YokedSurfaceCode {
    ...
    pub fn effective_distance(&self) -> usize {
        ...
        match self.config.yoke_direction {
            YokeDirection::RowOnly => d * self.config.grid_rows.min(self.config.grid_cols),
            YokeDirection::RowAndColumn => d * self.config.grid_rows.max(self.config.grid_cols),
        }
    }
    ...
    pub fn measure_syndrome(&mut self) -> YokedSyndrome {
        ...
        let patch_syndromes: Vec<Vec<bool>> =
            self.patches.iter().map(|p| p.get_syndrome()).collect();
    ...
    pub fn decode_and_correct(&mut self) -> Result<Vec<PatchCorrection>, YokedCodeError> {
    ...
    pub fn estimate_logical_error_rate(&mut self, num_trials: usize) -> YokedErrorEstimate {
...
mod tests {
    ...
    #[test]
    fn test_num_patches() {
        let cfg = YokedCodeConfig::new().grid_rows(2).grid_cols(3);
        let code = YokedSurfaceCode::new(cfg);
    ...
    fn test_outer_check_matrix_dims_row_only() {
        ...
        for r in 0..m.rows {
            assert_eq!(
                m.row_weight(r),
                2,
                "row {} should connect exactly 2 patches",
                r
            );
        }
    }
