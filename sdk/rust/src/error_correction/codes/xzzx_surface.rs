...
impl XZZXSurfaceCode {
    ...
    pub fn simulate_round(&self, seed: u64) -> SyndromeResult {
        ...
        for (s_idx, stab) in self.stabilizers.iter().enumerate() {
            ...
            for ((q_idx, &data_qubit), &pauli) in
                stab.data_qubits.iter().enumerate().zip(stab.paulis.iter())
            {
...
pub fn print_threshold_comparison() {
    ...
    println!(
        "{:<15} {:<15} {:<15} {:<10}",
        "Bias η", "Standard", "XZZX", "Improvement"
    );
    println!("{}", "-".repeat(70));
...
pub fn print_benchmark() {
    ...
    println!(
        "{:<10} {:<15} {:<15} {:<15}",
        "Distance", "Time (s)", "Success Rate", "Shots"
    );
    println!("{}", "-".repeat(70));
