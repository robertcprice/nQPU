...
pub enum SamplingError {
    ...
    InsufficientTrials { requested: usize, completed: usize },
...
fn union_find_correct(
    ...
    if defects.len() == 2 {
        ...
        let shared: Vec<u8> = checks[d1]
            .iter()
            .filter(|q| set0.contains(q))
            .cloned()
            .collect();

        if !shared.is_empty() {
            correction[shared[0] as usize] =
                combine_pauli_single(correction[shared[0] as usize], pauli);
            return correction;
        }
            ...
            for _ in 0..trials_in_chunk {
                ...
                let correction = match decoder {
                    ...
                    DecoderType::BeliefPropagation => {
                        ...
                        let z_syn = &syndrome[x_checks.len()
                            ..x_checks.len()
                                + z_syn_len
                                    .min(syndrome.len() - x_checks.len().min(syndrome.len()))];
...
fn estimate_threshold(results: &[SamplingResult], distances: &[usize]) -> (f64, f64) {
    ...
    for i in 0..distances.len() - 1 {
        ...
        for j in 0..small_results
            .len()
            .min(large_results.len())
            .saturating_sub(1)
        {
            ...
            let l1_opt = large_results
                .iter()
                .find(|r| (r.error_rate - p1).abs() < 1e-12);
            let l2_opt = large_results
                .iter()
                .find(|r| (r.error_rate - p2).abs() < 1e-12);
...
mod tests {
    ...
    fn test_surface_code_parity_check_d3() {
        ...
        for check in x_checks.iter().chain(z_checks.iter()) {
            for &q in check {
                assert!((q as usize) < 9, "Qubit index {} out of range for d=3", q);
            }
        }
    }
    ...
    fn test_config_validation() {
        ...
        let invalid_rate = SamplingConfig::builder().error_rates(vec![-0.1]).build();
        assert!(invalid_rate.validate().is_err());

        let invalid_dist = SamplingConfig::builder().code_distances(vec![2]).build();
    ...
    fn test_logical_operators() {
        ...
        col_x[6] = 1; // (2,0)
        assert!(
            is_logical_x(&col_x, d),
            "Full column of X should be logical X"
        );
