...
impl NoiseModel {
    ...
    pub fn total_error_rate(&self) -> f64 {
        self.p_depolarizing
            ...
            + self.p_y
            + self.p_z
            + self.p_amplitude_damping
            + self.p_phase_damping
    }
...
impl DifferentiableQEC {
    ...
    pub fn apply_gradients(&mut self, grads: &QECGradients) {
        self.decoder
            .apply_gradients(&grads.decoder, self.config.learning_rate);
    }
    ...
    fn apply_noise(&self, state: &[f64]) -> Vec<f64> {
        ...
        state.iter().map(|&amp| amp * (1.0 - p / 2.0)).collect()
    }
