    ...
    pub fn inverse(&self) -> Option<GateOp> {
        match self {
            ...
            GateOp::U3(q, theta, phi, lambda) => {
                Some(GateOp::U3(*q, -theta, -lambda, -phi))
            }
...
impl BuiltCircuit {
    ...
    pub fn stats(&self) -> CircuitStats {
        CircuitStats {
            ...
            three_qubit_count: self
                .gates
                .iter()
                .filter(|g| g.is_three_qubit())
                .count(),
...
impl CircuitBuilder {
    ...
    fn check_two_qubits(&mut self, a: usize, b: usize) -> bool {
        ...
        if a == b && ok {
            self.errors
                .push(CircuitError::DuplicateQubit { qubit: a });
            ok = false;
        }
        ok
    }
    ...
    fn check_three_qubits(&mut self, a: usize, b: usize, c: usize) -> bool {
        ...
        if ok {
            if a == b || a == c || b == c {
                let dup = if a == b { a } else if a == c { a } else { b };
                self.errors
...
mod tests {
    ...
    #[test]
    fn with_clbits() {
        let circuit = CircuitBuilder::with_clbits(2, 4)
            .h(0)
            .measure(0, 3)
            .build();
