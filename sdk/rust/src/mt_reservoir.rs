...
use crate::anharmonic::{AnharmonicConfig, AnharmonicOscillator, QuantumCouplingConfig, oscillator_to_rotation};
use crate::orch_or::{OrchORConfig, OrchORSimulator, ConsciousnessMeasure};
...
impl HeartCoherenceState {
    ...
    pub fn update(&mut self, hrv_signal: &[f64]) {
        ...
        let variance: f64 = hrv_signal.iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>() / hrv_signal.len() as f64;
...
impl MicrotubuleReservoir {
    ...
    pub fn with_config(config: MTReservoirConfig) -> Self {
        ...
        let orch_or = OrchORSimulator::new(orch_config)
            .expect("Valid Orch-OR configuration");
    ...
    fn couple_oscillator_to_quantum(&mut self) {
        let n = self.config.num_tubulins.min(self.oscillator.config.num_modes);
        ...
        for i in 0..n {
            ...
            GateOperations::rx(&mut self.orch_or.microtubule.quantum_state, i, angle * self.config.anharmonic_quantum_coupling);
        }
    ...
    fn apply_heart_coupling(&mut self) {
        ...
        self.orch_or.effective_coherence_ns = self.orch_or.effective_coherence_ns.clamp(1e-6, 1000.0);
    }
    ...
    pub fn consciousness_trend(&self) -> f64 {
        ...
        let recent: f64 = self.history[n-recent_count..n].iter()
            .map(|h| h.consciousness_score)
            .sum::<f64>() / recent_count as f64;

        let earlier_count = 5_usize.min(n.saturating_sub(1));
        let earlier: f64 = self.history[0..n.saturating_sub(earlier_count)].iter()
            .map(|h| h.consciousness_score)
            .sum::<f64>() / (n.saturating_sub(earlier_count)).max(1) as f64;
...
mod tests {
    ...
    fn test_initialize() {
        ...
        assert!(coherence > 0.0, "Coherence should be positive after initialization");
    }
