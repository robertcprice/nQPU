    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MemoryExceeded {
                ...
                write!(
                    f,
                    "GPU memory exceeded: need {} MB, have {} MB",
...
impl GpuDmrgConfig {
    ...
    pub fn with_max_sweeps(mut self, s: usize) -> Self {
        self.max_sweeps = s;
        self
    }
    pub fn with_energy_tolerance(mut self, t: f64) -> Self {
        self.energy_tolerance = t;
        self
    }
    pub fn with_lanczos_iterations(mut self, n: usize) -> Self {
        self.lanczos_iterations = n;
        self
    }
    pub fn with_noise_schedule(mut self, ns: Vec<f64>) -> Self {
    ...
    pub fn with_max_gpu_memory_mb(mut self, m: usize) -> Self {
        self.max_gpu_memory_mb = m;
        self
    }
    ...
    pub fn with_subspace_expansion(mut self, se: bool) -> Self {
        self.subspace_expansion = se;
        self
    }
...
impl GpuMps {
    ...
    pub fn entanglement_entropy(&self) -> f64 {
        if self.center >= self.num_sites - 1 {
            return 0.0;
        }
        let i = self.center;
        let dl = self.tensors[i].shape()[0];
                ...
                for bp in 0..dr {
                    ...
                    for a in 0..dl.min(l_rows) {
                        for ap in 0..dl.min(l.shape()[1]) {
                            for s in 0..d {
                                val = val
                                    + l[[a, ap]] * tensor[[a, s, b]] * tensor[[ap, s, bp]].conj();
                            }
                        }
                    }
                    new_l[[b, bp]] = val;
...
impl MpoHamiltonian {
    ...
    pub fn heisenberg(num_sites: usize, j: f64) -> Self {
        ...
        for i in 0..num_sites - 1 {
            bond_dims.push(if i == 0 || i == num_sites - 2 {
                mpo_d
            } else {
                mpo_d
            });
    ...
    pub fn ising(num_sites: usize, j: f64, h_field: f64) -> Self {
        ...
        let mut bond_dims = vec![1];
        for _ in 0..num_sites - 1 {
            bond_dims.push(mpo_d);
        }
        bond_dims.push(1);
...
impl GpuEnvironment {
    ...
    pub fn build(mps: &GpuMps, mpo: &MpoHamiltonian) -> Self {
        ...
        for i in 0..n {
            left_blocks[i + 1] =
                contract_left_step(&left_blocks[i], &mps.tensors[i], &mpo.tensors[i]);
        }

        // Build right blocks
        for i in (0..n).rev() {
            right_blocks[i] =
                contract_right_step(&right_blocks[i + 1], &mps.tensors[i], &mpo.tensors[i]);
        }
    ...
    pub fn update_left(&mut self, i: usize, mps_tensor: &Array3<C64>, mpo_tensor: &Array4<C64>) {
        if i + 1 < self.left_blocks.len() {
            self.left_blocks[i + 1] =
                contract_left_step(&self.left_blocks[i], mps_tensor, mpo_tensor);
        }
    }
    ...
    pub fn update_right(&mut self, i: usize, mps_tensor: &Array3<C64>, mpo_tensor: &Array4<C64>) {
        if i < self.right_blocks.len() - 1 {
            self.right_blocks[i] =
                contract_right_step(&self.right_blocks[i + 1], mps_tensor, mpo_tensor);
        }
    }
}
...
impl GpuDmrgEngine {
    ...
    pub fn run(&self, mps: &mut GpuMps, mpo: &MpoHamiltonian) -> Result<DmrgResult, GpuDmrgError> {
        if self.config.max_bond_dim == 0 {
            return Err(GpuDmrgError::InvalidBondDim(
        ...
        if mem > self.config.max_gpu_memory_mb {
            return Err(GpuDmrgError::MemoryExceeded {
                required_mb: mem,
                available_mb: self.config.max_gpu_memory_mb,
            });
        }
    ...
    fn optimize_two_site(
        ...
        let (eigenvalue, eigvec) = lanczos_two_site(
            ...
            d,
            dr,
            self.config.lanczos_iterations,
        )?;
...
fn lanczos_two_site(
    ...
    let dim = dl * d * d * dr;
    if dim == 0 {
        return Err(GpuDmrgError::LanczosFailed(
            "lanczos: dimension is zero".to_string(),
        ));
    }
                                ...
                                for bp in 0..rr {
                                    ...
                                    for w1_idx in 0..lw {
                                        for w2_idx in 0..wr1.min(wl2) {
                                            for w3_idx in 0..rw {
                                                let l_val = if a < ll && ap < lr {
                                                    l[[a, w1_idx, ap]]
                                                } else {
                                                    c0()
                                                };
                                                let r_val = if b < rl && bp < rr {
                                                    r[[b, w3_idx, bp]]
                                                } else {
                                                    c0()
                                                };
                                                if l_val.norm() < 1e-30 || r_val.norm() < 1e-30 {
                                                    continue;
                                                }
                                                h_elem = h_elem
                                                    + l_val
...
mod tests {
    ...
    fn test_error_display_convergence() {
        let e = GpuDmrgError::ConvergenceFailed {
            sweeps: 30,
            energy: -1.5,
            tolerance: 1e-10,
        };
    ...
    fn test_error_is_std_error() {
        let e: Box<dyn std::error::Error> = Box::new(GpuDmrgError::LanczosFailed("test".into()));
    ...
    fn test_svd_truncate_near_zero_matrix() {
        ...
        let near_zero =
            Array2::from_shape_fn((3, 4), |(i, j)| cr(((i * 4 + j + 1) as f64) * 1e-22));
    ...
    fn test_svd_truncate_degenerate_matrix() {
        ...
        assert!(
            s[0] > 20.0,
            "Dominant singular value should be ~24, got {}",
            s[0]
        );
