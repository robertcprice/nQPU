...
impl DmrgMetalEngine {
    ...
    pub fn new() -> Result<Self, DmrgMetalError> {
        let device = Device::system_default().ok_or(DmrgMetalError::NoDevice)?;
    ...
    pub fn matvec(
        ...
        if h.len() != m * n {
            return Err(DmrgMetalError::InvalidDimensions(format!(
                "H matrix size {} != m*n = {}*{} = {}",
                h.len(),
                m,
                n,
        ...
        let grid_size = MTLSize {
            width: grid_width as u64,
            height: grid_height as u64,
            depth: 1,
        };
