...
impl HaPPYCode {
    ...
    pub fn measure_logical(&mut self) -> Vec<bool> {
        ...
        (0..n_logical).map(|_| rand::random::<bool>()).collect()
    }
...
impl SurfaceCode3D {
        ...
        for _x in 0..lx {
            for y in 0..ly {
                for z in 0..lz {
                    let paulis = vec![
                        PauliOp::Z,
                        PauliOp::Z, // Two Z operators
                    ];
                    let qubits = vec![y * lz * lx + z, y * lz * lx + z + 1];
        ...
        for y in 0..ly {
            for x in 0..lx {
                for z in 0..lz {
                    let paulis = vec![
                        PauliOp::X,
                        PauliOp::X, // Two X operators
                    ];
                    let qubits = vec![x * lz * lx + y * lz * lx + z, x * lz * lx + y * lz * lx + z];
                    stabilizers.push(Stabilizer3D::from_3d(paulis, qubits));
                }
            }
        }
    ...
    pub fn logical_qubits(&self) -> usize {
        ...
        match (self.params.lx, self.params.ly, self.params.lz) {
            (_, _, _) if self.params.lx == self.params.ly && self.params.ly == self.params.lz => {
                d * d
            } // For cubic codes
            _ => self.params.d,
        }
