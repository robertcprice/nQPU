...
impl MagicState {
    ...
    pub fn t_state() -> Self {
        ...
        Self {
            state_vector: vec![Complex64::new(SQRT2_INV, 0.0), omega * SQRT2_INV],
            num_qubits: 1,
    ...
    pub fn tdg_state() -> Self {
        ...
        Self {
            state_vector: vec![Complex64::new(SQRT2_INV, 0.0), omega_conj * SQRT2_INV],
            num_qubits: 1,
...
fn contract_greedy(tn: &mut TensorNetwork, _config: &MastConfig) -> Result<Complex64, MastError> {
    // Convert all nodes to dense and do pairwise contraction
    let mut dense_states: Vec<Option<Vec<Complex64>>> =
        tn.tensors.iter().map(|t| Some(t.to_dense())).collect();
    ...
    while dense_states.iter().filter(|s| s.is_some()).count() > 1 {
        ...
        let nq_composed = if nq_a == nq_b { nq_a } else { nq_a.max(nq_b) };
