...
impl TTN {
    ...
    fn apply_two_qubit_gate_via_sv(&mut self, gate: &[[f64; 2]; 16], q1: usize, q2: usize) {
        ...
        for idx in 0..dim {
            ...
            for out_pair in 0..4usize {
                ...
                let ob2 = out_pair & 1;
                let out_idx = (idx & !(1usize << q1) & !(1usize << q2)) | (ob1 << q1) | (ob2 << q2);
    ...
    fn decompose_statevector_into_tree(&mut self, sv: &[[f64; 2]]) {
        ...
        fn decompose(
            ...
            for pl in 0..nl {
                for pr in 0..nr {
                    for b in 0..bond_up {
                        mat[pl * ncols + pr * bond_up + b] = tensor[(pl * nr + pr) * bond_up + b];
                    }
                }
            }
...
mod tests {
    ...
    fn test_cnot_bell_state() {
        ...
        assert!(
            (cnorm_sq(sv[0]).sqrt() - s).abs() < 0.01,
            "|00> amp should be 1/sqrt(2), got {}",
            cnorm_sq(sv[0]).sqrt()
        );
        assert!(cnorm_sq(sv[1]) < 0.01, "|01> should be ~0");
        assert!(cnorm_sq(sv[2]) < 0.01, "|10> should be ~0");
        assert!(
            (cnorm_sq(sv[3]).sqrt() - s).abs() < 0.01,
            "|11> amp should be 1/sqrt(2), got {}",
            cnorm_sq(sv[3]).sqrt()
        );
    }
    ...
    fn test_entanglement_entropy_bell() {
        ...
        assert!(
            (entropy - expected).abs() < 0.05,
            "Bell entropy should be ln(2)={:.4}, got {:.4}",
            expected,
            entropy
        );
    }
    ...
    fn test_entanglement_entropy_product() {
        ...
        assert!(
            entropy.abs() < TOL,
            "Product state entropy should be 0, got {:.6}",
            entropy
        );
    }
    ...
    fn test_statevector_ghz_3qubit() {
        ...
        // |0000> = index 0
        assert!(
            (cnorm_sq(sv[0]).sqrt() - s).abs() < 0.02,
            "|0000> should be 1/sqrt(2), got {}",
            cnorm_sq(sv[0]).sqrt()
        );
        // |0111> = qubits 0,1,2 set = bits 0,1,2 = 0b0111 = 7
        assert!(
            (cnorm_sq(sv[7]).sqrt() - s).abs() < 0.02,
            "|0111> should be 1/sqrt(2), got {}",
            cnorm_sq(sv[7]).sqrt()
        );
        for i in 0..sv.len() {
            if i != 0 && i != 7 {
    ...
    fn test_multiple_cnots() {
        ...
        // |1111> = index 15
        assert!(
            (cnorm_sq(sv[15]).sqrt() - 1.0).abs() < 0.02,
            "|1111> should be 1.0, got {}",
            cnorm_sq(sv[15]).sqrt()
        );
        for i in 0..sv.len() {
            if i != 15 {
    ...
    fn test_bell_sampling_correlations() {
        ...
        for (i, s) in samples.iter().enumerate() {
            assert_eq!(
                s[0], s[1],
                "Shot {}: Bell outcomes must correlate, got {:?}",
                i, s
            );
        }
    }
