...
impl StabilizerTensorNetwork {
    ...
    pub fn promote_node(&mut self, node_idx: usize) {
        ...
        self.stats.current_stabilizer_nodes = self.stats.current_stabilizer_nodes.saturating_sub(1);
    ...
    pub fn demote_node(&mut self, node_idx: usize) -> bool {
        ...
        if self.nodes[node_idx].try_demote_to_stabilizer() {
            self.stats.demotions += 1;
            self.stats.current_dense_nodes = self.stats.current_dense_nodes.saturating_sub(1);
            self.stats.current_stabilizer_nodes += 1;
            true
        } else {
            false
    ...
    pub fn contract(&self) -> Vec<C64> {
        ...
        let mut tensors: Vec<Vec<C64>> =
            self.nodes.iter().map(|n| node_to_statevector(n)).collect();
...
fn try_extract_stabilizer(data: &[C64], n: usize) -> Option<StabilizerState> {
    ...
    if count == dim {
        ...
        let matches = nonzero
            .iter()
            .all(|(_, a)| (a.re - expected).abs() < eps * 100.0 && a.im.abs() < eps * 100.0);
        if matches {
