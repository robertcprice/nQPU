...
impl ContractionPath {
    ...
    pub fn from_contractions(contractions: Vec<Contraction>) -> Self {
        ...
        let max_intermediate = contractions
            .iter()
            .map(|c| if c.memory > 0 { c.memory / 16 } else { 1 })
            .max()
            .unwrap_or(1);
...
impl fmt::Display for ContractionReport {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "Network: {} tensors, {} indices",
            self.num_tensors, self.num_indices
        )?;
...
fn boltzmann_select(
    ...
    let min_score = scored.iter().map(|s| s.2).fold(f64::INFINITY, f64::min);
...
fn random_contraction_path(tn: &TensorNetwork, seed: u64) -> ContractionResult<ContractionPath> {
    if tn.tensors.len() <= 1 {
        return Err(ContractionError::SingleTensor);
    }
...
fn recursive_partition_contract(
    ...
    if nodes.len() <= config.coarsening_threshold {
        ...
        if let Ok(path) = optimize_greedy(&sub_tn, &GreedyConfig::default(), rng.gen()) {
    ...
    let result_a_indices: Vec<IndexId> = internal_a
        .union(&cross.iter().copied().collect())
        .copied()
        .collect();
    let result_b_indices: Vec<IndexId> = internal_b
        .union(&cross.iter().copied().collect())
        .copied()
        .collect();
...
pub fn optimize_contraction(
    ...
    match &config.method {
        ...
        OptimizationMethod::HyperGraphPartition(pc) => optimize_partition(tn, pc, config.seed),
...
pub fn build_chain_network(num_tensors: usize, bond_dim: usize) -> TensorNetwork {
    ...
    tn.output_indices = (0..num_tensors)
        .map(|i| {
...
pub fn build_cubic_network(x: usize, y: usize, z: usize, bond_dim: usize) -> TensorNetwork {
    ...
    tn.output_indices = tn.tensors.iter().map(|t| t.indices[0]).collect();
...
mod tests {
    ...
    fn matmul_chain(dims: &[(usize, usize)]) -> TensorNetwork {
        ...
        for (tensor_id, &(rows, cols)) in dims.iter().enumerate() {
            ...
            tn.tensors.push(TensorInfo {
                ...
                shape: vec![*tn.index_sizes.get(&row_idx).unwrap(), cols],
                is_output: false,
            });
        }
    ...
    fn test_partition_recursive() {
        let tn = chain_network(8);
        let config = ContractionConfig::builder()
            .method(OptimizationMethod::HyperGraphPartition(PartitionConfig {
                num_partitions: 2,
                imbalance_factor: 1.5,
    ...
    fn test_large_network_completes() {
        ...
        assert!(
            elapsed < 10.0,
            "Optimization took too long: {:.1}s",
            elapsed
        );
    ...
    fn test_validation_unknown_index() {
        let mut tn = TensorNetwork::new();
        tn.tensors
            .push(TensorInfo::new(0, vec![0, 999], vec![2, 3]));
