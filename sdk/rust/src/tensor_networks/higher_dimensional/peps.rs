...
impl PEPS {
        ...
        F: FnOnce() -> Array2<c64>,
    {
        ...
        let gate = if (horizontal && coord1.x > coord2.x) || (!horizontal && coord1.y > coord2.y) {
        ...
        if horizontal {
            ...
            let (u_trunc, s_vals, vt_trunc, new_bond) = truncated_svd(&gated, self.max_bond_dim);
            ...
            let mut new_ta = Array::<c64, Ix5>::zeros((d, du_a, dd_a, dl_a, new_bond));
            for pa in 0..d {
                for bua in 0..du_a {
                    for bda in 0..dd_a {
                        for bla in 0..dl_a {
            ...
            let mut new_tb = Array::<c64, Ix5>::zeros((d, du_b, dd_b, new_bond, dr_b));
            for pb in 0..d {
                for bub in 0..du_b {
                    for bdb in 0..dd_b {
                        for brb in 0..dr_b {
                            let col = pb * other_b + bub * dd_b * dr_b + bdb * dr_b + brb;
                            for k in 0..new_bond {
            ...
            self.tensors[ca.y][ca.x] = PEPSTensor::from_data(new_ta, [du_a, dd_a, dl_a, new_bond]);
            self.tensors[cb.y][cb.x] = PEPSTensor::from_data(new_tb, [du_b, dd_b, new_bond, dr_b]);
        } else {
            ...
            let mut new_ta = Array::<c64, Ix5>::zeros((d, du_a, new_bond, dl_a, dr_a));
            for pa in 0..d {
                for bua in 0..du_a {
                    for bla in 0..dl_a {
                        for bra in 0..dr_a {
                            let row = pa * other_a + bua * dl_a * dr_a + bla * dr_a + bra;
                            for k in 0..new_bond {
            ...
            let mut new_tb = Array::<c64, Ix5>::zeros((d, new_bond, dd_b, dl_b, dr_b));
            for pb in 0..d {
                for bdb in 0..dd_b {
                    for blb in 0..dl_b {
                        for brb in 0..dr_b {
                            let col = pb * other_b + bdb * dl_b * dr_b + blb * dr_b + brb;
                            for k in 0..new_bond {
            ...
            self.tensors[ca.y][ca.x] = PEPSTensor::from_data(new_ta, [du_a, new_bond, dl_a, dr_a]);
            self.tensors[cb.y][cb.x] = PEPSTensor::from_data(new_tb, [new_bond, dd_b, dl_b, dr_b]);
        }
    }
    ...
    for _ in 0..max_iter {
        ...
        for i in 0..n {
            if i != p && i != q {
                ...
                a[[i, q]] = -c64::new(sin, 0.0) * aip * phase + c64::new(cos, 0.0) * aiq;
                a[[p, i]] = a[[i, p]].conj();
                a[[q, i]] = a[[i, q]].conj();
            }
        }
...
mod tests {
    ...
    fn test_bell_state_horizontal() {
        ...
        assert!(
            (amps[0].norm_sqr() - 0.5).abs() < 1e-6,
            "|00> amp^2 = {}",
            amps[0].norm_sqr()
        );
        assert!(
            amps[1].norm_sqr() < 1e-6,
            "|01> should be ~0, got {}",
            amps[1].norm_sqr()
        );
        assert!(
            amps[2].norm_sqr() < 1e-6,
            "|10> should be ~0, got {}",
            amps[2].norm_sqr()
        );
        assert!(
            (amps[3].norm_sqr() - 0.5).abs() < 1e-6,
            "|11> amp^2 = {}",
            amps[3].norm_sqr()
        );
    }
