...
pub enum ClusterTebdError {
    ...
    BondDimExceeded { actual: usize, max: usize },
}
...
fn compact_svd(m: &Array2<Complex64>) -> (Array2<Complex64>, Vec<f64>, Array2<Complex64>) {
    let (rows, cols) = (m.nrows(), m.ncols());
    let k = rows.min(cols);
...
pub fn random_cluster_mps(n_sites: usize, physical_dim: usize, bond_dim: usize) -> ClusterMps {
    ...
    for i in 0..(n_sites.saturating_sub(1)) {
        ...
        let from_right = physical_dim
            .saturating_pow((n_sites - 1 - i) as u32)
            .min(bond_dim);
...
pub fn measure_local(mps: &ClusterMps, operator: &Array2<Complex64>, site: usize) -> Complex64 {
    ...
    for a in 0..bl {
        for sa in 0..da {
            for m in 0..bond {
                for sb in 0..db {
                    for b in 0..br {
                        theta[[a * da + sa, sb * br + b]] += ta[[a, sa, m]] * tb[[m, sb, b]];
                    }
                }
            }
        }
...
pub fn compute_mutual_information(mps: &ClusterMps, site_a: usize, site_b: usize) -> f64 {
...
pub fn build_entanglement_graph(mps: &ClusterMps, threshold: f64) -> Vec<(usize, usize, f64)> {
    let n = mps.num_sites;
...
pub fn merge_cluster_tensors(mps: &ClusterMps, sites: &[usize]) -> ArrayD<Complex64> {
    assert!(!sites.is_empty());
    assert!(
        sites.windows(2).all(|w| w[1] == w[0] + 1),
        "sites must be contiguous and sorted"
    );
            ...
            for m in 0..bond {
                ...
                for s in 0..d {
                    for b in 0..br {
                        ...
                        let new_flat = new_result.as_slice_mut().unwrap();
...
pub fn decompose_cluster_tensor(
    ...
    assert_eq!(
        shape.len(),
        k + 2,
        "tensor must have k+2 dimensions for k sites"
    );
    ...
    if k == 1 {
        ...
        let t3 = Array3::from_shape_vec((bl, d, br), t.into_raw_vec()).unwrap();
        return (vec![t3], 0.0);
    }
    ...
    for _i in 0..k - 1 {
        ...
        let site_tensor = Array3::from_shape_vec((bl, d, new_bond), u.into_raw_vec()).unwrap();
...
pub fn standard_tebd_step(
    ...
    let mat = theta_prime.into_shape((bl * da, db * br)).unwrap();
...
pub fn cluster_tebd(
    ...
    for _step in 0..config.num_steps {
        ...
        for (ci, cluster) in clusters.iter().enumerate() {
            ...
            let (new_tensors, trunc_err) = decompose_cluster_tensor(
                &merged,
                &cluster.sites,
                config.max_bond_dim,
                config.svd_cutoff,
...
mod tests {
    ...
    fn test_svd_no_truncation() {
        ...
        assert!(
            trunc_err < TOL,
            "no truncation error expected, got {}",
            trunc_err
        );
    ...
    fn test_cluster_tebd_ising_8_sites() {
        ...
        assert!(result.time_evolved > 0.0, "time_evolved should be positive");
        assert!(
