    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DmrgError::ConvergenceFailed {
                ...
                write!(
                    f,
                    "DMRG not converged after {} sweeps (E={:.10}, tol={:.1e})",
                    sweeps_done, last_energy, tolerance
                )
            }
...
impl DmrgConfig {
    ...
    pub fn energy_tolerance(mut self, t: f64) -> Self {
        self.energy_tolerance = t;
        self
    }
    ...
    pub fn lanczos_iterations(mut self, n: usize) -> Self {
        self.lanczos_iterations = n;
        self
    }
    pub fn noise(mut self, n: Vec<f64>) -> Self {
        self.noise = n;
        self
    }
}
...
impl TdvpConfig {
    ...
    pub fn method(mut self, m: TdvpMethod) -> Self {
        self.method = m;
        self
    }
    pub fn max_bond_dim(mut self, d: usize) -> Self {
...
pub fn svd_truncate(
    ...
    let vt_full = svd
        .v_t
        .ok_or_else(|| DmrgError::SvdFailed("No Vt".into()))?;
...
/// Create a computational-basis product-state MPS from a bitstring like `0101`.
///
/// Bit `0` maps to the +1 eigenstate of `Z` and bit `1` maps to the -1 eigenstate.
pub fn computational_basis_mps(bitstring: &str) -> Result<Mps, DmrgError> {
    if bitstring.is_empty() {
        return Err(DmrgError::InvalidBondDim(
            "computational-basis product states require at least one site".into(),
        ));
    }
    if !bitstring.chars().all(|bit| bit == '0' || bit == '1') {
        return Err(DmrgError::InvalidBondDim(format!(
            "invalid computational-basis bitstring '{bitstring}'"
        )));
    }

    let mut sites = Vec::with_capacity(bitstring.len());
    for bit in bitstring.chars() {
        let mut tensor = Array3::zeros((1, 2, 1));
        tensor[[0, if bit == '0' { 0 } else { 1 }, 0]] = c1();
        sites.push(MpsSite {
            tensor,
            physical_dim: 2,
            left_bond: 1,
    ...
    Ok(Mps {
        sites,
        center_position: 0,
        num_sites: bitstring.len(),
    })
}

/// Compress a dense statevector into an MPS using sequential SVD splits.
///
/// The input state must have length `2^num_sites`. Singular values are truncated
/// to `max_bond_dim` and filtered by the relative cutoff `cutoff`.
pub fn dense_statevector_to_mps(
    state: &[Complex64],
    num_sites: usize,
    max_bond_dim: usize,
    cutoff: f64,
) -> Result<Mps, DmrgError> {
    if num_sites == 0 {
        return Err(DmrgError::InvalidBondDim(
            "dense-state compression requires at least one site".into(),
        ));
    }
    if max_bond_dim == 0 {
        return Err(DmrgError::InvalidBondDim(
            "max_bond_dim must be at least 1 for dense-state compression".into(),
        ));
    }
    let expected_dim = 1usize
        .checked_shl(num_sites as u32)
        .ok_or_else(|| DmrgError::InvalidBondDim("num_sites is too large".into()))?;
    if state.len() != expected_dim {
        return Err(DmrgError::InvalidBondDim(format!(
            "dense state has dimension {} but expected {} for {} sites",
            state.len(),
            expected_dim,
            num_sites,
        )));
    ...
    let norm_sq: f64 = state.iter().map(|value| value.norm_sqr()).sum();
    if norm_sq <= 1e-30 {
        return Err(DmrgError::InvalidBondDim(
            "dense-state compression requires a non-zero norm".into(),
        ));
    }
    let inv_norm = 1.0 / norm_sq.sqrt();
    let mut current: Vec<Complex64> = state.iter().map(|value| *value * inv_norm).collect();
    let mut left_bond = 1usize;
    let mut sites = Vec::with_capacity(num_sites);
    let relative_cutoff = cutoff.max(0.0);

    for site in 0..num_sites.saturating_sub(1) {
        let right_dim = 1usize << (num_sites - site - 1);
        let matrix = Array2::from_shape_vec((left_bond * 2, right_dim), current)
            .map_err(|error| DmrgError::SvdFailed(error.to_string()))?;
        let (u, singular_values, vt) = svd_truncate(&matrix, max_bond_dim)?;
        let max_sigma = singular_values.first().copied().unwrap_or(0.0);
        let threshold = relative_cutoff * max_sigma;
        let keep = singular_values
            .iter()
            .take_while(|sigma| **sigma > threshold)
            .count()
            .max(1)
            .min(singular_values.len());

        let site_tensor = u
            .slice(s![.., 0..keep])
            .to_owned()
            .into_shape((left_bond, 2, keep))
            .map_err(|error| DmrgError::SvdFailed(error.to_string()))?;
        sites.push(MpsSite {
            tensor: site_tensor,
            physical_dim: 2,
        ...
        let mut next = vt.slice(s![0..keep, ..]).to_owned();
        for (row, sigma) in singular_values.iter().take(keep).enumerate() {
            for column in 0..next.ncols() {
                next[[row, column]] *= cr(*sigma);
            }
        }
        current = next.into_raw_vec();
        left_bond = keep;
    }

    let last_tensor = Array3::from_shape_vec((left_bond, 2, 1), current)
        .map_err(|error| DmrgError::SvdFailed(error.to_string()))?;
    sites.push(MpsSite {
        tensor: last_tensor,
        physical_dim: 2,
    ...
    let mut mps = Mps {
        sites,
        center_position: num_sites - 1,
        num_sites,
    };
    canonicalize_mps(&mut mps, 0);
    let norm = mps_norm(&mps);
    if norm > 1e-15 {
        let scale = cr(1.0 / norm);
        for value in mps.sites[0].tensor.iter_mut() {
            *value *= scale;
        }
    }
    Ok(mps)
...
/// Create a named product-state MPS for common quench initial states.
///
/// Supported labels:
/// - `all_up`
/// - `all_down`
/// - `neel` (mapped to `1010...` for Python compatibility)
/// - `plus_x`, `minus_x`, `plus_y`, `minus_y`
/// - `domain_wall`, `anti_domain_wall`
/// - explicit per-site product strings over `0`, `1`, `+`, `-`, `R`, `L`
pub fn named_product_state_mps(n_sites: usize, label: &str) -> Result<Mps, DmrgError> {
    if n_sites == 0 {
        return Err(DmrgError::InvalidBondDim(
            "product states require at least one site".into(),
        ));
    }

    let canonical = match label {
        "all_up" => "0".repeat(n_sites),
        "all_down" => "1".repeat(n_sites),
        "neel" => (0..n_sites)
            .map(|site| if site % 2 == 0 { '1' } else { '0' })
            .collect(),
        "plus_x" => "+".repeat(n_sites),
        "minus_x" => "-".repeat(n_sites),
        "plus_y" => "R".repeat(n_sites),
        "minus_y" => "L".repeat(n_sites),
        "domain_wall" => (0..n_sites)
            .map(|site| if site < n_sites / 2 { '0' } else { '1' })
            .collect(),
        "anti_domain_wall" => (0..n_sites)
            .map(|site| if site < n_sites / 2 { '1' } else { '0' })
            .collect(),
        _ if label.len() == n_sites
            && label
                .chars()
                .all(|symbol| matches!(symbol, '0' | '1' | '+' | '-' | 'R' | 'L' | 'r' | 'l')) =>
        {
            label
                .chars()
        ...
        _ => {
            return Err(DmrgError::InvalidBondDim(format!(
                "unsupported product-state label '{label}'"
            )))
        }
    };

    let mut sites = Vec::with_capacity(canonical.len());
    let scale = 1.0 / std::f64::consts::SQRT_2;
    for symbol in canonical.chars() {
        let mut tensor = Array3::zeros((1, 2, 1));
        match symbol {
            '0' => {
                tensor[[0, 0, 0]] = c1();
            ...
            '+' => {
                tensor[[0, 0, 0]] = cr(scale);
                tensor[[0, 1, 0]] = cr(scale);
            }
            '-' => {
                tensor[[0, 0, 0]] = cr(scale);
                tensor[[0, 1, 0]] = cr(-scale);
            ...
            'R' => {
                tensor[[0, 0, 0]] = cr(scale);
                tensor[[0, 1, 0]] = Complex64::new(0.0, scale);
            }
            'L' => {
                tensor[[0, 0, 0]] = cr(scale);
                tensor[[0, 1, 0]] = Complex64::new(0.0, -scale);
            ...
            _ => {
                return Err(DmrgError::InvalidBondDim(format!(
                    "unsupported product-state symbol '{symbol}'"
                )))
            }
        }
        sites.push(MpsSite {
...
pub fn canonicalize_mps(mps: &mut Mps, center: usize) {
    ...
    for i in 0..center {
        left_normalize_site(mps, i);
    }
    for i in (center + 1..n).rev() {
        right_normalize_site(mps, i);
    }
...
fn left_normalize_site(mps: &mut Mps, i: usize) {
    ...
    if i + 1 < mps.num_sites {
        ...
        let next_mat = mps.sites[i + 1]
            .tensor
            .clone()
            .into_shape((old_lb, d_next * rb_next))
            .unwrap();
...
fn right_normalize_site(mps: &mut Mps, i: usize) {
    ...
    if i > 0 {
        ...
        let prev_mat = mps.sites[i - 1]
            .tensor
            .clone()
            .into_shape((lb_prev * d_prev, old_rb))
            .unwrap();
...
pub fn build_mpo_heisenberg(n_sites: usize, j: f64, h: f64) -> MpoHamiltonian {
    build_mpo_heisenberg_xxz(n_sites, j, 1.0, h)
}

/// Build the MPO for the Heisenberg XXZ model:
///   H = Jxy sum_i (Sx_i Sx_{i+1} + Sy_i Sy_{i+1} + delta Sz_i Sz_{i+1}) + h sum_i Sz_i
/// MPO bond dimension: 5. Uses lower-triangular encoding:
///   Row 0 = absorbing (completed), Row 4 = identity (start).
pub fn build_mpo_heisenberg_xxz(n_sites: usize, jxy: f64, delta: f64, h: f64) -> MpoHamiltonian {
    let d = 2usize;
    let mpo_dim = 5usize;
    ...
    for i in 0..n_sites {
        ...
        if n_sites == 1 {
            ...
            // Row 4: [h*Sz, Jxy/2*Sp, Jxy/2*Sm, Jxy*delta*Sz, I]
            for s in 0..d {
                for sp_ in 0..d {
                    w[[0, s, sp_, 0]] = cr(h) * sz[[s, sp_]];
                    w[[0, s, sp_, 1]] = cr(jxy / 2.0) * sp[[s, sp_]];
                    w[[0, s, sp_, 2]] = cr(jxy / 2.0) * sm[[s, sp_]];
                    w[[0, s, sp_, 3]] = cr(jxy * delta) * sz[[s, sp_]];
            ...
            for s in 0..d {
                for sp_ in 0..d {
                    ...
                    w[[3, s, sp_, 0]] = sz[[s, sp_]];
                    w[[4, s, sp_, 0]] = cr(h) * sz[[s, sp_]];
                    w[[4, s, sp_, 1]] = cr(jxy / 2.0) * sp[[s, sp_]];
                    w[[4, s, sp_, 2]] = cr(jxy / 2.0) * sm[[s, sp_]];
                    w[[4, s, sp_, 3]] = cr(jxy * delta) * sz[[s, sp_]];
...
/// Build the MPO for the Heisenberg XYZ model:
///   H = sum_i (Jx Sx_i Sx_{i+1} + Jy Sy_i Sy_{i+1} + Jz Sz_i Sz_{i+1}) + h sum_i Sz_i
/// MPO bond dimension: 7. Uses lower-triangular encoding:
///   Row 0 = absorbing (completed), Row 6 = identity (start).
pub fn build_mpo_heisenberg_xyz(
    n_sites: usize,
    jx: f64,
    jy: f64,
    jz: f64,
    ...
    let exchange_flip = (jx + jy) / 4.0;
    let exchange_pair = (jx - jy) / 4.0;
    ...
    for i in 0..n_sites {
        ...
        if n_sites == 1 {
            ...
            for s in 0..d {
                for sp_ in 0..d {
                    w[[0, s, sp_, 0]] = cr(h) * sz[[s, sp_]];
                    w[[0, s, sp_, 1]] = cr(exchange_flip) * sp[[s, sp_]];
                    w[[0, s, sp_, 2]] = cr(exchange_flip) * sm[[s, sp_]];
                    w[[0, s, sp_, 3]] = cr(exchange_pair) * sp[[s, sp_]];
                    w[[0, s, sp_, 4]] = cr(exchange_pair) * sm[[s, sp_]];
                    w[[0, s, sp_, 5]] = cr(jz) * sz[[s, sp_]];
                    w[[0, s, sp_, 6]] = id2[[s, sp_]];
                }
            }
        } else if i == n_sites - 1 {
            for s in 0..d {
                for sp_ in 0..d {
                    ...
                    w[[1, s, sp_, 0]] = sm[[s, sp_]];
                    w[[2, s, sp_, 0]] = sp[[s, sp_]];
                    w[[3, s, sp_, 0]] = sp[[s, sp_]];
                    w[[4, s, sp_, 0]] = sm[[s, sp_]];
                    w[[5, s, sp_, 0]] = sz[[s, sp_]];
                    w[[6, s, sp_, 0]] = cr(h) * sz[[s, sp_]];
                }
            }
        } else {
            for s in 0..d {
                for sp_ in 0..d {
                    ...
                    w[[5, s, sp_, 0]] = sz[[s, sp_]];
                    w[[6, s, sp_, 0]] = cr(h) * sz[[s, sp_]];
                    w[[6, s, sp_, 1]] = cr(exchange_flip) * sp[[s, sp_]];
                    w[[6, s, sp_, 2]] = cr(exchange_flip) * sm[[s, sp_]];
                    w[[6, s, sp_, 3]] = cr(exchange_pair) * sp[[s, sp_]];
                    w[[6, s, sp_, 4]] = cr(exchange_pair) * sm[[s, sp_]];
                    w[[6, s, sp_, 5]] = cr(jz) * sz[[s, sp_]];
                    w[[6, s, sp_, 6]] = id2[[s, sp_]];
                }
            }
        }
        sites.push(w);
...
///   H = -J sum_i Sz_i Sz_{i+1} - hx sum_i Sx_i - hz sum_i Sz_i
/// MPO bond dimension: 3. Lower-triangular encoding:
///   Row 0 = absorbing, Row 2 = start/identity.
pub fn build_mpo_ising_fields(n_sites: usize, j: f64, hx: f64, hz: f64) -> MpoHamiltonian {
    let d = 2usize;
    let mpo_dim = 3usize;
    ...
    let onsite = sx.mapv(|value| value * cr(-hx)) + sz.mapv(|value| value * cr(-hz));

    // Bulk W (3x3):
    //   | I       0     0 |  row 0: absorbing
    //   | Sz      0     0 |  row 1: absorb Sz
    //   | onsite -J*Sz  I |  row 2: start/identity
...
pub fn build_mpo_ising(n_sites: usize, j: f64, h: f64) -> MpoHamiltonian {
    build_mpo_ising_fields(n_sites, j, h, 0.0)
}
...
pub fn lanczos_ground_state(
    ...
    if dim == 0 {
        return Err(DmrgError::LanczosFailed("Zero dim".into()));
    }
    if dim == 1 {
        let v = vec![c1()];
        let hv = h_eff(&v);
    ...
    let norm = vec_norm(&v);
    if norm < 1e-15 {
        return Err(DmrgError::LanczosFailed("Zero initial vector".into()));
    }
    ...
    for j in 0..m {
        ...
        let mut r: Vec<Complex64> = w
            .iter()
            .enumerate()
            .map(|(k, &wk)| {
                wk - cr(alpha) * basis[j][k]
                    - if j > 0 {
                        cr(betas[j - 1]) * v_prev[k]
                    } else {
                        c0()
                    }
            })
...
fn expm_tridiag(alphas: &[f64], betas: &[f64], dt: f64) -> Vec<Complex64> {
    ...
    for k in 0..n {
        ...
        let coeff = evecs[0][k];
        for j in 0..n {
            result[j] += cr(evecs[j][k]) * cr(coeff) * exp_f;
        }
    }
    result
}
...
pub fn dmrg(hamiltonian: &MpoHamiltonian, config: &DmrgConfig) -> Result<DmrgResult, DmrgError> {
    let n = hamiltonian.sites.len();
    if n == 0 {
        return Err(DmrgError::InvalidBondDim("Empty Hamiltonian".into()));
    }
    if config.max_bond_dim == 0 {
    ...
    prepare_mps_for_dmrg(&mut mps, n, phys_dim)?;
    dmrg_from_initialized_mps(mps, hamiltonian, config)
}

/// Run DMRG starting from an existing MPS instead of a random initialization.
pub fn dmrg_from_mps(
    initial_mps: &Mps,
    ...
    let phys_dim = hamiltonian.sites[0].shape()[1];
    let mut mps = initial_mps.clone();
    prepare_mps_for_dmrg(&mut mps, n, phys_dim)?;
    dmrg_from_initialized_mps(mps, hamiltonian, config)
}

fn prepare_mps_for_dmrg(mps: &mut Mps, num_sites: usize, physical_dim: usize) -> Result<(), DmrgError> {
    if mps.num_sites != num_sites || mps.sites.len() != num_sites {
        return Err(DmrgError::InvalidBondDim(format!(
            "initial MPS has {} sites but Hamiltonian has {} sites",
            mps.num_sites, num_sites
        )));
    }
    for (site_index, site) in mps.sites.iter().enumerate() {
        if site.physical_dim != physical_dim {
            return Err(DmrgError::InvalidBondDim(format!(
                "initial MPS site {site_index} has physical dimension {} but Hamiltonian requires {}",
                site.physical_dim, physical_dim
            )));
        }
    }
        ...
        match self.previous_energy {
            ...
            Some(prev_e) => {
                    ...
                    && self.current_bond_dim < self.max_bond_dim
                {
                    ...
                    self.current_bond_dim = (self.current_bond_dim * 2).min(self.max_bond_dim);
                    (self.current_bond_dim, false)
                } else {
...
/// Apply a local single-site operator to an MPS and return the new state.
pub fn apply_local_operator(mps: &Mps, operator: &Array2<Complex64>, site: usize) -> Mps {
    let n = mps.num_sites;
    assert!(site < n);

    let mut updated = mps.clone();
    let tensor = &mps.sites[site].tensor;
    let lb = tensor.shape()[0];
    let d = tensor.shape()[1];
    let rb = tensor.shape()[2];
    assert_eq!(operator.shape(), &[d, d]);

    let mut new_tensor = Array3::zeros((lb, d, rb));
    for left in 0..lb {
        for output in 0..d {
            for input in 0..d {
                let op = operator[[output, input]];
                if op.norm_sqr() < 1e-30 {
                    continue;
                }
                for right in 0..rb {
                    new_tensor[[left, output, right]] += op * tensor[[left, input, right]];
                }
            }
        }
    }

    updated.sites[site].tensor = new_tensor;
    updated
}
...
/// Measure the transition matrix element <bra|O_site|ket>.
pub fn measure_transition_local_observable(
    bra: &Mps,
    ket: &Mps,
    ...
    let n = bra.num_sites;
    assert!(site < n);
    assert_eq!(ket.num_sites, n);

    let mut env = Array2::zeros((1, 1));
    env[[0, 0]] = c1();

    for i in 0..n {
        let bra_t = &bra.sites[i].tensor;
        let ket_t = &ket.sites[i].tensor;
        let d = bra_t.shape()[1];
        assert_eq!(ket_t.shape()[1], d);
        let bra_rb = bra_t.shape()[2];
        let ket_rb = ket_t.shape()[2];
        let mut new_env = Array2::zeros((bra_rb, ket_rb));

        for bra_lb in 0..env.nrows() {
            for ket_lb in 0..env.ncols() {
                let e = env[[bra_lb, ket_lb]];
                if e.norm_sqr() < 1e-30 {
                    continue;
                }
                if i == site {
                    for s in 0..d {
                        for sp in 0..d {
                            ...
                            for bra_rb_index in 0..bra_rb {
                                let eb = e * bra_t[[bra_lb, s, bra_rb_index]].conj() * op;
                                for ket_rb_index in 0..ket_rb {
                                    new_env[[bra_rb_index, ket_rb_index]] +=
                                        eb * ket_t[[ket_lb, sp, ket_rb_index]];
                                }
                            }
                        }
                    }
                    ...
                    for s in 0..d {
                        for bra_rb_index in 0..bra_rb {
                            let eb = e * bra_t[[bra_lb, s, bra_rb_index]].conj();
                            for ket_rb_index in 0..ket_rb {
                                new_env[[bra_rb_index, ket_rb_index]] +=
                                    eb * ket_t[[ket_lb, s, ket_rb_index]];
                            }
                        }
                    }
                }
...
pub fn measure_correlation(
    mps: &Mps,
    op_a: &Array2<Complex64>,
    site_a: usize,
    op_b: &Array2<Complex64>,
    site_b: usize,
) -> Complex64 {
    let n = mps.num_sites;
...
pub fn entanglement_spectrum(mps: &Mps, bond: usize) -> Result<Vec<f64>, DmrgError> {
    let n = mps.num_sites;
    if bond >= n - 1 {
        return Err(DmrgError::InvalidBondDim("Bond index out of range".into()));
    }
...
pub fn entanglement_entropy(mps: &Mps, bond: usize) -> Result<f64, DmrgError> {
    let spectrum = entanglement_spectrum(mps, bond)?;
    let mut entropy = 0.0;
    for p in spectrum {
        entropy -= p * p.ln();
    }
...
fn tdvp2_step(
    ...
    let n = mps.num_sites;
    if n < 2 {
        return tdvp1_step(mps, hamiltonian, dt, lanczos_iter);
    }
    ...
    for i in 0..n - 1 {
        ...
        if i < n - 2 {
            ...
            // Backward-evolve site i+1 by -dt/2
            {
                ...
                let h_fn = move |v: &[Complex64]| -> Vec<Complex64> {
                    apply_h_eff_one_site(
                        &left_1s, &mpo_next, &right_1s, v, lb_next, d_next, rb_next,
                    )
                };
                let evolved_back = lanczos_expm(&h_fn, &vec_next, -half_dt, lanczos_iter)?;
                mps.sites[i + 1].tensor =
                    Array3::from_shape_vec((lb_next, d_next, rb_next), evolved_back)
                        .map_err(|e| DmrgError::SvdFailed(format!("{}", e)))?;
            }
...
/// Run a single TDVP step using the requested integration method.
pub fn tdvp_step(
    mps: &mut Mps,
    hamiltonian: &MpoHamiltonian,
    dt: f64,
    ...
    match method {
        TdvpMethod::OneSite => tdvp1_step(mps, hamiltonian, dt, lanczos_iter),
        TdvpMethod::TwoSite => tdvp2_step(mps, hamiltonian, dt, max_bond_dim, lanczos_iter),
    }
}
...
pub fn tdvp_evolve(
    ...
    if hamiltonian.sites.len() != n {
        return Err(DmrgError::InvalidBondDim(format!(
            "MPS has {} sites but Hamiltonian has {} sites",
            n,
            hamiltonian.sites.len()
        )));
    }
...
mod tests {
    ...
    fn test_random_mps_creation_and_normalization() {
        ...
        for i in 0..5 {
            assert_eq!(
                mps.sites[i].right_bond,
                mps.sites[i + 1].left_bond,
                "Bond mismatch between sites {} and {}",
                i,
                i + 1
            );
        }
    ...
    #[test]
    fn test_build_heisenberg_xxz_mpo_4_sites() {
        let mpo = build_mpo_heisenberg_xxz(4, 1.5, 0.75, 0.2);
        assert_eq!(mpo.sites.len(), 4);
        assert_eq!(mpo.sites[0].shape(), &[1, 2, 2, 5]);
        assert_eq!(mpo.sites[1].shape(), &[5, 2, 2, 5]);
        assert_eq!(mpo.sites[3].shape(), &[5, 2, 2, 1]);
    ...
    #[test]
    fn test_build_heisenberg_xyz_mpo_4_sites() {
        let mpo = build_mpo_heisenberg_xyz(4, 1.5, 0.75, 1.2, 0.2);
        assert_eq!(mpo.sites.len(), 4);
        assert_eq!(mpo.sites[0].shape(), &[1, 2, 2, 7]);
        assert_eq!(mpo.sites[1].shape(), &[7, 2, 2, 7]);
        assert_eq!(mpo.sites[3].shape(), &[7, 2, 2, 1]);
    }
    ...
    #[test]
    fn test_build_ising_fields_mpo_4_sites() {
        let mpo = build_mpo_ising_fields(4, 1.0, 0.5, 0.25);
        assert_eq!(mpo.sites.len(), 4);
        assert_eq!(mpo.sites[0].shape(), &[1, 2, 2, 3]);
        assert_eq!(mpo.sites[1].shape(), &[3, 2, 2, 3]);
        assert_eq!(mpo.sites[3].shape(), &[3, 2, 2, 1]);
    ...
    fn test_dmrg_heisenberg_4_site() {
        ...
        assert!(
            (result.energy - (-1.6160254)).abs() < 0.5,
            "DMRG energy {} too far from exact -1.616",
            result.energy
        );
    }
    ...
    fn test_dmrg_ising_6_site_critical() {
        ...
        let config = DmrgConfig::new()
            ...
            .lanczos_iterations(30)
            .noise(vec![1e-3, 1e-4, 1e-5, 0.0]);

        let result = dmrg(&mpo, &config).unwrap();
        assert!(
            result.energy < 0.0,
            "Ising GS energy should be negative, got {}",
            result.energy
        );
    }
    ...
    fn test_tdvp_one_site_preserves_norm() {
        ...
        assert!(
            (norm_after - norm_before).abs() < 0.15,
    ...
    fn test_mps_overlap_orthogonality() {
        ...
        assert!(
            (self_overlap.re - norm_sq).abs() < 1e-6,
            "Self overlap {} should equal norm^2 {}",
            self_overlap.re,
            norm_sq
        );
    ...
    #[test]
    fn test_named_product_state_neel_matches_python_convention() {
        let mps = named_product_state_mps(4, "neel").unwrap();
        let sz = Array2::from_shape_vec((2, 2), vec![cr(0.5), c0(), c0(), cr(-0.5)]).unwrap();

        let site0 = measure_local_observable(&mps, &sz, 0);
        let site1 = measure_local_observable(&mps, &sz, 1);
        let site2 = measure_local_observable(&mps, &sz, 2);

        assert!((site0.re + 0.5).abs() < 1e-10, "site 0 should start down");
        assert!((site1.re - 0.5).abs() < 1e-10, "site 1 should start up");
        assert!((site2.re + 0.5).abs() < 1e-10, "site 2 should start down");
    }

    #[test]
    fn test_named_product_state_plus_x_has_zero_z_and_positive_x() {
        let mps = named_product_state_mps(3, "plus_x").unwrap();
        let sz = Array2::from_shape_vec((2, 2), vec![cr(0.5), c0(), c0(), cr(-0.5)]).unwrap();
        let sx = Array2::from_shape_vec((2, 2), vec![c0(), cr(0.5), cr(0.5), c0()]).unwrap();

        let z_expectation = measure_local_observable(&mps, &sz, 0);
        let x_expectation = measure_local_observable(&mps, &sx, 0);

        assert!(z_expectation.norm() < 1e-10, "plus_x should have zero Z polarization");
        assert!(
            (x_expectation.re - 0.5).abs() < 1e-10,
            "plus_x should have positive X polarization"
        );
    }

    #[test]
    fn test_named_product_state_domain_wall_matches_expected_pattern() {
        let mps = named_product_state_mps(5, "domain_wall").unwrap();
        let sz = Array2::from_shape_vec((2, 2), vec![cr(0.5), c0(), c0(), cr(-0.5)]).unwrap();

        let expectations: Vec<f64> = (0..5)
            .map(|site| measure_local_observable(&mps, &sz, site).re)
            .collect();

        assert!((expectations[0] - 0.5).abs() < 1e-10);
        assert!((expectations[1] - 0.5).abs() < 1e-10);
        assert!((expectations[2] + 0.5).abs() < 1e-10);
        assert!((expectations[3] + 0.5).abs() < 1e-10);
        assert!((expectations[4] + 0.5).abs() < 1e-10);
    }

    #[test]
    fn test_dense_statevector_to_mps_reconstructs_product_state() {
        let scale = 1.0 / std::f64::consts::SQRT_2;
        let dense = vec![
        ...
        let compressed = dense_statevector_to_mps(&dense, 2, 4, 1e-12).unwrap();
        let expected = named_product_state_mps(2, "0+").unwrap();

        let overlap = mps_overlap(&expected, &compressed);
        assert!(
            (overlap.norm() - 1.0).abs() < 1e-10,
            "compressed product state should match exactly, overlap={overlap}"
        );
    }

    #[test]
    fn test_dense_statevector_to_mps_preserves_bell_entanglement_spectrum() {
        let scale = 1.0 / std::f64::consts::SQRT_2;
        let dense = vec![
            Complex64::new(scale, 0.0),
            c0(),
        ...
        let compressed = dense_statevector_to_mps(&dense, 2, 4, 1e-12).unwrap();
        let spectrum = entanglement_spectrum(&compressed, 0).unwrap();

        assert_eq!(spectrum.len(), 2);
        assert!((spectrum[0] - 0.5).abs() < 1e-10);
        assert!((spectrum[1] - 0.5).abs() < 1e-10);
    }
    ...
    fn test_tdvp2_entanglement_growth() {
        ...
        assert!(
            ent_before < 1e-10,
            "Product state should have zero entanglement, got {}",
            ent_before
        );
        ...
        assert!(
            ent_after > 1e-6,
            "TDVP2 should generate entanglement from Neel state, got S={:.6}",
            ent_after
        );
    }
    ...
    fn test_tdvp_norm_conservation_tight() {
        ...
        assert!(
            (norm_final - 1.0).abs() < 0.05,
            "TDVP1 norm should be preserved: initial=1.0, final={:.8}",
            norm_final
        );
    }
    ...
    fn test_tdvp2_norm_conservation() {
        ...
        assert!(
            (norm_final - 1.0).abs() < 0.1,
            "TDVP2 norm should be approximately preserved: initial=1.0, final={:.8}",
            norm_final
        );
    }
    ...
    fn test_tdvp_ising_imaginary_time_ground_state() {
        ...
        assert!(
            e_final > gs_energy - 1.0,
            "TDVP energy {} should be above ground state {}",
            e_final,
            gs_energy
        );
    }
    ...
    fn test_tdvp_evolve_error_on_mismatch() {
        ...
        assert!(
            result.is_err(),
            "Should error on MPS/Hamiltonian size mismatch"
        );
    }
    ...
    fn test_tdvp1_bond_dimension_preserved() {
        ...
        let bonds_after: Vec<usize> = result
            .final_mps
            .sites
            .iter()
            .map(|s| s.right_bond)
        ...
        for (i, (&before, &after)) in bonds_before.iter().zip(bonds_after.iter()).enumerate() {
            assert!(
                after <= before,
                "TDVP1 bond at site {} grew from {} to {}",
                i,
                before,
                after
            );
    ...
    fn test_tdvp2_bond_dimension_can_grow() {
        ...
        let max_bond_after: usize = result
            .final_mps
            .sites
            .iter()
            .map(|s| s.right_bond)
        ...
        assert!(
            max_bond_after <= 8,
            "Bond dimension {} exceeds max_bond_dim=8",
            max_bond_after
        );
    }
    ...
    fn test_mps_energy_computation() {
        ...
        assert!(
            (energy - (-0.75)).abs() < 1e-8,
            "Neel state Heisenberg energy should be -0.75, got {}",
            energy
        );
    }
    ...
    fn test_zero_site_effective_hamiltonian() {
        ...
        for (i, (&r, &v)) in result.iter().zip(vec.iter()).enumerate() {
            assert!(
                (r - v).norm() < 1e-10,
                "Zero-site H_eff mismatch at index {}: got {}, expected {}",
                i,
                r,
                v
            );
    ...
    fn test_entanglement_entropy_product_state() {
        ...
        for bond in 0..n - 1 {
            ...
            assert!(
                ent < 1e-10,
                "Product state entanglement at bond {} should be 0, got {}",
                bond,
                ent
            );
        }
    ...
    #[test]
    fn test_entanglement_spectrum_product_state() {
        let n = 4;
        let mps = make_all_up_mps(n);
        for bond in 0..n - 1 {
            let spectrum = entanglement_spectrum(&mps, bond).unwrap();
    ...
    fn test_entanglement_entropy_bell_state() {
        ...
        assert!(
            (ent - expected).abs() < 1e-6,
            "Bell state entropy should be ln(2)={:.6}, got {:.6}",
            expected,
            ent
        );

        let spectrum = entanglement_spectrum(&mps, 0).unwrap();
        assert_eq!(spectrum.len(), 2);
        assert!((spectrum[0] - 0.5).abs() < 1e-6);
        assert!((spectrum[1] - 0.5).abs() < 1e-6);
    }
    ...
    fn test_tdvp_legacy_interface_compatibility() {
        ...
        assert!(
            (norm - 1.0).abs() < 0.1,
            "Legacy tdvp() norm not preserved: {}",
            norm
        );
    }
    ...
    fn test_svd_truncate_zero_matrix() {
        ...
        assert!(
            s.len() >= 1,
            "SVD of zero matrix must produce at least rank 1, got {}",
            s.len()
        );
        ...
        assert_eq!(
            u.shape()[1],
            s.len(),
            "U cols should match num singular values"
        );
        assert_eq!(
            vt.shape()[0],
            s.len(),
            "Vt rows should match num singular values"
        );
    ...
    fn test_svd_truncate_near_zero_matrix() {
        ...
        assert!(
            s.len() >= 1,
            "Near-zero matrix must produce at least rank 1, got {}",
            s.len()
        );
        ...
        for sv in &s {
            assert!(
                *sv < 1e-10,
                "Singular value {} unexpectedly large for near-zero matrix",
                sv
            );
        }
    ...
    fn test_adaptive_scheduler_growth() {
        ...
        assert_eq!(
            dim, 32,
            "Should double from 16 to 32 when delta_E < growth_threshold"
        );
        assert!(!conv);
    }
    ...
    fn test_dmrg_adaptive_heisenberg() {
        ...
        assert!(
            result.energy < -0.5,
            "Adaptive DMRG energy should be negative, got {}",
            result.energy
        );
    ...
    fn test_dmrg_adaptive_ising() {
        ...
        assert!(
            result.energy < 0.0,
            "Adaptive Ising DMRG energy should be negative, got {}",
            result.energy
        );
        assert!(result.energy.is_finite());
    }
