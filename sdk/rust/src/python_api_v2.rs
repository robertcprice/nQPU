...
use crate::{QuantumSimulator, gates::Gate};
use crate::jax_bridge::{JAXCircuit, JaxComplex, parameter_shift_grad, vmap_simulate, vmap_expectation};
...
impl PyQuantumCircuit {
    ...
    fn cx(&mut self, control: usize, target: usize) -> PyResult<()> {
        ...
        if control == target {
            return Err(PyValueError::new_err("Control and target must be different"));
        }
    ...
    fn __repr__(&self) -> String {
        format!("<QuantumCircuit {} qubits, {} gates>", self.num_qubits, self.gates.len())
    }
}
...
impl PySimulator {
    ...
    fn run(&self, circuit: &PyQuantumCircuit, shots: Option<usize>) -> PyResult<PySimulationResult> {
...
fn py_jax_gradient(circuit: &PyJAXCircuit, params: Vec<f32>, qubit: usize) -> PyResult<Vec<f32>> {
    ...
    Ok(parameter_shift_grad(&circuit.circuit, &params, qubit, shift))
}

/// VMAP: batch simulate multiple parameter sets
#[cfg(feature = "python")]
#[pyfunction]
fn py_jax_vmap_simulate(circuit: &PyJAXCircuit, batch_params: Vec<Vec<f32>>) -> PyResult<Vec<Vec<(f32, f32)>>> {
...
fn py_jax_vmap_expectation(
    ...
    Ok(vmap_expectation(&circuit.circuit, &batch_params, observable))
}
...
mod tests {
    ...
    fn test_qubit_bounds_validation_logic() {
        ...
        for q in 0..num_qubits {
            assert!(q < num_qubits, "qubit {} should be valid for {} qubits", q, num_qubits);
        }

        // Invalid: index == num_qubits
        assert!(!(num_qubits < num_qubits), "index == num_qubits must be rejected");
    ...
    fn test_jax_circuit_hadamard_superposition() {
        ...
        assert!((state[0].real - inv_sqrt2).abs() < 1e-5,
            "expected {}, got {}", inv_sqrt2, state[0].real);
        assert!((state[1].real - inv_sqrt2).abs() < 1e-5,
            "expected {}, got {}", inv_sqrt2, state[1].real);
    }
    ...
    fn test_jax_circuit_x_gate_flip() {
        ...
        assert!((state[1].real - 1.0).abs() < 1e-6, "|1> amplitude should be 1");
    }
    ...
    fn test_repr_format_strings() {
        ...
        let num_qubits = 5;
        let num_gates = 12;
        let repr = format!("<QuantumCircuit {} qubits, {} gates>", num_qubits, num_gates);
    ...
    fn test_jax_expectation_z_observable() {
        ...
        assert!((exp_pi - (-1.0)).abs() < 1e-3, "expected ~-1.0, got {}", exp_pi);
    ...
    fn test_parameter_shift_gradient() {
        ...
        assert!(grad_0[0].abs() < 0.15, "gradient at 0 should be near 0, got {}", grad_0[0]);
        ...
        assert!(grad_half[0].abs() > 0.5,
            "gradient at pi/2 should be significant, got {}", grad_half[0]);
    }
}
