...
impl QuboMatrix {
    ...
    pub fn add_linear(&mut self, i: usize, coeff: f64) {
        assert!(i < self.size, "Variable index {} out of bounds (size {})", i, self.size);
    ...
    pub fn add_quadratic(&mut self, i: usize, j: usize, coeff: f64) {
        assert!(i < self.size, "Variable index {} out of bounds (size {})", i, self.size);
        assert!(j < self.size, "Variable index {} out of bounds (size {})", j, self.size);
...
pub fn evaluate_qubo(qubo: &QuboMatrix, solution: &[u8]) -> f64 {
    assert_eq!(solution.len(), qubo.size, "Solution length must match QUBO size");
...
pub fn evaluate_ising(ising: &IsingModel, spins: &[i8]) -> f64 {
    assert_eq!(spins.len(), ising.num_spins, "Spins length must match Ising size");
...
pub fn brute_force_solve(qubo: &QuboMatrix) -> QuboSolution {
    let n = qubo.size;
    assert!(n <= 20, "Brute force is only feasible for n <= 20, got n = {}", n);
...
mod tests {
    ...
    fn test_ising_qubo_round_trip_energy() {
        ...
        for bits in 0u8..8 {
            let spins: Vec<i8> = (0..3).map(|k| if (bits >> k) & 1 == 0 { -1 } else { 1 }).collect();
    ...
    fn test_tsp_3_cities_brute_force() {
        ...
        for i in 0..n {
            ...
            assert_eq!(city_sum, 1, "City {} should be visited exactly once, got {}", i, city_sum);
        }

        // Each time step should have exactly one city
        for t in 0..n {
            let time_sum: u8 = (0..n).map(|i| assignment[i][t]).sum();
            assert_eq!(time_sum, 1, "Time {} should have exactly one city, got {}", t, time_sum);
        }
    }
    ...
    fn test_vertex_cover() {
        ...
        for &(i, j) in &edges {
            assert!(
                ...
                "Edge ({}, {}) not covered", i, j
            );
        }

        // Optimal: select node 1 covers both edges (cover size = 1)
        let cover_size: u8 = sol.bitstring.iter().sum();
        assert_eq!(cover_size, 1, "Minimum vertex cover of path 0-1-2 is 1, got {}", cover_size);
    }
    ...
    fn test_independent_set() {
        ...
        for &(i, j) in &edges {
            assert!(
                !(sol.bitstring[i] == 1 && sol.bitstring[j] == 1),
                "Adjacent nodes {} and {} both selected", i, j
            );
        }

        // For a triangle, max independent set = 1
        let set_size: u8 = sol.bitstring.iter().sum();
        assert_eq!(set_size, 1, "Max independent set of triangle is 1, got {}", set_size);
    }
    ...
    fn test_ising_evaluation_manual() {
        ...
        assert!((e_mp - (-0.75)).abs() < 1e-10, "E(-1,+1) = -0.75, got {}", e_mp);
    ...
    fn test_brute_force_global_minimum() {
        ...
        assert_eq!(sol.bitstring, vec![1, 0, 1], "Optimal solution should be [1, 0, 1]");
        assert!(
    ...
    fn test_qaoa_hamiltonian_energy_consistency() {
        ...
        for bits in 0u8..8 {
            let solution: Vec<u8> = (0..3).map(|k| (bits >> k) & 1).collect();
            let spins: Vec<i8> = solution.iter().map(|&b| if b == 0 { -1i8 } else { 1i8 }).collect();
            ...
            assert!(
                ...
                solution, qubo_energy, ham_energy
            );
        }
    }
}
