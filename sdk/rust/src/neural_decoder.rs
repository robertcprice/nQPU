...
impl GNNWeights {
    ...
    pub fn random(
        ...
        for l in 0..num_layers {
            ...
            let w = Array2::from_shape_fn((out_d, in_d), |(_i, _j)| {
    ...
    pub fn import_flat(
        ...
        for l in 0..num_layers {
            ...
            let w = Array2::from_shape_vec(
                (out_d, in_d),
                data[offset..offset + w_len].to_vec(),
            )
        ...
        for epoch in 0..self.config.max_epochs {
            ...
            for &sample_idx in &indices {
                ...
                for layer_idx in (0..self.config.num_layers).rev() {
                    ...
                    let in_d = input.len();
                    let d_w = Array2::from_shape_fn((out_d, in_d), |(i, j)| {
                        d_pre_act[i] * input[j]
                    });
                    ...
                    self.weights.layers[layer_idx]
                        .zip_mut_with(&d_w, |w, &dw| *w -= lr * dw);
                    self.weights.biases[layer_idx]
                        .zip_mut_with(&d_b, |b, &db| *b -= lr * db);
                }
            }
...
mod tests {
    ...
    fn test_different_activations() {
        ...
        for activation in &[ActivationFn::ReLU, ActivationFn::Sigmoid, ActivationFn::Tanh] {
            let config = NeuralDecoderConfig::new()
                .num_layers(2)
                .hidden_dim(8)
