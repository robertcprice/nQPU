    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SlcError::CircuitTooDeep(d) => {
                write!(f, "Circuit depth {} exceeds maximum {}", d, MAX_CIRCUIT_DEPTH)
            }
        }
    }
}
...
impl Lightcone {
    ...
    pub fn merge(&mut self, other: &Lightcone) {
        ...
        for (a, b) in self.active_qubits.iter_mut().zip(other.active_qubits.iter()) {
            *a = *a || *b;
        }
...
impl DependencyGraph {
    ...
    pub fn circuit_depth(&self) -> usize {
        self.gates.iter().map(|n| n.depth).max().map_or(0, |d| d + 1)
    }
}
...
pub fn build_dependency_graph(circuit: &QuantumCircuit) -> DependencyGraph {
    ...
    DependencyGraph { gates: nodes, edges }
}
...
/// on their qubits.
pub fn simplify_gates(gates: &[SlcGate], remove_identities: bool, merge_rotations: bool) -> Vec<SlcGate> {
...
pub fn circuit_depth(gates: &[SlcGate], num_qubits: usize) -> usize {
    ...
    for gate in gates {
        ...
        let qs = gate.qubits();
        let max_d = qs.iter().filter(|&&q| q < num_qubits).map(|&q| qubit_depth[q]).max().unwrap_or(0);
...
pub fn analyze_reduction(
    ...
    let original_gates = circuit
        .gates
        .iter()
        .filter(|g| !g.is_barrier())
        .count();
    let reduced_gates = shaded
        .gates
        .iter()
        .filter(|g| !g.is_barrier())
        .count();
...
mod tests {
    ...
    fn test_backward_lightcone_simple() {
        ...
        assert!(lc.active_qubits[0], "qubit 0 should be in backward cone of qubit 1");
        assert!(lc.active_qubits[1], "qubit 1 should be in backward cone");
        assert!(!lc.active_qubits[2], "qubit 2 should NOT be in backward cone");
        assert!(!lc.active_qubits[3], "qubit 3 should NOT be in backward cone");
    ...
    fn test_bidirectional_tighter() {
        ...
        for q in 0..circuit.num_qubits {
            if bidir.active_qubits[q] {
                assert!(back.active_qubits[q], "bidirectional should be subset of backward");
            }
        }
    ...
    fn test_all_to_all_full_lightcone() {
        ...
        for q in 0..5 {
            assert!(lc.active_qubits[q], "qubit {} should be active in all-to-all", q);
        }
    ...
    fn test_gate_outside_cone_removed() {
        ...
        assert!(shaded.gates.iter().any(|g| matches!(g, SlcGate::Measure(_))));
    }
    ...
    fn test_rotation_cancel_to_zero() {
        ...
        let simplified = simplify_gates(&gates, false, true);
        assert_eq!(simplified.len(), 0, "Rz(1.5) + Rz(-1.5) = identity, both removed");
    }
