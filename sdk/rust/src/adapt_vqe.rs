...
impl OperatorPool {
            ...
            for j in (i + 1)..occupied.len() {
                for a in 0..virtual_orbs.len() {
                    for b in (a + 1)..virtual_orbs.len() {
                        ...
                        let fermionic =
                            FermionicOperator::new(vec![p, q], vec![s, r], 1.0);
    ...
    pub fn qubit_adapt_pool(n_qubits: usize) -> Self {
        ...
        let two_body_patterns: [(char, char); 4] = [
            ('X', 'Y'),
            ('Y', 'X'),
            ('Y', 'Z'),
            ('Z', 'Y'),
...
fn jordan_wigner_double_excitation(
    p: usize,
    q: usize,
    r: usize,
    s: usize,
...
fn apply_pauli_rotation(state: &mut [Complex64], pauli_string: &[(usize, char)], theta: f64) {
    ...
    for i in 0..state.len() {
        state[i] = Complex64::new(cos_t, 0.0) * state[i]
            + Complex64::new(0.0, -sin_t) * p_state[i];
    }
}
...
fn lbfgs_minimize(
    ...
    for _iter in 0..max_iter {
        ...
        // Update position
        let x_new: Vec<f64> = x.iter().zip(direction.iter()).map(|(xi, di)| xi + alpha * di).collect();
...
fn lbfgs_two_loop(
    ...
    let gamma = if k > 0 {
        let ys: f64 = y_history[k - 1]
            ...
            .map(|(yi, si)| yi * si)
            .sum();
        let yy: f64 = y_history[k - 1]
            .iter()
            .map(|yi| yi * yi)
            .sum();
        if yy > 1e-20 { ys / yy } else { 1.0 }
    } else {
        1.0
    };
...
fn backtracking_line_search(
    ...
    let slope: f64 = grad.iter().zip(direction.iter()).map(|(gi, di)| gi * di).sum();
...
fn nelder_mead_minimize(
    ...
    for _iter in 0..max_iter {
        ...
        if f_reflected < values[0] {
            ...
            let ref_point = if use_reflected { &reflected } else { &simplex[n] };
            let f_ref = if use_reflected { f_reflected } else { values[n] };
...
impl AdaptVqe {
    ...
    pub fn run(&self, hamiltonian: &MolecularHamiltonian) -> AdaptVqeResult {
        ...
        for iteration in 0..self.max_iterations {
            ...
            for (idx, candidate) in self.pool.operators.iter().enumerate() {
                let grad = self.compute_pool_gradient(
                    hamiltonian,
                    &selected_ops,
                    &parameters,
                    candidate,
            ...
            let optimized = self.optimize_parameters(
                hamiltonian,
                &selected_ops,
                &parameters,
            );
    ...
    fn compute_pool_gradient(
        ...
        let mut state_plus = build_ansatz_state(
            self.n_qubits,
            self.n_electrons,
            current_ops,
            current_params,
        ...
        // E(-eps): current ansatz + candidate at -eps
        let mut state_minus = build_ansatz_state(
            self.n_qubits,
            self.n_electrons,
            current_ops,
            current_params,
...
mod tests {
    ...
    fn test_qubit_adapt_pool() {
        ...
        assert_eq!(
            single_y_count, 4,
            "Should have one Y operator per qubit"
        );
    ...
    fn test_h2_hamiltonian_construction() {
        ...
        assert_eq!(identity_terms.len(), 1, "Should have exactly one identity term");
        assert!(
    ...
    fn test_parameter_optimization() {
        ...
        let objective = |params: &[f64]| -> f64 {
            (params[0] - 1.0).powi(2) + (params[1] + 0.5).powi(2)
        };
        let grad = |params: &[f64]| -> Vec<f64> {
            vec![2.0 * (params[0] - 1.0), 2.0 * (params[1] + 0.5)]
        };
        ...
        assert!(result.value < 1e-6, "Minimum should be ~0, got {}", result.value);

        // Test Nelder-Mead fallback as well
        let (nm_params, nm_val) =
            nelder_mead_minimize(&objective, &[5.0, 5.0], 1000, 1e-12);
        assert!(
