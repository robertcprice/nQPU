    ...
    pub fn temporal_correlation(&self, t1: usize, t2: usize) -> C64 {
        ...
        for i in 0..self.dim {
            for j in 0..self.dim {
                ...
                correlation = correlation + self.elements[t2][t1][i][j] *
                    self.elements[t1][t1][j][i];
            }
        }
...
impl QuantumChannel {
    ...
    pub fn identity(dim: usize) -> Self {
        ...
        QuantumChannel { dim, matrix, kraus_ops }
    }
        ...
        for t in 0..self.n_steps {
            let channel = match environment {
                EnvironmentModel::Ohmic(cutoff) => {
                    ...
                    QuantumChannel::dephasing(self.dim, rate * (t as f64 + 1.0) / self.n_steps as f64)
                }
...
pub fn print_benchmark() {
    ...
    println!("{}", "-".repeat(70));
    println!("{:<10} {:<15} {:<15} {:<15}", "Steps", "Time (s)", "Non-Mark", "States");
    println!("{}", "-".repeat(70));
