...
mod tests {
    ...
    fn test_mean_variance_consistency() {
        ...
        let expected_mean =
            report.gradient_variances.iter().sum::<f64>() / report.gradient_variances.len() as f64;
        assert!(
    ...
    fn test_expressibility_product_vs_entangling() {
        let nq = 2;
        let kl_product = ExpressibilityAnalysis::kl_divergence(&product_circuit(nq), nq, nq, 200);
        let kl_entangling =
            ExpressibilityAnalysis::kl_divergence(&entangling_circuit(nq), nq, nq, 200);
    ...
    fn test_kl_divergence_nonneg() {
        let nq = 2;
        let kl = ExpressibilityAnalysis::kl_divergence(&entangling_circuit(nq), nq, nq, 100);
    ...
    fn test_average_entanglement_ordering() {
        let nq = 3;
        let avg_product =
            EntanglementCapability::average_entanglement(&product_circuit(nq), nq, nq, 30);
        let avg_entangling =
            EntanglementCapability::average_entanglement(&entangling_circuit(nq), nq, nq, 30);
    ...
    fn test_parameter_slice_values_bounded() {
        ...
        let slice =
            CostLandscapeVisualization::parameter_slice(&template, &params, 0, (0.0, PI), nq, 10);
    ...
    fn test_gradient_norm_distribution_count() {
        ...
        let norms =
            CostLandscapeVisualization::gradient_norm_distribution(&template, nq, nq, num_samples);
