...
impl JAXCircuit {
    ...
    fn apply_h(&self, state: &mut [JaxComplex], qubit: usize) {
        ...
        for i in (0..state.len()).step_by(2 * stride) {
            for j in 0..stride {
                ...
                state[i + j] =
                    JaxComplex::new((a.real + b.real) * inv_sqrt2, (a.imag + b.imag) * inv_sqrt2);
                state[i + j + stride] =
                    JaxComplex::new((a.real - b.real) * inv_sqrt2, (a.imag - b.imag) * inv_sqrt2);
            }
        }
    ...
    fn apply_z(&self, state: &mut [JaxComplex], qubit: usize) {
        ...
        for i in (0..state.len()).step_by(2 * stride) {
            for j in 0..stride {
                state[i + j + stride] =
                    JaxComplex::new(-state[i + j + stride].real, -state[i + j + stride].imag);
            }
        }
    }
    ...
    fn apply_rx(&self, state: &mut [JaxComplex], qubit: usize, angle: f32) {
        ...
        for i in (0..state.len()).step_by(2 * stride) {
            for j in 0..stride {
                ...
                state[i + j + stride] =
                    JaxComplex::new(b.real * c - a.real * s, b.imag * c - a.imag * s);
            }
        }
    }
    ...
    fn apply_ry(&self, state: &mut [JaxComplex], qubit: usize, angle: f32) {
        ...
        for i in (0..state.len()).step_by(2 * stride) {
            for j in 0..stride {
                ...
                state[i + j + stride] =
                    JaxComplex::new(a.real * s + b.real * c, a.imag * s + b.imag * c);
            }
        }
    }
...
pub fn vmap_expectation(
    circuit: &JAXCircuit,
    batch_params: &[Vec<f32>],
    observable: &str,
) -> Vec<f32> {
    batch_params
        .iter()
        .map(|params| circuit.expectation(params, observable))
        .collect()
}
