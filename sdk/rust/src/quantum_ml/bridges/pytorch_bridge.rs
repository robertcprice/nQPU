...
fn mat_z() -> [C64; 4] {
    [C64::one(), C64::zero(), C64::zero(), C64::new(-1.0, 0.0)]
}
...
impl Default for GradientMethod {
    fn default() -> Self {
        GradientMethod::ParameterShift { shift: PI / 2.0 }
    }
}
...
impl DifferentiableCircuit {
    ...
    fn apply_gate(sv: &mut StateVec, gate: &ParametricGate, params: &[f64]) {
        match gate {
            ...
            ParametricGate::CRx(c, t, p) => sv.apply_controlled(*c, *t, mat_rx(params[*p])),
            ParametricGate::CRy(c, t, p) => sv.apply_controlled(*c, *t, mat_ry(params[*p])),
            ParametricGate::CRz(c, t, p) => sv.apply_controlled(*c, *t, mat_rz(params[*p])),
    ...
    fn apply_gate_adjoint(sv: &mut StateVec, gate: &ParametricGate, params: &[f64]) {
        ...
        match gate {
            ...
            ParametricGate::U3(q, ti, pi, li) => {
                ...
                sv.apply_single(*q, mat_u3(-params[*ti], -params[*li], -params[*pi]))
            }
        }
    }
    ...
    fn measure_observable(sv: &StateVec, obs: &Observable) -> f64 {
        match obs {
            ...
            Observable::Hamiltonian(weighted) => weighted
                .iter()
    ...
    fn expectation_pauli_y(sv: &StateVec, qubit: usize) -> f64 {
        ...
        let sdg = [C64::one(), C64::zero(), C64::zero(), C64::new(0.0, -1.0)];
    ...
    pub fn backward(&self, observables: &[Observable]) -> Result<BackwardResult, BridgeError> {
    ...
    pub fn backward_with_params(
        ...
        match &self.gradient_method {
            ...
            GradientMethod::AdjointDiff => self.gradient_adjoint(params, observables),
            GradientMethod::FiniteDifference { epsilon } => {
                self.gradient_finite_difference(params, observables, *epsilon)
            }
            GradientMethod::Backprop => self.gradient_backprop(params, observables),
        }
    }
        ...
        for (o_idx, obs) in observables.iter().enumerate() {
            ...
            for (g_idx, gate) in self.gates.iter().enumerate().rev() {
                ...
                for p_idx in gate.param_indices() {
                    ...
                    let contrib =
                        self.gate_derivative_overlap(gate, p_idx, params, psi_before, &lambda);
...
impl BatchExecutor {
    ...
    pub fn execute(&self, param_batch: &[Vec<f64>]) -> Result<Vec<ForwardResult>, BridgeError> {
...
impl AdamState {
    ...
    fn step(&mut self, params: &mut [f64], grads: &[f64], lr: f64, beta1: f64, beta2: f64) {
        self.t += 1;
        let eps = 1e-8;
        ...
        match &self.optimizer {
                ...
                for iter in 0..self.max_iterations {
                    ...
                    for _ in 0..max_ls {
                        let trial: Vec<f64> = params
                            ...
                            .map(|(p, g)| p - alpha * g)
                            .collect();
                        let trial_fwd = self.circuit.forward_with_params(&trial, &obs)?;
            ...
            VqeOptimizer::SPSA { a, c } => {
                ...
                for iter in 0..self.max_iterations {
                    ...
                    let diff = ep.expectation_values[0] - em.expectation_values[0];
...
impl QuantumKernelML {
    ...
    pub fn kernel_matrix(&self, data: &[Vec<f64>]) -> Result<Vec<Vec<f64>>, BridgeError> {
        let n = data.len();
        let mut matrix = vec![vec![0.0; n]; n];

        // Pre-compute all state vectors
        let states: Vec<StateVec> = data.iter().map(|x| self.feature_map.execute(x)).collect();
...
impl QuantumGAN {
    pub fn new(generator: DifferentiableCircuit, latent_dim: usize, data_dim: usize) -> Self {
...
mod tests {
    ...
    fn test_param_shift_matches_numerical() {
        ...
        assert!((g_ps.parameter_gradients[0][0] - g_fd.parameter_gradients[0][0]).abs() < 1e-4,);
    }
    ...
    fn test_adjoint_matches_param_shift() {
        ...
        for p in 0..2 {
            ...
            assert!(
                (g_ps.parameter_gradients[0][p] - g_adj.parameter_gradients[0][p]).abs() < 1e-2,
                "param {}: ps={}, adj={}",
                p,
    ...
    fn test_backprop_matches_param_shift() {
        ...
        assert!((g_ps.parameter_gradients[0][0] - g_bp.parameter_gradients[0][0]).abs() < 1e-4,);
    }
    ...
    fn test_observable_hamiltonian() {
        ...
        // <H> = 0.5*1 + 0.3*1 = 0.8
        let c = CircuitBuilder::new(2).build();
        let h = Observable::Hamiltonian(vec![(0.5, vec![(0, 'Z')]), (0.3, vec![(1, 'Z')])]);
    ...
    fn test_kernel_self_overlap() {
        ...
        assert!((k - 1.0).abs() < TOL, "K(x,x) should be 1.0, got {}", k);
    }
    ...
    fn test_kernel_matrix_psd() {
        ...
        for v in &test_vecs {
            ...
            assert!(vtmv >= -TOL, "v^T K v = {} < 0 for v={:?}", vtmv, v);
        }
    }
    ...
    fn test_batch_parallel_matches_sequential() {
        ...
        for (i, p) in params_list.iter().enumerate() {
            ...
            assert!(
                (batch_results[i].expectation_values[0] - single.expectation_values[0]).abs() < TOL,
            );
        }
    }
    ...
    fn test_gan_generator() {
        ...
        assert!((sum - 1.0).abs() < TOL, "probs sum to {}, not 1.0", sum);
        // All non-negative
        for p in &probs {
            assert!(*p >= -TOL);
        }
    ...
    fn test_probabilities_sum_to_one() {
        let c = CircuitBuilder::new(3).h(0).h(1).cx(0, 2).ry(1, 0.7).build();
        let fwd = c.forward(&[Observable::PauliZ(0)]).unwrap();
    ...
    fn test_batch_with_gradients() {
        ...
        assert!((results[0].1.parameter_gradients[0][0] + 0.5_f64.sin()).abs() < TOL,);
    }
}
