...
impl CircuitComplexityAnalyzer {
    ...
    pub fn analyze(circuit: &AnalysisCircuit) -> ComplexityReport {
        ...
        let circuit_depth = circuit.gates.iter().map(|g| g.layer).max().unwrap_or(0) + 1;
        ...
        let two_qubit_gate_count = circuit.gates.iter().filter(|g| g.qubits.len() >= 2).count();
    ...
    fn compute_entanglement_graph(circuit: &AnalysisCircuit) -> Vec<(usize, usize, usize)> {
    ...
    fn max_entanglement_width(circuit: &AnalysisCircuit) -> usize {
        ...
        let max_layer = circuit.gates.iter().map(|g| g.layer).max().unwrap_or(0);
    ...
    fn estimate_code_distance(circuit: &AnalysisCircuit) -> usize {
        ...
        let depth = circuit.gates.iter().map(|g| g.layer).max().unwrap_or(0) + 1;
        let t_count = circuit
            .gates
            .iter()
            .filter(|g| g.class == GateClass::TGate)
    ...
    fn analyze_barren_plateaus(circuit: &AnalysisCircuit) -> BarrenPlateauRisk {
        ...
        let entangling_count = circuit.gates.iter().filter(|g| g.qubits.len() >= 2).count();
    ...
    fn compute_expressibility(circuit: &AnalysisCircuit) -> (Option<f64>, Option<f64>) {
        let rotation_count = circuit
            .gates
            .iter()
            .filter(|g| g.class == GateClass::Rotation)
        ...
        let max_expressibility =
            (rotation_count as f64 * entangling_count as f64).sqrt() / (n * n) as f64;
