    ...
    fn create_batches(&self, gates: &[Gate]) -> Vec<GateBatch> {
        if !self.config.enable_parallel {
            ...
            return vec![GateBatch {
                ...
                single_qubit_only: gates
                    .iter()
                    .all(|g| g.targets.len() == 1 && g.controls.is_empty()),
            }];
        }
...
mod tests {
    ...
    fn test_native_shader_syntax() {
        ...
        for (idx, shader) in compiled.native_shaders.iter().enumerate() {
            ...
            assert!(
                shader.contains("kernel void"),
                "Shader must contain kernel function"
            );
            assert!(shader.contains("batch_"), "Kernel must be named batch_N");
            assert!(shader.contains("Complex"), "Shader must use Complex type");
            assert!(
                shader.contains("device Complex* state"),
                "Kernel must have state buffer"
            );
            assert!(shader.contains("[[buffer(0)]]"), "State must be buffer 0");
            assert!(
                shader.contains("[[thread_position_in_grid]]"),
                "Must use thread_position_in_grid"
            );
            assert!(
                shader.contains("complex_mul"),
                "Must define complex_mul helper"
            );
            assert!(
                shader.contains("complex_add"),
                "Must define complex_add helper"
            );
        }
    }
    ...
    fn test_native_shader_gate_matrices() {
        ...
        for shader in &compiled.native_shaders {
            ...
            if shader.contains("gate_0_m") {
                assert!(
                    shader.contains("constant Complex gate_"),
                    "Shader must have gate matrix constants"
                );
                assert!(
                    shader.contains("[2][2]") || shader.contains("[4][4]"),
                    "Matrix must be 2x2 or 4x4"
                );
            }
        }
    }
    ...
    fn test_native_compilation_cnot() {
        ...
        assert!(
            shader.contains("gate_0_m[4][4]"),
            "CNOT should have 4x4 matrix"
        );
    }
    ...
    fn test_native_compilation_rotation_gate() {
        ...
        assert!(
            shader.contains("gate_0_m[2][2]"),
            "Rx should have 2x2 matrix"
        );
