...
impl ParameterExpression {
    ...
    pub fn evaluate(&self, binding: &ParameterBinding) -> Result<f64> {
        match self {
            ...
            ParameterExpression::Add(a, b) => Ok(a.evaluate(binding)? + b.evaluate(binding)?),
            ParameterExpression::Mul(a, b) => Ok(a.evaluate(binding)? * b.evaluate(binding)?),
    ...
    pub fn compose(&self, other: &ParametricCircuit) -> Result<ParametricCircuit> {
        ...
        for (name, param) in &other.parameters {
            if self_names.contains(name) {
                ...
                combined.parameters.insert(new_name, renamed_param);
            } else {
                combined.parameters.insert(name.clone(), param.clone());
            }
        }
...
mod tests {
    ...
    fn expression_composition() {
        ...
        let expr = ParameterExpression::sin(ParameterExpression::cos(ParameterExpression::param(
            "theta",
        )));
