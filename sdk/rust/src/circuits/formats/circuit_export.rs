...
pub enum VisGate {
    ...
    Measure { qubit: usize, cbit: Option<usize> },
    /// A qubit reset to |0>.
    Reset { qubit: usize },
}
...
impl VisCircuit {
    ...
    pub fn new(num_qubits: usize) -> Self {
        let qubit_labels = (0..num_qubits).map(|i| format!("q_{}", i)).collect();
        Self {
            num_qubits,
            num_cbits: 0,
            gates: Vec::new(),
    ...
    pub fn with_classical(num_qubits: usize, num_cbits: usize) -> Self {
        let qubit_labels = (0..num_qubits).map(|i| format!("q_{}", i)).collect();
        let cbit_labels = (0..num_cbits).map(|i| format!("c_{}", i)).collect();
        Self {
            num_qubits,
            num_cbits,
            gates: Vec::new(),
...
impl CircuitExporter {
    ...
    pub fn to_svg(&self, circuit: &VisCircuit) -> String {
        ...
        svg.push_str(
            "  .cwire { stroke: #333; stroke-width: 1.5; stroke-dasharray: 4,3; fill: none; }\n",
        );
        svg.push_str("  .gate-box { fill: #e8f4fd; stroke: #2980b9; stroke-width: 1.5; rx: 4; }\n");
        svg.push_str(
            "  .gate-text { text-anchor: middle; dominant-baseline: central; fill: #2c3e50; }\n",
        );
    ...
    pub fn to_ascii(&self, circuit: &VisCircuit) -> String {
        ...
        let mut connector: Vec<Vec<bool>> = vec![vec![false; num_cols]; circuit.num_qubits];
...
mod tests {
    ...
    fn test_ascii_toffoli() {
        ...
        assert!(
            lines[0].contains("*"),
            "Control 0 should have *, got: {}",
            lines[0]
        );
        assert!(
            lines[1].contains("*"),
            "Control 1 should have *, got: {}",
            lines[1]
        );
        assert!(
            lines[2].contains("X"),
            "Target should have X, got: {}",
            lines[2]
        );
    }
