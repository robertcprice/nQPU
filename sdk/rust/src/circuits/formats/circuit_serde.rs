...
impl SerializableGate {
    ...
    pub fn to_gate_type(&self) -> Option<crate::gates::GateType> {
        match self.gate_type.as_str() {
            ...
            "Phase" => self
                .params
                .first()
                .map(|&p| crate::gates::GateType::Phase(p)),
    ...
    pub fn from_gate_type(
        ...
        let (name, params) = match gate {
            ...
            crate::gates::GateType::U { theta, phi, lambda } => ("U", vec![*theta, *phi, *lambda]),
            ...
            crate::gates::GateType::CU {
                theta,
                phi,
                lambda,
                gamma,
            } => ("CU", vec![*theta, *phi, *lambda, *gamma]),
...
fn circuit_from_json_value(val: &JsonValue) -> Result<SerializableCircuit> {
    ...
    let metadata = val
        .get("metadata")
        .and_then(|m| metadata_from_json_value(m).ok());
...
fn extract_usize_array(val: &JsonValue, key: &str) -> Result<Vec<usize>> {
    let arr = val.get(key).and_then(|v| v.as_array()).unwrap_or(&[]);
    arr.iter()
        .map(|v| {
            v.as_u64().map(|n| n as usize).ok_or_else(|| {
                CircuitSerdeError::DeserializationFailed(format!("non-integer in '{}'", key))
            })
        })
        .collect()
}
...
fn extract_f64_array(val: &JsonValue, key: &str) -> Result<Vec<f64>> {
    ...
    arr.iter()
        .map(|v| {
            v.as_f64().ok_or_else(|| {
                CircuitSerdeError::DeserializationFailed(format!("non-number in '{}'", key))
            })
        })
        .collect()
}
...
fn metadata_from_json_value(val: &JsonValue) -> Result<CircuitMetadata> {
    Ok(CircuitMetadata {
        ...
        creation_date: val
            ...
            .unwrap_or("")
            .to_string(),
        qubit_count: val.get("qubit_count").and_then(|v| v.as_u64()).unwrap_or(0) as usize,
        gate_count: val.get("gate_count").and_then(|v| v.as_u64()).unwrap_or(0) as usize,
        depth: val.get("depth").and_then(|v| v.as_u64()).unwrap_or(0) as usize,
        tags: val
            .get("tags")
...
fn circuit_from_binary(data: &[u8]) -> Result<SerializableCircuit> {
    ...
    let num_qubits = u32::from_le_bytes([data[8], data[9], data[10], data[11]]) as usize;
    let gate_count = u32::from_le_bytes([data[12], data[13], data[14], data[15]]) as usize;
    ...
    for _ in 0..gate_count {
        ...
        let needed = target_count * 2 + control_count * 2 + param_count * 8;
        ...
        for _ in 0..param_count {
            let bytes: [u8; 8] = data[pos..pos + 8].try_into().map_err(|_| {
                CircuitSerdeError::DeserializationFailed("failed to read f64 parameter".to_string())
            })?;
...
fn circuit_to_qasm3(circuit: &SerializableCircuit) -> Result<String> {
    ...
    for gate in &circuit.gates {
        match gate.gate_type.as_str() {
            ...
            "Rx" => {
                if let (Some(&t), Some(&p)) = (gate.targets.first(), gate.params.first()) {
            ...
            "CNOT" => {
                if let (Some(&c), Some(&t)) = (gate.controls.first(), gate.targets.first()) {
...
pub fn deserialize_circuit(data: &[u8], format: CircuitFormat) -> Result<SerializableCircuit> {
    match format {
...
impl CircuitLibrary {
    ...
    pub fn filter_by_qubit_count(&self, min_qubits: usize) -> Vec<(&str, &SerializableCircuit)> {
        self.circuits
            .iter()
...
mod tests {
    ...
    fn serialize_empty_circuit() {
        ...
        let restored = deserialize_circuit(&bytes, CircuitFormat::Json).unwrap();
    ...
    fn compact_json() {
        ...
        let compact_bytes =
            serialize_circuit(&circuit, CircuitFormat::CompactJson, &compact_config).unwrap();
        ...
        let restored_pretty = deserialize_circuit(&pretty_bytes, CircuitFormat::Json).unwrap();
    ...
    fn binary_round_trip() {
        ...
        let config = SerdeConfig::default();
        let binary = serialize_circuit(&circuit, CircuitFormat::Binary, &config).unwrap();
        let restored = deserialize_circuit(&binary, CircuitFormat::Binary).unwrap();
        ...
        assert_eq!(restored.gates[1].controls, vec![0]);
        assert_eq!(restored.gates[2].gate_type, "Rz");
        assert!((restored.gates[2].params[0] - std::f64::consts::FRAC_PI_4).abs() < 1e-15);
    ...
    fn binary_smaller() {
        ...
        let config = SerdeConfig::default();
        let json_bytes = serialize_circuit(&circuit, CircuitFormat::Json, &config).unwrap();
        let binary_bytes = serialize_circuit(&circuit, CircuitFormat::Binary, &config).unwrap();
    ...
    fn format_conversion() {
        ...
        .unwrap();
        let restored = deserialize_circuit(&json_back, CircuitFormat::Json).unwrap();
        assert_eq!(restored.gates.len(), 3);
        assert_eq!(restored.gates[0].gate_type, "H");
    ...
    fn qasm3_export() {
        ...
        let config = SerdeConfig::default();
        let bytes = serialize_circuit(&circuit, CircuitFormat::OpenQASM3, &config).unwrap();
    ...
    fn large_circuit() {
        ...
        // JSON round-trip
        let json_bytes = serialize_circuit(&circuit, CircuitFormat::Json, &config).unwrap();
        let restored_json = deserialize_circuit(&json_bytes, CircuitFormat::Json).unwrap();
        ...
        // Binary round-trip
        let binary_bytes = serialize_circuit(&circuit, CircuitFormat::Binary, &config).unwrap();
        let restored_binary = deserialize_circuit(&binary_bytes, CircuitFormat::Binary).unwrap();
    ...
    fn gate_type_conversion() {
        ...
        let restored = SerializableGate::from_gate_type(&gate_type, vec![0], vec![]);
    ...
    fn binary_version_mismatch() {
        let circuit = SerializableCircuit::new(1);
        let config = SerdeConfig::default();
        let mut binary = serialize_circuit(&circuit, CircuitFormat::Binary, &config).unwrap();
