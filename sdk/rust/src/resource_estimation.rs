...
impl fmt::Display for ResourceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ResourceError::InfeasibleErrorBudget { target, best_achievable } => {
                write!(
                    f,
...
impl MagicStateFactory {
    ...
    pub fn output_t_states(&self) -> usize {
        match self {
            ...
            MagicStateFactory::Custom { output_t_states, .. } => *output_t_states,
        }
    }
...
impl fmt::Display for MagicStateFactory {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MagicStateFactory::Custom { input_t_states, output_t_states, .. } => {
...
impl ResourceEstimate {
    ...
    pub fn summary(&self) -> String {
        ...
        s.push_str(&format!("Physical qubits:     {:>15}\n", format_with_commas(self.physical_qubits)));
        s.push_str(&format!("  Data qubits:       {:>15}\n", format_with_commas(self.data_qubits)));
        s.push_str(&format!("  Routing qubits:    {:>15}\n", format_with_commas(self.routing_qubits)));
        s.push_str(&format!("  Factory qubits:    {:>15}\n", format_with_commas(self.factory_qubits)));
        s.push_str(&format!("Code distance:       {:>15}\n", self.code_distance));
        s.push_str(&format!("T-factories:         {:>15}\n", self.num_t_factories));
        s.push_str(&format!("T-states consumed:   {:>15}\n", format_with_commas(self.t_states_consumed)));
        s.push_str(&format!("Logical error rate:  {:>15.2e}\n", self.logical_error_rate));
        s.push_str(&format!("Wall-clock cycles:   {:>15}\n", format_with_commas(self.wall_clock_cycles)));
        s.push_str(&format!("Wall-clock time:     {:>15}\n", format_duration(self.wall_clock_time_seconds)));
        s.push_str(&format!("Space-time volume:   {:>15.2e}\n", self.total_volume));
        s
    }
...
pub fn data_qubits(num_logical: usize, code_distance: usize, scheme: &QecScheme) -> usize {
    match scheme {
        ...
        QecScheme::BicycleCode144 => {
            ...
            let physical_per_block = (base_n as f64 * distance_scale * distance_scale).ceil() as usize;
...
pub fn estimate_resources(
    ...
    let data = data_qubits(circuit.num_logical_qubits, code_distance, &config.qec_scheme);
    let routing = routing_qubits(circuit.num_logical_qubits, code_distance, &config.qec_scheme);
...
mod tests {
    ...
    fn test_surface_code_logical_error_decreases_with_distance() {
        ...
        // d=3: 0.1 * 0.1^2 = 1e-3
        // d=5: 0.1 * 0.1^3 = 1e-4
        assert!(rate_d3 < 1e-2, "d=3 error should be reasonable: {}", rate_d3);
    ...
    fn test_optimal_distance_increases_with_lower_target() {
        ...
        let d_1e6 = optimal_code_distance(p, 1e-6, n_qubits, n_cycles, &QecScheme::SurfaceCode, 51).unwrap();
        let d_1e9 = optimal_code_distance(p, 1e-9, n_qubits, n_cycles, &QecScheme::SurfaceCode, 51).unwrap();
        let d_1e12 = optimal_code_distance(p, 1e-12, n_qubits, n_cycles, &QecScheme::SurfaceCode, 51).unwrap();
    ...
    fn test_wall_clock_proportional_to_code_distance() {
        ...
        let (cycles_d5, time_d5) = estimate_wall_clock(t_depth, clifford_depth, measurements, 5, 100.0);
        let (cycles_d11, time_d11) = estimate_wall_clock(t_depth, clifford_depth, measurements, 11, 100.0);
    ...
    fn test_infeasible_error_budget_returns_error() {
        ...
        assert!(
            result.is_err(),
            "Should fail with infeasible error budget"
        );
    ...
    fn test_total_volume_equals_qubits_times_cycles() {
        ...
        assert!(
            ...
            est.total_volume, expected_volume
        );
    }
    ...
    fn test_bicycle_code_logical_error_rate() {
        ...
        assert!(rate_d11 < rate_d5, "Higher distance should give lower error");
        assert!(rate_d5 > 0.0 && rate_d5 < 1.0, "Error rate should be in (0,1)");
    }
}
