...
pub fn measure_two_qubit_pauli(
    ...
    assert_eq!(1 << n_qubits, n_states, "State vector size must be a power of 2");
...
impl FloquetSimulator {
    ...
    pub fn step(&mut self) -> Vec<bool> {
        ...
        self.stats
            .record_round(round.len(), num_defects);
...
impl FloquetDecoder {
    ...
    pub fn build_syndrome_graph(
        ...
        let num_measurement_types = schedule
            .rounds
            .iter()
            .map(|r| r.len())
            .max()
                ...
                for j in (i + 1)..round_def.len() {
                    ...
                    if shared {
                        ...
                        if let (Some(f), Some(t)) = (from, to) {
                            ...
                            let shared_qubit = if mi.qubit_a == mj.qubit_a
                                || mi.qubit_a == mj.qubit_b
                            {
                                mi.qubit_a
                            } else {
...
mod tests {
    ...
    fn minimal_schedule() -> FloquetSchedule {
        let round = MeasurementRound::new(vec![PauliMeasurement::new(
            0,
            1,
            PauliOp::Z,
            PauliOp::Z,
    ...
    fn test_measurement_round_structure() {
        ...
        for r in 0..schedule.period() {
            ...
            assert!(
                !round.is_empty(),
                "Round {} should have measurements",
                r
            );
    ...
    fn test_no_error_no_defects_after_warmup() {
        ...
        assert_eq!(s0, s1, "Repeated ZZ measurement on |00> should be deterministic");
    }
    ...
    fn test_syndrome_graph_construction() {
        ...
        assert!(
            graph.num_nodes() > 0,
            "Syndrome graph should have nodes"
        );
        assert!(
            graph.num_edges() > 0,
            "Syndrome graph should have edges"
        );
    ...
    fn test_error_free_evolution() {
        ...
        for (r, syn) in syndromes.iter().enumerate() {
            ...
            assert!(
                !syn[0],
                "Error-free ZZ on |00> should be +1 at round {}",
                r
            );
        }
    }
