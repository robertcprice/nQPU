...
mod tests {
        ...
        for i in 0..7 {
            for j in 0..7 {
                assert!(
                    ...
                    "H[{}][{}] = {} != H[{}][{}] = {}",
                    i, j, fmo.hamiltonian[i][j],
                    j, i, fmo.hamiltonian[j][i]
                );
            }
        }
    }
    ...
    fn fmo_evolution_conserves_population_initially() {
        ...
        assert!(approx_eq(total, 1.0, 1e-10), "Initial population = {}", total);
    }
    ...
    fn enzyme_kie_greater_than_one() {
        ...
        assert!(kie > 1.0, "KIE should be > 1 (H tunnels faster), got {}", kie);
    }
    ...
    fn enzyme_rate_vs_temperature_returns_correct_length() {
        let e = EnzymeTunneling::new(0.3, 0.05);
        let (temps, total, classical, tunnel) =
            e.rate_vs_temperature(10.0, 500.0, 50).unwrap();
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                assert!(
                    ...
                    "exp(0)[{},{}] = {:?}, expected {}",
                    i, j, result[idx(i, j, n)], expected
                );
                assert!(
                    result[idx(i, j, n)].im.abs() < 1e-10,
                    "exp(0)[{},{}] imaginary part = {}",
                    i, j, result[idx(i, j, n)].im
                );
            }
        }
    }
