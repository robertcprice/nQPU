...
impl FockState {
    ...
    pub fn from_number(n: usize, n_max: usize) -> Self {
        assert!(n <= n_max, "Fock state |{}> exceeds truncation n_max={}", n, n_max);
        let mut amplitudes = vec![C64::new(0.0, 0.0); n_max + 1];
...
impl GKPCode {
    ...
    pub fn logical_one(&self) -> FockState {
        ...
        for s in -s_max..=s_max {
            ...
            let weight =
                (-self.delta * self.delta * (2.0 * s as f64 + 1.0).powi(2) / 2.0).exp();
            if weight < 1e-15 {
                continue;
            }
        ...
        for ix in 0..resolution {
            ...
            for ip in 0..resolution {
                ...
                for n in 0..dim {
                    ...
                    let sign = if n % 2 == 0 { 1.0 } else { -1.0 };
                    let w_nn =
                        sign * (-r_sq).exp() * laguerre(n, 0.0, 2.0 * r_sq) / PI;
                    w += rho_nn * w_nn;
                }
                ...
                for m in 0..dim {
                    for n in 0..m {
                        ...
                        let w_mn_complex =
                            z_power * (sign_n * ratio * (-r_sq).exp() * lag / PI);
    ...
    pub fn logical_x(&mut self) {
        match &self.code_type {
            BosonicCodeType::Cat(_alpha) => {
                ...
                for n in 0..=self.n_max {
                    new_state.amplitudes[n] =
                        c1 * zero_l.amplitudes[n] + c0 * one_l.amplitudes[n];
                }
                new_state.normalize();
    ...
    pub fn logical_z(&mut self) {
        match &self.code_type {
            ...
            BosonicCodeType::Binomial(spacing) => {
                ...
                for n in 0..=self.n_max {
                    new_state.amplitudes[n] =
                        c0 * zero_l.amplitudes[n] - c1 * one_l.amplitudes[n];
                }
                new_state.normalize();
                self.state = new_state;
            }
