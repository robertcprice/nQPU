...
pub fn generate_tiling(
    ...
    let find_or_create_vertex =
        |vertices: &mut Vec<HyperbolicVertex>,
        ...
        for &(ea, eb) in &boundary_edges {
            ...
            for i in 1..p {
                if i == p - 1 {
                    ...
                    angle += step;
                    let new_z = new_center_approx
                        + C64::from_polar(new_radius, angle);
                    let nx = new_z.re;
                    let ny = new_z.im;
            ...
            result[i] = match (a, b) {
                ...
                (Some(pa), Some(pb)) => {
                    if pa == pb {
                        ...
                        match (pa, pb) {
                            (PauliType::X, PauliType::Y)
                            | (PauliType::Y, PauliType::X) => Some(PauliType::Z),
                            (PauliType::Y, PauliType::Z)
                            | (PauliType::Z, PauliType::Y) => Some(PauliType::X),
                            (PauliType::X, PauliType::Z)
                            | (PauliType::Z, PauliType::X) => Some(PauliType::Y),
                            _ => None,
                        }
                    }
                }
...
impl ISGTracker {
    ...
    pub fn process_measurement_round(&mut self, round: &HyperbolicMeasurementRound) {
        ...
        for &(a, b) in &round.measurements {
            ...
            self.stabilizers
                .retain(|s| s.commutes_with(&new_stab));
...
impl HyperbolicDecoder {
    ...
    pub fn from_tiling(tiling: &HyperbolicTiling) -> Self {
        let positions: Vec<(f64, f64)> = tiling
            .vertices
            .iter()
            .map(|v| (v.x, v.y))
            .collect();
    ...
    pub fn decode(
        ...
        for i in 0..defect_qubits.len() {
            for j in (i + 1)..defect_qubits.len() {
                ...
                let weight = if qi < self.vertex_positions.len()
                    && qj < self.vertex_positions.len()
                {
        ...
        let (pairs, unmatched) = if defect_count <= self.exact_mwpm_defect_limit && defect_count <= 63
        {
...
impl SemiHyperbolicCode {
    ...
    pub fn monte_carlo_estimate(
        ...
        for _trial in 0..num_trials {
            let syndromes = extract_syndromes_noisy(
                schedule,
                num_rounds,
                &noise,
                self.adjusted_n,
...
pub fn threshold_study(
    ...
    for &phys_rate in error_rates {
        ...
        for _trial in 0..num_trials {
            let syndromes =
                extract_syndromes_noisy(&schedule, num_rounds, &noise, n, &mut rng);
...
pub fn demo() {
    ...
    match threshold_study(
        ...
        Ok(result) => {
            for (&p, &l) in result.physical_rates.iter().zip(result.logical_rates.iter()) {
...
mod tests {
    ...
    fn test_isg_logical_qubits_uses_independent_rank() {
        ...
        assert_eq!(isg.num_stabilizers(), 3, "raw generator count should include duplicate");
        assert_eq!(
    ...
    fn test_exact_pairing_global_minimum() {
        ...
        assert!((cost - 4.0).abs() < 1e-12, "expected global optimum cost 4, got {}", cost);
    ...
    fn test_tiling_73() {
        ...
        assert!(tiling.num_vertices() >= 7, "Should have at least 7 vertices for {{{{7,3}}}}");
    ...
    fn test_semi_hyperbolic_construction() {
        ...
        assert_eq!(n1, n2, "Physical qubit count should be independent of alpha");
    ...
    fn test_semi_hyperbolic_alpha_extremes() {
        ...
        assert_eq!(k_flat, 1, "Flat limit should encode 1 logical qubit, got {}", k_flat);
    ...
    fn test_semi_hyperbolic_threshold() {
        ...
        assert!(t_flat > 0.0 && t_flat < 0.2, "Flat threshold unreasonable: {}", t_flat);
        assert!(t_hyp > 0.0 && t_hyp < 0.2, "Hyp threshold unreasonable: {}", t_hyp);
    }
    ...
    fn test_noise_model_display() {
        ...
        assert_eq!(format!("{}", NoiseModel::Phenomenological), "Phenomenological");
