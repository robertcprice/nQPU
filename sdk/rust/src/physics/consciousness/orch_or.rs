...
impl OrchORSimulator {
    ...
    pub fn check_objective_reduction(&mut self) -> Option<ReductionEvent> {
        ...
        let e_g = (num_superposed as f64) * G_NEWTON * TUBULIN_MASS_KG * TUBULIN_MASS_KG
    ...
    pub fn apply_anesthetic(&mut self, concentration: f64) {
        ...
        let temp_factor = (-ACTIVATION_ENERGY_J / (K_B * self.config.temperature_kelvin)).exp();
    ...
    pub fn consciousness_measure(&self) -> ConsciousnessMeasure {
        ...
        let gravitational_self_energy = if num_superposed > 0 {
            (num_superposed as f64) * G_NEWTON * TUBULIN_MASS_KG * TUBULIN_MASS_KG
                / TUBULIN_DISPLACEMENT_M
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_gravitational_self_energy() {
        ...
        let e_g_single = G_NEWTON * TUBULIN_MASS_KG * TUBULIN_MASS_KG / TUBULIN_DISPLACEMENT_M;

        // E_G should scale linearly with number of superposed tubulins
        let e_g_double = 2.0 * e_g_single;

        assert!(e_g_single > 0.0, "E_G should be positive");
        assert!(
    ...
    fn test_tubulin_state_probabilities() {
        ...
        let sup = TubulinState::Superposition(C64::new(inv_sqrt2, 0.0), C64::new(inv_sqrt2, 0.0));
