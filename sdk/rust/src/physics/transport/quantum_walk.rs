    ...
    pub fn adjacency_matrix(&self) -> Vec<Vec<f64>> {
        ...
        for (v, neighbors) in self.adjacency.iter().enumerate() {
            for (idx, &u) in neighbors.iter().enumerate() {
                let w = self.weights.as_ref().map_or(1.0, |ws| ws[v][idx]);
                mat[v][u] = w;
            }
        }
        mat
    ...
    for j in 0..m {
        ...
        if j + 1 < m {
            ...
            if b < 1e-14 {
                ...
                let m_eff = j + 1;
                return lanczos_reconstruct(&q, &alpha[..m_eff], &beta[..m_eff], t, beta0, m_eff);
            }
...
mod tests {
    ...
    fn test_dtqw_hadamard_line() {
        ...
        let spread: f64 =
            probs[0..center - 2].iter().sum::<f64>() + probs[center + 3..].iter().sum::<f64>();
        assert!(
            spread > 0.01,
