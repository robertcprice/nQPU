...
pub fn apply_gate(state: &mut Vec<Complex64>, gate: &EchoGate) {
    ...
    match gate {
        ...
        EchoGate::Rx(q, theta) => apply_single_qubit_gate(state, *q, n_qubits, &rx_matrix(*theta)),
        EchoGate::Ry(q, theta) => apply_single_qubit_gate(state, *q, n_qubits, &ry_matrix(*theta)),
        EchoGate::Rz(q, theta) => apply_single_qubit_gate(state, *q, n_qubits, &rz_matrix(*theta)),
...
pub fn loschmidt_echo(
    ...
    if perturbation.abs() > 1e-15 {
        for q in 0..n_qubits {
            h_perturbed.push(HamiltonianTerm::new(vec![(q, Pauli::Z)], perturbation));
        }
    }
...
pub fn scrambling_time(otoc_values: &[Complex64], times: &[f64], threshold: f64) -> Option<f64> {
...
mod tests {
    ...
    fn test_invalid_operator_detected() {
        let config = OtocConfig::new(2).operator_a(PauliOp::single(5, Pauli::X)); // qubit 5 out of range for 2-qubit system

        let result = compute_otoc(&config);
        assert!(
            result.is_err(),
            "Should detect invalid operator qubit index"
        );
        match result.unwrap_err() {
            OtocError::InvalidOperator(msg) => {
                assert!(
                    msg.contains("qubit 5"),
                    "Error message should mention qubit 5: {}",
                    msg
                );
            }
