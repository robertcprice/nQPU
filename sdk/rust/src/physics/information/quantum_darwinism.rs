...
impl fmt::Display for RedundancyResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let plateau_marker = if self.classical_plateau {
            " [PLATEAU]"
        } else {
            ""
        };
...
pub fn eigen_hermitian(matrix: &[Vec<C64>]) -> Result<(Vec<f64>, Vec<Vec<C64>>), DarwinismError> {
    let n = matrix.len();
    if n == 0 {
        return Ok((vec![], vec![]));
    }
...
fn eigen_2x2(m: &[Vec<C64>]) -> (Vec<f64>, Vec<Vec<C64>>) {
    ...
    let vecs = if b_norm < 1e-14 {
        ...
        let v0 = vec![C64::new(1.0 / norm0, 0.0), ratio0 / C64::new(norm0, 0.0)];

        let ratio1 = -(a - lambda1) / b;
        let norm1 = (1.0 + ratio1.norm_sqr()).sqrt();
        let v1 = vec![C64::new(1.0 / norm1, 0.0), ratio1 / C64::new(norm1, 0.0)];

        // Return as row-major: vecs[row][col] = row-th component of col-th eigenvector
        vec![vec![v0[0], v1[0]], vec![v0[1], v1[1]]]
    };
...
pub fn mutual_information(
    ...
    let trace_out_for_a: Vec<usize> = (0..n).filter(|q| !system_qubits.contains(q)).collect();
    ...
    let trace_out_for_b: Vec<usize> = (0..n).filter(|q| !fragment_qubits.contains(q)).collect();
    ...
    let trace_out_for_ab: Vec<usize> = (0..n).filter(|q| !all_qubits[*q]).collect();
...
pub fn zurek_decoherence_factor(theta: f64, n_env: usize, rounds: usize) -> f64 {
...
mod tests {
    ...
    fn test_full_darwinism_workflow() {
        ...
        assert!(
            history.last().unwrap().off_diagonal_norm < history.first().unwrap().off_diagonal_norm,
