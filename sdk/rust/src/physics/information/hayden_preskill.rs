...
impl std::fmt::Display for HaydenPreskillError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            HaydenPreskillError::InsufficientQubits {
                available,
                required,
            } => {
                write!(
                    f,
                    "Insufficient qubits: have {}, need {}",
                    available, required
                )
            }
...
impl QuantumFirewall {
    ...
    pub fn check_monogamy(&self, a: &[usize], b: &[usize], c: &[usize]) -> MonogamyResult {
...
impl BlackHoleEvaporationSimulator {
    ...
    pub fn full_evaporation(&mut self) -> EvaporationResult {
        ...
        for snap in &self.snapshots {
            radiation_fraction.push(snap.step as f64 / self.initial_bh_qubits as f64);
...
impl HaydenPreskillSimulator {
    ...
    pub fn setup_initial_state(&mut self) {
        ...
        let pairs = self
            .config
            .black_hole_qubits
            .min(self.config.radiation_qubits);
        for i in 0..pairs {
    ...
    pub fn recover_diary(&self, radiation_qubits: &[usize]) -> RecoveryResult {
        let diary_ref: Vec<usize> = (self.diary_ref_start..self.diary_ref_end).collect();
        ...
        let early_rad: Vec<usize> = (self.early_rad_start..self.early_rad_end).collect();
        let mut all_radiation: Vec<usize> = radiation_qubits.to_vec();
        all_radiation.extend_from_slice(&early_rad);
        all_radiation.sort();
        all_radiation.dedup();

        let mi_with_early = mutual_information_between(&self.state, &diary_ref, &all_radiation);
    ...
    pub fn page_curve(&self) -> PageCurve {
        ...
        for k in 0..=n_bh {
            ...
            let entropy = if k == 0 || k == n_bh {
                ...
                let rad_indices: Vec<usize> = (self.bh_start..self.bh_start + k).collect();
...
fn apply_scrambling_layer(state: &mut QuantumState, start: usize, end: usize, rng: &mut Lcg) {
    let n = end - start;
    if n == 0 {
        return;
    }
...
fn mutual_information_between(state: &QuantumState, a: &[usize], b: &[usize]) -> f64 {
...
mod tests {
    ...
    fn test_diary_recovery_with_early_radiation() {
        ...
        assert!(result.scrambling_was_necessary, "Scrambling was applied");
    }
