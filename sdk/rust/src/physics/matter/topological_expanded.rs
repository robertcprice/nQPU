...
impl IsingAnyonState {
    ...
    pub fn new(num_anyons: usize) -> Self {
        assert!(
            num_anyons >= 2,
            "need at least 2 anyons for a non-trivial space"
        );
...
impl MajoranaChain {
    ...
    pub fn new(num_sites: usize) -> Self {
        assert!(
            num_sites > 0 && num_sites <= 16,
            "num_sites must be in [1, 16]"
        );
        let dim = 1 << num_sites;
            ...
            for idx in 0..dim {
                ...
                if occ_k == occ_k1 {
                    ...
                    new_state[idx] = self.state[idx] * inv_sqrt2
                        + self.state[partner_idx] * C64::new(0.0, parity_sign as f64 * inv_sqrt2);
                } else {
...
impl BraidCompiler {
        ...
        match gate {
            ...
            GateType::Phase(theta) => {
                ...
                let best_angle = [0.0, PI / 2.0, PI]
                    .iter()
                    .copied()
                    .min_by(|a, b| (a - theta).abs().partial_cmp(&(b - theta).abs()).unwrap())
                    .unwrap();
    ...
    pub fn compile_circuit(&self, gates: &[Gate]) -> Vec<BraidWord> {
        gates
            .iter()
            .map(|g| self.compile_gate(&g.gate_type))
            .collect()
    }
...
mod tests {
    ...
    fn test_vacuum_fusion_is_identity() {
        for anyon in &[
            IsingAnyonType::Vacuum,
            IsingAnyonType::Sigma,
            IsingAnyonType::Psi,
        ] {
    ...
    fn test_quantum_dimensions() {
        assert!(
            (IsingFusionRules::quantum_dimension(IsingAnyonType::Vacuum) - 1.0).abs() < EPSILON
        );
        assert!(
    ...
    fn test_compile_clifford_gates_exact() {
        ...
        for gate in &[
            ...
            assert!(
                !braid.is_empty(),
                "gate {:?} should produce non-empty braid",
                gate
            );
        }
    }
    ...
    fn test_compile_t_gate_approximate() {
        ...
        assert!(
            braid.fidelity > 0.8,
            "T gate fidelity should be reasonable (>0.8)"
        );
    }
