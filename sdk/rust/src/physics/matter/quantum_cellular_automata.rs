...
impl QuantumCellularAutomaton {
    ...
    pub fn step(&mut self) {
        ...
        if self.boundary == BoundaryCondition::Periodic && n >= 3 {
            ...
            let wrap_already_covered = odd
                .iter()
                .any(|&(a, b)| (a == n - 1 && b == 0) || (a == 0 && b == n - 1));
...
mod tests {
    ...
    fn test_identity_preserves_state() {
        let mut qca =
            QuantumCellularAutomaton::new(4, QCARuleType::Identity, BoundaryCondition::Open);
    ...
    fn test_identity_evolve_multiple_steps() {
        let mut qca =
            QuantumCellularAutomaton::new(3, QCARuleType::Identity, BoundaryCondition::Periodic);
    ...
    fn test_goldilocks_j0_product_evolution() {
        ...
        let mut qca =
            QuantumCellularAutomaton::new(4, QCARuleType::Goldilocks(0.0), BoundaryCondition::Open);
        GateOperations::x(&mut qca.state, 2);
        let probs_before = qca.state.probabilities();
        ...
        for (a, b) in probs_before.iter().zip(probs_after.iter()) {
            assert!((a - b).abs() < TOL, "J=0 Goldilocks should act as identity");
        }
    }
    ...
    fn test_spacetime_data_recording() {
        let mut qca =
            QuantumCellularAutomaton::new(4, QCARuleType::Goldilocks(0.2), BoundaryCondition::Open);
        GateOperations::x(&mut qca.state, 0);
    ...
    fn test_open_vs_periodic_differ() {
        ...
        let mut qca_open =
            QuantumCellularAutomaton::new(n, QCARuleType::Goldilocks(j), BoundaryCondition::Open);
    ...
    fn test_light_cone_initial_zero() {
        let mut qca =
            QuantumCellularAutomaton::new(4, QCARuleType::Goldilocks(0.3), BoundaryCondition::Open);
        GateOperations::x(&mut qca.state, 0);
    ...
    fn test_custom_closure_rule() {
        ...
        assert!((probs[2] - 1.0).abs() < TOL, "CustomClosure SWAP failed");
    }
    ...
    fn test_qubit_probabilities() {
        let mut qca =
            QuantumCellularAutomaton::new(3, QCARuleType::Identity, BoundaryCondition::Open);
        GateOperations::x(&mut qca.state, 1);
    ...
    fn test_norm_preservation() {
        ...
        for _ in 0..20 {
            ...
            assert!((norm - 1.0).abs() < 1e-8, "Norm not preserved: {}", norm);
        }
    }
}
