...
impl QuantumBattery {
    ...
    pub fn new(config: BatteryConfig) -> Result<Self, BatteryError> {
        ...
        let state = match &config.initial_state {
            ...
            BatteryState::Custom(vec) => vec.clone(),
        };
    ...
    pub fn state_norm(&self) -> f64 {
        let norm_sq: f64 = self.state.iter().map(|(re, im)| re * re + im * im).sum();
        norm_sq.sqrt()
    }
                ...
                for qi in 0..n {
                    for qj in (qi + 1)..n {
                        ...
                        for k in 0..dim {
                            ...
                            let phase_i = if bit_i == 0 {
                                C64::new(0.0, 1.0)
                            } else {
                                C64::new(0.0, -1.0)
                            };
                            let phase_j = if bit_j == 0 {
                                C64::new(0.0, 1.0)
                            } else {
                                C64::new(0.0, -1.0)
                            };
    ...
    fn apply_trotter_step(&mut self, charging_h: &[Vec<C64>], dt: f64) {
        let dim = self.dim;
        ...
        for k in 0..dim {
            ...
            self.state[k] = (re * phase_re - im * phase_im, re * phase_im + im * phase_re);
        }
    ...
    fn apply_dissipation(&mut self, dt: f64) {
        ...
        match diss {
            ...
            DissipationModel::Dephasing { gamma } => {
                ...
                for k in 0..self.dim {
                    let n_excited = (0..self.num_qubits).filter(|&q| (k >> q) & 1 == 1).count();
...
impl ScalingAnalysis {
    ...
    pub fn run(
        ...
        for &n in cell_counts {
            ...
            // Find peak power
            let peak_power = result.power_history.iter().cloned().fold(0.0_f64, f64::max);
...
mod tests {
    ...
    fn test_battery_creation_empty() {
        ...
        for k in 1..battery.dim {
            let prob =
                battery.state[k].0 * battery.state[k].0 + battery.state[k].1 * battery.state[k].1;
            assert!(
                prob < EPSILON,
                "Non-ground amplitude at index {}: {}",
                k,
                prob
            );
        }
    ...
    fn test_charging_power_peak_exists() {
        ...
        let max_power = result.power_history.iter().cloned().fold(0.0_f64, f64::max);
    ...
    fn test_scaling_exponent_parallel() {
        ...
        let result =
            ScalingAnalysis::run(&cell_counts, &|_n| ChargingModel::Parallel, 1.0).unwrap();
    ...
    fn test_config_validation_errors() {
        ...
        // Negative coupling
        let config =
            BatteryConfig::new().with_charging_model(ChargingModel::Collective { coupling: -1.0 });
        assert!(config.validate().is_err());
        ...
        let config = BatteryConfig::new().with_initial_state(BatteryState::Partial(1.5));
        assert!(config.validate().is_err());
    }
