...
impl KitaevChain {
    ...
    pub fn pfaffian_sign(&self) -> i32 {
        // At k=0: d_z = -mu - 2t
        let pf0 = if -self.mu - 2.0 * self.t > 0.0 { 1 } else { -1 };
        // At k=pi: d_z = -mu + 2t
        let pf_pi = if -self.mu + 2.0 * self.t > 0.0 { 1 } else { -1 };
        pf0 * pf_pi
    }
...
mod tests {
    ...
    fn test_phase_diagram_boundary() {
        ...
        for p in &points {
            ...
            if p.mu.abs() > 2.5 {
                assert!(!p.topological, "mu={} should be trivial", p.mu);
            }
        }
    }
    ...
    fn test_winding_number() {
        ...
        assert_eq!(
            trivial.winding_number(),
            0,
            "trivial phase should have winding number 0"
        );
    ...
    fn test_pfaffian_sign() {
        ...
        assert_eq!(
            topo.pfaffian_sign(),
            -1,
            "topological phase should have pfaffian = -1"
        );

        // Trivial: pfaffian should be positive
        let trivial = KitaevChain::new(10, 1.0, 1.0, 5.0);
        assert_eq!(
            trivial.pfaffian_sign(),
            1,
            "trivial phase should have pfaffian = +1"
        );
    }
    ...
    fn test_braiding_clockwise() {
        ...
        assert!(
            (norm - 1.0).abs() < TOL,
            "state should be normalized after braiding"
        );

        // Single-qubit exchange is a phase gate: |0> -> phase * |0>
        assert!(
            sim.state[0].norm_sqr() > 1.0 - TOL,
            "should remain in |0> subspace"
        );
    ...
    fn test_braiding_gate_is_clifford() {
        assert!(
            verify_clifford_property(),
            "braiding exchange should produce a Clifford gate"
        );
    }
    ...
    fn test_fermion_parity_measurement() {
        ...
        assert!(
            (norm - 1.0).abs() < TOL,
            "state should be normalized after measurement"
        );
    ...
    fn test_magic_state_distillation() {
        ...
        assert!(
            !clifford.is_universal,
            "braiding alone should not be universal"
        );
    ...
    fn test_majorana_1_device() {
        ...
        assert_eq!(
            device.junctions.len(),
            7,
            "should have 7 junctions (nearest-neighbour)"
        );
    ...
    fn test_prebuilt_kitaev_chain_spectrum() {
        ...
        assert_eq!(
            spectrum.len(),
            8,
            "4-site chain should have 8 BdG eigenvalues"
        );
    ...
    fn test_junction_coupling() {
        ...
        assert!(
            !open_junction.is_closed(),
            "zero coupling should not be closed"
        );
        ...
        assert!(
            !closed_junction.is_open(),
            "nonzero coupling should not be open"
        );
        assert!(
            closed_junction.is_closed(),
            "nonzero coupling should be closed"
        );
    }
    ...
    fn test_kitaev_sweet_spot() {
        ...
        assert!(
            gap > 0.5,
            "sweet spot should have a substantial gap, got {}",
            gap
        );
