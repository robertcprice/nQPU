...
impl CTCSimulator {
    ...
    pub fn simulate_deutsch(
        ...
        let rho_sys_final = self.partial_trace_ctc(
            &self.apply_unitary(&self.tensor_product(&rho_sys, &rho_ctc), unitary),
            dim_sys,
            dim_ctc,
        );
    ...
    pub fn perfect_discrimination(&self, state: &QuantumState) -> Result<usize, String> {
        if self.n_ctc < 2 {
...
pub fn print_benchmark() {
    ...
    println!(
        "{:<10} {:<15} {:<15} {:<10}",
        "Qubits", "Time (s)", "Iterations", "Converged"
    );
    println!("{}", "-".repeat(70));
