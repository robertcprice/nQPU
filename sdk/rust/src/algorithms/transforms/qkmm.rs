...
use crate::quantum_kernels::{state_overlap, zero_state};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            QKMMError::InvalidDimension { got, expected } => {
                write!(
                    f,
                    "Invalid dimension: got {}, expected power of 2 <= {}",
                    got, expected
                )
            }
            QKMMError::IncompatibleMatrixDims {
                a_rows,
                a_cols,
                b_rows,
                b_cols,
...
impl QKMM {
    ...
    pub fn matrix_chain(&mut self, matrices: &[Array2<f64>]) -> Result<Array2<f64>, QKMMError> {
        ...
        for (i, mat) in matrices.iter().skip(1).enumerate() {
            result = self.matrix_matrix(&result, mat).map_err(|e| {
                QKMMError::StateError(format!("Chain multiplication failed at step {}: {}", i, e))
            })?;
        }
...
pub fn amplitude_encode_matrix(
    state: &mut Vec<Complex64>,
    matrix: &Array2<f64>,
    num_qubits: usize,
) {
