...
pub fn pauli_to_matrix(paulis: &[(usize, char)], num_qubits: usize) -> Array2<Complex64> {
    ...
    let mut ops: Vec<Array2<Complex64>> =
        (0..num_qubits).map(|_| single_pauli_matrix('I')).collect();
...
pub fn build_hamiltonian_matrix(terms: &[HamiltonianTerm], num_qubits: usize) -> Array2<Complex64> {
...
pub fn exact_evolution(terms: &[HamiltonianTerm], time: f64, num_qubits: usize) -> Vec<Complex64> {
...
fn qswift_samples_for_error(lambda: f64, time: f64, target_error: f64, order: usize) -> usize {
...
mod tests {
    ...
    fn test_qswift_error_decreases_with_order() {
        ...
        assert!(
            err4 < err2,
            "order 4 should have smaller error than order 2"
        );
        assert!(
            err6 < err4,
            "order 6 should have smaller error than order 4"
        );
