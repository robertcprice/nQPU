...
fn mat2_identity() -> Mat2 {
    [[c64_one(), c64_zero()], [c64_zero(), c64_one()]]
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SpectrumError::InvalidEigenvalue { value, reason } => {
                write!(f, "Invalid eigenvalue {:.6}: {}", value, reason)
            }
    ...
    pub fn chebyshev_approximation(&self, degree: usize) -> Vec<f64> {
        ...
        for k in 0..n {
            ...
            for j in 0..num_samples {
                let x_j = ((2 * j + 1) as f64 * PI / (2 * num_samples) as f64).cos();
...
impl EigenvalueTransform {
    ...
    pub fn threshold(lambda: f64, degree: usize) -> Self {
        ...
        Self {
            ...
            description: format!("Threshold at {:.4} (degree {})", lambda, degree),
        }
    }
    ...
    pub fn bandpass(low: f64, high: f64, degree: usize) -> Self {
        ...
        Self {
            coefficients: filter.chebyshev_approximation(degree),
            description: format!("Bandpass [{:.4}, {:.4}] (degree {})", low, high, degree),
        }
    }
...
pub fn standard_amplification(
    ...
    for _ in 0..num_iter {
        ...
        let prob: f64 = target_indices.iter().map(|&i| state[i].norm_sqr()).sum();
        convergence_history.push(prob);
    }

    let success_probability = target_indices.iter().map(|&i| state[i].norm_sqr()).sum();
...
impl FixedPointAmplification {
    ...
    pub fn new(num_iterations: usize, target_delta: f64) -> Result<Self, SpectrumError> {
        ...
        let (alphas, betas) = compute_fixed_point_angles(num_iterations, target_delta);
    ...
    pub fn amplify(&self, initial_state: &[C64], target_indices: &[usize]) -> AmplificationResult {
        let dim = initial_state.len();
        let mut state = initial_state.to_vec();
        let mut convergence_history = Vec::with_capacity(self.num_iterations + 1);
        let mut oracle_calls = 0;

        let initial_prob: f64 = target_indices.iter().map(|&i| state[i].norm_sqr()).sum();
        convergence_history.push(initial_prob);
        ...
        let success_probability: f64 = target_indices.iter().map(|&i| state[i].norm_sqr()).sum();
...
pub fn compute_fixed_point_angles(num_iterations: usize, delta: f64) -> (Vec<f64>, Vec<f64>) {
...
impl ObliviousAmplification {
    ...
    pub fn amplify(&self, input_state: &[C64]) -> Result<AmplificationResult, SpectrumError> {
        ...
        let initial_prob: f64 = good_indices.iter().map(|&i| state[i].norm_sqr()).sum();
        convergence_history.push(initial_prob);
        ...
        let prob_after_u: f64 = good_indices.iter().map(|&i| state[i].norm_sqr()).sum();
        ...
        for _ in 0..self.num_rounds {
            ...
            let prob: f64 = good_indices.iter().map(|&i| state[i].norm_sqr()).sum();
            convergence_history.push(prob);
        }
...
pub fn apply_spectral_filter(
    ...
    for k in 2..=degree {
        ...
        for i in 0..dim {
            t_next[i] = c64(2.0, 0.0) * h_t_curr[i] - t_prev[i];
        }
...
fn apply_hermitian_part(u: &[C64], u_adj: &[C64], state: &[C64], dim: usize) -> Vec<C64> {
...
pub fn multi_target_amplification(
    ...
    for j in 0..num_samples {
        let x = ((2 * j + 1) as f64 * PI / (2 * num_samples) as f64).cos();
        let mut val = 0.0;
...
pub fn compare_with_qsvt(
    ...
    let config = SpectrumAmplificationConfig::new().num_iterations(num_iterations);
    let sa_result = standard_amplification(oracle, initial_state, target_indices, &config)?;
    ...
    let qsvt_prob = ((2 * num_iterations + 1) as f64 * theta).sin().powi(2);
...
mod tests {
    ...
    fn test_standard_grover() {
        ...
        let result =
            standard_amplification(&oracle, &initial_state, &target_indices, &config).unwrap();
    ...
    fn test_spectral_filter_threshold() {
        ...
        let result =
            eigenvalue_threshold_amplification(&h, &state, threshold, 20, &config).unwrap();

        // The amplified state should have more weight on eigenvalues > 0.2
        // (indices 2 and 3 with eigenvalues 0.4 and 0.9)
        let weight_above: f64 =
            result.amplified_state[2].norm_sqr() + result.amplified_state[3].norm_sqr();
        let weight_below: f64 =
            result.amplified_state[0].norm_sqr() + result.amplified_state[1].norm_sqr();
    ...
    fn test_multi_target_amplification() {
        ...
        let result = multi_target_amplification(&u, &state, &targets, 0.3, 10, &config).unwrap();
    ...
    fn test_comparison_with_qsvt() {
        ...
        assert!(
            (sa_result.success_probability - qsvt_result.success_probability).abs() < 0.1,
    ...
    fn test_degree_exceeded_error() {
        ...
        let result = eigenvalue_threshold_amplification(&h, &state, 0.0, 10, &config);
        assert!(matches!(result, Err(SpectrumError::DegreeExceeded { .. })));
    }
}
