...
impl PauliPropagator {
    ...
    pub fn validate(&self) -> Result<(), GpuPauliError> {
        if self.num_qubits == 0 {
            return Err(GpuPauliError::InvalidPauli("zero qubits".to_string()));
        }
...
impl MetalPauliKernel {
    ...
    pub fn dispatch_clifford(
        ...
        let tg = MTLSize::new(std::cmp::min(256, self.num_strings as u64), 1, 1);
...
mod tests {
    ...
    fn prop_single(ps: PackedPauliString, gate: &CliffordGate) -> (PackedPauliString, (f64, f64)) {
        ...
        assert_eq!(
            results.len(),
            1,
            "Clifford gate should produce exactly 1 term"
        );
        let r = &results[0];
    ...
    fn test_near_clifford_count_t_gates() {
        ...
        let obs = WeightedPackedPauli::unit(PackedPauliString::single_z(2, 0));
        let estimator = NearCliffordEstimator::new(2, obs, circuit, NearCliffordConfig::default());
