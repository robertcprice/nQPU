...
impl std::fmt::Display for TrotterError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            TrotterError::InvalidOrder(o) => {
                write!(f, "invalid Trotter order {o}; must be 1, 2, or 4")
            }
...
pub fn time_dependent_trotter(
    ...
    let base_error =
        trotter_error_estimate(terms, total_time / num_time_steps as f64, config.order);
...
pub fn improved_trotter_evolve(
    ...
    match config.method {
        TrotterMethod::Standard => Ok(standard_trotter(
            terms,
            total_time,
            config.num_steps,
            config.order,
        )),
        TrotterMethod::TimeDependentPF => time_dependent_trotter(terms, total_time, config),
        TrotterMethod::VariationalPF => variational_trotter(terms, total_time, config),
    }
}
...
mod tests {
    ...
    fn test_standard_1st_order_pauli_x() {
        ...
        assert!(f > 0.9999, "fidelity with exact = {f}, expected > 0.9999");
    ...
    fn test_energy_scale_detection() {
        ...
        assert!(ratio > 10.0, "scale ratio = {ratio}, expected > 10.0");
    ...
    fn test_variational_heisenberg_2qubit() {
        ...
        let xx = Array2::from_shape_vec(
            (4, 4),
            vec![
                ZERO, ZERO, ZERO, ONE, ZERO, ZERO, ONE, ZERO, ZERO, ONE, ZERO, ZERO, ONE, ZERO,
                ZERO, ZERO,
            ],
        )
        .unwrap();
