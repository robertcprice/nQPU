...
impl IsingModel {
    ...
    pub fn from_qubo(n: usize, q: &[(usize, usize, f64)]) -> Self {
        ...
        let j_vec: Vec<(usize, usize, f64)> =
            j_map.into_iter().map(|((a, b), v)| (a, b, v)).collect();
...
pub enum QuantumAnnealingSchedule {
    ...
    Pause { start: f64, end: f64, s_pause: f64 },
}
...
impl QuantumAnnealer {
                ...
                for _sweep in 0..self.config.n_sweeps_per_step {
                    for slice_idx in 0..p {
                        for spin_idx in 0..n {
                            ...
                            let delta_e =
                                self.flip_energy_change(&slices, slice_idx, spin_idx, s, j_perp, p);
    ...
    fn flip_energy_change(
        ...
        let neighbour_sum = spin_val(slices[prev][spin_idx]) + spin_val(slices[next][spin_idx]);
    ...
    fn apply_rx(&self, state: &mut [Complex64], q: usize, theta: f64) {
        ...
        while i < dim {
            for k in 0..s {
                ...
                state[idx0] = Complex64::new(a.re * c + b.im * sn, a.im * c - b.re * sn);
                state[idx1] = Complex64::new(b.re * c + a.im * sn, b.im * c - a.re * sn);
            }
            i += s << 1;
        }
    }
...
mod tests {
    ...
    fn test_max_cut_triangle() {
        ...
        assert_eq!(
            cut_size, 2,
            "Triangle max cut should be 2, got {}",
            cut_size
        );
    }
    ...
    fn test_statevector_norm_preservation() {
        ...
        let model =
            IsingModel::new(3, vec![0.5, -0.3, 0.1], vec![(0, 1, 0.8), (1, 2, -0.5)]).unwrap();
    ...
    fn test_random_ising_generation() {
        ...
        assert!(model.j.len() < 50, "Should not exceed total possible pairs");
    ...
    fn test_custom_schedule() {
        let schedule =
            QuantumAnnealingSchedule::Custom(vec![(0.0, 0.0), (0.3, 0.5), (0.7, 0.5), (1.0, 1.0)]);
