...
pub fn optimal_fixed_angles(problem_type: &ProblemType, num_layers: usize) -> QaoaParams {
    match problem_type {
        ProblemType::MaxCut => match num_layers {
            ...
            2 => QaoaParams::new(vec![0.6155, 0.4050], vec![0.3925, 0.1850]),
            p => {
...
pub fn cold_start_qaoa(
    ...
    let gammas: Vec<f64> = (0..num_layers).map(|i| 0.1 + 0.05 * (i as f64)).collect();
    let betas: Vec<f64> = (0..num_layers).map(|i| 0.1 + 0.03 * (i as f64)).collect();
...
pub fn warm_start_transfer(
    ...
    let (_, cold_energy) = cold_start_qaoa(
        target_problem,
        config.num_layers,
        config.cold_start_iterations,
    );
...
pub fn random_3_regular_graph(num_nodes: usize, rng: &mut impl Rng) -> ProblemInstance {
    ...
    for _ in 0..max_attempts {
        ...
        if edges
            .iter()
            .any(|&(a, b, _): &(usize, usize, f64)| (a == i && b == j) || (a == j && b == i))
        {
            continue;
        }
        edges.push((i, j, 1.0));
...
pub fn nelder_mead(f: &dyn Fn(&[f64]) -> f64, initial: &[f64], max_iter: usize) -> (Vec<f64>, f64) {
    let n = initial.len();
    if n == 0 {
        return (vec![], f(&[]));
    }
...
mod tests {
    ...
    fn test_nelder_mead_converges() {
        ...
        assert!((best[0] - 3.0).abs() < 1e-4, "x not converged: {}", best[0]);
        assert!((best[1] - 5.0).abs() < 1e-4, "y not converged: {}", best[1]);
