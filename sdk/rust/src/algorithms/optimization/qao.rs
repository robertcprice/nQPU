    ...
    pub fn optimize(&mut self) -> QAOResult {
        ...
        for iteration in 0..self.max_iterations {
            ...
            if cost_history.len() > 10 {
                ...
                let prev_10: f64 = cost_history.iter().rev().skip(10).take(10).sum::<f64>() / 10.0;
...
mod tests {
    ...
    fn test_graph_coloring_cost_function() {
        let adj = Array2::from_shape_vec((2, 2), vec![false, true, true, false]).unwrap();
    ...
    fn test_constrained_qubo() {
        let matrix = Array2::from_shape_vec((2, 2), vec![1.0, 0.0, 0.0, 1.0]).unwrap();
        let constraints = Array2::from_shape_vec((2, 2), vec![true, false, false, true]).unwrap();
