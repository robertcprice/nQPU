...
pub fn expectation_value(state: &[Complex64], hamiltonian: &[(Vec<(usize, char)>, f64)]) -> f64 {
    let num_qubits = (state.len() as f64).log2() as usize;
    let dim = state.len();
    let mut expval = 0.0;
    for basis in 0..dim {
...
pub fn portfolio_bi_objective(returns: &[f64], covariance: &[Vec<f64>]) -> Vec<ObjectiveFunction> {
    let n = returns.len();
...
pub fn run_qamoo(
    ...
    for (k, obj) in objectives.iter().enumerate() {
        let (params, energy, _bs) = qaoa_optimize(
    ...
    for wv in &weight_vectors {
        ...
        let scalarized_terms =
            build_scalarized_hamiltonian(objectives, wv, &ideal, config.scalarization);
        ...
        let (params, _energy, bitstring) = if let Some(ref prev) = prev_params {
            ...
            let state = simulate_qaoa_circuit(
                &opt_params,
                &scalarized_terms,
                num_qubits,
                config.num_layers,
        ...
        let obj_values: Vec<f64> = objectives
            ...
            .map(|obj| {
                ...
                evaluate_hamiltonian_on_basis(&obj.hamiltonian_terms, basis, num_qubits)
            })
            .collect();
