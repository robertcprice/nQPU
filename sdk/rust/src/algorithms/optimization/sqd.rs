...
impl FermionicHamiltonian {
    ...
    #[inline]
    pub fn set_two_body_element(&mut self, p: usize, q: usize, r: usize, s: usize, val: f64) {
        let n = self.num_spin_orbitals;
    ...
    pub fn h2_sto3g_4so() -> Self {
        ...
        let n: usize = 4;
        let mut two_body = vec![0.0; n * n * n * n];
        let idx = |p: usize, q: usize, r: usize, s: usize| -> usize { p * 64 + q * 16 + r * 4 + s };
    ...
    pub fn lih_sto3g() -> Self {
        ...
        let coupling = [(0, 1, -0.095), (0, 2, -0.013), (1, 2, 0.030), (1, 3, 0.015)];
    ...
    pub fn matrix_element(&self, bra: usize, ket: usize) -> f64 {
        ...
        match annihilated.len() {
            1 => {
                let p = annihilated[0];
                let q = created[0];
                let occ: Vec<usize> = (0..n).filter(|&i| (bra >> i) & 1 == 1 && i != p).collect();

                let mut val = self.one_body[p][q];
                for &r in &occ {
                    val += self.two_body_element(p, r, q, r) - self.two_body_element(p, r, r, q);
                }
                sign * val
            }
            2 => {
                ...
                let r = created[0];
                let s = created[1];
                let val = self.two_body_element(p, q, r, s) - self.two_body_element(p, q, s, r);
                sign * val
            }
            _ => 0.0,
        }
...
fn jacobi_eigenvalues(matrix: &Array2<f64>, tol: f64, max_sweeps: usize) -> SQDResult<Vec<f64>> {
    let n = matrix.nrows();
    if n != matrix.ncols() {
...
pub fn generate_thermal_samples(
    ...
    let energies: Vec<f64> = (0..dim).map(|b| hamiltonian.diagonal_element(b)).collect();
...
mod tests {
    ...
    fn test_hamiltonian_rejects_mismatched_one_body() {
        let result = FermionicHamiltonian::new(2, vec![vec![0.0; 3]; 2], vec![0.0; 16], 0.0, 2);
        assert!(result.is_err());
    }

    #[test]
    fn test_hamiltonian_rejects_mismatched_two_body() {
        let result = FermionicHamiltonian::new(2, vec![vec![0.0; 2]; 2], vec![0.0; 8], 0.0, 2);
        assert!(result.is_err());
    }
    ...
    fn test_bootstrap_display() {
        let h2 = FermionicHamiltonian::h2_sto3g(0.74);
        let samples = vec![0b00, 0b01, 0b10, 0b11];
        let config = SQDConfig::default().with_bootstrap_samples(5).with_seed(42);
        let result = sqd_solve(&h2, &samples, &config).unwrap();
        let bs = result.bootstrap.as_ref().unwrap();
        let s = format!("{}", bs);
    ...
    fn test_hamming_distance_symmetric() {
        assert_eq!(
            hamming_distance(0b1010, 0b0011),
            hamming_distance(0b0011, 0b1010)
        );
    }
    ...
    fn test_error_display_dimension_mismatch() {
        let e = SQDError::DimensionMismatch {
            expected: 4,
            got: 8,
        };
        let msg = format!("{}", e);
    ...
    fn test_error_display_eigen_convergence() {
        let e = SQDError::EigenConvergence {
            iterations: 100,
            residual: 1e-5,
        };
        let msg = format!("{}", e);
    ...
    fn test_error_is_error_trait() {
        let e: Box<dyn std::error::Error> = Box::new(SQDError::EmptySubspace);
    ...
    fn test_sqd_result_display() {
        let h2 = FermionicHamiltonian::h2_sto3g(0.74);
        let samples = vec![0b00, 0b01, 0b10, 0b11];
        let config = SQDConfig::default().with_bootstrap_samples(0).with_seed(42);
        let result = sqd_solve(&h2, &samples, &config).unwrap();
        let s = format!("{}", result);
    ...
    fn test_sqd_with_hamming_expansion_improves_energy() {
        ...
        assert!(
            ...
            result_yes.ground_state_energy,
            result_no.ground_state_energy
        );
    }
}
