...
impl LowDepthUCC {
    ...
    pub fn n_params(&self) -> usize {
        let n_singles = if self.config.singles {
            self.singles.len()
        } else {
            0
        };
        let n_doubles = if self.config.doubles {
            self.doubles.len()
        } else {
            0
        };
        ...
        for _step in 0..self.config.trotter_steps {
            for &idx in &self.order {
                if idx < self.singles.len() {
                    let theta =
                        params.get(idx).copied().unwrap_or(0.0) / self.config.trotter_steps as f64;
    ...
    fn single_excitation_circuit(&self, exc: &SingleExcitation, theta: f64) -> Vec<Gate> {
        ...
        if self.config.qubit_excitation {
            ...
            gates.push(Gate {
                gate_type: GateType::Rz(2.0 * theta),
                targets: vec![a],
                controls: vec![],
                params: None,
            });
