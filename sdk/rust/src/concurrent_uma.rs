...
use crate::uma_dispatch::{
    DispatchCostModel, GateAnalysis, UmaDispatchConfig,
    UmaDispatcher, UmaError,
};
use crate::{C32, C64, QuantumState};
    ...
    pub fn find_concurrent_partitions(&self) -> Vec<Vec<usize>> {
        ...
        for i in 0..self.num_gates {
            ...
            let dep_max_layer = self.deps[i]
                .iter()
                .filter_map(|&d| {
                    layers.iter().position(|(indices, _)| indices.contains(&d))
                })
                .max();
...
impl ConcurrentUmaExecutor {
    ...
    fn execute_layer_sequential(
        ...
        let gpu_gate_list: Vec<Gate> = partition.gpu_gates.iter().map(|&i| gates[i].clone()).collect();
...
fn apply_rz_chunk(
    ...
    for base in (0..chunk.len()).step_by(stride * 2) {
        for i in 0..stride {
            ...
            let b = chunk[i1];
            chunk[i0] = C64::new(
                a.re * p0_re - a.im * p0_im,
                a.re * p0_im + a.im * p0_re,
            );
            chunk[i1] = C64::new(
                b.re * p1_re - b.im * p1_im,
                b.re * p1_im + b.im * p1_re,
            );
        }
    }
    true
...
mod tests {
    ...
    fn test_concurrent_partitions_all_independent() {
        ...
        assert_eq!(layers.len(), 1, "all independent gates should be in one layer");
    ...
    fn test_concurrent_partitions_mixed() {
        ...
        assert_eq!(layers[0].len(), 4, "first layer should have H(0),H(1),H(2),H(3)");
        assert_eq!(layers[1].len(), 2, "second layer should have CNOT(0,1),CNOT(2,3)");
    }
