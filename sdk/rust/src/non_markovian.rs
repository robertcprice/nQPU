...
impl ProcessTensor {
    ...
    pub fn from_rtn(params: &RTNParams, num_steps: usize, dt: f64) -> Self {
        ...
        let correlation_steps = if params.switching_rate > 0.0 {
            ((1.0 / params.switching_rate).ceil() as usize).min(num_steps).max(1)
        } else {
            num_steps
        };
    ...
    pub fn from_one_over_f(params: &OneOverFParams, num_steps: usize, dt: f64) -> Self {
        ...
        let correlation_steps = if params.gamma_min > 0.0 {
            ((1.0 / params.gamma_min).ceil() as usize).min(num_steps).max(1)
        } else {
            num_steps
        };
...
impl NonMarkovianSimulator {
    ...
    pub fn step(&mut self) {
        ...
        let evolved = pt_step.channel.apply_to_density_matrix(&self.state.elements);
    ...
    pub fn apply_gate(&mut self, gate: &Gate) {
        ...
        if gate_dim == self.state.dim {
            ...
            self.state.elements =
                unitary_channel.apply_to_density_matrix(&self.state.elements);
        } else {
            ...
            self.state.elements =
                full_channel.apply_to_density_matrix(&self.state.elements);
        }
    }
    ...
    pub fn non_markovianity_measure(&self) -> f64 {
        ...
        for _ in 0..num_steps {
            ...
            let current_dist =
                trace_distance(&sim_0.state.elements, &sim_1.state.elements, dim);
    ...
    fn renormalize_trace(&mut self) {
        let dim = self.state.dim;
        let trace: f64 = (0..dim)
            .map(|i| self.state.elements[i * dim + i].re)
            .sum();
