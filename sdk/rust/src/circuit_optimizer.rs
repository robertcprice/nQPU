...
impl CircuitOptimizer {
    ...
    fn try_fuse_sequence(
        ...
        let fused_gate = Gate::new(
            GateType::Custom(fused_matrix),
            vec![target_qubit],
            vec![],
        );
    ...
    fn commutation_analysis(&self, gates: &[Gate], stats: &mut OptimizationStats) -> Vec<Gate> {
        ...
        while changed && pass < max_passes {
            ...
            for i in 0..result.len().saturating_sub(1) {
                if self.gates_commute(&result[i], &result[i + 1])
                    && self.should_swap(&result, i)
                {
                    result.swap(i, i + 1);
    ...
    fn gates_commute(&self, a: &Gate, b: &Gate) -> bool {
        ...
        match (&a.gate_type, &b.gate_type) {
            ...
            (GateType::CNOT, GateType::X) => {
                !b.targets.is_empty() && a.targets[0] == b.targets[0]
            }
            (GateType::X, GateType::CNOT) => {
                !a.targets.is_empty() && b.targets[0] == a.targets[0]
            }
    ...
    fn peephole_optimization(&self, gates: &[Gate], stats: &mut OptimizationStats) -> Vec<Gate> {
        ...
        while i < gates.len() {
            ...
            if i + 2 < gates.len() {
                if let Some(replacement) = self.match_3gate_pattern(&gates[i], &gates[i + 1], &gates[i + 2]) {
