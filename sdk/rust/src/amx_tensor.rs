...
impl AmxMPSEngine {
    ...
    pub fn apply_two_qubit_gate(
        ...
        let cols_b = d_r * br;
        let theta_contract =
            AmxComplexGemm::multiply(&tensors[site], &tensors[site + 1], rows_a, chi_inner, cols_b);
        ...
        let (u_trunc, s_trunc, vt_trunc, trunc_error) =
            contractor.truncate(&u_full, &s_full, &vt_full, svd_rows, svd_cols, effective_rank);
...
mod tests {
    ...
    fn test_gemm_known_product() {
        ...
        let a = vec![
            ...
            C64::new(3.0, 0.0), C64::new(4.0, -1.0),
        ];
        let b = vec![
            c64_one(),  c64_zero(),
            c64_zero(), c64_one(),
    ...
    fn test_gemm_non_square() {
        ...
        let a = vec![
            ...
            C64::new(4.0, 0.0), C64::new(5.0, 0.0), C64::new(6.0, 0.0),
        ];
        let b = vec![
            C64::new(7.0, 0.0),  C64::new(8.0, 0.0),
            C64::new(9.0, 0.0),  C64::new(10.0, 0.0),
            C64::new(11.0, 0.0), C64::new(12.0, 0.0),
        ];
        ...
        let expected = vec![
            C64::new(58.0, 0.0),  C64::new(64.0, 0.0),
            C64::new(139.0, 0.0), C64::new(154.0, 0.0),
        ];
    ...
    fn test_gemm_complex_product() {
        ...
        let result = AmxComplexGemm::multiply(&a, &b, 2, 2, 2);
        let expected = vec![
            C64::new(0.0, 2.0), C64::new(0.0, 0.0),
    ...
    fn test_gemm_multiply_add() {
        ...
        let c0 = vec![
            C64::new(10.0, 0.0), C64::new(10.0, 0.0),
            C64::new(10.0, 0.0), C64::new(10.0, 0.0),
        ];
        ...
        // 2*I*B + 3*C0 = 2*B + 3*C0
        let expected = vec![
            C64::new(32.0, 0.0), C64::new(34.0, 0.0),
            C64::new(36.0, 0.0), C64::new(38.0, 0.0),
        ];
        assert_approx_eq_c64(&result, &expected, TOL);
    }
    ...
    fn test_svd_reconstruction() {
        ...
        let a = vec![
            C64::new(1.0, 2.0),  C64::new(3.0, -1.0), C64::new(0.5, 0.5),
            C64::new(-1.0, 0.0), C64::new(2.0, 1.0),  C64::new(1.0, -2.0),
        ];
    ...
    fn test_svd_singular_values_descending() {
        let a = vec![
            C64::new(3.0, 1.0), C64::new(0.0, 0.0),
            C64::new(0.0, 0.0), C64::new(1.0, 0.5),
        ];
    ...
    fn test_batch_gemm() {
        ...
        let b1 = vec![
            C64::new(2.0, 0.0), C64::new(3.0, 0.0),
            C64::new(4.0, 0.0), C64::new(5.0, 0.0),
        ];
        let a2 = vec![
            C64::new(0.0, 1.0), c64_zero(),
            c64_zero(),          C64::new(0.0, 1.0),
        ];
        let b2 = vec![
        ...
        let batch = vec![
            GemmItem { a: &a1, b: &b1, rows: 2, k: 2, cols: 2 },
            GemmItem { a: &a2, b: &b2, rows: 2, k: 2, cols: 2 },
        ];
    ...
    fn test_tensor_contractor_svd() {
        let contractor = AmxTensorContractor::new();
        let a = vec![
            C64::new(1.0, 0.0), C64::new(2.0, 1.0),
    ...
    fn test_truncation_preserves_dominant() {
        ...
        let a = vec![
            C64::new(10.0, 0.0), c64_zero(),          c64_zero(),          c64_zero(),
            c64_zero(),          C64::new(5.0, 0.0),  c64_zero(),          c64_zero(),
            c64_zero(),          c64_zero(),           C64::new(1.0, 0.0),  c64_zero(),
            c64_zero(),          c64_zero(),           c64_zero(),          C64::new(0.01, 0.0),
        ];
    ...
    fn test_truncation_max_rank_larger_than_available() {
        let contractor = AmxTensorContractor::new();
        let a = vec![
            C64::new(3.0, 0.0), C64::new(1.0, 0.0),
            C64::new(1.0, 0.0), C64::new(3.0, 0.0),
        ];
    ...
    fn test_mps_engine_cnot_like_gate() {
        ...
        let cnot = vec![
            c64_one(),  c64_zero(), c64_zero(), c64_zero(),
            c64_zero(), c64_one(),  c64_zero(), c64_zero(),
            c64_zero(), c64_zero(), c64_zero(), c64_one(),
            c64_zero(), c64_zero(), c64_one(),  c64_zero(),
        ];
        ...
        assert!((state[0].norm() - 1.0).abs() < 1e-9, "Expected |00>=1, got {:?}", state[0]);
        assert!(state[1].norm() < 1e-9, "Expected |01>=0, got {:?}", state[1]);
        assert!(state[2].norm() < 1e-9, "Expected |10>=0, got {:?}", state[2]);
        assert!(state[3].norm() < 1e-9, "Expected |11>=0, got {:?}", state[3]);
        assert!(err < 1e-10, "Truncation error should be negligible for product state");
    }
