...
impl DualCameraSystem {
    ...
    pub fn capture(&mut self, duration_us: u64) -> Result<(Vec<CameraEvent>, Vec<CameraEvent>), String> {
        self.events_a.clear();
        self.events_b.clear();
        ...
        self.frames_captured.fetch_add(duration_us, Ordering::Relaxed);
...
impl AntibunchingTest {
    ...
    pub fn run_test(&mut self, duration_us: u64) -> Result<AntibunchingResult, String> {
        ...
        for event_a in &events_a {
            for event_b in &events_b {
                let dt = (event_a.timestamp_us as i64 - event_b.timestamp_us as i64).abs()
                    as u64;
        ...
        let confidence = if is_quantum {
            ...
            if sigma > 5.0 { 0.9999 }
            else if sigma > 3.0 { 0.95 + (sigma - 3.0) * 0.025 }
            else if sigma > 2.0 { 0.85 + (sigma - 2.0) * 0.1 }
            else { 0.5 + sigma * 0.175 }
        } else {
            0.0
        };
    ...
    pub fn run_classical_test(&mut self, duration_us: u64) -> Result<AntibunchingResult, String> {
        ...
        let n_total = events_a.len() + events_b.len();
        let uncertainty = if n_total > 0 { 1.0 / (n_total as f64).sqrt() } else { 1.0 };
...
impl LeggettGargTest {
    ...
    pub fn run_test(&mut self, n_trials: usize) -> Result<LeggettGargResult, String> {
        ...
        for _ in 0..n_trials {
            ...
            rng_state ^= rng_state << 25;
            rng_state = rng_state.wrapping_mul(0x2545F4914F6CDD1D);
            let m1 = if (rng_state as f64 / u64::MAX as f64) < 0.5 { 1 } else { -1 };

            // Quantum evolution between t1 and t2
            // System returns to superposition (for quantum)
            rng_state ^= rng_state >> 12;
            let m2 = if (rng_state as f64 / u64::MAX as f64) < 0.5 { 1 } else { -1 };

            // Quantum evolution between t2 and t3
            rng_state ^= rng_state >> 12;
            let m3 = if (rng_state as f64 / u64::MAX as f64) < 0.5 { 1 } else { -1 };
...
impl QuantumVerificationSuite {
    ...
    pub fn run_all(&mut self, duration_us: u64, n_trials: usize) -> Result<QuantumVerificationReport, String> {
    ...
    pub fn generate_report(&self, report: &QuantumVerificationReport) -> String {
        ...
        output.push_str(&format!(
            ...
            if report.any_quantum_detected { "YES ✅" } else { "NO  ❌" }
        ));
        output.push_str("╚══════════════════════════════════════════════════════════════╝\n");
...
mod tests {
    ...
    fn test_dual_camera_capture() {
        ...
        assert!(ratio > 0.4 && ratio < 0.6, "Events should be split roughly evenly");
    }
