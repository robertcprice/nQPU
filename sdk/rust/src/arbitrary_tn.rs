...
pub enum TNError {
    ...
    DimensionMismatch { index_a: usize, dim_a: usize, index_b: usize, dim_b: usize },
...
impl fmt::Display for TNError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TNError::DimensionMismatch { index_a, dim_a, index_b, dim_b } => {
                write!(
                    f,
...
impl Tensor {
    ...
    fn flat_index(&self, indices: &[usize]) -> usize {
        ...
        for i in (0..shape.len()).rev() {
            assert!(indices[i] < shape[i], "index {} out of range for dim {}", indices[i], shape[i]);
    ...
    pub fn contract_with(&self, other: &Tensor, contract_indices: &[(usize, usize)]) -> Tensor {
        ...
        let free_a: Vec<usize> = (0..shape_a.len()).filter(|i| !contracted_a.contains(i)).collect();
        let free_b: Vec<usize> = (0..shape_b.len()).filter(|i| !contracted_b.contains(i)).collect();
        ...
        let result_numel: usize = if result_shape.is_empty() { 1 } else { result_shape.iter().product() };
        let mut result_data = vec![Complex64::new(0.0, 0.0); result_numel];

        // Contracted dimensions
        let contracted_dims: Vec<usize> = contract_indices.iter().map(|&(ia, _)| shape_a[ia]).collect();
        let contracted_numel: usize = if contracted_dims.is_empty() { 1 } else { contracted_dims.iter().product() };
        ...
        let free_a_numel: usize = if free_a.is_empty() { 1 } else { free_a.iter().map(|&i| shape_a[i]).product() };
        let free_b_numel: usize = if free_b.is_empty() { 1 } else { free_b.iter().map(|&i| shape_b[i]).product() };
        ...
        for fa_flat in 0..free_a_numel {
            ...
            for fb_flat in 0..free_b_numel {
                ...
                for c_flat in 0..contracted_numel {
                    ...
                    let offset_a: usize = idx_a.iter().zip(strides_a.iter()).map(|(&i, &s)| i * s).sum();
                    let offset_b: usize = idx_b.iter().zip(strides_b.iter()).map(|(&i, &s)| i * s).sum();
                ...
                let r_flat = if result_shape.is_empty() {
                    ...
                    let r_multi: Vec<usize> = fa_multi.iter().chain(fb_multi.iter()).copied().collect();
                    r_multi.iter().zip(strides_r.iter()).map(|(&i, &s)| i * s).sum()
                };
    ...
    pub fn trace(&self, idx_a: usize, idx_b: usize) -> Tensor {
        ...
        assert_eq!(shape[idx_a], shape[idx_b], "trace indices must have same dimension");
        ...
        let result_numel = if result_shape.is_empty() { 1 } else { result_shape.iter().product() };
        let mut result_data = vec![Complex64::new(0.0, 0.0); result_numel];
        ...
        let result_indices: Vec<TensorIndex> = remaining
            .iter()
            .map(|&i| self.indices[i].clone())
            .collect();
    ...
    pub fn svd(&self, left_indices: &[usize], max_bond: usize) -> (Tensor, Vec<f64>, Tensor) {
        ...
        let m: usize = if left_dims.is_empty() { 1 } else { left_dims.iter().product() };
        let n: usize = if right_dims.is_empty() { 1 } else { right_dims.iter().product() };

        // Reshape to matrix by permuting indices: left first, right second
        let mut matrix = vec![Complex64::new(0.0, 0.0); m * n];

        let _all_indices: Vec<usize> = left_indices.iter().chain(right_indices.iter()).copied().collect();
...
fn jacobi_svd(matrix: &[Complex64], m: usize, n: usize) -> (Vec<Complex64>, Vec<f64>, Vec<Complex64>) {
    let k = m.min(n);
...
fn hermitian_eigen(matrix: &[Complex64], n: usize) -> (Vec<f64>, Vec<Complex64>) {
    ...
    for _iter in 0..SVD_MAX_ITER {
        ...
        for i in 0..n {
            let aip = a[i * n + p];
            let aiq = a[i * n + q];
            a[i * n + p] = Complex64::new(c, 0.0) * aip + Complex64::new(s, 0.0) * phase.conj() * aiq;
        ...
        for j in 0..n {
            ...
            a[q * n + j] = -Complex64::new(s, 0.0) * phase.conj() * apj + Complex64::new(c, 0.0) * aqj;
        }
        ...
        for i in 0..n {
            let vip = v[i * n + p];
            let viq = v[i * n + q];
            v[i * n + p] = Complex64::new(c, 0.0) * vip + Complex64::new(s, 0.0) * phase.conj() * viq;
...
impl TensorNetwork {
    ...
    pub fn connect(&mut self, tensor_a: usize, idx_a: usize, tensor_b: usize, idx_b: usize) {
        ...
        match found {
            ...
            None => {
                self.contractions.push((tensor_a, tensor_b, vec![(idx_a, idx_b)]));
            }
        }
    }
    ...
    pub fn contraction_cost(&self, order: &[(usize, usize)]) -> f64 {
        ...
        for (id, &pos) in &self.id_map {
            ...
            index_maps.insert(*id, self.tensors[pos].indices.iter().map(|idx| idx.id).collect());
        }
        ...
        for &(a, b) in order {
            ...
            let shared: Vec<usize> = ids_b.iter().filter(|id| set_a.contains(id)).copied().collect();
            ...
            let free_a: Vec<(usize, usize)> = ids_a.iter().zip(shape_a.iter())
                .filter(|(id, _)| !shared.contains(id))
                .map(|(&id, &dim)| (id, dim))
                .collect();
            let free_b: Vec<(usize, usize)> = ids_b.iter().zip(shape_b.iter())
                .filter(|(id, _)| !shared.contains(id))
                .map(|(&id, &dim)| (id, dim))
                .collect();

            let output_product: f64 = free_a.iter().chain(free_b.iter())
            ...
            // Register result
            let result_ids: Vec<usize> = free_a.iter().chain(free_b.iter()).map(|&(id, _)| id).collect();
            let result_shape: Vec<usize> = free_a.iter().chain(free_b.iter()).map(|&(_, d)| d).collect();
    ...
    pub fn contract_all(&mut self, order: &[(usize, usize)]) -> Tensor {
        ...
        assert_eq!(work.len(), 1, "contraction order did not reduce to single tensor");
...
impl ContractionOptimizer {
    ...
    pub fn greedy_order(network: &TensorNetwork) -> ContractionOrder {
        ...
        for (&id, &pos) in &network.id_map {
            ...
            index_id_maps.insert(id, network.tensors[pos].indices.iter().map(|idx| idx.id).collect());
        }
        ...
        while active.len() > 1 {
            ...
            for i in 0..active_vec.len() {
                for j in (i + 1)..active_vec.len() {
                    ...
                    let result_size: usize = free_a.iter().chain(free_b.iter())
                        .map(|&(_, d)| d)
                        .product::<usize>()
                        .max(1);

                    let contracted_product: f64 = shared.iter()
                        .map(|sid| {
                            let pos = ids_a.iter().position(|x| x == sid).unwrap();
                            shape_a[pos] as f64
                        })
                    ...
                    if cost < best_cost {
                        best_cost = cost;
                        best_pair = (a, b);
                        best_result_ids = free_a.iter().chain(free_b.iter()).map(|&(id, _)| id).collect();
                        best_result_shape = free_a.iter().chain(free_b.iter()).map(|&(_, d)| d).collect();
                        best_flops = flops;
                    }
                }
            }
    ...
    pub fn kahypar_like(network: &TensorNetwork) -> ContractionOrder {
        ...
        for (&id, &pos) in &network.id_map {
            shapes.insert(id, network.tensors[pos].shape());
            id_maps.insert(id, network.tensors[pos].indices.iter().map(|idx| idx.id).collect());
        }
        ...
        for i in 0..active.len() {
            for j in (i + 1)..active.len() {
                ...
                if cost < best_cost {
                    best_cost = cost;
                    best_pair = (a, b);
                    best_shape = free_a.iter().chain(free_b.iter()).map(|&(_, d)| d).collect();
                    best_ids = free_a.iter().chain(free_b.iter()).map(|&(id, _)| id).collect();
                    best_flops = flops;
                }
            }
        }
...
impl CircuitToTN {
    ...
    pub fn from_circuit(
        ...
        for q in 0..num_qubits {
            ...
            let mut t = Tensor::new(&[2], vec![Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0)]);
        ...
        for (name, qubits, matrix_data) in gates {
            ...
            for (k, &q) in qubits.iter().enumerate() {
                ...
                t.indices[nq + k] = TensorIndex::with_id(current_wires[q], 2, &format!("q{}_{}_in", q, name));
...
impl TNSimulator {
    ...
    pub fn amplitude(&self, network: &TensorNetwork, bitstring: &[bool]) -> Complex64 {
        ...
        for (pos, t) in net.tensors.iter().enumerate() {
            for (idx_pos, idx) in t.indices.iter().enumerate() {
                if index_count.get(&idx.id) == Some(&1) && !idx.name.contains("_in") && idx.name.starts_with('q') {
    ...
    pub fn expectation(&self, network: &TensorNetwork, observable: &Tensor) -> Complex64 {
        ...
        let bra_pairs: Vec<(usize, usize)> = (0..psi_conj.rank())
            .zip(0..o_psi.rank())
            .collect();
    ...
    for (i, mps_t) in mps_tensors.iter().enumerate() {
        ...
        if t.rank() >= 2 {
            ...
            if let Some(prev_id) = prev_bond_id {
                t.indices[0] = TensorIndex::with_id(prev_id, t.indices[0].dim, &format!("bond_{}", i - 1));
            }
...
mod tests {
    ...
    fn test_contraction_matrix_multiply() {
        // A = [[1, 2], [3, 4]], B = [[5, 6], [7, 8]]
        // A * B = [[19, 22], [43, 50]]
        let a = Tensor::new(&[2, 2], vec![c(1.0, 0.0), c(2.0, 0.0), c(3.0, 0.0), c(4.0, 0.0)]);
        let b = Tensor::new(&[2, 2], vec![c(5.0, 0.0), c(6.0, 0.0), c(7.0, 0.0), c(8.0, 0.0)]);
    ...
    fn test_tensor_trace() {
        ...
        assert!((tr.data[0] - c(3.0, 0.0)).norm() < TOL, "trace of 3x3 identity should be 3");
    }
    ...
    fn test_tensor_svd() {
        ...
        for i in 0..2 {
            for j in 0..2 {
                ...
                assert!(diff < 1e-8, "SVD reconstruction error at [{},{}]: {}", i, j, diff);
            }
        }

        // Singular values should be positive and sorted descending
        assert!(svals[0] >= svals[1], "singular values should be descending");
        assert!(svals.iter().all(|&s| s >= 0.0), "singular values must be non-negative");
    }
    ...
    fn test_contract_pair() {
        ...
        let mut a = Tensor::new(&[2, 2], vec![c(1.0, 0.0), c(2.0, 0.0), c(3.0, 0.0), c(4.0, 0.0)]);
        a.indices[1].id = shared_id;

        let mut b = Tensor::new(&[2, 2], vec![c(5.0, 0.0), c(6.0, 0.0), c(7.0, 0.0), c(8.0, 0.0)]);
    ...
    fn test_full_contraction_chain() {
        ...
        for i in 0..result.data.len() {
            ...
            assert!(diff < 1e-8, "chain contraction mismatch at {}: diff={}", i, diff);
        }
    }
    ...
    fn test_random_greedy() {
        ...
        assert!(random.estimated_flops <= greedy.estimated_flops + 1e-6,
            "random greedy should be at least as good: {} vs {}", random.estimated_flops, greedy.estimated_flops);
    }
    ...
    fn test_cost_estimation() {
        ...
        assert!(approx_eq(cost, 48.0, 1e-6), "cost should be 48, got {}", cost);
    }
    ...
    fn test_amplitude_bell_state() {
        ...
        assert!(approx_eq(amp_00.norm(), inv_sqrt2, 1e-8),
            "|<00|psi>| should be 1/sqrt(2), got {}", amp_00.norm());
        assert!(approx_eq(amp_11.norm(), inv_sqrt2, 1e-8),
            "|<11|psi>| should be 1/sqrt(2), got {}", amp_11.norm());
        assert!(approx_eq(amp_01.norm(), 0.0, 1e-8),
            "|<01|psi>| should be 0, got {}", amp_01.norm());
        assert!(approx_eq(amp_10.norm(), 0.0, 1e-8),
            "|<10|psi>| should be 0, got {}", amp_10.norm());
    }
    ...
    fn test_inner_product() {
        ...
        assert!(approx_eq(ip.re, 1.0, 1e-8), "<psi|psi> should be 1.0, got {}", ip);
    }
    ...
    fn test_bond_compression() {
        ...
        assert!(compressed_norm <= original_norm + TOL,
            "compressed norm {} should be <= original {}", compressed_norm, original_norm);
    }
    ...
    fn test_small_circuit_x_gate() {
        ...
        assert!(approx_eq(amp_0.norm(), 0.0, 1e-8), "X|0> -> |1>: amp(0) should be 0");
        assert!(approx_eq(amp_1.norm(), 1.0, 1e-8), "X|0> -> |1>: amp(1) should be 1");
    }
    ...
    fn test_order_affects_cost_not_result() {
        ...
        assert_eq!(r1.shape(), r2.shape(), "shapes must match regardless of order");
        for i in 0..r1.data.len() {
            let diff = (r1.data[i] - r2.data[i]).norm();
            assert!(diff < 1e-6, "results must match regardless of order: diff={} at {}", diff, i);
        }
    ...
    fn test_tensor_reshape() {
        let t = Tensor::new(&[2, 3], vec![c(1.0,0.0), c(2.0,0.0), c(3.0,0.0),
