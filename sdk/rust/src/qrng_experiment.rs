...
impl QrngExperiment {
    ...
    pub fn run(&mut self) -> Result<ExperimentResult, String> {
        ...
        if self.config.test_ssd {
            println!("▶ Running SSD timing test ({} samples)...", self.config.n_samples);
            ...
            println!("  Mean: {:.2} µs, Std: {:.2} µs, CV: {:.2}%",
                     result.mean_ns / 1000.0, result.std_ns / 1000.0, result.cv * 100.0);
        ...
        if self.config.test_cpu {
            println!("▶ Running CPU jitter test ({} samples)...", self.config.n_samples);
            ...
            println!("  Mean: {:.2} ns, Std: {:.2} ns, CV: {:.2}%",
                     result.mean_ns, result.std_ns,
        ...
        if self.config.test_entropy {
            println!("▶ Collecting bits from entropy pool ({} samples)...", self.config.n_samples);
            ...
            println!("  Fano factor: {:.4} (ideal: 1.0 for Poisson)", result.fano_factor);
        ...
        if self.config.run_nist && !bits.is_empty() {
            println!("▶ Running NIST SP 800-22 test suite ({} bits)...", bits.len() * 8);

            // Convert bytes to bits for NIST tests
            let bit_vec: Vec<u8> = bits.iter()
        ...
        let interpretation = self.interpret_results(quantum_score, &ssd_stats, &cpu_stats, &nist_result);
    ...
    fn save_results(&self, result: &ExperimentResult) -> Result<(), String> {
        ...
        // Save main report
        let report_path = self.config.output_dir.join(format!("{}_report.md", result.id));
        ...
        // Save raw data
        let data_path = self.config.output_dir.join(format!("{}_data.csv", result.id));
...
╚══════════════════════════════════════════════════════════════╝"#,
            ...
            if result.ssd_stats.is_some() { "✅ Tested" } else { "⏭ Skipped" },
            if result.cpu_stats.is_some() { "✅ Tested" } else { "⏭ Skipped" },
