...
use crate::{c64_zero, c64_one, C64};
...
impl BarrettKokProtocol {
    ...
    pub fn success_probability(&self) -> f64 {
        let single_arm = self.emission_prob * self.channel_transmittance * self.detection_efficiency;
...
impl QuantumRepeater {
    ...
    fn segment_channel(&self) -> FiberChannel {
        FiberChannel::new(self.segment_distance, self.loss_db_per_km, self.depolarizing_rate)
    }
    ...
    pub fn distribute(&self) -> (Vec<C64>, f64, f64) {
        ...
        if pairs.len() == 1 {
            ...
            return (pairs.into_iter().next().unwrap(), total_success_prob, fidelity);
        }
    ...
    pub fn beats_direct_transmission(&self) -> bool {
        let direct_eta = 10.0_f64.powf(
            -self.loss_db_per_km * self.total_distance() / 10.0,
        );
...
impl QuantumNetwork {
    ...
    pub fn distribute_entanglement(
        &self,
        src: usize,
        dst: usize,
    ) -> Option<(Vec<C64>, f64, f64)> {
...
mod tests {
    ...
    fn test_erasure_channel_reduces_fidelity() {
        ...
        assert!(
            f_out < 1.0,
            "Erasure should reduce fidelity, got {}",
            f_out
        );
    }
    ...
    fn test_dlcz_bell_pair_generation() {
        ...
        let tr = dm_trace(&rho, 4);
        assert!((tr - 1.0).abs() < TOLERANCE, "Trace should be 1, got {}", tr);
    }
