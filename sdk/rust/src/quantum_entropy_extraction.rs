...
use crate::{QuantumState, GateOperations, C64};
...
impl QuantumEntropyExtractor {
    ...
    fn extract_process_tensor_entropy(&self) -> Vec<u8> {
        ...
        for t in 0..self.config.depth {
            ...
            for q in 0..n {
                ...
                if !memory.is_empty() {
                    let mem_influence: f64 = memory.iter()
                        .flat_map(|m| m.iter())
                        .sum();
            ...
            if memory.len() >= 2 {
                let corr = self.temporal_correlation(&memory[memory.len() - 2], &memory[memory.len() - 1]);
    ...
    fn pauli_matrix_2q(&self, i: usize, j: usize, p0: char, p1: char) -> C64 {
        ...
        // First qubit
        el *= match p0 {
            'I' => if b0_i == b0_j { Complex64::new(1.0, 0.0) } else { Complex64::new(0.0, 0.0) },
            'X' => if b0_i != b0_j { Complex64::new(1.0, 0.0) } else { Complex64::new(0.0, 0.0) },
            'Y' => if b0_i != b0_j { Complex64::new(0.0, if b0_i == 0 { 1.0 } else { -1.0 }) } else { Complex64::new(0.0, 0.0) },
            'Z' => if b0_i == b0_j { Complex64::new(if b0_i == 0 { 1.0 } else { -1.0 }, 0.0) } else { Complex64::new(0.0, 0.0) },
            _ => Complex64::new(1.0, 0.0),
        };

        // Second qubit
        el *= match p1 {
            'I' => if b1_i == b1_j { Complex64::new(1.0, 0.0) } else { Complex64::new(0.0, 0.0) },
            'X' => if b1_i != b1_j { Complex64::new(1.0, 0.0) } else { Complex64::new(0.0, 0.0) },
            'Y' => if b1_i != b1_j { Complex64::new(0.0, if b1_i == 0 { 1.0 } else { -1.0 }) } else { Complex64::new(0.0, 0.0) },
            'Z' => if b1_i == b1_j { Complex64::new(if b1_i == 0 { 1.0 } else { -1.0 }, 0.0) } else { Complex64::new(0.0, 0.0) },
            _ => Complex64::new(1.0, 0.0),
        };
...
pub fn generate_quantum_seeds(count: usize) -> Vec<u64> {
    let extractor = get_global_extractor(8);
    (0..count).map(|_| extractor.extract_seed().seed_u64).collect()
}
...
pub fn print_benchmark() {
    ...
    println!("{}", "-".repeat(70));
    println!("{:<10} {:<12} {:<12} {:<12} {:<15}", "Qubits", "Rounds", "Time (s)", "Avg Shannon", "Seeds/sec");
    println!("{}", "-".repeat(70));

    for n in [4, 6, 8, 10].iter() {
        let (time, shannon, throughput) = benchmark_extraction(*n, 100);
        println!("{:<10} {:<12} {:<12.4} {:<12.2} {:<15}", n, 100, time, shannon, throughput);
    }
