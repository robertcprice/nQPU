...
impl Metal4Capabilities {
    ...
    pub fn detect() -> Self {
        ...
        let max_threads_per_group = device
...
impl Metal4TensorContraction {
    ...
    fn contract_pair_cpu(
        ...
        let free_a: Vec<usize> = (0..rank_a)
            .filter(|i| !contracted_a.contains(i))
            .collect();
        let free_b: Vec<usize> = (0..rank_b)
        ...
        let contracted_dims: Vec<usize> = contracted_a
            .iter()
            .map(|&ax| dims_a[ax])
            .collect();
...
impl Metal4AdaptiveML {
    ...
    pub fn predict_backend(&mut self, analysis: &CircuitAnalysis) -> SimBackend {
        ...
        if analysis.is_clifford_only || features.clifford_fraction >= self.clifford_stabilizer_threshold {
    ...
    fn extract_features(analysis: &CircuitAnalysis) -> CircuitFeatures {
        ...
        let two_qubit_fraction =
            analysis.num_two_qubit as f64 / total_gates as f64;
        ...
        CircuitFeatures {
            num_qubits: analysis.num_single_qubit
...
impl Metal4Profiler {
    ...
    pub fn end_capture(&mut self) -> ProfilingReport {
        ...
        let bottleneck =
            ProfilingReport::identify_bottleneck(elapsed, bandwidth_gbps, occupancy);
...
mod tests {
    ...
    fn test_shader_compiler_generates_hadamard() {
        ...
        assert!(result.is_ok(), "Hadamard shader should compile: {:?}", result.err());
