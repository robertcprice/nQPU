    ...
    for i in 0..phase_nodes.len() {
        ...
        for j in i + 1..phase_nodes.len() {
            ...
            if phase_is_zero(sum) {
                ...
                let nj: HashSet<usize> = diagram.neighbor_ids(phase_nodes[j].0).into_iter().collect();
        ...
        for (parity, phase) in &self.terms {
            ...
            if active_qubits.len() == 1 {
                ...
                for &ctrl in &active_qubits[..active_qubits.len() - 1] {
                    let zc = diagram.add_spider_with_info(
                        SpiderType::ZSpider(0.0),
                        Some(ctrl),
                        None,
                    );
                    let xt = diagram.add_spider_with_info(
                        SpiderType::XSpider(0.0),
                        Some(target),
                        None,
                    );
...
mod tests {
    ...
    fn test_clifford_simplify_reaches_fixpoint() {
        ...
        assert!(after <= before, "Simplification should not increase node count");
    }
    ...
    fn test_full_simplify_reduces_gate_count() {
        ...
        assert!(after <= before, "Full simplify should reduce gate count: before={}, after={}", before, after);
    }
