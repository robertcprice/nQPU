...
impl DistributedQuantumState {
    ...
    fn apply_global_single_qubit(&mut self, qubit: usize, matrix: [[C64; 2]; 2]) -> Result<(), String> {
        #[cfg(feature = "distributed")]
        {
            let rank = self.mpi.rank();
            let num_local_qubits = self.global_num_qubits - (self.mpi.size() as f64).log2() as usize;
            ...
            self.mpi.world.process_at_rank(partner_rank as i32)
