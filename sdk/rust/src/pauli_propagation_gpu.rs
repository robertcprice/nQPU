...
impl PauliSum {
    ...
    pub fn merge_duplicates(&mut self, threshold: f64) {
        ...
        for term in &self.terms {
            ...
            key_to_paulis.entry(key).or_insert_with(|| term.paulis.clone());
        }
    ...
    pub fn truncate_by_count(&mut self, max_terms: usize) {
        if self.terms.len() > max_terms {
            self.terms
                .sort_by(|a, b| b.coeff_magnitude().partial_cmp(&a.coeff_magnitude()).unwrap());
    ...
    pub fn commuting_groups(&self) -> Vec<Vec<usize>> {
        ...
        for (idx, term) in self.terms.iter().enumerate() {
            ...
            for group in &mut groups {
                let all_commute = group.iter().all(|&g_idx| {
                    term.commutes_with(&self.terms[g_idx])
                });
                if all_commute {
                    group.push(idx);
                    placed = true;
...
impl PropGate {
    ...
    pub fn max_qubit(&self) -> usize {
        match self {
            ...
            | PropGate::Ry(q, _)
            | PropGate::Rz(q, _) => *q,
            PropGate::CX(a, b) | PropGate::CZ(a, b) | PropGate::Swap(a, b) => {
                std::cmp::max(*a, *b)
            }
    ...
    for _ in 0..depth {
        for q in 0..num_qubits {
            ...
            match r {
                ...
                2 => {
                    if num_qubits > 1 {
                        let other = (q + 1 + (next_rand() as usize % (num_qubits - 1)))
                            % num_qubits;
...
mod tests {
    ...
    fn test_weighted_pauli_creation() {
        let wp = WeightedPauli::new(
            vec![PauliOp::X, PauliOp::Y, PauliOp::Z],
            (0.5, -0.3),
        );
    ...
    fn test_batch_propagation_matches_sequential() {
        ...
        for obs in &[obs1.clone(), obs2.clone(), obs3.clone()] {
            let config = PropagationConfig::default();
            let mut prop =
                PauliPropagator::new(config, obs.clone(), circuit.clone());
    ...
    fn test_performance_1000_terms() {
        ...
        assert!(result.elapsed_secs < 10.0, "took too long: {}s", result.elapsed_secs);
    }
