...
impl fmt::Display for LatticeSurgeryError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            LatticeSurgeryError::PatchOverlap { patch_a, patch_b, position } => {
                write!(
                    f,
            ...
            LatticeSurgeryError::InvalidMerge { patch_a, patch_b, reason } => {
                write!(
                    f,
            ...
            LatticeSurgeryError::ResourceExhausted { resource, required, available } => {
                write!(
                    f,
...
impl LatticeSurgeryEngine {
    ...
    fn measure_xx(state: &mut QuantumState) -> bool {
        assert!(state.num_qubits >= 2, "Need at least 2 qubits for XX measurement");
        ...
        let prob_plus = plus_0.norm_sqr() + plus_1.norm_sqr()
    ...
    fn measure_zz(state: &mut QuantumState) -> bool {
        assert!(state.num_qubits >= 2, "Need at least 2 qubits for ZZ measurement");
        let amps = state.amplitudes_mut();
        let a00 = amps[0];
        let a01 = amps[1];
        let a10 = amps[2];
    ...
    pub fn merge(
        ...
        let parity_outcome = if let (Some(sa), Some(sb)) = (state_a, state_b) {
            ...
            let effective_parity = if noise_flips % 2 == 1 { !parity } else { parity };
    ...
    pub fn logical_cnot(
        ...
        let state_c = self.patches.get(&control_id).and_then(|p| p.logical_state.clone());
        let state_t = self.patches.get(&target_id).and_then(|p| p.logical_state.clone());
    ...
    pub fn logical_hadamard(
        &mut self,
        patch_id: usize,
    ) -> Result<(), LatticeSurgeryError> {
        let patch = self.patches.get_mut(&patch_id).ok_or_else(|| {
            LatticeSurgeryError::SplitFailed {
                patch_id,
                reason: "Patch does not exist".to_string(),
            }
        })?;
    ...
    pub fn logical_s_gate(
        &mut self,
        patch_id: usize,
    ) -> Result<bool, LatticeSurgeryError> {
    ...
    pub fn logical_t_gate(
        &mut self,
        patch_id: usize,
    ) -> Result<(bool, bool), LatticeSurgeryError> {
        if !self.patches.contains_key(&patch_id) {
            return Err(LatticeSurgeryError::SplitFailed {
                patch_id,
                reason: "Patch does not exist".to_string(),
    ...
    pub fn measure_z(&mut self, patch_id: usize) -> Result<bool, LatticeSurgeryError> {
        let patch = self.patches.get(&patch_id).ok_or_else(|| {
            LatticeSurgeryError::SplitFailed {
                patch_id,
                reason: "Patch does not exist".to_string(),
            }
...
mod tests {
    ...
    fn test_merge_xx() {
        ...
        assert!(!merge.parity_outcome, "XX merge of |00> should give +1 (even) parity");
    ...
    fn test_merge_zz() {
        ...
        assert!(!merge.parity_outcome, "ZZ merge of |00> should give +1 (even) parity");
        ...
        assert!(merge2.parity_outcome, "ZZ merge of |01> should give -1 (odd) parity");
    }
    ...
    fn test_logical_cnot() {
        ...
        let probs_c = engine.patches[&control].measurement_probabilities().unwrap();
    ...
    fn test_compile_bell_circuit() {
        ...
        let total_instructions: usize = schedule
            .steps
            .iter()
    ...
    fn test_measure_z_with_frame() {
        ...
        assert!(result2, "X frame correction should flip measurement of |0> to 1");
    }
    ...
    fn test_measure_x_basis() {
        ...
        assert!(!result, "Measuring |+> in X basis should give 0 (+1 eigenvalue)");
    }
