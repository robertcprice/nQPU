...
) -> ([[num_complex::Complex64; 4]; 4], [[num_complex::Complex64; 4]; 4]) {
    use num_complex::Complex64 as C;
...
mod tests {
    ...
    fn test_dejmps_distillation() {
        let mut distiller = EntanglementDistiller::new(2)
            .with_protocol(DistillationProtocol::DEJMPS);

        // Starting fidelity 0.75 should improve
        let f_after = distiller.distill(0.75);
        assert!(f_after > 0.75, "Fidelity should improve: {} > 0.75", f_after);
    ...
    fn test_bbpssw_distillation() {
        let mut distiller = EntanglementDistiller::new(2)
            .with_protocol(DistillationProtocol::BBPSSW);
    ...
    fn test_nested_swapping() {
        ...
        assert!(f2 < f1, "More levels should reduce fidelity: {} < {}", f2, f1);
    }
