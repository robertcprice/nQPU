...
impl GoogleProvider {
    ...
    pub fn from_env() -> Result<Self, QPUError> {
        let project_id = std::env::var("GOOGLE_CLOUD_PROJECT")
            .map_err(|_| QPUError::ConfigError("GOOGLE_CLOUD_PROJECT not set".into()))?;
...
impl QPUProvider for GoogleProvider {
    ...
    async fn list_backends(&self) -> Result<Vec<BackendInfo>, QPUError> {
        let path = format!("/projects/{}/processors", self.project_id);
    ...
    async fn submit_circuit(
        ...
        let path = format!("/projects/{}/programs", self.project_id,);
    ...
    async fn estimate_cost(
        ...
        Ok(Some(CostEstimate {
            ...
            is_free_tier: true,
            details: Some(
                "Google Quantum access is via research partnership — no per-job cost".into(),
            ),
        }))
    }
                    ...
                    for meas in measurements {
                        if let Some(bits) = meas.as_array() {
                            let bitstring: String = bits
                                .iter()
                                .map(|b| if b.as_i64() == Some(1) { '1' } else { '0' })
                                .collect();
...
fn known_google_backends() -> Vec<BackendInfo> {
    vec![
        BackendInfo {
            ...
            queue_length: 0,
            basis_gates: vec!["syc".into(), "phased_xz".into(), "cz".into()],
            coupling_map: None,
            max_shots: 1_000_000,
            max_depth: None,
            avg_gate_error_1q: Some(0.001),
