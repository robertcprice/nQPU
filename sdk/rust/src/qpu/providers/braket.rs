...
impl BraketProvider {
    ...
    async fn signed_get(&self, path: &str) -> Result<serde_json::Value, QPUError> {
        ...
        let mut headers_list: Vec<(&str, String)> =
            vec![("host", host.to_string()), ("x-amz-date", amz_date.clone())];
        if let Some(ref tok) = self.session_token {
            headers_list.push(("x-amz-security-token", tok.clone()));
        }
        let headers_ref: Vec<(&str, &str)> =
            headers_list.iter().map(|(k, v)| (*k, v.as_str())).collect();
    ...
    async fn send_and_parse(
        ...
        let body = response.text().await.map_err(|e| {
            QPUError::NetworkError(format!("Failed to read Braket response: {}", e))
        })?;
...
impl QPUProvider for BraketProvider {
    ...
    async fn submit_circuit(
        ...
        let task_id = json["quantumTaskArn"]
            ...
            .ok_or_else(|| QPUError::SubmissionError("Missing task ARN in Braket response".into()))?
            .to_string();
    ...
    async fn estimate_cost(
        ...
        let (task_cost, shot_cost, details) = if arn.contains("ionq") {
            ...
            return Ok(Some(CostEstimate {
                ...
                is_free_tier: false,
                details: Some(format!("SV1 simulator: ~{:.3} min at $0.075/min", minutes)),
            }));
        } else {
...
fn parse_braket_device(json: &serde_json::Value) -> Option<BackendInfo> {
    ...
    let caps: serde_json::Value = serde_json::from_str(caps_str).unwrap_or(serde_json::json!({}));
    ...
    let coupling_map: Option<Vec<(usize, usize)>> = caps["paradigm"]["connectivity"]
        ["fullyConnected"]
        .as_bool()
        .and_then(|fully| {
            if fully {
...
mod tests {
    ...
    fn test_arn_to_display_name_known() {
        assert_eq!(
            BraketProvider::arn_to_display_name("arn:aws:braket:us-east-1::device/qpu/ionq/Aria-1"),
            "ionq_aria"
        );
    }
    ...
    fn test_sign_request_deterministic() {
        ...
        assert!(
            auth.contains("Credential=AKIAIOSFODNN7EXAMPLE/20240101/us-east-1/braket/aws4_request")
        );
    ...
    async fn test_braket_cost_estimate_ionq() {
        let provider =
            BraketProvider::new("AKID".into(), "SECRET".into(), "us-east-1".into(), None);
