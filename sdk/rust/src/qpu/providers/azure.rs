...
impl AzureProvider {
    ...
    pub fn new(
        ...
        let base_url = format!("https://{}.{}.quantum.azure.com", workspace, location);
        Self {
    ...
    pub fn from_env() -> Result<Self, QPUError> {
        ...
        let location =
            std::env::var("AZURE_QUANTUM_LOCATION").unwrap_or_else(|_| "eastus".to_string());
    ...
    async fn get_token(&self) -> Result<String, QPUError> {
        // Check cache first.
        {
            let cache = self
                .token_cache
                .read()
                .map_err(|_| QPUError::ProviderError {
    ...
    fn map_http_error(status: reqwest::StatusCode, body: &str) -> QPUError {
        match status.as_u16() {
            ...
            404 => {
                QPUError::BackendUnavailable(format!("Not found: {}", &body[..body.len().min(300)]))
            }
...
impl QPUProvider for AzureProvider {
    ...
    async fn list_backends(&self) -> Result<Vec<BackendInfo>, QPUError> {
        ...
        let providers = json["value"].as_array().or_else(|| json.as_array());
        ...
        if backends.is_empty() {
            for &(target_id, provider_name, qubits, is_sim) in KNOWN_TARGETS {
                backends.push(make_known_target_info(
                    target_id,
                    provider_name,
                    qubits,
                    is_sim,
    ...
    async fn submit_circuit(
        ...
        let (input_format, input_data) = if backend.contains("ionq") {
            ...
            (
                "ionq.circuit.v1",
                serde_json::to_string(&ionq_json).map_err(|e| {
    ...
    async fn estimate_cost(
        ...
        let (cost, details) = if backend.contains("ionq") {
            ...
            if is_simulator {
                ...
                (cost, "Quantinuum QPU: HQC-based pricing (~$1/HQC)")
            }
        } else {
...
impl AzureJob {
    ...
    async fn get_token(&self) -> Result<String, QPUError> {
        ...
        let body = response
            .text()
            .await
            .map_err(|e| QPUError::AuthenticationError(format!("Read Azure AD response: {}", e)))?;
        ...
        let json: serde_json::Value = serde_json::from_str(&body)
            .map_err(|e| QPUError::AuthenticationError(format!("Invalid token JSON: {}", e)))?;
...
fn required_env(name: &str) -> Result<String, QPUError> {
    std::env::var(name)
        .map_err(|_| QPUError::ConfigError(format!("{} environment variable not set", name)))
}
...
fn parse_azure_target(json: &serde_json::Value, provider_id: &str) -> Option<BackendInfo> {
    ...
    if let Some(desc) = json["description"].as_str() {
        metadata.insert("description".into(), serde_json::Value::String(desc.into()));
    }
    metadata.insert(
        "provider_id".into(),
...
fn parse_azure_result(
    ...
    if let Some(histogram) = output_parsed["histogram"].as_object() {
        let shots = json["inputParams"]["shots"].as_u64().unwrap_or(1000) as f64;
    ...
    else if let Some(count_obj) = output_parsed["Counts"]
        .as_object()
    ...
    let shots = json["inputParams"]["shots"]
        .as_u64()
        .unwrap_or_else(|| counts.values().sum::<usize>() as u64) as usize;

    let is_simulator = backend_name.contains("sim");

    let execution_time = json["beginExecutionTime"].as_str().and_then(|_begin| {
    ...
    if let Some(cost) = json["costEstimate"].as_object() {
        metadata.insert(
            "cost_estimate".into(),
            serde_json::Value::Object(cost.clone()),
        );
    }
...
mod tests {
    ...
    fn test_azure_map_http_error_auth() {
        let err = AzureProvider::map_http_error(reqwest::StatusCode::FORBIDDEN, "access denied");
        match err {
    ...
    fn test_azure_map_http_error_not_found() {
        let err =
            AzureProvider::map_http_error(reqwest::StatusCode::NOT_FOUND, "workspace not found");
        match err {
    ...
    fn test_azure_map_http_error_server() {
        ...
        match err {
            QPUError::ProviderError { status_code, .. } => assert_eq!(status_code, Some(500)),
