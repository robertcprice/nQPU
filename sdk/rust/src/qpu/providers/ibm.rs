...
impl QPUProvider for IBMProvider {
    ...
    async fn list_backends(&self) -> Result<Vec<BackendInfo>, QPUError> {
        ...
        let backends_arr = json.as_array().ok_or_else(|| QPUError::ProviderError {
            provider: IBM_PROVIDER_NAME.into(),
    ...
    async fn submit_circuit(
        ...
        let json = self.execute(self.post("/api/jobs").json(&body)).await?;
...
impl QPUJob for IBMJob {
    ...
    async fn status(&self) -> Result<JobStatus, QPUError> {
        let json = self.get_json(&format!("/api/jobs/{}", self.job_id)).await?;
    ...
    async fn cancel(&self) -> Result<(), QPUError> {
        let resp = self
            .client
            .post(format!("{}/api/jobs/{}/cancel", self.base_url, self.job_id))
            .bearer_auth(&self.token)
            .send()
            .await
...
impl IBMJob {
    ...
    async fn fetch_result(&self) -> Result<JobResult, QPUError> {
        ...
        let is_simulator =
            self.backend_name.contains("simulator") || self.backend_name.contains("sim");
...
fn parse_ibm_backend(json: &serde_json::Value) -> Option<BackendInfo> {
    ...
    let status_str = json["status"]
        ...
        .or_else(|| {
            json["operational"]
                .as_bool()
                .map(|b| if b { "online" } else { "offline" })
        })
        .unwrap_or("offline");
    ...
    let coupling_map: Option<Vec<(usize, usize)>> = json["coupling_map"].as_array().map(|arr| {
        arr.iter()
            .filter_map(|pair| {
                let p = pair.as_array()?;
    ...
    if let Some(processor) = json["processor_type"].as_object() {
        metadata.insert(
            "processor_type".into(),
            serde_json::Value::Object(processor.clone()),
        );
    }
...
fn parse_retry_after_from_body(body: &str) -> Option<u64> {
    serde_json::from_str::<serde_json::Value>(body).ok()?["retry_after"].as_u64()
}
...
mod tests {
    ...
    fn test_map_http_error_auth() {
        let err = IBMProvider::map_http_error(reqwest::StatusCode::UNAUTHORIZED, "bad token");
        match err {
    ...
    fn test_map_http_error_not_found() {
        let err = IBMProvider::map_http_error(reqwest::StatusCode::NOT_FOUND, "no such backend");
        match err {
