...
mod tests {
    ...
    async fn test_reject_empty_circuit() {
        ...
        let config = JobConfig::default();
        let result = provider.submit_circuit(&circuit, "mock_5q", &config).await;
    ...
    async fn test_circuit_fits_on_larger_backend() {
        ...
        circuit.measure_all();
        let config = JobConfig::default();
        let result = provider.submit_circuit(&circuit, "mock_27q", &config).await;
        assert!(
            result.is_ok(),
    ...
    async fn test_validate_too_many_qubits_report() {
        ...
        let has_qubit_error = report.errors.iter().any(|e| e.contains("qubits"));
        assert!(
            has_qubit_error,
