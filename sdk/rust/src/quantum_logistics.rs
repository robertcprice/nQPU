...
impl fmt::Display for LogisticsError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ConvergenceFailure { iterations, best_cost } => {
                write!(
                    f,
...
impl CVRPEncoder {
    ...
    pub fn encode(
        ...
        let var = |c: usize, v: usize, p: usize| -> usize {
            Self::var_index(c, v, p, nv, max_stops)
        };
            ...
            for c1 in 0..nc {
                for c2 in (c1 + 1)..nc {
                    ...
                    if pair_demand > config.vehicle_capacity {
                        let pen = self.capacity_penalty
                            * (pair_demand - config.vehicle_capacity)
...
impl Schedule {
    ...
    pub fn is_valid(&self, problem: &JobShopProblem) -> bool {
        ...
        for j in 0..problem.num_jobs {
            ...
            let mut ordered: Vec<Option<(f64, f64)>> =
                vec![None; problem.operations[j].len()];
    ...
    pub fn optimize_angles(&self, num_iterations: usize) -> (Vec<f64>, Vec<f64>) {
        ...
        for _ in 0..num_iterations {
            for k in 0..p {
                ...
                g_minus[k] -= shift;
                let grad_g =
                    (self.expected_cost(&g_plus, &beta) - self.expected_cost(&g_minus, &beta))
                        / (2.0 * shift.sin());
                gamma[k] -= lr * grad_g;
                ...
                b_minus[k] -= shift;
                let grad_b =
                    (self.expected_cost(&gamma, &b_plus) - self.expected_cost(&gamma, &b_minus))
                        / (2.0 * shift.sin());
                beta[k] -= lr * grad_b;
            }
        }
...
impl QuantumLogisticsOptimizer {
        ...
        match problem.encode_qubo(max_time) {
            Ok(qubo) => {
                ...
                if let Some((best_bits, _)) = samples.first() {
                    ...
                    Schedule { assignments: Vec::new() }
                }
            }
            Err(_) => Schedule { assignments: Vec::new() },
        }
    }
    ...
    fn brute_force_tsp(
        ...
        if n == 0 {
            return TSPSolution { tour: Vec::new(), total_distance: 0.0 };
        }
        if n == 1 {
            return TSPSolution { tour: vec![0], total_distance: 0.0 };
        }
...
fn nearest_neighbour_tsp(distances: &DistanceMatrix) -> TSPSolution {
    ...
    TSPSolution { tour, total_distance: total_dist }
}
...
mod tests {
    ...
    fn test_config_builder_invalid_locations() {
        assert!(LogisticsConfigBuilder::new().num_locations(0).build().is_err());
        assert!(LogisticsConfigBuilder::new().num_locations(1).build().is_err());
        assert!(LogisticsConfigBuilder::new().num_locations(201).build().is_err());
        assert!(LogisticsConfigBuilder::new().num_locations(2).build().is_ok());
        assert!(LogisticsConfigBuilder::new().num_locations(200).build().is_ok());
    }
    ...
    fn test_config_builder_invalid_vehicles_and_capacity() {
        assert!(LogisticsConfigBuilder::new().num_vehicles(0).build().is_err());
        assert!(LogisticsConfigBuilder::new().num_vehicles(51).build().is_err());
        assert!(LogisticsConfigBuilder::new().vehicle_capacity(-1.0).build().is_err());
        assert!(LogisticsConfigBuilder::new().vehicle_capacity(0.0).build().is_err());
    }
    ...
    fn test_solver_display() {
        ...
        assert_eq!(format!("{}", LogisticsSolver::QuantumAnnealing), "Quantum Annealing");
        assert_eq!(format!("{}", LogisticsSolver::HybridClassicalQuantum), "Hybrid Classical-Quantum");
    ...
    fn test_error_display() {
        ...
        let e = LogisticsError::ProblemTooLarge { size: 100, limit: 50 };
        let msg = format!("{}", e);
        assert!(msg.contains("100"));
        assert!(msg.contains("50"));

        let e = LogisticsError::ConvergenceFailure { iterations: 42, best_cost: 3.14 };
        let msg = format!("{}", e);
