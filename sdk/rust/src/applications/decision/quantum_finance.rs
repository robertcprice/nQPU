...
impl FinanceQuantumState {
    ...
    pub fn h(&mut self, q: usize) {
        ...
        while i < dim {
            for k in 0..s {
                ...
                self.amplitudes[i + k] = Complex64::new((a.re + b.re) * inv, (a.im + b.im) * inv);
                self.amplitudes[i + k + s] =
                    Complex64::new((a.re - b.re) * inv, (a.im - b.im) * inv);
            }
            i += s << 1;
        }
    }
    ...
    pub fn ry(&mut self, q: usize, theta: f64) {
        ...
        while i < dim {
            for k in 0..s {
                let a = self.amplitudes[i + k];
                let b = self.amplitudes[i + k + s];
                self.amplitudes[i + k] = Complex64::new(a.re * c - b.re * sn, a.im * c - b.im * sn);
                self.amplitudes[i + k + s] =
                    Complex64::new(a.re * sn + b.re * c, a.im * sn + b.im * c);
            }
            i += s << 1;
        }
    }
    ...
    pub fn rx(&mut self, q: usize, theta: f64) {
        ...
        while i < dim {
            for k in 0..s {
                let a = self.amplitudes[i + k];
                let b = self.amplitudes[i + k + s];
                self.amplitudes[i + k] = Complex64::new(a.re * c + b.im * sn, a.im * c - b.re * sn);
                self.amplitudes[i + k + s] =
                    Complex64::new(b.re * c + a.im * sn, b.im * c - a.re * sn);
            }
            i += s << 1;
        }
    }
    ...
    pub fn rzz(&mut self, q0: usize, q1: usize, angle: f64) {
        ...
        for i in 0..dim {
            ...
            let phase = Complex64::new((angle * parity).cos(), -(angle * parity).sin());
    ...
    pub fn cry(&mut self, ctrl: usize, tgt: usize, theta: f64) {
        ...
        for i in 0..dim {
            if (i >> ctrl) & 1 == 1 && (i >> tgt) & 1 == 0 {
                ...
                self.amplitudes[i] = Complex64::new(a.re * c - b.re * sn, a.im * c - b.im * sn);
                self.amplitudes[j] = Complex64::new(a.re * sn + b.re * c, a.im * sn + b.im * c);
            }
        }
    }
...
impl Portfolio {
    ...
    pub fn new(
        ...
        if covariance.len() != n {
            return Err(FinanceError::InvalidPortfolio(format!(
                "Covariance rows ({}) != assets ({})",
                covariance.len(),
                n
            )));
        }
...
pub fn portfolio_to_qubo(
    ...
    if total > 30 {
        return Err(FinanceError::InvalidParameters(format!(
            "QUBO size {} exceeds practical limit (30 variables)",
            total
        )));
    }
...
impl Default for PortfolioConfig {
    fn default() -> Self {
        Self {
            ...
            optimization_method: ClassicalOptimizer::COBYLA {
                max_iterations: 200,
            },
            num_shots: 1024,
...
impl PortfolioConfigBuilder {
    ...
    pub fn num_qaoa_layers(mut self, p: usize) -> Self {
        self.config.num_qaoa_layers = p;
        self
    }
    ...
    pub fn penalty_strength(mut self, p: f64) -> Self {
        self.config.penalty_strength = p;
        self
    }
    pub fn optimization_method(mut self, m: ClassicalOptimizer) -> Self {
        self.config.optimization_method = m;
        self
    }
    pub fn num_shots(mut self, s: usize) -> Self {
        self.config.num_shots = s;
        self
    }
    pub fn num_bits_per_asset(mut self, b: usize) -> Self {
        self.config.num_bits_per_asset = b;
        self
    }
...
impl PortfolioOptimizer {
    ...
    fn evaluate_cost(&self, ising: &IsingHamiltonian, gammas: &[f64], betas: &[f64]) -> f64 {
    ...
    fn validate_config(&self) -> FinanceResult<()> {
        if self.config.strike_price <= 0.0 {
            return Err(FinanceError::InvalidParameters(
                "Strike must be positive".into(),
            ));
        }
        if self.config.spot_price <= 0.0 {
            return Err(FinanceError::InvalidParameters(
                "Spot must be positive".into(),
            ));
        }
        ...
        if self.config.time_to_maturity <= 0.0 {
            return Err(FinanceError::InvalidParameters(
                "Time to maturity must be positive".into(),
            ));
        }
        Ok(())
    }
...
impl RiskAnalyzer {
    ...
    pub fn analyze(
        ...
        let vol: f64 = (scenarios
            .iter()
            .map(|r| (r - mean_ret).powi(2))
            .sum::<f64>()
            / (scenarios.len() - 1).max(1) as f64)
            .sqrt();
        let sharpe_ratio = if vol > 1e-15 { mean_ret / vol } else { 0.0 };

        // Sortino ratio (downside deviation only)
        let downside_var: f64 = scenarios
            .iter()
            .filter(|&&r| r < 0.0)
            .map(|r| r * r)
            .sum::<f64>()
        ...
        let sortino_ratio = if downside_vol > 1e-15 {
    ...
    fn compute_max_drawdown(&self, scenarios: &[f64]) -> f64 {
        ...
        for &r in scenarios
            .iter()
            .take(self.config.time_horizon.max(scenarios.len()))
        {
            cumulative *= 1.0 + r;
            if cumulative > peak {
                peak = cumulative;
...
impl CreditScorer {
    ...
    pub fn train_svm(
        ...
        if data.len() != labels.len() {
            return Err(FinanceError::InvalidParameters(
                "Data/label length mismatch".into(),
            ));
        }
...
mod tests {
    ...
    fn test_portfolio_construction() {
        let assets = vec![Asset::new("A", 0.10, 0.5), Asset::new("B", 0.15, 0.5)];
        ...
        assert!(
            (pf.variance(&[0.5, 0.5])
                - (0.5 * 0.5 * 0.04 + 2.0 * 0.5 * 0.5 * 0.01 + 0.5 * 0.5 * 0.09))
                .abs()
                < 1e-12
        );
    }
    ...
    fn test_cobyla_step() {
        ...
        assert!(
            result.cost < 0.1,
            "COBYLA should find near-minimum, got cost={}",
            result.cost
        );
    }
    ...
    fn test_two_asset_portfolio() {
        let assets = vec![Asset::new("A", 0.10, 0.5), Asset::new("B", 0.20, 0.5)];
    ...
    fn test_four_asset_cardinality() {
        let assets: Vec<Asset> = (0..4)
            .map(|i| Asset::new(&format!("A{}", i), 0.05 + 0.05 * i as f64, 0.25))
            .collect();
        ...
        assert!(
            nonzero <= 2,
            "Cardinality constraint violated: {} assets",
            nonzero
        );
    }
    ...
    fn test_asian_option_range() {
        ...
        assert!(
            result.price < european * 1.1,
            "Asian <= European (got {} vs {})",
            result.price,
            european
        );
    }
    ...
    fn test_barrier_less_than_vanilla() {
        ...
        let result = pricer.price().unwrap();
        assert!(
            result.price <= vanilla + 0.5,
            "Barrier ({:.2}) should be <= vanilla ({:.2})",
            result.price,
            vanilla
        );
    }
    ...
    fn test_cvar_geq_var() {
        ...
        let analyzer = RiskAnalyzer::new(config);
        let metrics = analyzer.analyze(&[0.05], &[vec![0.04]], &[1.0]).unwrap();
        assert!(
    ...
    fn test_sharpe_ratio() {
        ...
        let metrics = analyzer.analyze(&[0.15], &[vec![0.01]], &[1.0]).unwrap();
        assert!(
            metrics.sharpe_ratio > 0.0,
            "Sharpe should be positive for positive return portfolio"
        );
    }
    ...
    fn test_kernel_symmetric() {
        ...
        let data = vec![vec![0.5, 0.3], vec![0.1, 0.8], vec![0.9, 0.2]];
    ...
    fn test_kernel_psd() {
        ...
        for trial in 0..10 {
            let x: Vec<f64> = (0..3)
                .map(|i| hash_to_uniform((trial * 3 + i) as u64 + 777) * 2.0 - 1.0)
                .collect();
            let mut xtKx = 0.0;
            for i in 0..3 {
                for j in 0..3 {
            ...
            assert!(
                xtKx >= -1e-10,
                "Kernel matrix not PSD: x^T K x = {} for trial {}",
                xtKx,
                trial,
            );
        }
    ...
    fn test_historical_distribution() {
        let historical_returns = vec![vec![0.01, -0.02, 0.03, -0.01, 0.02, 0.005, -0.015, 0.025]];
        let config = RiskConfig {
            confidence_level: 0.95,
            num_scenarios: 5000,
        ...
        assert!(
            metrics.var > 0.0,
            "Historical VaR should detect negative returns"
        );
    }
    ...
    fn test_student_t_fatter_tails() {
        ...
        let normal_metrics = RiskAnalyzer::new(config_normal)
            .analyze(&[0.0], &[vec![0.04]], &[1.0])
            .unwrap();
        let t_metrics = RiskAnalyzer::new(config_t)
            .analyze(&[0.0], &[vec![0.04]], &[1.0])
            .unwrap();
    ...
    fn test_large_portfolio_no_hang() {
        ...
        let assets: Vec<Asset> = (0..n)
            .map(|i| Asset::new(&format!("A{}", i), 0.05 + 0.005 * i as f64, 1.0 / n as f64))
            .collect();
    ...
    fn test_gradient_descent_convergence() {
        ...
        assert!(
            result.cost < 0.01,
            "GD should converge, cost={}",
            result.cost
        );
        assert!(
            result.parameters[0].abs() < 0.1,
            "x0={}",
            result.parameters[0]
        );
        assert!(
            result.parameters[1].abs() < 0.1,
            "x1={}",
            result.parameters[1]
        );
    }
    ...
    fn test_pauli_feature_map() {
        let scorer = CreditScorer::new(
            2,
            2,
            QuantumKernel::PauliFeatureMap {
                paulis: vec!["ZZ".into(), "XX".into()],
            },
        );
        let k = scorer.kernel_value(&[0.5, 0.3], &[0.5, 0.3]).unwrap();
