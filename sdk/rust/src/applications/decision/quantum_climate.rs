...
impl ChemicalSpecies {
    pub fn new(name: &str, concentration: f64, lifetime_seconds: f64, is_greenhouse: bool) -> Self {
        Self {
            name: name.to_string(),
            concentration,
            lifetime_seconds,
...
impl AtmosphericChemistry {
    ...
    pub fn encode_rate_equations(&self) -> Array2<Complex64> {
        ...
        for rxn in &self.reaction_network {
            ...
            for &(react_idx, react_stoich) in &rxn.reactants {
                a[[react_idx, react_idx]] += Complex64::new(-rxn.rate_constant * react_stoich, 0.0);
            }
        }
    ...
    pub fn evolve_concentrations(&self, initial: &[f64], dt: f64, steps: usize) -> Vec<Vec<f64>> {
        let n = self.species.len();
        assert_eq!(
            initial.len(),
            n,
...
impl ClimateSimulator {
    ...
    pub fn run_lbm_simulation(&self) -> ClimateResult<LBMResult> {
        ...
        lbm.distributions[idx] = LatticeBoltzmannQuantum::equilibrium_distribution(1.1, 0.05, 0.02);
    ...
    pub fn run_carbon_cycle(&self, emissions_scenario: &[f64]) -> ClimateResult<CarbonResult> {
        ...
        let total_carbon_history: Vec<f64> = history.iter().map(|step| step.iter().sum()).collect();
    ...
    pub fn project_warming(
        ...
        let sensitivity = (self.config.sensitivity_range.0 + self.config.sensitivity_range.1) / 2.0;
        let f_2x = 5.35 * 2.0_f64.ln();
...
mod tests {
    ...
    fn test_config_validation_inverted_sensitivity() {
        assert!(ClimateConfig::new()
            .sensitivity_range(5.0, 1.5)
            .validate()
            .is_err());
    }
    ...
    fn test_lbm_streaming_periodic_conservation() {
        ...
        lbm.distributions[10] = LatticeBoltzmannQuantum::equilibrium_distribution(2.0, 0.1, 0.1);
    ...
    fn test_carbon_hamiltonian_has_off_diagonal() {
        ...
        assert!(
            has_offdiag,
            "Hamiltonian must have off-diagonal coupling terms"
        );
    }
