...
impl QuantumDecisionMaker {
    ...
    ///
    /// The method does not modify the internal belief state.
    pub fn conjunction_fallacy(&self, a: &Projector, b: &Projector) -> ConjunctionResult {
...
fn perturbed_belief(dim: usize, respondent: u64, seed: u64) -> BeliefState {
    ...
    for _ in 0..dim {
        ...
        rng_state = rng_state
            .wrapping_mul(6364136223846793005)
            .wrapping_add(1442695040888963407);
        let phase = ((rng_state >> 33) as f64 / (1u64 << 31) as f64) * 2.0 * std::f64::consts::PI;
...
mod tests {
    ...
    fn test_belief_state_normalized() {
        ...
        let belief =
            BeliefState::from_state(vec![Complex64::new(3.0, 0.0), Complex64::new(4.0, 0.0)]);
    ...
    fn test_commuting_projectors_no_order_effect() {
        ...
        assert!(
            ...
            "basis projectors should commute"
        );
    ...
    fn test_conjunction_fallacy_possible() {
        ...
        assert!(result.p_a <= 1.0 + TOL, "P(A) out of range: {}", result.p_a);
        assert!(result.p_b <= 1.0 + TOL, "P(B) out of range: {}", result.p_b);
    }
    ...
    fn test_sure_thing_classical() {
        ...
        let result = dm.sure_thing_violation(&[action], &[cond_0, cond_1, cond_2, cond_3]);
