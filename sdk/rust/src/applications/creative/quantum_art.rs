...
use crate::{c64_one, c64_zero, GateOperations, QuantumState, C64};
...
impl fmt::Display for QuantumArtError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::DegenerateRegion => write!(f, "fractal region has zero area"),
            Self::WignerError { reason } => write!(f, "Wigner function error: {}", reason),
        }
    }
}
        ...
        for _step in 0..self.steps {
            ...
            for row in 0..n {
                ...
                while col + 1 < n {
                    ...
                    let b = psi[j];
                    psi_new[i] =
                        C64::new(a.re * cos_dt + b.im * sin_dt, a.im * cos_dt - b.re * sin_dt);
                    psi_new[j] =
                        C64::new(b.re * cos_dt + a.im * sin_dt, b.im * cos_dt - a.re * sin_dt);
                    col += 2;
                }
            }
            psi = psi_new;
        ...
        for py in 0..h {
            for px in 0..w {
                ...
                for (i, &slit_y) in slit_positions.iter().enumerate() {
                    ...
                    let dx = screen_x;
                    let r =
                        (dx * dx + dy * dy + self.screen_distance * self.screen_distance).sqrt();
...
impl QuantumPalette {
    ...
    fn generate_bell_palette(&self) -> Result<Vec<Pixel>, QuantumArtError> {
        ...
        for i in 0..self.num_colors {
            ...
            let v =
                self.value_range.0 + (1.0 - prob_mod) * (self.value_range.1 - self.value_range.0);
    ...
    fn generate_ghz_palette(&self) -> Result<Vec<Pixel>, QuantumArtError> {
        ...
        let z_expectations: Vec<f64> = (0..n_qubits).map(|q| state.expectation_z(q)).collect();
        let x_expectations: Vec<f64> = (0..n_qubits).map(|q| state.expectation_x(q)).collect();
        ...
        for i in 0..self.num_colors {
            ...
            let s = self.saturation_range.0
                + (z_expectations[qi].abs()) * (self.saturation_range.1 - self.saturation_range.0);
            let v = self.value_range.0
                + (1.0 - z_expectations[qi].abs()) * (self.value_range.1 - self.value_range.0);
    ...
    fn generate_w_palette(&self) -> Result<Vec<Pixel>, QuantumArtError> {
        ...
        for i in 0..self.num_colors {
            ...
            let v = self.value_range.0
                + (0.5 - z_val * 0.3).clamp(0.0, 1.0) * (self.value_range.1 - self.value_range.0);
    ...
    fn generate_random_palette(&self) -> Result<Vec<Pixel>, QuantumArtError> {
        ...
        for i in 0..self.num_colors {
            ...
            let s = self.saturation_range.0
                + (z_expectations[qi].abs()) * (self.saturation_range.1 - self.saturation_range.0);
            let v = self.value_range.0
                + ((x_expectations[qi] + 1.0) / 2.0) * (self.value_range.1 - self.value_range.0);
...
impl QuantumFractal {
    ...
    #[inline]
    fn apply_parameterized_gate(&self, alpha: C64, beta: C64, mag: f64, arg: f64) -> (C64, C64) {
...
impl WignerPortrait {
            ...
            WignerStateType::Fock(n) => {
                if *n >= fock_dim {
                    return Err(QuantumArtError::WignerError {
                        reason: format!(
                            "Fock state |{}> exceeds truncation dimension {}",
                            n, fock_dim
                        ),
                    });
                }
    ...
    pub fn integrate(&self, _config: &QuantumArtConfig) -> Result<f64, QuantumArtError> {
        let fock_dim = 32;
        let state_amps = self.prepare_state(fock_dim)?;
        let (x_min, x_max, p_min, p_max) = self.bounds;
...
fn laguerre_polynomial(n: usize, x: f64) -> f64 {
    ...
    for k in 1..n {
        let l_next = ((2.0 * k as f64 + 1.0 - x) * l_prev1 - k as f64 * l_prev2) / (k as f64 + 1.0);
        l_prev2 = l_prev1;
        l_prev1 = l_next;
    }
    l_prev1
...
mod tests {
    ...
    fn test_qca_deterministic_evolution() {
        ...
        assert_eq!(
            ...
            "QCA evolution should be deterministic"
        );
    }
    ...
    fn test_svg_output_valid() {
        ...
        assert!(
            ...
            "SVG should contain the red pixel"
        );
    }
    ...
    fn test_error_display() {
        let err = QuantumArtError::InvalidDimensions {
            width: 0,
            height: 100,
        };
        let msg = format!("{}", err);
        ...
        let err = QuantumArtError::LatticeTooLarge {
            size: 500,
            max: 128,
        };
        let msg = format!("{}", err);
