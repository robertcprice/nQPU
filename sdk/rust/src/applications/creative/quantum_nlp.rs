...
impl QuantumNlpConfig {
    ...
    pub fn total_qubits_for_type(&self, gram_type: &GrammaticalType) -> usize {
        match gram_type {
            ...
            GrammaticalType::Transitive => 2 * self.noun_qubits + self.sentence_qubits,
            // Intransitive verb: n^r . s => subject + sentence
            GrammaticalType::Intransitive => self.noun_qubits + self.sentence_qubits,
            // Adjective: n . n^l => output noun + input noun
            GrammaticalType::Adjective => 2 * self.noun_qubits,
...
impl Lexicon {
    ...
    pub fn english_basic() -> Self {
        ...
        // --- Nouns ---
        let nouns = [
            "alice", "bob", "charlie", "cat", "dog", "fish", "bird", "food", "water", "person",
            "robot", "computer", "book", "house", "car", "tree", "sun", "moon",
        ];
        for noun in &nouns {
        ...
        // --- Transitive verbs ---
        let transitive = [
            "loves", "sees", "chases", "eats", "knows", "hates", "reads", "builds", "catches",
            "teaches", "helps",
        ];
        ...
        // --- Intransitive verbs ---
        let intransitive = [
            "sleeps", "runs", "swims", "flies", "thinks", "jumps", "walks", "talks", "exists",
        ];
...
fn build_diagram(words: &[&Word], typed_wires: &[(AtomicType, usize)]) -> SentenceDiagram {
...
pub fn sentence_to_circuit(sentence: &Sentence, config: &QuantumNlpConfig) -> QuantumNlpCircuit {
...
fn apply_iqp_ansatz(gates: &mut Vec<NlpGate>, qubits: &[usize], params: &[f64], num_layers: usize) {
    let n = qubits.len();
    let single_params = n;
...
fn apply_tn_ansatz(gates: &mut Vec<NlpGate>, qubits: &[usize], params: &[f64], num_layers: usize) {
    let n = qubits.len();
...
pub fn sentence_similarity(s1: &Sentence, s2: &Sentence, config: &QuantumNlpConfig) -> f64 {
    ...
    match config.similarity_measure {
        SimilarityMeasure::Fidelity => compute_fidelity(&state1, &state2),
        SimilarityMeasure::SwapTest => compute_swap_test_similarity(&state1, &state2),
        SimilarityMeasure::InnerProduct => compute_inner_product(&state1, &state2),
    }
}
...
pub fn batch_evaluate(sentences: &[Sentence], config: &QuantumNlpConfig) -> Vec<f64> {
    sentences
        .iter()
...
pub fn similarity_matrix(sentences: &[Sentence], config: &QuantumNlpConfig) -> Vec<Vec<f64>> {
    let n = sentences.len();
...
pub fn compose_sentences(s1: &Sentence, s2: &Sentence, config: &QuantumNlpConfig) -> QuantumState {
    let circuit1 = sentence_to_circuit(s1, config);
    let circuit2 = sentence_to_circuit(s2, config);
...
fn tensor_product_states(s1: &QuantumState, s2: &QuantumState) -> QuantumState {
    ...
    let total_qubits = (amps1.len() as f64).log2() as usize + (amps2.len() as f64).log2() as usize;
...
pub fn sentence_entropy(sentence: &Sentence, config: &QuantumNlpConfig) -> f64 {
...
pub fn run_discocat_demo() -> String {
    ...
    let test_sentences = ["alice sleeps", "bob sleeps", "alice runs", "cat sleeps"];
...
mod tests {
    ...
    fn test_word_type_assignment() {
        let lex = Lexicon::english_basic();
        assert_eq!(
            lex.lookup("alice").unwrap().gram_type,
            GrammaticalType::Noun
        );
        assert_eq!(
            lex.lookup("loves").unwrap().gram_type,
            GrammaticalType::Transitive
        );
        assert_eq!(
            lex.lookup("sleeps").unwrap().gram_type,
            GrammaticalType::Intransitive
        );
        assert_eq!(
            lex.lookup("big").unwrap().gram_type,
            GrammaticalType::Adjective
        );
        assert_eq!(
            lex.lookup("the").unwrap().gram_type,
            GrammaticalType::Determiner
        );
    }
    ...
    fn test_fidelity_similarity_in_range() {
        ...
        let config = QuantumNlpConfig::new().with_similarity(SimilarityMeasure::Fidelity);
        let sim = sentence_similarity(&s1, &s2, &config);
        assert!(
    ...
    fn test_swap_test_similarity_in_range() {
        ...
        let s1 = parse_sentence(&lex, "cat sleeps").unwrap();
        let s2 = parse_sentence(&lex, "dog runs").unwrap();
        let config = QuantumNlpConfig::new().with_similarity(SimilarityMeasure::SwapTest);
        let sim = sentence_similarity(&s1, &s2, &config);
        assert!(
            (0.0..=1.0).contains(&sim),
    ...
    fn test_inner_product_similarity() {
        let lex = Lexicon::english_basic();
        let s = parse_sentence(&lex, "alice sleeps").unwrap();
        let config = QuantumNlpConfig::new().with_similarity(SimilarityMeasure::InnerProduct);
        let sim = sentence_similarity(&s, &s, &config);
        assert!(
            (sim - 1.0).abs() < 1e-10,
    ...
    fn test_training_runs() {
        ...
        let config = QuantumNlpConfig::new();
        let dataset = vec![TrainingExample::new("alice sleeps", 1.0)];

        let trainer = NlpTrainer::new().with_learning_rate(0.3).with_epochs(5);
    ...
    fn test_training_reduces_loss() {
        ...
        let config = QuantumNlpConfig::new();
        let dataset = vec![TrainingExample::new("cat runs", 0.8)];

        let trainer = NlpTrainer::new().with_learning_rate(0.5).with_epochs(20);
    ...
    fn test_semantic_functor_determiner_empty() {
        ...
        assert!(
            gates.is_empty(),
            "Functor should produce no gates for determiner"
        );
    }
