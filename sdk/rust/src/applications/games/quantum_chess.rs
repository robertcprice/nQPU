...
impl QuantumChessGame {
    ...
    pub fn make_move(&mut self, mv: QuantumChessMove) -> Result<MoveResult, ChessError> {
        ...
        let result = match &mv {
            QuantumChessMove::Classical { from, to } => self.apply_classical(*from, *to)?,
            QuantumChessMove::QuantumSplit { from, to_a, to_b } => {
                self.apply_quantum_split(*from, *to_a, *to_b)?
            }
            QuantumChessMove::Measure { square } => self.apply_measure(*square)?,
        };
    ...
    fn apply_classical(
        ...
        for (board, _) in &self.branches {
            if let Some(p) = board.piece_at(from) {
                if p.color == self.turn && is_legal_piece_move(p.kind, p.color, from, to, board) {
                    any_legal = true;
                    break;
                }
            }
        ...
        let desc = if let Some(cap) = captured {
            format!("{} {} -> {} captures {}", piece.kind, from, to, cap.kind)
        } else {
    ...
    fn apply_measure(
        ...
        if (occupied_prob - 0.0).abs() < 1e-12 || (occupied_prob - 1.0).abs() < 1e-12 {
            ...
            return Ok((
                ...
                false,
                format!(
                    "Measure {}: no superposition (prob = {:.2})",
                    square, occupied_prob
                ),
            ));
        }
        ...
        for row in 0..8u8 {
            for col in 0..8u8 {
                ...
                if let Some((piece, prob)) = probs.first() {
                    if *prob > 1e-6 {
                        annotations.insert((row, col), (piece.display_char(), *prob));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_piece_probability_sum() {
        ...
        assert!((prob_f3 - 0.5).abs() < 1e-10, "f3 probability: {}", prob_f3);
        assert!((prob_h3 - 0.5).abs() < 1e-10, "h3 probability: {}", prob_h3);
        // The knight left g1 in both branches, so g1 is empty.
        let prob_g1 = game.square_occupied_probability(Square::from_algebraic("g1"));
        assert!(prob_g1 < 1e-10, "g1 should be empty, prob = {}", prob_g1);
    }
    ...
    fn test_seed_determinism() {
        ...
        let board_b = run(999);
        assert!(
            board_a.is_identical_to(&board_b),
            "same seed must produce identical collapse"
        );
        ...
        let any_differ = !board_a.is_identical_to(&board_c) || !board_a.is_identical_to(&board_d);
        assert!(
            any_differ,
            "different seeds should usually produce different collapses"
        );
    }
    ...
    fn test_knight_l_shape_exhaustive() {
        ...
        let valid_targets = ["c6", "e6", "f5", "f3", "e2", "c2", "b3", "b5"];
