...
impl OptGate {
    ...
    pub fn is_two_qubit(&self) -> bool {
        matches!(
            self,
            OptGate::CX(..) | OptGate::CZ(..) | OptGate::Swap(..)
        )
    }
    ...
    pub fn unitary_2x2(&self) -> Option<[Complex; 4]> {
        ...
        match self {
            OptGate::H(_) => Some([s2, s2, s2, Complex::new(-std::f64::consts::FRAC_1_SQRT_2, 0.0)]),
            ...
            OptGate::T(_) => {
                let t = Complex::new(std::f64::consts::FRAC_1_SQRT_2, std::f64::consts::FRAC_1_SQRT_2);
                Some([o, z, z, t])
            }
            OptGate::Tdg(_) => {
                let t = Complex::new(std::f64::consts::FRAC_1_SQRT_2, -std::f64::consts::FRAC_1_SQRT_2);
...
impl OptCircuit {
    ...
    pub fn metrics(&self) -> CircuitMetrics {
        ...
        let mut gate_counts: Vec<(String, usize)> =
...
fn gates_cancel(a: &OptGate, b: &OptGate) -> bool {
    match (a, b) {
        ...
        (OptGate::Rx(q1, a1), OptGate::Rx(q2, a2)) if q1 == q2 && (a1 + a2).abs() < 1e-10 => {
            true
        }
        (OptGate::Ry(q1, a1), OptGate::Ry(q2, a2)) if q1 == q2 && (a1 + a2).abs() < 1e-10 => {
            true
        }
        (OptGate::Rz(q1, a1), OptGate::Rz(q2, a2)) if q1 == q2 && (a1 + a2).abs() < 1e-10 => {
            true
        }
        _ => false,
    }
...
fn gates_commute(a: &OptGate, b: &OptGate) -> bool {
    ...
    match (a, b) {
        ...
        (OptGate::X(q), OptGate::CX(_, tgt)) | (OptGate::CX(_, tgt), OptGate::X(q))
            if q == tgt =>
        {
            return true;
        }
...
mod tests {
    ...
    fn test_template_swap_detected() {
        ...
        let cx_count = result.iter().filter(|g| matches!(g, OptGate::CX(..))).count();
