...
impl CosmicRayDetector {
    ...
    fn simulate_cosmic_ray(&self, frame: &mut [u8]) -> Option<CosmicRayEvent> {
        ...
        rng ^= rng >> 12;
        let track_length = if (rng % 10) < 3 { 0 } else { ((rng % 20) + 1) as u32 };
    ...
    pub fn analyze_frame(&mut self, frame: &[u8]) -> Result<Vec<CosmicRayEvent>, String> {
        ...
        self.events_detected.fetch_add(events.len() as u64, Ordering::Relaxed);
...
impl MultiReceiverExtractor {
    ...
    pub fn measure(&mut self) -> Result<Vec<ReceiverMeasurement>, String> {
        ...
        self.measurements_taken.fetch_add(self.num_receivers as u64, Ordering::Relaxed);
    ...
    pub fn extract_entropy(&mut self, count: usize) -> Result<Vec<u8>, String> {
        ...
        for _ in 0..count {
            ...
            let combined: f64 = measurements.iter()
                .map(|m| m.signal)
                .fold(0.0, |acc, s| acc + s);

            // Also use noise directly
            let noise_combined: f64 = measurements.iter()
                .map(|m| m.noise)
                .sum();
    ...
    pub fn stats(&self) -> (usize, f64) {
        let avg_quantum = if self.correlations.is_empty() {
            ...
            self.correlations.iter()
                ...
                .sum::<f64>() / self.correlations.len() as f64
        };
...
impl SsdTunnelingExtractor {
    ...
    pub fn stats(&self) -> (u64, f64) {
        ...
        let mean: f64 = self.measurements.iter()
...
impl GpuComputeExtractor {
    ...
    pub fn extract_entropy(&mut self, count: usize) -> Result<Vec<u8>, String> {
        ...
        for _ in 0..count {
            ...
            let byte = (((m.duration_us & 0xF) << 4) as u8) |
                       ((m.core_timing_std as u8) & 0xF);
            entropy.push(byte);
        }
    ...
    pub fn stats(&self) -> (u64, bool) {
        (self.computations.load(Ordering::Relaxed), self.gpu_available)
    }
}
...
impl PhotonStatisticsExtractor {
    ...
    fn simulate_intensity(&self, count: u32) -> f64 {
        ...
        count as f64 + (count as f64).sqrt() * (Instant::now().elapsed().as_nanos() as f64 % 2.0 - 1.0)
    }
    ...
    pub fn extract_entropy(&mut self, count: usize) -> Result<Vec<u8>, String> {
        ...
        for _ in 0..count {
            ...
            let byte = (measurement.count & 0xFF) as u8 ^
                       ((measurement.mean_intensity * 100.0) as u8);
            entropy.push(byte);
        }
    ...
    /// Get statistics
    pub fn stats(&self) -> (u64, f64) {
        (self.frames_analyzed.load(Ordering::Relaxed), self.verify_poisson())
    }
}
