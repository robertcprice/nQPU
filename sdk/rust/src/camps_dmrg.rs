...
impl Element {
    ...
    pub fn atomic_number(&self) -> usize {
        match self {
            Element::H => 1, Element::He => 2, Element::Li => 3, Element::Be => 4,
            Element::B => 5, Element::C => 6, Element::N => 7, Element::O => 8,
            Element::F => 9, Element::Ne => 10, Element::Na => 11, Element::Mg => 12,
            Element::Al => 13, Element::Si => 14, Element::P => 15, Element::S => 16,
    ...
    pub fn ammonia() -> Self {
        ...
        Self {
            atoms: vec![
                ...
                Atom::new(Element::H, bond_length, 0.0, 0.0),
                Atom::new(Element::H, bond_length * angle.cos(), bond_length * angle.sin(), 0.0),
                Atom::new(Element::H, bond_length * angle.cos(), -bond_length * angle.sin() * 0.5, bond_length * 0.866),
            ],
            charge: 0,
            multiplicity: 1,
            n_orbitals: 8,
    ...
    pub fn methane() -> Self {
        ...
        Self {
            atoms: vec![
                ...
                Atom::new(Element::H, bond_length, 0.0, 0.0),
                Atom::new(Element::H, bond_length * tetrahedral_angle.cos(), bond_length * tetrahedral_angle.sin(), 0.0),
                Atom::new(Element::H, bond_length * tetrahedral_angle.cos(), -bond_length * tetrahedral_angle.sin() * 0.333, bond_length * 0.943),
                Atom::new(Element::H, bond_length * tetrahedral_angle.cos(), -bond_length * tetrahedral_angle.sin() * 0.333, -bond_length * 0.943),
            ],
            charge: 0,
            multiplicity: 1,
    ...
    pub fn benzene() -> Self {
        ...
        Self {
            atoms: vec![
                ...
                Atom::new(Element::H, (radius + ch_bond) * 0.5, (radius + ch_bond) * 0.866, 0.0),
                Atom::new(Element::H, -(radius + ch_bond) * 0.5, (radius + ch_bond) * 0.866, 0.0),
                Atom::new(Element::H, -(radius + ch_bond), 0.0, 0.0),
                Atom::new(Element::H, -(radius + ch_bond) * 0.5, -(radius + ch_bond) * 0.866, 0.0),
                Atom::new(Element::H, (radius + ch_bond) * 0.5, -(radius + ch_bond) * 0.866, 0.0),
...
impl FermionicHamiltonian {
    ...
    pub fn hydrogen_hamiltonian(bond_length: f64) -> Self {
        ...
        Self {
            one_body: vec![
                OneBodyIntegral { p: 0, q: 0, value: h11 },
                OneBodyIntegral { p: 1, q: 1, value: h22 },
                OneBodyIntegral { p: 0, q: 1, value: h12 },
                OneBodyIntegral { p: 1, q: 0, value: h12 },
            ],
            two_body: vec![
                TwoBodyIntegral { p: 0, q: 0, r: 0, s: 0, value: g1111 },
                TwoBodyIntegral { p: 1, q: 1, r: 1, s: 1, value: g2222 },
                TwoBodyIntegral { p: 0, q: 0, r: 1, s: 1, value: g1122 },
                TwoBodyIntegral { p: 1, q: 1, r: 0, s: 0, value: g1122 },
                TwoBodyIntegral { p: 0, q: 1, r: 0, s: 1, value: g1212 },
                TwoBodyIntegral { p: 1, q: 0, r: 1, s: 0, value: g1212 },
            ],
...
impl MPSState {
    ...
    pub fn normalize(&mut self) {
        let norm: f64 = self.tensors.iter()
            .flat_map(|t| t.iter())
            .map(|x| x * x)
            .sum::<f64>()
            .sqrt();
...
impl DMRGEngine {
    ...
    fn optimize_site(&self, state: &mut MPSState, site: usize, _hamiltonian: &FermionicHamiltonian) {
    ...
    fn compute_energy(&self, state: &MPSState, hamiltonian: &FermionicHamiltonian) -> f64 {
        ...
        for integral in &hamiltonian.one_body {
            ...
            let site_factor = if integral.p < state.tensors.len() {
                state.tensors[integral.p].iter().map(|x| x * x).sum::<f64>().sqrt()
            } else {
                1.0
            };
        ...
        for integral in &hamiltonian.two_body {
            if integral.p < state.tensors.len() && integral.r < state.tensors.len() {
                let factor_p = state.tensors[integral.p].iter().map(|x| x * x).sum::<f64>().sqrt();
                let factor_r = state.tensors[integral.r].iter().map(|x| x * x).sum::<f64>().sqrt();
...
mod tests {
    ...
    fn test_mps_normalize() {
        ...
        let norm: f64 = mps.tensors.iter().flat_map(|t| t.iter()).map(|x| x * x).sum();
