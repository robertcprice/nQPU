...
pub fn run_rb(config: &RbConfig) -> Result<RbResult> {
    if config.num_qubits == 0 || config.num_qubits > 2 {
        return Err(QcvvError::InvalidConfig("RB supports 1 or 2 qubits".into()));
    }
...
pub fn gate_error_from_interleaved(
    rb_ref: &RbResult,
    rb_interleaved: &RbResult,
    num_qubits: usize,
) -> f64 {
...
mod tests {
    ...
    fn test_random_circuit_generation() {
        ...
        let has_single = circuit
            .gates
            .iter()
            .any(|g| matches!(g, RandomGate::SingleQubit { .. }));
        let has_two = circuit
            .gates
            .iter()
            .any(|g| matches!(g, RandomGate::TwoQubit { .. }));
    ...
    fn test_exponential_fit() {
        ...
        let y: Vec<f64> = x
            .iter()
            .map(|&m: &f64| known_a * known_p.powf(m) + known_b)
            .collect();
