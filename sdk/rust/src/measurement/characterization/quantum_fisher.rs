                    ...
                    for k in 0..self.dim {
                        for l in 0..self.dim {
                            ...
                            matrix_element = matrix_element
                                + v_i_conj * generator.matrix[k][l] * eigenvectors[j][l];
                        }
                    }

                    let denom = eigenvalues[i] + eigenvalues[j];
                    qfi += 2.0 / denom
                        * (matrix_element.re * matrix_element.re
                            + matrix_element.im * matrix_element.im);
                }
            }
        }
...
pub fn print_benchmark() {
    ...
    for n in [2, 5, 10, 20, 50].iter() {
        ...
        println!(
            "N={:<5}: Heisenberg={:.2e}, Shot Noise={:.2e}, Improvement={:.1}x",
...
mod tests {
    ...
    fn test_qfi_ghz_state() {
        ...
        assert!(
            result.qfi > 5.0,
            "QFI for GHZ + J_z should be ~N² = 9, got {}",
            result.qfi
        );
