...
impl FermionicGaussianState {
    ...
    pub fn from_covariance(gamma: Vec<Vec<f64>>) -> Result<Self, FermionicGaussianError> {
        let dim = gamma.len();
        if dim == 0 || dim % 2 != 0 {
    ...
    pub fn from_occupation(
        ...
        for &idx in occupied {
            if idx >= n_modes {
                return Err(FermionicGaussianError::ModeOutOfRange { mode: idx, n_modes });
            }
        }
    ...
    pub fn two_point_correlator(&self, i: usize, j: usize) -> C64 {
        ...
        let real_part =
            (delta + self.covariance[2 * i + 1][2 * j] - self.covariance[2 * i][2 * j + 1]) / 4.0;

        let imag_part =
            (self.covariance[2 * i][2 * j] + self.covariance[2 * i + 1][2 * j + 1]) / 4.0;
    ...
    pub fn entropy_of_subsystem(&self, sites: &[usize]) -> f64 {
        ...
        for (si, &qi) in sites.iter().enumerate() {
            for (sj, &qj) in sites.iter().enumerate() {
                ...
                reduced[2 * si + 1][2 * sj + 1] = self.covariance[2 * qi + 1][2 * qj + 1];
            }
        }
...
impl FreeHamiltonian {
    ...
    pub fn from_hopping_and_pairing(hopping: Vec<Vec<f64>>, pairing: Vec<Vec<f64>>) -> Self {
        let n_modes = hopping.len();
        FreeHamiltonian {
            n_modes,
            hopping,
