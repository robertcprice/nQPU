...
impl ReferenceFrame {
        ...
        match *gate {
            ...
            FrameGate::M { qubit, result_idx } => {
                ...
                pauli.zs = self
                    ...
                    .iter()
                    .enumerate()
                    .map(|(i, z)| if i == qubit { true } else { *z })
                    .collect();
    ...
    pub fn sample_batch(&mut self, num_measurements: usize, num_shots: usize) -> Vec<Vec<bool>> {
        (0..num_shots)
            .map(|_| self.sample(num_measurements))
            .collect()
    }
}
...
impl StabilizerSampler {
    ...
    pub fn preprocess(&mut self, gates: &[FrameGate]) {
        // Count measurements
        self.num_measurements = gates
            .iter()
            .filter(|g| matches!(g, FrameGate::M { .. }))
            .count();
...
pub fn benchmark_sampling(n: usize, depth: usize, shots: usize) -> (f64, f64) {
    ...
    for _ in 0..depth {
        ...
        match gate_type % 4 {
            ...
            2 => gates.push(FrameGate::CX {
                control: q1,
                target: if q2 == q1 { (q1 + 1) % n } else { q2 },
            }),
            _ => gates.push(FrameGate::CZ {
                a: q1,
                b: if q2 == q1 { (q1 + 1) % n } else { q2 },
            }),
        }
    }
...
pub fn print_benchmark() {
    ...
    println!(
        "{:<10} {:<10} {:<15} {:<15}",
        "Qubits", "Depth", "K shots/sec", "us/shot"
    );
    println!("{}", "-".repeat(70));
