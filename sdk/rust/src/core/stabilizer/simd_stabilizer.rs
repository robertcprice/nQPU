...
mod simd_neon {
    ...
    #[inline]
    pub unsafe fn commutation_parity(x1: &[u64], z1: &[u64], x2: &[u64], z2: &[u64]) -> bool {
        let w = x1.len();
        let chunks = w / 2;
...
impl PackedPauliRow {
    ...
    pub fn rowmul_assign(&mut self, other: &Self) {
        ...
        let phase_sum: i64 = unsafe {
            simd_neon::rowmul_phase(&self.x_bits, &self.z_bits, &other.x_bits, &other.z_bits)
        };
        #[cfg(not(target_arch = "aarch64"))]
        let phase_sum: i64 = {
            let w = self.x_bits.len();
...
impl StabilizerTableau {
    ...
    pub fn cx(&mut self, c: usize, t: usize) -> Result<()> {
        ...
        if c == t {
            return Err(SimdStabilizerError::SameQubit {
                gate: "CX",
                qubit: c,
            });
        }
...
impl InverseTableau {
    ...
    pub fn deterministic_outcome(
        &self,
        forward_tableau: &StabilizerTableau,
        q: usize,
    ) -> Option<bool> {
        let n = self.num_qubits;
        ...
        match instr {
            ...
            StabilizerInstruction::Depolarize2(a, b, p) => {
                ...
                if r < *p {
                    ...
                    let apply_pauli =
                        |tab: &mut StabilizerTableau, q: usize, idx: u32| -> Result<()> {
                            match idx {
                                1 => tab.pauli_x(q),
                                2 => tab.pauli_y(q),
                                3 => tab.pauli_z(q),
...
mod tests {
    ...
    fn test_bulk_sampling_surface_code() {
        ...
        let config = SimdStabilizerConfig::new(7)
            .with_seed(100)
            .with_bulk_sampling();
    ...
    fn test_bulk_sampling_error_rate_accuracy() {
        ...
        let circuit = build_3qubit_bitflip_code(0.0);
        let config = SimdStabilizerConfig::new(5)
            .with_seed(0)
            .with_bulk_sampling();
        let mut sampler = BulkQecSampler::new(config, circuit);
    ...
    fn test_simd_commutes_with_large() {
        ...
        assert!(
            a.commutes_with(&b),
            "500 anti-commuting positions (even) should commute"
        );
        ...
        assert!(
            !a.commutes_with(&b),
            "499 anti-commuting positions (odd) should anti-commute"
        );
    }
    ...
    fn test_simd_weight_large() {
        ...
        assert_eq!(
            row.weight(),
            expected,
            "weight should count non-identity positions"
        );
    }
    ...
    fn test_simd_weight_boundary_sizes() {
        ...
        for n in &[1, 63, 64, 65, 127, 128, 129, 191, 192, 193] {
            ...
            assert_eq!(
                row.weight(),
                n,
                "n={}: all-X row should have weight {}",
                n,
                n
            );
