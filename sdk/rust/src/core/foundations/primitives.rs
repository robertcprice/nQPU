...
impl Observable {
    ...
    pub fn xx(q0: usize, q1: usize) -> Self {
        Self {
            terms: vec![PauliTerm::new(1.0, vec![(q0, Pauli::X), (q1, Pauli::X)])],
        }
    }
...
pub fn apply_gate(state: &mut Vec<Complex64>, gate: &PrimitiveGate, num_qubits: usize) {
    match gate {
        ...
        PrimitiveGate::X(q) => apply_single_qubit_gate(state, *q, num_qubits, |a, b| (b, a)),
        ...
        PrimitiveGate::Z(q) => apply_single_qubit_gate(state, *q, num_qubits, |a, b| (a, -b)),
        ...
        PrimitiveGate::Rx(q, theta) => {
            ...
            apply_single_qubit_gate(state, *q, num_qubits, move |a, b| {
                ...
                (a * cos_h + b * neg_i * sin_h, a * neg_i * sin_h + b * cos_h)
            })
        }
...
fn apply_single_qubit_gate<F>(state: &mut [Complex64], q: usize, num_qubits: usize, gate_fn: F)
where
    ...
    pub fn slice_register(&self, register_bits: &[usize]) -> BitArray {
        ...
        for shot in 0..self.num_shots {
            ...
            for (i, &bit_idx) in register_bits.iter().enumerate() {
                ...
                let bit_val = (full_bs >> (self.num_bits - 1 - bit_idx)) & 1;
...
impl SamplerPubV2 {
    ...
    pub fn with_parameters(circuit: Circuit, parameter_values: Vec<HashMap<String, f64>>) -> Self {
        Self {
            circuit,
            parameter_values,
            shots: None,
    ...
    fn execute_estimator_v2(
        ...
        let (values, standard_errors) = match self.config.shots {
            ...
            Some(base_shots) => {
                ...
                let precision_shots = (1.0 / (target_precision * target_precision)).ceil() as usize;
        ...
        let mut metadata = ExecutionMetadata::new_nqpu(circuit.num_qubits, shots_used, elapsed);
        metadata.extra.insert(
            "target_precision".to_string(),
            format!("{:.6}", target_precision),
        );
        metadata.extra.insert(
            "mode".to_string(),
            if self.config.shots.is_some() {
...
mod tests {
    ...
    fn test_sampler_batch_execution() {
        ...
        let pubs = vec![SamplerPub::new(circuit_h), SamplerPub::new(circuit_x)];
        ...
        assert_eq!(results[0].counts.values().sum::<usize>(), 1024);
    ...
    fn test_estimator_shot_based_matches_exact() {
        ...
        // Shot-based
        let shot_estimator =
            Estimator::new(EstimatorConfig::default().with_shots(100_000).with_seed(42));
    ...
    fn test_per_pub_shots_override() {
        ...
        assert_eq!(results[1].counts.values().sum::<usize>(), 500);
    }
    ...
    fn test_sampler_v2_classical_registers() {
        ...
        let r0_counts = result
            .get_register_counts("r0")
            .expect("r0 register missing");
        assert_eq!(
        ...
        let r1_counts = result
            .get_register_counts("r1")
            .expect("r1 register missing");
        assert_eq!(
    ...
    fn test_estimator_v2_single_observable() {
        ...
        assert!(
            result.standard_errors[0].abs() < 1e-10,
            "Exact mode: zero error"
        );
    ...
    fn test_estimator_v2_multiple_observables() {
        ...
        let estimator = Estimator::new(EstimatorConfig::default());
        let pubs = vec![EstimatorPubV2::new(circuit, vec![obs_zz, obs_xx, obs_zi])];
        let results = estimator.run_v2(&pubs);
    ...
    fn test_estimator_v2_precision_targeting() {
        ...
        let estimator = Estimator::new(EstimatorConfig::default().with_shots(100).with_seed(42));
    ...
    fn test_estimator_v2_pauli_string() {
        ...
        let sparse_op = SparsePauliOp::new(vec![("ZZ".to_string(), 0.5), ("XX".to_string(), 0.5)]);
    ...
    fn test_estimator_v2_parametric() {
        ...
        let circuit = CircuitBuilder::new(1).ry_param(0, "theta").build();
    ...
    fn test_pub_construction() {
        ...
        let pub_full = SamplerPubV2::with_parameters_and_shots(circuit.clone(), vec![params], 4096);
        ...
        let epub_prec = EstimatorPubV2::with_precision(circuit.clone(), vec![obs.clone()], 0.001);
    ...
    fn test_estimator_v2_standard_error() {
        ...
        let shot_est = Estimator::new(EstimatorConfig::default().with_shots(10000).with_seed(42));
