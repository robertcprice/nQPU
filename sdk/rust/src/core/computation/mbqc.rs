    ...
    fn translate_gate(&mut self, gate: &Gate) -> Result<(), MbqcError> {
        match &gate.gate_type {
            ...
            GateType::CNOT => {
                ...
                self.pattern
                    .add_command(MeasurementCommand::new(ctrl_phys, MeasurementBasis::XBasis));

                // Bridge A: X-basis measurement, X-correction from control.
                self.pattern
                    .add_command(MeasurementCommand::with_corrections(
                        bridge_a,
                ...
                self.pattern
                    .add_command(MeasurementCommand::new(tgt_phys, MeasurementBasis::XBasis));
...
mod tests {
    ...
    fn test_brickwork_cluster_creation() {
        ...
        assert_eq!(
            cluster.edges().len(),
            horizontal_edges + vertical_r0 + vertical_r1
        );
    }
    ...
    fn test_translator_custom_gate_returns_error() {
        ...
        let custom_gate = Gate::new(GateType::Custom(identity_matrix), vec![0], vec![]);
    ...
    fn test_translator_rxx_zero_angle() {
        ...
        let result = GateModelTranslator::to_mbqc(&gates, 2);
        assert!(result.is_ok(), "Rxx(0) should translate without error");
    ...
    fn test_translator_rzz_pi() {
        ...
        let result = GateModelTranslator::to_mbqc(&gates, 2);
        assert!(result.is_ok(), "Rzz(pi) should translate without error");
        let (cluster, pattern) = result.unwrap();
        assert_eq!(pattern.output_qubits().len(), 2);
        assert!(cluster.num_qubits() > 2);
    }
    ...
    fn test_translator_cu_identity_params() {
        ...
        let result = GateModelTranslator::to_mbqc(&gates, 2);
        assert!(result.is_ok(), "CU(0,0,0,0) should translate without error");
        let (_cluster, pattern) = result.unwrap();
        assert_eq!(pattern.output_qubits().len(), 2);
    }
