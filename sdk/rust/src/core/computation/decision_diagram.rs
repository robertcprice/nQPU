...
impl DDNodePool {
    ...
    pub fn get_or_create(&mut self, var: usize, low: DDEdge, high: DDEdge) -> usize {
        ...
        let node = DDNode { id, var, low, high };
    ...
    pub fn mark_and_sweep(&mut self, root: usize) -> usize {
        ...
        let mut new_nodes: Vec<DDNode> = Vec::with_capacity(self.nodes.len() - unreachable_count);
...
impl DDSimulator {
    ...
    fn apply_cnot_ctrl_above(
        ...
        if node.var == control {
            ...
            let new_high = self.apply_cnot_ctrl_above(&child_high, control, target, true);
        ...
        let new_low = self.apply_cnot_ctrl_above(&child_low, control, target, ctrl_active);
        let new_high = self.apply_cnot_ctrl_above(&child_high, control, target, ctrl_active);
    ...
    fn apply_cnot_tgt_above(&mut self, edge: &DDEdge, control: usize, target: usize) -> DDEdge {
        if edge.is_zero() {
            return DDEdge::zero();
        }
    ...
    fn apply_controlled_gate(&mut self, control: usize, target: usize, gate_type: &GateType) {
        let mat = gate_matrix_2x2(gate_type);
        let root_clone = self.root.clone();
        self.root = self.apply_controlled_recursive(&root_clone, control, target, &mat, false);
        self.add_cache.clear();
    }
    ...
    fn apply_controlled_recursive(
        ...
        if node.var == control {
            ...
            let new_low = self.apply_controlled_recursive(&child_low, control, target, mat, false);
            let new_high = self.apply_controlled_recursive(&child_high, control, target, mat, true);
    ...
    fn apply_toffoli_recursive(
        ...
        if node.var == c1 {
            let child_low = self.weighted_edge(&node.low, w);
            let child_high = self.weighted_edge(&node.high, w);
            let new_low = self.apply_toffoli_recursive(&child_low, c1, c2, target, false, c2_on);
            let new_high = self.apply_toffoli_recursive(&child_high, c1, c2, target, true, c2_on);
            return self.build_node(node.var, new_low, new_high);
        }
        ...
        if node.var == c2 {
            let child_low = self.weighted_edge(&node.low, w);
            let child_high = self.weighted_edge(&node.high, w);
            let new_low = self.apply_toffoli_recursive(&child_low, c1, c2, target, c1_on, false);
            let new_high = self.apply_toffoli_recursive(&child_high, c1, c2, target, c1_on, true);
            return self.build_node(node.var, new_low, new_high);
        }
        ...
        let child_low = self.weighted_edge(&node.low, w);
        let child_high = self.weighted_edge(&node.high, w);
        let new_low = self.apply_toffoli_recursive(&child_low, c1, c2, target, c1_on, c2_on);
        let new_high = self.apply_toffoli_recursive(&child_high, c1, c2, target, c1_on, c2_on);
        self.build_node(node.var, new_low, new_high)
    }
...
mod tests {
    ...
    fn test_initial_state_single_qubit() {
        ...
        assert!(amp_close(amp0, C64::new(1.0, 0.0)), "|0> amp = {:?}", amp0);
        assert!(amp_close(amp1, C64::new(0.0, 0.0)), "|1> amp = {:?}", amp1);
    }
