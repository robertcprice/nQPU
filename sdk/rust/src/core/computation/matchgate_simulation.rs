...
pub enum MatchgateError {
    ...
    NotAMatchgate { gate_desc: String },
    /// Matchgates require adjacent qubits (j, j+1).
    NonAdjacentQubits { qubit_a: usize, qubit_b: usize },
    ...
    NonMatchgateBudgetExceeded { current: usize, max: usize },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MatchgateError::NonMatchgateBudgetExceeded { current, max } => {
                write!(f, "Non-matchgate budget exceeded: {} > {}", current, max)
            }
        }
    }
}
...
pub fn matchgate_from_blocks(a: [[C64; 2]; 2], b: [[C64; 2]; 2], qa: usize, qb: usize) -> Gate {
    let zero = C64::new(0.0, 0.0);
...
mod tests {
    ...
    fn test_vacuum_state_gamma_squared_is_neg_identity() {
        let cov = CovarianceMatrix::new(4);
        assert!(cov.is_valid(), "Vacuum state must satisfy Gamma^2 = -I");
    }
    ...
    fn test_occupation_state_initialization() {
        ...
        assert!(
            cov.is_antisymmetric(TOL),
            "Occupation state must be antisymmetric"
        );
        assert!(cov.is_valid(), "Occupation state must satisfy Gamma^2 = -I");

        // Qubit 0 (empty): Gamma_{0,1} = 1
        assert!(
            (cov.get(0, 1) - 1.0).abs() < TOL,
            "Empty qubit should have Gamma=+1"
        );
        // Qubit 1 (occupied): Gamma_{2,3} = -1
        assert!(
            (cov.get(2, 3) - (-1.0)).abs() < TOL,
            "Occupied qubit should have Gamma=-1"
        );
        // Qubit 2 (occupied): Gamma_{4,5} = -1
        assert!(
            (cov.get(4, 5) - (-1.0)).abs() < TOL,
            "Occupied qubit should have Gamma=-1"
        );
        // Qubit 3 (empty): Gamma_{6,7} = 1
        assert!(
            (cov.get(6, 7) - 1.0).abs() < TOL,
            "Empty qubit should have Gamma=+1"
        );
    }
    ...
    fn test_covariance_set_enforces_antisymmetry() {
        ...
        assert!(
            (cov.get(2, 0) - (-0.5)).abs() < TOL,
            "set() must enforce antisymmetry"
        );
    }
    ...
    fn test_non_adjacent_qubits_returns_error() {
        ...
        assert!(result.is_err(), "Non-adjacent iSWAP should return error");
    ...
    fn test_extended_simulator_handles_mixed_circuit() {
        ...
        assert_eq!(
            ext.num_branches(),
            2,
            "Matchgate should not increase branches"
        );
        ...
        assert_eq!(
            ext.num_branches(),
            4,
            "Second non-matchgate should double branches"
        );
    }
    ...
    fn test_xx_yy_matchgate_parameterization() {
        ...
        assert!(
            (mat[0][0] - one).norm_sqr() < TOL,
            "XX+YY(0) should be identity"
        );
        assert!(
            (mat[1][1] - one).norm_sqr() < TOL,
            "XX+YY(0) should be identity"
        );
        assert!(
            (mat[2][2] - one).norm_sqr() < TOL,
            "XX+YY(0) should be identity"
        );
        assert!(
            (mat[3][3] - one).norm_sqr() < TOL,
            "XX+YY(0) should be identity"
        );
        assert!(
            (mat[1][2] - zero).norm_sqr() < TOL,
            "XX+YY(0) off-diag should be zero"
        );
