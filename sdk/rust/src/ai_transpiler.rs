...
impl NoiseModel {
    ...
    pub fn set_edge_error(&mut self, q1: usize, q2: usize, error_rate: f64) {
        self.two_qubit_errors.insert((q1.min(q2), q1.max(q2)), error_rate);
    }
    ...
    pub fn swap_error(&self, q1: usize, q2: usize) -> f64 {
        ...
        let cx_error = self.two_qubit_errors.get(&edge_key).copied().unwrap_or(0.01);
...
impl PPOAgent {
    ...
    pub fn new(cmap: &AiCouplingMap, config: &PPOConfig) -> Self {
        ...
        let policy_weights: Vec<Vec<f64>> = (0..num_actions)
            .map(|_| (0..feature_dim).map(|_| rng.gen::<f64>() * 0.1 - 0.05).collect())
            .collect();
        let value_weights: Vec<f64> = (0..feature_dim).map(|_| rng.gen::<f64>() * 0.1 - 0.05).collect();
    ...
    fn extract_features(
        ...
        features[0] = if count > 0 { total_dist / count as f64 } else { 0.0 };
        ...
        let l2p_mean = layout.l2p.iter().map(|&p| p as f64).sum::<f64>() / layout.l2p.len().max(1) as f64;
        features[4] = l2p_mean;
    ...
    pub fn update(&mut self, experiences: &[Experience]) {
        ...
        let std_adv = (advantages.iter().map(|a| (a - mean_adv).powi(2)).sum::<f64>() / advantages.len() as f64).sqrt() + 1e-8;
                ...
                if new_prob > 1e-10 {
                    for (j, &f) in exp.state_features.iter().enumerate() {
                        if j < self.policy_weights[exp.action].len() {
                            ...
                            self.policy_weights[exp.action][j] += self.policy_lr * normalized_adv * grad;
                        }
                    }
                }
    ...
    pub fn route(&self, circuit: &[AiGate], cmap: &AiCouplingMap) -> RoutingResult {
        let mut rng = StdRng::seed_from_u64(42);
        let n_logical = circuit.iter().map(|g| g.qubits()).flatten().max().map_or(0, |q| q + 1);
        let n_physical = cmap.num_qubits;
...
pub fn kak_decompose(u: &Array2<Complex64>) -> Result<KakResult> {
    ...
    let u_tl = Array2::from_shape_vec(
        (2, 2),
        vec![u[[0, 0]], u[[0, 1]], u[[1, 0]], u[[1, 1]]],
    )
    .unwrap();
    let (gp0, phi0, theta0, lambda0) = zyz_decompose(&u_tl);

    let u_br = Array2::from_shape_vec(
        (2, 2),
        vec![u[[2, 2]], u[[2, 3]], u[[3, 2]], u[[3, 3]]],
    )
    .unwrap();
    ...
    while changed {
        ...
        while i + 1 < circuit.len() {
            let merged = match (&circuit[i], &circuit[i + 1]) {
                (AiGate::Rz(q0, a), AiGate::Rz(q1, b)) if q0 == q1 => {
                    Some(AiGate::Rz(*q0, a + b))
                }
                (AiGate::Rx(q0, a), AiGate::Rx(q1, b)) if q0 == q1 => {
                    Some(AiGate::Rx(*q0, a + b))
                }
                (AiGate::Ry(q0, a), AiGate::Ry(q1, b)) if q0 == q1 => {
                    Some(AiGate::Ry(*q0, a + b))
                }
                _ => None,
            };
...
fn zx_gate_to_ai(gate_type: &crate::zx_calculus::GateType, qubits: &[usize], params: &[f64]) -> Option<AiGate> {
    use crate::zx_calculus::GateType;
    ...
    match gate_type {
        ...
        GateType::Rx if qubits.len() == 1 && !params.is_empty() => Some(AiGate::Rx(qubits[0], params[0])),
        GateType::Ry if qubits.len() == 1 && !params.is_empty() => Some(AiGate::Ry(qubits[0], params[0])),
        GateType::Rz if qubits.len() == 1 && !params.is_empty() => Some(AiGate::Rz(qubits[0], params[0])),
...
fn zx_optimize(circuit: &[AiGate], num_qubits: usize) -> Vec<AiGate> {
    ...
    let zx_gates: Vec<_> = circuit
        .iter()
        .filter_map(|g| ai_gate_to_zx(g))
        .collect();
...
mod tests {
    ...
    fn test_kak_cnot() {
        ...
        assert!(kak.cnot_count <= 3, "CNOT count {} exceeds maximum 3", kak.cnot_count);
    }
    ...
    fn test_kak_to_gates_one_cnot() {
        ...
        let gates = kak_to_gates(&kak, 0, 1);
        let cx_count = gates.iter().filter(|g| matches!(g, AiGate::CX(_, _))).count();
    ...
    fn test_transpile_optimization_reduces_gates() {
        ...
        assert!(result.stats.gate_count <= 4, "Gate count {} should be <= 4", result.stats.gate_count);
    }
    ...
    fn test_transpile_ccx_unrolled() {
        ...
        let result = ai_transpile(&circ, &cm, &AiTranspileConfig::default());
        assert!(!result.circuit.iter().any(|g| matches!(g, AiGate::CCX(_, _, _))));
    ...
    fn test_zx_gate_to_ai_conversion() {
        ...
        assert!(matches!(zx_gate_to_ai(&GateType::H, &[0], &[]), Some(AiGate::H(0))));
        assert!(matches!(zx_gate_to_ai(&GateType::T, &[0], &[]), Some(AiGate::T(0))));
        assert!(matches!(zx_gate_to_ai(&GateType::CNOT, &[0, 1], &[]), Some(AiGate::CX(0, 1))));
    }
    ...
    fn test_noise_model_uniform() {
        ...
        assert!(noise.single_qubit_errors.iter().all(|&e| (e - 0.01).abs() < 1e-10));
    }
    ...
    fn test_ai_transpile_with_noise_aware_routing() {
        ...
        let sabre_config = SabreConfig::default()
            .noise_model(noise)
            .noise_weight(1.0);
