...
pub enum ErrorDiffingError {
    ...
    ObservableOutOfRange { index: usize, num_observables: usize },
    ...
    ConvergenceError { shots_run: usize, target_stderr: f64 },
}
    ...
    pub fn run_with_engine(
        ...
        for batch_idx in 0..num_batches {
            ...
            if config.target_stderr > 0.0
                && shots_completed >= config.min_shots_for_convergence
            {
                let ler =
                    total_logical_errors as f64 / shots_completed as f64;
    ...
    pub fn compile(
        ...
        for (noise_idx, channel, qubits) in &noise_locations {
            ...
            for (prob, qubit_paulis) in pauli_errors {
                ...
                let (det_flips, obs_flips) = propagate_error(
                    instructions,
                    num_qubits,
                    *noise_idx,
                    &qubit_paulis,
...
fn channel_to_paulis(
    channel: &NoiseChannel,
    qubits: &[usize],
) -> Vec<(f64, Vec<(usize, Pauli)>)> {
    match channel {
...
mod tests {
    ...
    fn test_depolarizing_channel_paulis() {
        let paulis =
            channel_to_paulis(&NoiseChannel::Depolarizing { p: 0.03 }, &[0]);
    ...
    fn test_correlated_depolarizing_paulis() {
        let paulis = channel_to_paulis(
            &NoiseChannel::CorrelatedDepolarizing { p: 0.15 },
            &[0, 1],
        );
