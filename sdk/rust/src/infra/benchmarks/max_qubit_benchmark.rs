...
impl MaxQubitFinder {
    ...
    fn test_mps_1d(&mut self) -> BenchmarkResult {
        ...
        for attempt in 0..10 {
            ...
            match self.try_mps_1d(qubits, &config) {
                Ok(r) => {
                    println!(
                        "  [X] {} qubits: SUCCESS ({:.3} ms)",
                        qubits,
                        r.time_seconds * 1000.0
                    );
                    max_qubits = qubits;
    ...
    pub fn print_summary(&self) {
        ...
        for result in sorted {
            println!(
                "Method: {:25} | Max Qubits: {:4} | Time: {:8.3}s | Memory: {:8.1} MB | Status: {}",
                result.method,
                result.num_qubits,
                result.time_seconds,
                result.memory_mb,
...
fn estimate_memory_mera(qubits: usize) -> f64 {
    ...
    // Rough upper bound
    (qubits as f64) * 1024.0 * (qubits as f64).log2().powi(3) * 16.0 / (1024.0 * 1024.0)
}
...
mod tests {
    ...
    fn test_benchmark_result_creation() {
        let result =
            BenchmarkResult::success("Test Method".to_string(), 100, 5.0, 1024.0, 200.0, 1.0);
