...
impl SurfaceCodeLattice {
    ...
    fn measure_x_syndrome(&self, errors: &DataQubitErrors) -> Vec<bool> {
        self.x_stabilizers
            .iter()
            .map(|stab| stab.iter().filter(|&&q| errors.has_z_error(q)).count() % 2 == 1)
            .collect()
    }
    ...
    fn measure_z_syndrome(&self, errors: &DataQubitErrors) -> Vec<bool> {
        self.z_stabilizers
            .iter()
            .map(|stab| stab.iter().filter(|&&q| errors.has_x_error(q)).count() % 2 == 1)
            .collect()
    }
...
impl MwpmDecoder {
    ...
    fn boundary_distance(&self, detection: (usize, usize), stabilizers: &[Vec<usize>]) -> usize {
...
impl UnionFindDecoder {
    ...
    /// 4. Peel corrections.
    fn decode(&self, syndrome_history: &[Vec<bool>], stabilizers: &[Vec<usize>]) -> Vec<usize> {
        // Compute detections
        let detections = self.compute_detections(syndrome_history);
        if detections.is_empty() {
            return Vec::new();
...
pub fn estimate_threshold(
    ...
    if d3_errors.last().map(|e| e.1).unwrap_or(0.0) > d5_errors.last().map(|e| e.1).unwrap_or(0.0) {
...
pub fn run_benchmark(config: &WillowConfig) -> WillowResult<WillowResults> {
    ...
    for &d in &config.code_distances {
        ...
        for _ in 0..config.num_shots {
            let (is_error, per_round) = run_memory_experiment_shot(
                &lattice,
                &config.noise_model,
                config.decoder,
                config.num_rounds,
...
impl fmt::Display for LambdaAnalysis {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "  Lambda = {:.3} +/- {:.3}",
            self.lambda, self.lambda_uncertainty
        )?;
...
mod tests {
    ...
    fn test_syndrome_no_errors() {
        ...
        assert!(
            x_syn.iter().all(|&s| !s),
            "X syndrome should be all-zero with no errors"
        );
        assert!(
            z_syn.iter().all(|&s| !s),
            "Z syndrome should be all-zero with no errors"
        );
    }
    ...
    fn test_syndrome_single_x_error() {
        ...
        assert!(
            num_triggered > 0,
            "Z syndrome should detect X error on qubit 4"
        );
        ...
        assert_eq!(
            x_triggered, 0,
            "X syndrome should not detect a pure X error"
        );
    }
    ...
    fn test_syndrome_single_z_error() {
        ...
        assert!(
            num_triggered > 0,
            "X syndrome should detect Z error on qubit 4"
        );
        ...
        assert_eq!(
            z_triggered, 0,
            "Z syndrome should not detect a pure Z error"
        );
    }
    ...
    fn test_mwpm_single_error() {
        ...
        assert!(
            ...
            "MWPM should produce corrections for a single X error"
        );
    }
    ...
    fn test_mwpm_two_errors() {
        ...
        assert!(
            !corrections.is_empty(),
            "MWPM should produce corrections for two X errors"
        );
    }
    ...
    fn test_union_find_basic() {
        ...
        assert!(
            !corrections.is_empty(),
            "Union-Find should produce corrections"
        );
    }
    ...
    fn test_lambda_below_threshold() {
        ...
        assert!(
            analysis.lambda > 1.0,
            "Lambda should be > 1 below threshold, got {}",
            analysis.lambda
        );
    }
    ...
    fn test_lambda_above_threshold() {
        ...
        assert!(
            analysis.lambda < 1.0,
            "Lambda should be < 1 above threshold, got {}",
            analysis.lambda
        );
    }
    ...
    fn test_google_comparison() {
        ...
        assert!(
            comparison.agreement_score > 0.5,
            "Agreement should be reasonable, got {}",
    ...
    fn test_rcs_xeb_defined() {
        ...
        assert!(
            rcs.xeb_fidelity.is_finite(),
            "XEB fidelity should be finite"
        );
    }
    ...
    fn test_full_benchmark_runs() {
        ...
        assert!(
            result.is_ok(),
            "Quick benchmark should complete without error"
        );
    ...
    fn test_si1000_noise_model() {
        ...
        let p_meas = noise.effective_meas_error();
        assert!(
            p_data > 0.0 && p_data < 0.1,
            "SI1000 data error should be small"
        );
        assert!(
            p_meas > 0.0 && p_meas < 0.1,
            "SI1000 meas error should be small"
        );
    }
    ...
    fn test_above_threshold_lambda_less_than_one() {
        ...
        for &e in &results.logical_error_rates {
            assert!(
                e > 0.01,
                "Logical error rate should be high at p=0.20, got {:.4}",
                e
            );
        }
    }
