        ...
        DistributedGradientMethod::Adjoint => {
            if cfg.world_size == 1 {
                ...
                distributed_parameter_shift_impl(
                    ...
                    cfg,
                    Some(
                        "adjoint requested with world_size>1; fell back to parameter-shift"
                            .to_string(),
                    ),
                )
            } else {
...
mod tests {
    ...
    fn test_distributed_parameter_shift_matches_adjoint_reference_single_rank() {
        ...
        assert_eq!(
            out.method_used,
            DistributedGradientMethodUsed::ParameterShift
        );
    ...
    fn test_distributed_gradient_auto_uses_adjoint_mirror_multi_rank() {
        ...
        assert_eq!(
            out.method_used,
            DistributedGradientMethodUsed::AdjointMirror
        );
        assert_eq!(out.num_evaluations, 1);
