    ...
    fn execute(&self, state: &mut InternalQuantumState) -> Result<(), NqpuError> {
        for gate in &self.gates {
            match gate {
                ...
                CircuitGate::Custom2Q(q1, q2, m) => state.apply_2q(*q1 as usize, *q2 as usize, m),
            }
        }
        Ok(())
    }
...
macro_rules! gate_1q_param {
    ($fn_name:ident, $method:ident) => {
        #[no_mangle]
        pub extern "C" fn $fn_name(state: *mut NqpuState, qubit: u32, angle: f64) -> NqpuError {
            unsafe {
                let s = match state_mut(state) {
                    Ok(s) => s,
                    Err(e) => return e,
...
#[no_mangle]
pub extern "C" fn nqpu_sample(state: *const NqpuState, num_shots: u32, out: *mut u64) -> NqpuError {
    unsafe {
        let s = match state_ref(state) {
            Ok(s) => s,
            Err(e) => return e,
...
pub extern "C" fn nqpu_set_seed(seed: u64) {
    ...
    THREAD_RNG_STATE.with(|cell| {
        *cell.borrow_mut() = if seed == 0 { 0x853c49e6748fea9b } else { seed };
    });
}
...
mod tests {
    ...
    fn test_gate_cx_bell_state() {
        ...
        assert!((a00.re - inv_sqrt2).abs() < EPS, "Bell: a00 = {:?}", a00);
        assert!(a01.norm_sqr() < EPS, "Bell: a01 = {:?}", a01);
        assert!(a10.norm_sqr() < EPS, "Bell: a10 = {:?}", a10);
        assert!((a11.re - inv_sqrt2).abs() < EPS, "Bell: a11 = {:?}", a11);
        nqpu_state_destroy(state);
    }
    ...
    fn test_probabilities_sum_to_one() {
        ...
        assert!((sum - 1.0).abs() < EPS, "Probability sum = {} != 1.0", sum);
        nqpu_state_destroy(state);
    }
    ...
    fn test_null_pointer_errors() {
        ...
        assert_eq!(nqpu_circuit_add_h(null_circuit, 0), NqpuError::NullPointer);
        assert_eq!(
    ...
    fn test_invalid_qubit_errors() {
        ...
        assert_eq!(nqpu_circuit_add_cx(circuit, 0, 5), NqpuError::InvalidQubit);
        nqpu_circuit_destroy(circuit);
    }
    ...
    fn test_gate_y() {
        ...
        assert!((a1.im - 1.0).abs() < EPS, "Y|0>: a1.im = {} != 1.0", a1.im);
        nqpu_state_destroy(state);
    }
    ...
    fn test_gate_z() {
        ...
        let a1 = get_amp(state, 1);
        assert!((a1.re + 1.0).abs() < EPS, "Z|1>: a1.re = {} != -1.0", a1.re);
        assert!(a1.im.abs() < EPS);
        nqpu_state_destroy(state);
    }
    ...
    fn test_nan_angle_rejected() {
        let state = nqpu_state_create(2);
        assert_eq!(
            nqpu_gate_rx(state, 0, f64::NAN),
            NqpuError::InvalidParameter
        );
        assert_eq!(
            nqpu_gate_ry(state, 0, f64::INFINITY),
            NqpuError::InvalidParameter
        );
        assert_eq!(
