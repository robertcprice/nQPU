...
impl RelevanceScorer {
    ...
    pub fn rank(&self, papers: &[ArxivPaper]) -> Vec<(ArxivPaper, f64)> {
        let mut scored: Vec<(ArxivPaper, f64)> =
            papers.iter().map(|p| (p.clone(), self.score(p))).collect();
...
impl ArxivMonitor {
    ...
    fn parse_entry(&self, entry: &str) -> Result<ArxivPaper, ArxivError> {
        ...
        let published = extract_tag(entry, "published").unwrap_or("").to_string();

        let updated = extract_tag(entry, "updated").unwrap_or("").to_string();
    ...
    pub fn detect_trends(&self, papers: &[ArxivPaper]) -> Vec<ResearchTrend> {
        ...
        for keyword in &trend_keywords {
            ...
            let is_emerging = growth_rate > self.config.trend_growth_threshold && recent_count >= 2;
...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <entry>
    ...
    fn test_sort_order_display() {
        ...
        assert_eq!(format!("{}", SortOrder::LastUpdatedDate), "lastUpdatedDate");
    }
