    ...
    pub fn infer_num_qubits(&self) -> usize {
        match &self.circuit {
            CircuitSpec::Builtin { name, params } => match name {
                ...
                BuiltinCircuit::Ghz => params.first().map(|&n| n as usize).unwrap_or(3),
                BuiltinCircuit::Qft => params.first().map(|&n| n as usize).unwrap_or(3),
                BuiltinCircuit::Grover => params.first().map(|&n| n as usize).unwrap_or(3),
                BuiltinCircuit::VqeAnsatz => params.first().map(|&n| n as usize).unwrap_or(4),
            },
                ...
                if let Some(bracket_start) = trimmed.find('[') {
                    if let Some(bracket_end) = trimmed.find(']') {
                        if bracket_end > bracket_start + 1 {
                            if let Ok(n) = trimmed[bracket_start + 1..bracket_end]
                                .trim()
                                .parse::<usize>()
                            {
                                total_qubits += n;
...
impl RunResult {
    ...
    pub fn to_json(&self) -> String {
        ...
        lines.push(format!("  \"timing_ms\": {},", self.timing.as_millis()));
...
fn build_builtin_circuit(name: &BuiltinCircuit, params: &[f64], sim: &mut QuantumSimulator) {
    match name {
        ...
        BuiltinCircuit::VqeAnsatz => {
            ...
            // Second Ry layer
            for i in 0..n {
                let angle = params.get(n + i + 1).copied().unwrap_or(PI / 3.0);
                sim.ry(i, angle);
            }
        }
    }
...
impl ExperimentRunner {
    ...
    fn run_single(
        ...
        let expectation_values = Self::compute_expectation_values(&config.analysis, &sim.state);
    ...
    fn run_sweep(config: &ExperimentConfig, sweep: &SweepConfig) -> ExperimentResult<RunResult> {
        let start = Instant::now();
    ...
    fn compute_expectation_values(
        ...
        for obs in &analysis.expectation_values {
            let (label, value) = match obs {
                Observable::PauliZ(q) => (format!("Z{}", q), expectation_pauli_z(state, *q)),
                Observable::PauliX(q) => (format!("X{}", q), expectation_pauli_x(state, *q)),
                Observable::PauliY(q) => (format!("Y{}", q), expectation_pauli_y(state, *q)),
            ...
            "backend" => {
                config.backend = match value {
                    ...
                    s if s.starts_with("mps") => {
                        ...
                        let inner = s.trim_start_matches("mps(").trim_end_matches(')');
                        let bd_str = inner.trim_start_matches("bond_dim=").trim();
                    ...
                    s if s.starts_with("gpu") => {
                        let inner = s.trim_start_matches("gpu(").trim_end_matches(')');
                        let dev_str = inner.trim_start_matches("device=").trim();
...
fn parse_noise_spec(value: &str) -> ExperimentResult<NoiseSpec> {
    ...
    if value.starts_with("depolarizing(") {
        ...
        let p: f64 = inner.parse().map_err(|_| {
            ConfigError::ParseError(format!("invalid depolarizing parameter: {}", inner))
        })?;
    ...
    if value.starts_with("amplitude_damping(") {
        ...
        let gamma: f64 = inner.parse().map_err(|_| {
            ConfigError::ParseError(format!("invalid amplitude damping parameter: {}", inner))
        })?;
    ...
    if value.starts_with("readout_error(") {
        ...
        let p01: f64 = parts[0]
            .parse()
            .map_err(|_| ConfigError::ParseError(format!("invalid p01: {}", parts[0])))?;
        let p10: f64 = parts[1]
            .parse()
            .map_err(|_| ConfigError::ParseError(format!("invalid p10: {}", parts[1])))?;
        return Ok(NoiseSpec::ReadoutError { p01, p10 });
    }

    if value.starts_with("device(") {
        let inner = value.trim_start_matches("device(").trim_end_matches(')');
...
mod tests {
    ...
    fn config_builder() {
        ...
        assert_eq!(
            config.description,
            Some("Testing the builder pattern".to_string())
        );
    ...
    fn noise_injection() {
        ...
        assert_eq!(
            clean_01, 1000,
            "Without noise, X(0) should always give |01>"
        );
    ...
    fn experiment_timing() {
        ...
        assert!(result.timing.as_nanos() > 0, "timing should be non-zero");
    }
    ...
    fn sweep_results_shape() {
        ...
        let values: Vec<f64> = result.sweep_results.iter().map(|r| r.param_value).collect();
...
"#;
    ...
    fn fidelity_computation() {
        ...
        let target = vec![C64::new(sqrt2_inv, 0.0), C64::new(sqrt2_inv, 0.0)];
    ...
    fn pauli_string_expectation() {
        ...
        let zz = result.expectation_values.values().next().unwrap();
        assert!(
            (*zz - 1.0).abs() < 1e-10,
    ...
    fn backend_display() {
        ...
        assert_eq!(
            format!("{}", BackendSpec::Gpu { device: 0 }),
            "gpu(device=0)"
        );
    }
