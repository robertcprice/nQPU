...
impl PatternTracker {
    ...
    fn update(&mut self, syndrome: &[bool], config: &AdaptiveConfig) {
        ...
        for (i, &bit) in syndrome.iter().enumerate() {
            if i < self.syndrome_ema.len() {
                self.syndrome_ema[i] = config.ema_decay * self.syndrome_ema[i]
                    + alpha * if bit { 1.0 } else { 0.0 };
            }
        }
        ...
        if let Some(ref prev) = self.prev_syndrome {
            ...
            if count > 0 {
                ...
                self.temporal_ema =
                    config.ema_decay * self.temporal_ema + alpha * temporal;
            }
        }
    ...
    fn get_context(&self) -> NoiseContext {
        let error_rate = self.syndrome_ema.iter().sum::<f64>() / self.syndrome_ema.len().max(1) as f64;
...
impl DriftDetector {
    ...
    fn update(&mut self, syndrome: &[bool], tracker: &PatternTracker, round: usize, config: &AdaptiveConfig) -> f64 {
...
impl ParameterAdapter {
    ...
    fn compute_corrections(&self, syndrome: &[bool], config: &AdaptiveConfig) -> (Vec<bool>, Vec<f64>) {
...
impl AdaptiveDecoder {
    ...
    pub fn decode(&mut self, syndrome: &[bool]) -> AdaptiveCorrection {
        ...
        self.noise_context.rounds_since_drift = self.round_count - self.drift_detector.last_drift_round;
        ...
        let (x_corr, x_conf) = self.param_adapter.compute_corrections(syndrome, &self.config);
        let (z_corr, z_conf) = self.param_adapter.compute_corrections(syndrome, &self.config);

        // Update average confidence
        let avg_conf = x_conf.iter().chain(z_conf.iter()).sum::<f64>()
            / (x_conf.len() + z_conf.len()).max(1) as f64;
        self.stats.avg_confidence =
            0.99 * self.stats.avg_confidence + 0.01 * avg_conf;
...
mod tests {
    ...
    fn test_pattern_tracker_update() {
        ...
        let syndrome = vec![true, false, true, false, false, false,
