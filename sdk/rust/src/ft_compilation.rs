...
impl RossSeligerDecomposer {
    ...
    fn decompose_by_halving(
        ...
        for _ in 0..max_iterations {
            ...
            if let Some(entry) = best_entry {
                ...
                return Err(FTCompilationError::DecompositionFailed(
                    format!("No approximation found for residual angle {:.6}", remaining_theta),
                ));
            }
        }
    ...
    fn is_valid_extension(&self, sequence: &[RSToken], token: RSToken) -> bool {
        if let Some(&last) = sequence.last() {
            ...
            if token == RSToken::T {
                let consecutive = sequence.iter().rev().take_while(|&&t| t == RSToken::T).count();
                if consecutive >= 7 {
                    return false;
                }
            }
            if token == RSToken::Tdg {
                let consecutive = sequence.iter().rev().take_while(|&&t| t == RSToken::Tdg).count();
                if consecutive >= 7 {
                    return false;
                }
            }
        ...
        for gate in gates {
            ...
            if cancel {
                ...
                let merge_to_s = if let Some(last) = result.last() {
                    match (last, &gate) {
                        (CliffordTGate::T(q1), CliffordTGate::T(q2)) if q1 == q2 => {
                            Some(*q1)
                        }
                        _ => None,
                    }
                ...
                let merge_to_sdg = if let Some(last) = result.last() {
                    match (last, &gate) {
                        (CliffordTGate::Tdg(q1), CliffordTGate::Tdg(q2)) if q1 == q2 => {
                            Some(*q1)
                        }
                        _ => None,
                    }
...
mod tests {
    ...
    fn test_litinski_cnot_spread() {
        ...
        let has_z0 = rot.paulis.iter().any(|(q, p)| *q == 0 && *p == PauliType::Z);
        let has_z1 = rot.paulis.iter().any(|(q, p)| *q == 1 && *p == PauliType::Z);
    ...
    fn test_t_cancellation() {
        ...
        let compiler = FTCompiler::new(
            FTCompilationConfig::new().optimization_level(1),
        );
    ...
    fn test_pauli_rotation_properties() {
        ...
        // Weight.
        let rot_2q = PauliRotation::new(
            vec![(0, PauliType::X), (1, PauliType::Z)],
            0.5,
        );
    ...
    fn test_multi_qubit_independence() {
        ...
        assert_eq!(optimized.len(), 2, "different-qubit T gates should not cancel");
    }
    ...
    fn test_t_count_optimality_comprehensive() {
        ...
        for &theta in &angles {
            for &epsilon in &epsilons {
                ...
                assert!(
                    ...
                    theta, epsilon, actual_t_count, theoretical_bound, overhead
                );
                ...
                assert!(
                    ...
                    theta, epsilon, decomp.actual_error
                );
            }
        }
        ...
        for (theta, eps, actual, bound, overhead) in &results {
            eprintln!("{:.2}\t{:.4}\t{}\t{}\t{:.2}x", theta, eps, actual, bound, overhead);
        }
        eprintln!("All {} configurations within 2x theoretical bound", results.len());
    }
    ...
    fn test_t_count_precision_scaling() {
        ...
        for epsilon in precisions {
            ...
            assert!(
                ...
                prev_epsilon, epsilon, prev_t_count, decomp.t_count
            );
    ...
    fn test_special_angles_optimal_t_count() {
        ...
        let decomp = compiler.decompose_rz(0, FRAC_PI_2).unwrap();
        assert_eq!(decomp.t_count, 0, "Rz(π/2) should need 0 T gates (Clifford)");
