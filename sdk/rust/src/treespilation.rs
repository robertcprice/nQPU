...
impl TreeDecomposition {
    ...
    pub fn validate(&self, graph: &InteractionGraph) -> Result<(), String> {
        ...
        for &(u, v) in &graph.edges {
            let covered = self.bags.iter().any(|bag| bag.contains(&u) && bag.contains(&v));
            if !covered {
                return Err(format!(
        ...
        let all_vertices: HashSet<usize> = self.bags.iter().flat_map(|b| b.iter().copied()).collect();
        for &v in &all_vertices {
            let containing_bags: Vec<usize> = (0..n_bags)
    ...
    pub fn contraction_order(&self) -> Vec<(usize, BTreeSet<usize>)> {
        ...
        while let Some(bag_idx) = queue.pop_front() {
            ...
            let parent_bag: Option<usize> = adj[bag_idx]
                .iter()
                .find(|&&nb| !processed[nb])
                .copied();
...
pub fn reorder_gates_for_treewidth(
    gates: &[TreeGate],
    n_qubits: usize,
) -> (Vec<TreeGate>, usize) {
    if gates.is_empty() {
...
impl Treespiler {
    ...
    pub fn treespile(&self, gates: &[TreeGate], n_qubits: usize) -> TreespilationResult {
        ...
        let original_elim =
            compute_elimination_ordering(&original_graph, self.heuristic);
...
pub fn to_native_gate(gate: &TreeGate) -> crate::gates::Gate {
    let (gate_type, targets, controls) = match &gate.gate_type {
        ...
        TreeGateType::Rx => {
            ...
            (crate::gates::GateType::Rx(angle), gate.qubits.clone(), vec![])
        }
        TreeGateType::Ry => {
            let angle = gate.params.first().copied().unwrap_or(0.0);
            (crate::gates::GateType::Ry(angle), gate.qubits.clone(), vec![])
        }
        TreeGateType::Rz => {
            let angle = gate.params.first().copied().unwrap_or(0.0);
            (crate::gates::GateType::Rz(angle), gate.qubits.clone(), vec![])
        }
        TreeGateType::CX => {
            if gate.qubits.len() >= 2 {
                (
...
mod tests {
    ...
    fn test_tree_decomposition_covers_all_vertices() {
        ...
        for v in 0..graph.n_vertices {
            if graph.degree(v) > 0 {
                assert!(
                    all_bag_vertices.contains(&v),
                    "Vertex {} not in any bag",
                    v
                );
            }
        }
    ...
    fn test_gate_reordering_commuting() {
        ...
        let original_types: HashSet<String> = gates
            .iter()
    ...
    fn test_treespilation_qaoa_circuit() {
        ...
        assert!(result.speedup_estimate > 1.0, "Should have significant speedup");
