...
impl FiberChannel {
    ...
    pub fn standard(length_km: f64) -> Self {
        Self::new(length_km, DEFAULT_FIBER_LOSS_DB_PER_KM, DEFAULT_DARK_COUNT_PROB)
    }
...
impl QkdLink {
    ...
    pub fn fiber_channel(&self) -> FiberChannel {
        FiberChannel::new(self.fiber_length_km, self.loss_db_per_km, DEFAULT_DARK_COUNT_PROB)
    }
...
pub fn decoy_state_bb84_key_rate(
    ...
    let skr_per_pulse = q1 * (1.0 - binary_entropy(e1))
        - DEFAULT_EC_EFFICIENCY * q_mu * binary_entropy(qber);
...
pub fn cv_qkd_key_rate(
    ...
    let v_mod = match modulation {
        ...
        CVModulation::DiscreteModulation { num_states } => {
            2.0 * (num_states as f64).sqrt()
        }
    };
...
pub fn estimate_link_key_rate(link: &QkdLink, node_a: &QkdNode, node_b: &QkdNode) -> KeyRateEstimate {
    ...
    match link.protocol {
        QkdProtocol::BB84 { decoy_states: false } => bb84_key_rate(
...
pub fn security_analysis(
    ...
    let privacy_amplification_factor =
        (effective_min_ent - error_correction_leaked).max(0.0);
...
impl Default for AuthMethod {
    fn default() -> Self {
        AuthMethod::WegmanCarter { hash_family_size: 256 }
    }
}
...
impl QkdNetwork {
    ...
    fn adjacency_list(&self) -> HashMap<usize, Vec<(usize, usize)>> {
        ...
        for (i, link) in self.links.iter().enumerate() {
            ...
            adj.entry(link.node_a)
                .or_default()
                .push((link.node_b, i));
            adj.entry(link.node_b)
                .or_default()
                .push((link.node_a, i));
        }
        adj
            ...
            for spur_idx in 0..prev_path.len().saturating_sub(1) {
                ...
                for existing_path in &a_paths {
                    if existing_path.len() > spur_idx
                        && existing_path[..=spur_idx] == root_path[..]
                    {
...
impl NetworkLibrary {
    ...
    pub fn bearlinq() -> QkdNetwork {
        let config = NetworkConfig::new("BearlinQ Berlin")
            .with_auth(AuthMethod::WegmanCarter { hash_family_size: 256 })
        ...
        net.add_node(QkdNode::new(1, "HU-Berlin", NodeType::EndUser, (12.0, 10.0)));
    ...
    pub fn tokyo_qkd() -> QkdNetwork {
        let config = NetworkConfig::new("Tokyo QKD Network")
    ...
    pub fn cambridge_network() -> QkdNetwork {
        let config = NetworkConfig::new("Cambridge QKD Network")
            .with_routing(RoutingAlgorithm::MaxKeyRate);
    ...
    pub fn intercity(distance_km: f64) -> QkdNetwork {
        ...
        for i in 0..=num_segments.saturating_sub(1) {
            if i + 1 <= num_segments {
                let _ = net.add_link(
                    QkdLink::new(i, i + 1, actual_segment).with_protocol(protocol),
                );
            }
        }
...
mod tests {
    ...
    fn test_link_transmittance() {
        ...
        assert!((t - 0.1).abs() < 1e-10, "50 km link transmittance should be ~0.1");
    }
    ...
    fn test_routing_shortest_path() {
        let config = NetworkConfig::new("Routing Test")
            .with_routing(RoutingAlgorithm::ShortestPath);
        let mut net = QkdNetwork::new(config);
    ...
    fn test_routing_max_key_rate() {
        let config = NetworkConfig::new("Rate Routing Test")
            .with_routing(RoutingAlgorithm::MaxKeyRate);
        let mut net = QkdNetwork::new(config);
    ...
    fn test_finite_key_lower_rate() {
        ...
        assert!(finite.finite_key_effects, "10k bits should trigger finite-key effects");
        assert!(
    ...
    fn test_bearlinq_topology() {
        ...
        assert_eq!(net.links.len(), 4, "BearlinQ should have 4 links (partial mesh)");
    ...
    fn test_star_network_construction() {
        ...
        assert_eq!(net.nodes.len(), n + 1, "Star should have N+1 nodes (center + clients)");
    ...
    fn test_rate_matrix_symmetry() {
        ...
        for i in 0..4 {
            ...
            assert!(
                matrix[i][i].abs() < 1e-10,
                "Self-rate should be zero"
            );
        }
    }
}
