...
impl NeutralAtomGates {
    ...
    pub fn cz_gate(atoms: &mut [AtomState], a: usize, b: usize) -> NeutralAtomResult<()> {
        let na = atoms.len();
        if a >= na {
            return Err(NeutralAtomError::AtomOutOfBounds { index: a, num_atoms: na });
        }
        if b >= na {
            return Err(NeutralAtomError::AtomOutOfBounds { index: b, num_atoms: na });
        }
        if atoms[a].is_lost {
        ...
        let phase_ag = if c00.norm_sqr() > 1e-30 { c00 / c00.norm() }
                       else if c01.norm_sqr() > 1e-30 { c01 / c01.norm() }
                       else { Complex64::new(1.0, 0.0) };
        let phase_ar = if c10.norm_sqr() > 1e-30 { c10 / c10.norm() }
                       else if c11.norm_sqr() > 1e-30 { c11 / c11.norm() }
                       else { Complex64::new(1.0, 0.0) };
        let phase_bg = if c00.norm_sqr() > 1e-30 { c00 / c00.norm() }
                       else if c10.norm_sqr() > 1e-30 { c10 / c10.norm() }
                       else { Complex64::new(1.0, 0.0) };
        let phase_br = if c01.norm_sqr() > 1e-30 { c01 / c01.norm() }
                       else if c11.norm_sqr() > 1e-30 { c11 / c11.norm() }
                       else { Complex64::new(1.0, 0.0) };
    ...
    pub fn ccz_gate(
        ...
        for &idx in &[a, b, c] {
            if idx >= na {
                return Err(NeutralAtomError::AtomOutOfBounds { index: idx, num_atoms: na });
            }
        ...
        let norm_ag = (coeffs[0].norm_sqr() + coeffs[1].norm_sqr()
            + coeffs[2].norm_sqr() + coeffs[3].norm_sqr())
        .sqrt();
        let norm_ar = (coeffs[4].norm_sqr() + coeffs[5].norm_sqr()
            + coeffs[6].norm_sqr() + coeffs[7].norm_sqr())
        .sqrt();
        // Find first non-zero coefficient for phase
        let ph_ag = coeffs[0..4].iter().find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
        let ph_ar = coeffs[4..8].iter().find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
        atoms[a].qubit_state = [ph_ag * norm_ag, ph_ar * norm_ar];

        // Atom B: |g> from even k indices where j=0, |r> where j=1.
        let norm_bg = (coeffs[0].norm_sqr() + coeffs[1].norm_sqr()
            + coeffs[4].norm_sqr() + coeffs[5].norm_sqr())
        .sqrt();
        let norm_br = (coeffs[2].norm_sqr() + coeffs[3].norm_sqr()
            + coeffs[6].norm_sqr() + coeffs[7].norm_sqr())
        .sqrt();
        let ph_bg = [coeffs[0], coeffs[1], coeffs[4], coeffs[5]].iter()
            .find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
        let ph_br = [coeffs[2], coeffs[3], coeffs[6], coeffs[7]].iter()
            .find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
        atoms[b].qubit_state = [ph_bg * norm_bg, ph_br * norm_br];

        // Atom C: |g> from even indices (k=0), |r> from odd (k=1).
        let norm_cg = (coeffs[0].norm_sqr() + coeffs[2].norm_sqr()
            + coeffs[4].norm_sqr() + coeffs[6].norm_sqr())
        .sqrt();
        let norm_cr = (coeffs[1].norm_sqr() + coeffs[3].norm_sqr()
            + coeffs[5].norm_sqr() + coeffs[7].norm_sqr())
        .sqrt();
        let ph_cg = [coeffs[0], coeffs[2], coeffs[4], coeffs[6]].iter()
            .find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
        let ph_cr = [coeffs[1], coeffs[3], coeffs[5], coeffs[7]].iter()
            .find(|c| c.norm_sqr() > 1e-30)
            .map(|c| c / c.norm()).unwrap_or(Complex64::new(1.0, 0.0));
...
impl AtomSortingAlgorithm {
    ...
    pub fn heuristic_rearrange(
        ...
        for (atom_id, target_trap, _cost) in &sorted_targets {
            ...
            let current_trap = array
                .traps
                .iter()
                .position(|t| t.atom_id == Some(atom_id));
...
impl NeutralAtomSimulator {
    ...
    pub fn load_atoms(&mut self, seed: u64) -> usize {
        ...
        for (i, trap) in self.array.traps.iter_mut().enumerate() {
            ...
            if r < p {
                ...
                self.array
                    .atoms
                    .push(AtomState::new_ground(trap.position));
                loaded += 1;
            } else {
                trap.occupied = false;
                trap.atom_id = None;
    ...
    pub fn apply_gate(&mut self, gate: &NeutralAtomGate) -> NeutralAtomResult<()> {
        match gate {
            NeutralAtomGate::Rotation { atom, theta, phi } => {
                ...
                NeutralAtomGates::single_qubit_rotation(
                    &mut self.array.atoms[*atom],
                    *theta,
                    *phi,
                );
    ...
    pub fn run_circuit(&mut self, circuit: &[NeutralAtomGate]) -> NeutralAtomResult<SimulationResult> {
...
mod tests {
    ...
    fn test_ccz_gate_all_rydberg() {
        ...
        // Note: The -1 global phase is not stored in separable qubit representation.
        for i in 0..3 {
            assert!(atoms[i].rydberg_population() > 0.999, "atom {} not in |r>", i);
    ...
    #[test]
    fn test_move_cost() {
        let cost = AtomSortingAlgorithm::move_cost(
            (0.0, 0.0, 0.0),
            (10.0, 0.0, 0.0),
        );
    ...
    fn test_connectivity_equality() {
        assert_eq!(AtomConnectivity::NearestNeighbor, AtomConnectivity::NearestNeighbor);
        assert_ne!(AtomConnectivity::NearestNeighbor, AtomConnectivity::AllToAll);
        assert_eq!(
