...
pub fn small_bicycle_code() -> BicycleCode {
    ...
    let mut code = build_bicycle_code(&config).expect("Small bicycle code construction should not fail");
...
pub fn random_error(n: usize, error_rate: f64, rng: &mut impl Rng) -> Vec<u8> {
    (0..n).map(|_| if rng.gen_bool(error_rate.min(1.0).max(0.0)) { 1 } else { 0 }).collect()
}
...
pub fn decode(
    ...
    for cluster in &clusters {
        if cluster.len() <= config.max_cluster_size {
            let cluster_correction =
                solve_cluster(code, cluster, syndrome, &posteriors_x);
...
mod tests {
    ...
    fn test_small_bicycle_code_parameters() {
        ...
        assert!(verify_css_condition(&code.hx, &code.hz), "CSS condition must hold");
    ...
    fn test_ambiguity_detection() {
        ...
        assert!(ambiguous.contains(&2), "Node 2 (p=0.48) should be ambiguous");
        assert!(ambiguous.contains(&3), "Node 3 (p=0.52) should be ambiguous");
        assert!(ambiguous.contains(&5), "Node 5 (p=0.45) should be ambiguous");
        assert!(ambiguous.contains(&7), "Node 7 (p=0.55) should be ambiguous");
        assert!(
    ...
    fn test_clustering_isolated_nodes() {
        ...
        assert_eq!(clusters.len(), 3, "Isolated nodes should form singleton clusters");
    }
    ...
    fn test_decoder_single_qubit_error_gross() {
        ...
        assert!(result.correction.iter().any(|&c| c == 1) || error.iter().all(|&e| e == 0),
    ...
    fn test_logical_error_rate_scaling() {
        ...
        let config_small = BicycleCodeConfig::new(
            3,
            2,
            vec![(1, 0), (0, 1)],
            vec![(2, 0), (0, 1)],
    ...
    fn test_sparse_transpose_round_trip() {
        ...
        assert_eq!(mtt.entries, mat.entries, "Double transpose should equal original");
    }
    ...
    fn test_random_error_statistics() {
        ...
        assert!(weight > 30 && weight < 200, "Error weight {} seems off for rate {}", weight, rate);
    }
