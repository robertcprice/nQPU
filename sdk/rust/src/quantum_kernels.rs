...
pub fn encode_feature_map(data: &[f64], feature_map: &FeatureMap) -> Result<Vec<Complex64>, KernelError> {
    match feature_map {
        FeatureMap::AngleEncoding { num_qubits } => {
...
pub fn train_qsvm(
    ...
    assert_eq!(n, train_labels.len(), "Data and labels must have same length");
    ...
    while passes < max_passes {
        ...
        for i in 0..n {
            ...
            if (yi * ei < -tol && alphas[i] < c) || (yi * ei > tol && alphas[i] > 0.0) {
                ...
                // Update bias
                let b1 = bias - ei - yi * (alphas[i] - alpha_i_old) * k[[i, i]]
                    - yj * (alphas[j] - alpha_j_old) * k[[i, j]];
                let b2 = bias - ej - yi * (alphas[i] - alpha_i_old) * k[[i, j]]
        ...
        .map(|j| {
            if model.alphas[j] > 1e-7 {
                let kval = match &config.kernel_type {
                    ...
                    KernelType::Projected { observables } => {
                        projected_kernel(&train_data[j], test_point, &config.feature_map, observables)
                    }
                };
...
pub fn evaluate_qsvm(
    ...
    let correct = test_data
        ...
        .filter(|(point, &label)| predict_qsvm(model, train_data, train_labels, point, config) == label)
        .count();
...
pub fn kernel_alignment(k: &KernelMatrix, labels: &[i8]) -> f64 {
    ...
    let fro_inner: f64 = k.matrix.iter().zip(k_ideal.iter()).map(|(a, b)| a * b).sum();
...
pub fn kernel_concentration(k: &KernelMatrix) -> f64 {
    ...
    let variance = off_diag.iter().map(|x| (x - mean) * (x - mean)).sum::<f64>()
        / off_diag.len() as f64;
    variance
}
...
mod tests {
    ...
    fn test_kernel_config_builder_defaults() {
        let config = KernelConfig::default();
        assert!(matches!(config.feature_map, FeatureMap::AngleEncoding { num_qubits: 2 }));
    ...
    fn test_qsvm_linearly_separable() {
        ...
        let config = QsvmConfig::default()
            .with_kernel_config(
                KernelConfig::default().with_feature_map(fm),
            )
