...
impl StrategicCode {
    ...
    pub fn repetition(num_qubits: usize, noise_model: NoiseModel) -> Self {
        assert!(num_qubits >= 3, "Need at least 3 qubits for repetition code");
...
impl PetzRecoveryChannel {
    ...
    pub fn new(num_qubits: usize, noise_model: NoiseModel) -> Self {
        assert!(num_qubits <= 10, "Petz recovery limited to 10 qubits (density matrix)");
    ...
    pub fn recover(&self, rho: &[C64]) -> RecoveryResult {
        ...
        let overhead = dim as f64 / (dim as f64 / 2.0_f64.powi(self.num_qubits as i32 - 1)).max(1.0);
...
impl ApproximateCodeAnalysis {
    ...
    pub fn overhead_reduction(&self, _exact_distance: usize, exact_qubits: usize, approx_qubits: usize) -> f64 {
        if approx_qubits == 0 {
            return 0.0;
        }
...
impl NoiseAdaptedOptimizer {
    ...
    pub fn optimize(&self) -> OptimizationResult {
        ...
        let mut best_code =
            StrategicCode::repetition(self.num_qubits, self.noise_model.clone());
...
pub fn compare_dynamical_vs_static(
    distance: usize,
    noise_model: &NoiseModel,
) -> ComparisonResult {
...
mod tests {
    ...
    fn test_noise_model_sampling() {
        ...
        assert!(non_identity > 90, "Full depolarizing should rarely produce I");
    }
    ...
    fn test_adaptive_code_mode() {
        ...
        assert!(!code.enhanced_mode, "Should exit enhanced mode after cooldown");
    }
    ...
    fn test_approximate_code_analysis() {
        ...
        assert!(analysis.fidelity_bound > 0.9, "Fidelity bound should be high for low noise");
        assert!(
    ...
    fn test_noise_adapted_optimizer() {
        let mut optimizer = NoiseAdaptedOptimizer::new(
            5,
            3,
            NoiseModel::Depolarizing { error_rate: 0.005 },
        );
    ...
    fn test_petz_channel_applies_amplitude_damping_population_decay() {
        ...
        assert!((noisy[0].re - 0.2).abs() < 1e-9, "rho00 expected 0.2, got {}", noisy[0].re);
        assert!((noisy[3].re - 0.8).abs() < 1e-9, "rho11 expected 0.8, got {}", noisy[3].re);
    ...
    fn test_petz_channel_custom_kraus_identity_is_noop() {
        ...
        let model = NoiseModel::CustomKraus { kraus_ops: vec![i2] };
