    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TJMError::ConvergenceFailed { step, norm } => {
                write!(
                    f,
                    "Convergence failed at step {} (norm={:.6e})",
                    step, norm
                )
            }
        ...
        for l in 0..dl {
            for pp_out in 0..4 {
                for r in 0..dr {
                    let mut val = c0();
                    for pp_in in 0..4 {
                        val += gate[pp_out][pp_in]
                            * theta[l * 4 * dr + pp_in * dr + r];
                    }
            ...
            for br2 in 0..t0_ket.dr {
                ...
                for bl in 0..t0_bra.dl {
                    for bl2 in 0..t0_ket.dl {
                        for p in 0..self.physical_dim {
                            val += t0_bra.get(bl, p, br).conj()
                                * t0_ket.get(bl2, p, br2);
                        }
                    }
                }
                transfer.set(br, br2, val);
...
impl TensorJumpSimulator {
    ...
    fn average_time_series(
        &self,
        all_series: &[Vec<(f64, Vec<f64>)>],
    ) -> Vec<(f64, Vec<f64>)> {
...
pub fn heisenberg_dephasing_sim(
    ...
    let jump_ops: Vec<JumpOperator> = (0..num_sites)
        .map(|s| dephasing(s, gamma))
        .collect();
    let observables: Vec<Observable> = (0..num_sites)
        .map(|s| Observable::z(s))
        .collect();
...
mod tests {
    ...
    fn test_trajectory_no_jumps() {
        ...
        let (traj, _ts) = evolve_trajectory(
            &initial,
            &config,
            &hamiltonian,
            &jump_ops,
    ...
    fn test_observable_averaging() {
        ...
        let sim = TensorJumpSimulator::new(
            config,
            hamiltonian,
            jump_ops,
            observables,
