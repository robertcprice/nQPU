...
impl LinearOpticalNetwork {
    ...
    fn apply_rotation(
        ...
        for col in 0..m {
            ...
            u[[b, col]] = Complex64::new(-s, 0.0) * eiphi.conj() * ua
                + Complex64::new(c, 0.0) * ub;
        }
    }
}
...
impl PhotonicIsingMachine {
    ...
    pub fn encode_ising(
        j_matrix: &Array2<f64>,
        h: &[f64],
    ) -> PhotonicResult<LinearOpticalNetwork> {
        ...
        for i in 0..n {
            for j in (i + 1)..n {
                ...
                for col in 0..n {
                    ...
                    let uj = u[[j, col]];
                    new_u[[i, col]] =
                        Complex64::new(c, 0.0) * ui + Complex64::new(s, 0.0) * uj;
                    new_u[[j, col]] =
                        Complex64::new(-s, 0.0) * ui + Complex64::new(c, 0.0) * uj;
                }
                u = new_u;
            }
        }
        ...
        Ok(LinearOpticalNetwork { modes: n, unitary: u })
    }
...
impl PhotonicSimulator {
    ...
    pub fn run_boson_sampling(
        ...
        let samples =
            BosonSamplingClassical::sample(&self.network, input_state, num_samples);
        Ok(BSResult {
            samples,
    ...
    pub fn validate_advantage(
        &self,
        samples: &[Vec<usize>],
    ) -> PhotonicResult<ValidationResult> {
        let sampler = self.sampler.as_ref().ok_or_else(|| {
            PhotonicError::UnsupportedProtocol(
                "Validation requires a GBS sampler".into(),
            )
        })?;
...
mod tests {
    ...
    fn test_boson_sampling_probability_single_photon() {
        ...
        let net = LinearOpticalNetwork::from_beamsplitters(
            2,
            &[(0, 1, 0.5, 0.0)],
        )
        .unwrap();
    ...
    fn test_clements_decomposition_output() {
        ...
        let elements =
            LinearOpticalNetwork::from_clements_decomposition(&net.unitary).unwrap();
    ...
    fn test_ising_encoding_unitary() {
        let j = Array2::from_shape_vec(
            (3, 3),
            vec![0.0, 0.5, -0.3, 0.5, 0.0, 0.2, -0.3, 0.2, 0.0],
        )
        .unwrap();
    ...
    fn test_ising_solve() {
        let j = Array2::from_shape_vec(
            (3, 3),
            vec![0.0, 1.0, 0.5, 1.0, 0.0, 0.8, 0.5, 0.8, 0.0],
        )
        .unwrap();
        let h = vec![0.0; 3];
