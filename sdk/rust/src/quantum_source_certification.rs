...
impl QuantumSourceCertifier {
    ...
    pub fn certify_source(
        ...
        if physics.passed {
            quantum_evidence.push(format!(
                "Physics mechanism verified: {:?}", mechanism
            ));
        } else {
        ...
        if poisson.passed {
            quantum_evidence.push(
                "Timing follows Poisson distribution (quantum signature)".to_string()
            );
        }
        ...
        if corr.passed {
            quantum_evidence.push(
                "No long-range correlations (quantum signature)".to_string()
            );
        } else {
            classical_evidence.push(
                "Long-range correlations detected (classical signature)".to_string()
            );
        }
        ...
        if bias.passed {
            quantum_evidence.push(
                "Bias drifts over time (thermal/quantum signature)".to_string()
            );
        }
        ...
        if fine.passed {
            quantum_evidence.push(
                "No structure at fine scales (quantum signature)".to_string()
            );
        }
    ...
    fn certify_mechanism(&self, mechanism: &QuantumMechanism) -> CertificationTest {
        let (score, description) = match mechanism {
            QuantumMechanism::FowlerNordheimTunneling { oxide_thickness_nm, electric_field_mv_cm } => {
            ...
            QuantumMechanism::AvalancheNoise { .. } => {
                ...
                let score = 0.6;
                (score, "Avalanche noise - quantum initiation, classical gain".to_string())
            }
            ...
            QuantumMechanism::Unknown => {
                (0.2, "Unknown mechanism - cannot certify".to_string())
            }
        };
    ...
    fn test_poisson_distribution(&self, values: &[u64]) -> CertificationTest {
        ...
        CertificationTest {
            ...
            description: format!(
                "CV = {:.2} (Poisson ≈ 1.0) - {}",
                cv,
                if score >= 0.6 { "QUANTUM-LIKE" } else { "CLASSICAL-LIKE" }
            ),
        }
    }
    ...
    fn test_correlation_decay(&self, bits: &[u8]) -> CertificationTest {
        ...
        CertificationTest {
            ...
            description: format!(
                "Max autocorr = {:.4} - {}",
                max_corr,
                if score >= 0.5 { "UNCORRELATED (quantum)" } else { "CORRELATED (classical)" }
            ),
        }
    }
    ...
    fn test_bias_drift(&self, bits: &[u8]) -> CertificationTest {
        ...
        let bias_var: f64 = biases.iter()
            .map(|b| (b - mean_bias).powi(2))
            .sum::<f64>() / biases.len() as f64;
        ...
        CertificationTest {
            ...
            description: format!(
                ...
                bias_var,
                if drift_score >= 0.4 { "DRIFTING (quantum/thermal)" } else { "STABLE (classical)" }
            ),
        }
    }
    ...
    fn test_fine_scale_randomness(&self, bits: &[u8]) -> CertificationTest {
        ...
        for chunk in bits[..bits.len() - 3].chunks(4) {
            if chunk.len() == 4 {
                let pattern = chunk.iter().enumerate()
        ...
        let chi_sq: f64 = pattern_counts.iter()
            .map(|&c| {
        ...
        CertificationTest {
            ...
            description: format!(
                "Chi² = {:.2} (ideal ~15) - {}",
                chi_sq,
                if score >= 0.5 { "UNIFORM (quantum)" } else { "NON-UNIFORM" }
            ),
        }
    }
    ...
    fn test_min_entropy(&self, bits: &[u8]) -> CertificationTest {
        ...
        CertificationTest {
            ...
            description: format!(
                "H_min = {:.4} bits/bit - {}",
                min_entropy,
                if score >= 0.9 { "HIGH ENTROPY" } else { "LOW ENTROPY" }
            ),
        }
    }
    ...
    pub fn generate_report(&self, cert: &QuantumCertification) -> String {
        ...
        report.push_str(&format!("║  Mechanism: {:<50}║\n", format!("{:?}", cert.mechanism).chars().take(50).collect::<String>()));
        report.push_str(&format!("║  Confidence: {:>48.1}% ║\n", cert.quantum_confidence));
        report.push_str(&format!("║  Verdict: {:<50}║\n", format!("{:?}", cert.verdict)));
        report.push_str("╠══════════════════════════════════════════════════════════════╣\n");
...
mod tests {
    ...
    fn test_certify_ssd() {
        ...
        let timing: Vec<u64> = (0..10000).map(|i| i as u64 * 1000 + (i % 17) * 50).collect();
        let bits: Vec<u8> = (0..8000).map(|i| (i % 7) % 2).collect();
