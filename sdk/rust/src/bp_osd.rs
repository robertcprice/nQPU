...
impl Osd {
    ...
    fn osd_cs(
        ...
        let (osd0_correction, pivot_cols, col_order, augmented) =
            Self::osd0(h, syndrome, llrs);
        ...
        let pivot_set: std::collections::HashSet<usize> =
            pivot_cols.iter().copied().collect();
        let info_bits: Vec<usize> = (0..n)
...
impl BpOsdDecoder {
    ...
    pub fn decode(&self, syndrome: &[bool], p: f64) -> DecodingResult {
        ...
        // Run BP
        let (bp_correction, bp_llrs, bp_iterations, bp_converged) =
            self.run_bp(syndrome, p);
        ...
        let (osd_correction, osd_combos) = if self.osd_order == 0 {
            // OSD-0 only
            let (corr, _, _, _) =
                Osd::osd0(&self.parity_check, syndrome, &bp_llrs);
            (corr, 1)
        } else {
            // OSD-CS
            Osd::osd_cs(
...
mod tests {
    ...
    fn test_bp_converges_simple_code() {
        ...
        assert!(result.converged, "BP should converge on simple repetition code");
    ...
    fn test_osd0_basic() {
        ...
        for error_pos in 0..7 {
            ...
            let llrs: Vec<f64> = (0..7).map(|i| if i == error_pos { -1.0 } else { 2.0 }).collect();
    ...
    fn test_osd_cs_finds_lower_weight() {
        ...
        let (osd_cs_correction, combos_tried) =
            Osd::osd_cs(&h, &syn, &llrs, 3, 100);
    ...
    fn test_bp_method_sum_product() {
        ...
        assert!(result.converged, "Sum-product should converge on repetition code");
        assert!(
            result.correction[2],
    ...
    fn test_bp_method_min_sum() {
        ...
        assert!(result.converged, "Min-sum should converge on repetition code");
        assert!(
            result.correction[3],
    ...
    fn test_large_code_performance() {
        ...
        assert!(result.converged, "BP should converge on large repetition code");
        assert!(
            result.correction[250],
            "Should identify error on qubit 250"
        );
        assert_eq!(result.weight, 1);
    ...
    fn test_max_osd_combinations_limit() {
        let h = SparseMatrix::from_dense(&[
            vec![true, true, false, true, false, false, true, false, false, false],
            vec![false, true, true, false, true, false, false, true, false, false],
            vec![true, false, true, false, false, true, false, false, true, false],
            vec![false, false, false, true, true, true, false, false, false, true],
        ]);
